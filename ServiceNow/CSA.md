# ServiceNow CSA - Practice Questions

This document contains practice questions for the ServiceNow CSA (Certified System Administrator) certification.

## How to use:
- 📝 Read the question and answer choices
- 🤔 Think and choose your answer
- 🔍 Click "**View Correct Answer**" to see the result
- 💬 Click "**View Discussion**" to read comments and explanations from the community

> **Note:** Answer and discussion sections can be collapsed/expanded by clicking on the corresponding headers.

## Question 1

### A Service Catalog may include which of the following components?

### Answer Choices:
**A.** Order Guides, Exchange Rates, Calendars

**B.** Order Guides, Catalog Items, and Interceptors

**C.** Catalog Items, Asset Contracts, Task Surveys

**D.** Record Producers, Order Guides, and Catalog Items



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (35 comments)</strong></summary>

**👤 sowoc** ⬆️ 5

**Selected Answer:** D
D is the correct answer

Just cleared my CSA exam with great confidence thanks to ValidIt-Exams!
Their updated CSA study material was accurate and very helpful.
Most of the questions I practiced came in the actual exam.
If you're serious about passing, ValidIt-Exams is the best choice.

---

**👤 9d1bc03** ⬆️ 1

**Selected Answer:** D
D is the correct answer

---

**👤 rijig** ⬆️ 2

**Selected Answer:** D
D is the Correct Option
I’ve tried a few mock test platforms, but Examforsure stood out. Their content is top-notch and very similar to what you see on the actual exam.

---

**👤 jessicamoneu** ⬆️ 2

**Selected Answer:** D
D is the correct answer IT EXAMS PRO

---

**👤 Jp_Singh** ⬆️ 1

**Selected Answer:** D
D is the correct answer for this question

---

**👤 EllisAudrey** ⬆️ 3

**Selected Answer:** D
Standard catalog items

Record producers: giving alternative ways of adding information, such as Incidents via the service catalog.
Order guides: to group multiple catalog items in one request.

Click Here: https://zig.ht/ServiceNowCSA

The overall catalog is made up of a collection of discrete catalog items.

---

**👤 KMC_MNJ** ⬆️ 1

**Selected Answer:** D
perfect example of a question I hate. I know how to setup service catalogs, including flows for fulfilling requests. Don't know the exact terminology of the parts so I have to fail the exam over

---

**👤 CarlaWI** ⬆️ 1

**Selected Answer:** D
Perfect example of a question I hate. I know how to setup service catalogs, including flows for fulfilling requests. Don't know the exact terminology of the parts so I have to fail the exam over this.

---

**👤 Jk11122333444555666** ⬆️ 1

D is correct

---

**👤 TipoZico** ⬆️ 1

**Selected Answer:** D
It is.

*...and 25 more comments*


</details>

---

## Question 2

### From the User menu, which actions can a user select? (Choose three.)

### Answer Choices:
**A.** Send Notifications

**B.** Log Out ServiceNow

**C.** Elevate Roles

**D.** Impersonate Users

**E.** Order from Service Catalog

**F.** Approve Records



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BCD
**Community voting results:** BCD (79%), 14%, 7%

</details>

<details>
<summary><strong>💬 View Discussion (20 comments)</strong></summary>

**👤 2pagesServiceNowSolutions** ⬆️ 11

badly worded...a *user cant select any!
a user with both admin and security roles can select the three answers they're seeking.

---

**👤 dalvisa** ⬆️ 1

**Selected Answer:** BCD
correct answer

---

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** BCD
Correct answer

---

**👤 Nexfo** ⬆️ 1

**Selected Answer:** BCD
Just drop the menu :)

---

**👤 TipoZico** ⬆️ 1

**Selected Answer:** BCD
"um... actually..."

---

**👤 Lucifer01** ⬆️ 2

**Selected Answer:** BEF
From ChatGPT

---

**👤 David_Wu** ⬆️ 1

B, D, E should be the answers

---

**👤 Certs_Hub** ⬆️ 1

B, D, E are correct answers.

B. Log Out ServiceNow: Logging out is a common action available in user menus to securely sign out of the system.

D. Impersonate Users: Impersonating users allows administrators or users with specific permissions to temporarily assume the identity of another user for troubleshooting or testing purposes.

E. Order from Service Catalog: If the system includes a Service Catalog, users may have the option to order services or items from it through the User menu.

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** BCD
B, C, D is correct

---

**👤 Mahindra_Reddie** ⬆️ 1

**Selected Answer:** BCD
Is correct answer

*...and 10 more comments*


</details>

---

## Question 3

### What is a quick way to create a report from a list view?

### Answer Choices:
**A.** Click on filter breadcrumb, drag and drop on the Report > Create New module

**B.** Click Funnel, define filter conditions, click Create Report

**C.** Click Context Menu, select Create Report

**D.** Apply filter, right click on column header, select Bar Chart

**E.** Apply filter, right click on column header, select Create Report



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (21 comments)</strong></summary>

**👤 Rwin0001** ⬆️ 15

Shouldn't this answer be D then?

---

**👤 w8w8w6v8o8** ⬆️ 6

**Selected Answer:** D
There's no option anywhere in List View to select "Create Report"

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D is the only viable option from the list, tested in the PDI.

---

**👤 40a783b** ⬆️ 2

**Selected Answer:** D
B cannot be an answer, Click Funnel is for filtering the list.
D is correct one either Bar Chart or Pie Chart can be created out of there

---

**👤 Atul1711** ⬆️ 1

**Selected Answer:** D
D is the right answer

---

**👤 tim4896** ⬆️ 1

D is correct

---

**👤 murphyalex** ⬆️ 1

D is correct

---

**👤 sithupraji** ⬆️ 1

D is the answer

---

**👤 [Removed]** ⬆️ 3

**Selected Answer:** D
This is so scuffed... even the explanation says it's D.

---

**👤 Samitinjay** ⬆️ 2

**Selected Answer:** D
Option D is correct. 
Because once you go to the report section you can change any type of report you want so this is the fastest way to generate a report

*...and 11 more comments*


</details>

---

## Question 4

### What import utility do you use when the field names on the import set match the name of the fields on the Target table?

### Answer Choices:
**A.** Schema Mapping

**B.** Automatic Mapping

**C.** Mapping Assist

**D.** Mapping Dashboard



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (79%), C (21%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 aper** ⬆️ 1

**Selected Answer:** B
B 
"The simplest mapping method is where all of the field names of the import sets match the names of the fields on the production tables onto which the data will be transformed"
https://www.servicenow.com/docs/bundle/xanadu-integrate-applications/page/script/server-scripting/concept/c_MappingOptions.html

---

**👤 562ae40** ⬆️ 3

**Selected Answer:** C
Its literally in the documentation picture that it is C

---

**👤 Anna47** ⬆️ 2

**Selected Answer:** C
Related Links
Auto Map Matching Fields
Mapping Assist 
Validate Coalesce Fields
Transform
Index Coalesce Fields
Run Point Scan

---

**👤 NatusyaZira** ⬆️ 4

**Selected Answer:** B
Yes, B is right : Automatic Mapping Utility
The simplest mapping method is where all of the field names of the import sets match the names of the fields on the production tables onto which the data will be transformed. In this case, simply click Auto map matching fields in the related list in the Table Transform Map form and confirm proper matching. If there are any discrepancies in terms of how fields were automatically matched, use the mapping assist utility to correct them. When all fields are matched properly, click the Transform related link to begin transforming data onto the destination table.

---

**👤 rcn8278** ⬆️ 1

The correct answer is C. Mapping Assist.

---

**👤 LovMi21** ⬆️ 4

**Selected Answer:** B
Automatic Mapping Utility
The simplest mapping method is where all of the field names of the import sets match the names of the fields on the production tables onto which the data will be transformed. In this case, simply click Auto map matching fields in the related list in the Table Transform Map form and confirm proper matching. If there are any discrepancies in terms of how fields were automatically matched, use the mapping assist utility to correct them. When all fields are matched properly, click the Transform related link to begin transforming data onto the destination table.

---

**👤 bagoly** ⬆️ 4

**Selected Answer:** B
B is correct

---

**👤 Markster** ⬆️ 1

B is correct if all fields match

---

**👤 Bog2022** ⬆️ 2

**Selected Answer:** B
The simplest mapping method is where all of the field names of the Import Set match the name of the fields on the target table where the data will be transformed.

---

**👤 Matz2021** ⬆️ 4

Automatic Mapping Utility
The simplest mapping method is where all of the field names of the import sets match the names of the fields on the production tables onto which the data will be transformed. In this case, simply click Auto map matching fields in the related list in the Table Transform Map form and confirm proper matching. If there are any discrepancies in terms of how fields were automatically matched, use the mapping assist utility to correct them. When all fields are matched properly, click the Transform related link to begin transforming data onto the destination table.

*...and 2 more comments*


</details>

---

## Question 5

### As an IT employee what interface would you use, if you wanted to browse internal IT documentation, like troubleshooting scripts and FAQs?

### Answer Choices:
**A.** Knowledge

**B.** ServiceNow Wiki

**C.** Knowledge Now

**D.** SharePoint

**E.** Stack Overflow



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Foodiepanda** ⬆️ 1

**Selected Answer:** A
In many IT environments, Knowledge Management systems or platforms, like Knowledge in ServiceNow, are used to store and organize internal documentation, scripts, and FAQs for easy access by IT employees. ServiceNow Wiki (option B) is also a plausible choice, as it is often used for documentation within the ServiceNow platform. However, without specific details about your organization's setup, "Knowledge" is a general term for this type of documentation repository.

---

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** A
Knowledge A.

---

**👤 YMijoe** ⬆️ 1

**Selected Answer:** A
"Knowledge" is correct.



</details>

---

## Question 6

### A new Service Desk employee in Latin America complains that the create dates and times are incorrect on their Incident list. What would you suggest to fix this issue?

### Answer Choices:
**A.** Have them clear their cache.

**B.** Have them use the gear icon to set the employee's time zone.

**C.** Recommend they use Chrome, instead of Explorer.

**D.** Use the system properties to correct the instance's time zone.

**E.** Have them correct the time zone on their computer.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (81%), Other

</details>

<details>
<summary><strong>💬 View Discussion (24 comments)</strong></summary>

**👤 connal308876** ⬆️ 5

In Washingington, it appears you would click on user menu, select Preferences, and Language & Region and select Timezone

---

**👤 Remo878** ⬆️ 5

D option

---

**👤 bcc5e85** ⬆️ 1

D OPTION

---

**👤 40a783b** ⬆️ 4

**Selected Answer:** B
B is correct. Use the gear icon means under User Menu choose gear icon near Preferences. there Language & Region ...

---

**👤 JinDG** ⬆️ 3

**Selected Answer:** B
Yeah just because this one employee need the time zone to be in Latin we are just gonna change the time zone setting for the whole instance.
Some people needs to get their brain checked fr.

---

**👤 wer3ball** ⬆️ 1

https://docs.servicenow.com/bundle/washingtondc-platform-administration/page/administer/time/task/t_SetASystemTimeZone.html

Wouldn't it be D as answer then

---

**👤 38d386e** ⬆️ 5

B is correct. D is incorrect because he isn't an admin user; he can't change the system settings in SN.

---

**👤 Adamkrt** ⬆️ 4

**Selected Answer:** B
Employee complaints so its setting must be fixed - B

---

**👤 PeterK0** ⬆️ 2

**Selected Answer:** D
gear icon under the profile icon , which refers to preferences , there we can customize our time zone and everything

---

**👤 Certs_Hub** ⬆️ 1

D is correct option. 
The issue of incorrect create dates and times on the Incident list could be related to the time zone settings of the ServiceNow instance. ServiceNow allows administrators to configure the time zone for the instance through system properties.

*...and 14 more comments*


</details>

---

## Question 7

### What are three security modules often used by the System Administrator? (Choose three.)

### Answer Choices:
**A.** System Properties > Security

**B.** Utilities > Migrate Security

**C.** System Security > Security

**D.** Self-Service > My Access

**E.** System Security > Access Control (ACL)

**F.** Password Management > Security Questions

**G.** System Security > High Security Settings



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AEG
**Community voting results:** AEG (89%), 11%

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 Rikart2009** ⬆️ 2

**Selected Answer:** AEG
AEG is correct

---

**👤 Mihaf92** ⬆️ 1

It is AEG

---

**👤 evoken** ⬆️ 1

**Selected Answer:** AEG
A E G are correct

---

**👤 bagoly** ⬆️ 2

**Selected Answer:** AEG
correct

---

**👤 Mohammedhz** ⬆️ 3

**Selected Answer:** AEG
Correct



</details>

---

## Question 8

### When testing a catalog item, having a manager approval flows, which of these best practices would you follow? (Choose three.)

### Answer Choices:
**A.** Make sure the latest flows are activated.

**B.** Use the instance Incognito setting to quickly toggle between requester and approver.

**C.** Impersonate the requester to ensure the form works.

**D.** Make sure the requester's user record has a manager specified.

**E.** Create and select your Testing Update Set, before starting the test cases.

**F.** Use your Admin account, so you can approve the items quickly.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACD
**Community voting results:** ACD (58%), CDF (26%), Other

</details>

<details>
<summary><strong>💬 View Discussion (29 comments)</strong></summary>

**👤 MattMeyer2022** ⬆️ 9

**Selected Answer:** CDF
The correct answers are C, D and F
FYI -- Answer E is wrong. I don't think there is something called a Testing Update set, and if there is, there is no need to use it when testing a catalog item.

---

**👤 PLiza2** ⬆️ 8

A is for sure one of the correct answers. Can't test the flow without Activating it. Just follow the steps in the lab 4.3 of SN Administration Fundamentals course e-book. 
Also C, is correct based on the steps in the LAB. D makes sense since there is Manager approval involved. B - there is no such thing as Instance Incognito Mode, E - there is no Testing Update Set

---

**👤 JuanSP** ⬆️ 1

**Selected Answer:** ACD
ACD is correct

---

**👤 Nexfo** ⬆️ 1

**Selected Answer:** ADE
ACD is correct

---

**👤 naab143** ⬆️ 7

The best practices to follow when testing a catalog item with a manager approval flow are:

A. Make sure the latest flows are activated.

Ensuring that the latest approval flows are activated helps ensure that any recent changes or updates to the approval process are reflected in the testing.
C. Impersonate the requester to ensure the form works.

Impersonating the requester allows you to test the catalog item from the perspective of the end-user, ensuring that the form functions correctly and provides a seamless user experience.
D. Make sure the requester's user record has a manager specified.

Ensuring that the requester's user record has a manager specified is essential for triggering the manager approval flow correctly. Without a specified manager, the approval process may not function as intended.
These practices ensure that the catalog item and its associated approval flow are thoroughly tested and function correctly in a real-world scenario.

---

**👤 ba14741** ⬆️ 3

**Selected Answer:** ACD
ACD are correct

---

**👤 2pagesServiceNowSolutions** ⬆️ 3

When testing a catalog item, having a manager approval flows, which of these ***best practices*** would you follow? (Choose three.)

A (Probably) assuming you are testing that version...the latest version is actually in dev...mid dev maybe???
B (nonsense answer, just ignore this)
C (YES) you have to test as the user, admin and dev may have access to tables that ACLs mean reference fields don't work for a user who does not have access.
D (YES) if you're dot walking from who the requester is/or for you need the manager field populated otherwise teh approval you are testing won't work!
E (Maybe) you test the update set on transfer to other environments, we're not sure if we're testing in early development...in the DEV environment, that being the case A is valid too?
F (NO! )the clue here is "Best practices" and for the same reason as C admin to approve is a good and quick idea...but is it testing the approver? hoes the manager have approver licence? 

Therefore the correct 3 best answers are A,C,D

---

**👤 38d386e** ⬆️ 4

ACD. F is wrong because there isn't an "Admin account" role.

---

**👤 Adamkrt** ⬆️ 3

**Selected Answer:** ACD
ACD are most logical

---

**👤 PeterK0** ⬆️ 3

**Selected Answer:** ACD
ACD because only those options contains words like "make sure", "to ensure" :DD

*...and 19 more comments*


</details>

---

## Question 9

### What is a no-code approach to control the mandatory or read-only state of a form field?

### Answer Choices:
**A.** UI Action

**B.** Client Script

**C.** UI Script

**D.** UI Rule

**E.** UI Policy



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** E
**Community voting results:** E (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 6d50eb7** ⬆️ 4

**Selected Answer:** E
UI Actions:
Purpose: UI Actions are used to define custom buttons, links, or context menu items that perform a specific action when clicked.
Scope: They can be applied to forms, lists, and other UI elements.
Triggers: UI Actions are triggered by user interaction, such as clicking a button or selecting a context menu item.
Use Cases: UI Actions are often used to trigger server-side scripts, client-side scripts, or other operations when a user needs to perform a specific action, like opening a new record, updating fields, or running a script.


UI Policies:
Purpose: UI Policies are used to dynamically change the visibility and behavior of form elements based on predefined conditions.
Scope: They are applied at the form level and are used to control the visibility, mandatory status, and read-only state of form fields.
Triggers: UI Policies are evaluated when a form is loaded, and their conditions determine whether certain fields should be shown, hidden, made mandatory, or set to read-only.
Use Cases: UI Policies are commonly used to simplify and streamline the user interface by showing or hiding fields based on the values of other fields, making forms more context-aware and user-friendly.

---

**👤 Sallex** ⬆️ 2

**Selected Answer:** E
E is correct

---

**👤 mrnow** ⬆️ 2

**Selected Answer:** E
E is correct



</details>

---

## Question 10

### When moving multiple update sets at one time, what might you do to facilitate the move?

### Answer Choices:
**A.** Batch

**B.** Verify

**C.** Test

**D.** Preview



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (15 comments)</strong></summary>

**👤 Muhammadosama** ⬆️ 6

A is correct ( "Batch update sets enable you to group update sets together so you can preview and commit them in bulk.")
another think confirming 'A' is correct ("o
Batch update sets enable you to group update sets together so you can preview and commit them in bulk.

Dealing with multiple update sets can lead to problems, including committing update sets in the wrong order or inadvertently leaving out one or more sets. You can avoid these problems by grouping completed update sets into a batch.

The system organizes update set batches into a hierarchy. One update set can act as the parent for multiple child update sets. A given set can be both a child and parent, enabling multiple-level hierarchies. One update set at the top level of the hierarchy acts as the base update set.

When you preview or commit the base update set, you preview or commit the entire batch. The system determines the processing order, and checks for collisions, based on the dates the changes were recorded, and on their sequential ancestry. Their ancestries are the specific instances in which the changes in the update sets took place.")

---

**👤 Mayur444** ⬆️ 1

A is the one

---

**👤 ba14741** ⬆️ 2

**Selected Answer:** A
A batch

---

**👤 GoldBear** ⬆️ 4

**Selected Answer:** A
A is correct.
https://docs.servicenow.com/bundle/vancouver-application-development/page/build/system-update-sets/hier-update-sets/concept/us-hier-overview.html

---

**👤 BenBose** ⬆️ 1

Seems very ambiguous to me - The answer could be A or D as both facilitate the moving of multiple update sets

---

**👤 Lilymich** ⬆️ 1

Batch update sets

---

**👤 Alice_Steele** ⬆️ 1

**Selected Answer:** A
docs.servicenow: 'Batch update sets enable you to group update sets together so you can preview and commit them in bulk.'

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is the answer.

---

**👤 rittersportler** ⬆️ 1

**Selected Answer:** A
https://docs.servicenow.com/bundle/tokyo-application-development/page/build/system-update-sets/hier-update-sets/concept/us-hier-overview.html

---

**👤 EAMTL** ⬆️ 1

**Selected Answer:** A
A is correct

*...and 5 more comments*


</details>

---

## Question 11

### What is specified in an Access Control rule?

### Answer Choices:
**A.** Groups, Conditional Expressions and Workflows

**B.** Table Schema, CRUD, and User Authentication

**C.** Object and Operation being secured; Permissions required to access the object

**D.** security_admin



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Nasman096** ⬆️ 10

**Selected Answer:** C
WHAT IS AN ACCESS CONTROL (ACL)?
An instance uses access control list (ACL) rules, also called access control rules, to control what data users can access and how they can access it. ACL rules require users to pass a set of requirements in order to gain access to particular data. Each ACL rule specifies:
The object and operation being secured
The permissions required to access the object

---

**👤 PeterK0** ⬆️ 1

It is very ambiguous when you use Access control rule instead of Access control list (ACL) and you think ACL, but you say Access control rule anyway

---

**👤 tonystark01** ⬆️ 1

**Selected Answer:** C
correct



</details>

---

## Question 12

### Which icon would you double click, to expand and collapse the list of all Applications and Modules?
//IMG//

### Answer Choices:
**A.** Star

**B.** Clock

**C.** Application

**D.** Funnel



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (75%), D (25%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 96d187b** ⬆️ 1

This is outdated, but answer would be C.

---

**👤 40a783b** ⬆️ 2

**Selected Answer:** C
C is correct but but valid only for core UI not for new expirience

---

**👤 38d386e** ⬆️ 1

C is the answer

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** C
It only takes a "single" click to expand the Application and Modules.

---

**👤 kohliaashish** ⬆️ 1

**Selected Answer:** C
Correct Answer Is C

---

**👤 tonystark01** ⬆️ 1

**Selected Answer:** C
Correct

---

**👤 amit_p** ⬆️ 1

**Selected Answer:** C
https://docs.servicenow.com/bundle/utah-platform-user-interface/page/administer/navigation-and-ui/task/ui16-navigator-tasks.html

---

**👤 Melll1972** ⬆️ 2

**Selected Answer:** D
I thought the answer was D, as clicking the funnel expands all of the applications in the menu.

---

**👤 Ghouse1** ⬆️ 1

C is correct

---

**👤 Markster** ⬆️ 2

I don't believe this is correct for the new San Diego NEXT UI.

*...and 1 more comments*


</details>

---

## Question 13

### Buttons, form links, and context menu items are all examples of what type of functionality?

### Answer Choices:
**A.** Business Rule

**B.** UI Action

**C.** Client Script

**D.** UI Policy



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (15 comments)</strong></summary>

**👤 heeyyyy** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 clausi** ⬆️ 2

**Selected Answer:** B
B is correct

---

**👤 Andit10** ⬆️ 2

**Selected Answer:** B
B is correct

---

**👤 denono** ⬆️ 2

https://docs.servicenow.com/bundle/washingtondc-platform-administration/page/administer/list-administration/concept/c_UIActions.html

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Augusto0007** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 saiakhilpalle** ⬆️ 1

B is correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 DANX1** ⬆️ 1

B is correct

*...and 5 more comments*


</details>

---

## Question 14

### What do you call any component that needs to be managed in order to deliver services?

### Answer Choices:
**A.** CSDM Items

**B.** CMDB

**C.** Configuration item

**D.** Service Offerings

**E.** Asset



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
C is the ITIL definition of a CI

---

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** C
C configuration item

---

**👤 Mohammedhz** ⬆️ 2

Correct



</details>

---

## Question 15

### A new service catalog item is being developed, but should only be visible to managers inside the HR Department. What method would you use to fulfill this requirement?

### Answer Choices:
**A.** Specify the Dept_Mgr role on the catalog content block

**B.** Add the Department Manager group to the catalog item's user criteria

**C.** Add the Department Manager group to the catalog item's ACL

**D.** Only publish the item in the HR service catalog

**E.** Use a Dept_Mgr ACL on the HR service catalog



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (15 comments)</strong></summary>

**👤 sithupraji** ⬆️ 8

Am totally confused, is this the real answers for the exam?Can some one pls correct it if its a wrong answer given by verifying snow documents

---

**👤 naab143** ⬆️ 7

The method that best fulfills the requirement of making a new service catalog item visible only to managers inside the HR Department is:

B. Add the Department Manager group to the catalog item's user criteria.

By adding the Department Manager group to the catalog item's user criteria, you restrict visibility to only users who are part of that group. This ensures that only managers within the HR Department can see and access the catalog item, fulfilling the requirement accurately.

---

**👤 uwbe** ⬆️ 4

**Selected Answer:** B
Catalog items are secured using user criteria's in the related links Available For/Not Available For

---

**👤 pielouis** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 PraiseDabira** ⬆️ 1

https://docs.servicenow.com/bundle/utah-employee-service-management/page/product/employee-center/reference/ec-user-criteria-form.html

---

**👤 FastAndy** ⬆️ 1

**Selected Answer:** B
Accoring to Snow docu

---

**👤 evoken** ⬆️ 2

**Selected Answer:** B
What’s with all the wrong answers. It’s B

---

**👤 Ranaway** ⬆️ 1

User Criteria

---

**👤 MattMeyer2022** ⬆️ 4

**Selected Answer:** B
User Criteria is used for catalog items

---

**👤 mitashki** ⬆️ 2

**Selected Answer:** B
B is the correct answer

*...and 5 more comments*


</details>

---

## Question 16

### A user wants to create a set of filter conditions, where they want to show records which satisfy two conditions:
✑ Incidents where the state is Closed
✑ Incidents where Assignment Group is Network
After clicking the Funnel icon, what should the user do?

### Answer Choices:
**A.** Define the first condition; click AND button; define second condition; click Run

**B.** Define the first condition; click AND button; define second condition; press enter

**C.** Define the first condition; click OR button; define second condition; press enter

**D.** Define the first condition; click > icon on breadcrumb, define second condition; click Run

**E.** Define the first condition; click > icon on breadcrumb, define second condition; press enter



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 sexyney** ⬆️ 1

clearly A" is the answer

---

**👤 MattMeyer2022** ⬆️ 2

**Selected Answer:** A
AND is for satisfying both

---

**👤 sephereth** ⬆️ 2

**Selected Answer:** A
"AND" is correct, is used coz u need to meet both conditions



</details>

---

## Question 17

### Access Control rules are applied to a specific table, like the Incident table. What is the object name for a rule that is specific to the Incident table and the Major
Incident field?

### Answer Choices:
**A.** Incident.Major_Incident

**B.** incident=>major_incident

**C.** incident<=>major_incident

**D.** incident||major_incident

**E.** incident.major_incident



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** E
**Community voting results:** E (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 BlackTECH** ⬆️ 1

**Selected Answer:** E
Small letters and underscore are key items in this question

---

**👤 cindy2023** ⬆️ 2

what's the difference between A and E ??? looks same/

---

**👤 evoken** ⬆️ 2

**Selected Answer:** E
E is correct

---

**👤 Ghouse1** ⬆️ 1

https://docs.servicenow.com/bundle/sandiego-platform-security/page/administer/contextual-security/concept/access-control-rules.html



</details>

---

## Question 18

### Two departments (HR Onboarding and Facilities) have come to you, asking for a way for employees to request event room set up services. The requirements are the same for the form and the task routing to the Facilities' assignment group.
✑ For HR, the item will be used primarily for the Onboarding coordinators, for employee orientation sessions.
✑ For Facilities, the item will be used for anyone in the company who needs room set up services.
However, both departments have their own service catalogs. What do you do, to support these requirements?

### Answer Choices:
**A.** Create one Catalog Item for HR Event Room Set Up and one for Facilities Event Room Set Up; then publish each to the appropriate Catalog.

**B.** Create one Catalog Item for Event Room Set Up; then publish to both Catalogs.

**C.** Create one Catalog Item for Event Room Set Up; then publish to the Parent Catalog, which is accessible to both HR and Facilities.

**D.** Create one Catalog Item for Event Room Set Up; then use ACLs to control access.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 bagoly** ⬆️ 9

**Selected Answer:** B
I don't get it, why the answer C is, why should be a parent-child relationship established? I've tested it, the catalog item can be added to both catalogs.

---

**👤 foolishsoul4556** ⬆️ 6

**Selected Answer:** B
Correct

---

**👤 ba14741** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 PinkAndBlack** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 PinkAndBlack** ⬆️ 1

In my opinion the correct answer is B. https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/service-catalog-management/task/t_DefineACatalogItem.html

---

**👤 Spiderman5000** ⬆️ 2

**Selected Answer:** B
In Tokyo, the catalog item can be associated with more than one catalog. Open a catalog item, scroll to the bottom reference list, select Catalog tab, add multiple catalog references. Maybe the design changed since this question was added?



</details>

---

## Question 19

### After finishing your work on High Security Settings, what do you do to return to normal admin security levels?

### Answer Choices:
**A.** Select Normal role

**B.** Log out and back in

**C.** Use System Administration > Normal Security module

**D.** Select Global Update Set

**E.** End Impersonation



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (67%), A (33%)

</details>

<details>
<summary><strong>💬 View Discussion (16 comments)</strong></summary>

**👤 timothy_moore** ⬆️ 16

In Tokyo, you select Elevate Role under the user menu then uncheck security_admin and click update. So, while the terminology is wrong, A is (now) the most correct answer

---

**👤 kcheemal007** ⬆️ 1

**Selected Answer:** A
None of these answers seem to match the actual situation but A and B seem to the the closest.

---

**👤 skibumvermont** ⬆️ 3

**Selected Answer:** A
A is the correct answer for the Washington DC release. 

https://www.servicenow.com/docs/csh?topicname=t_ElevateToAPrivilegedRole.html&version=latest

---

**👤 Lavel** ⬆️ 4

User Menu then Elevate Role and switch on/off security_admin

---

**👤 FATMASAAL2** ⬆️ 2

**Selected Answer:** B
The elevated role will end when you log out(UTAH)

---

**👤 Manny1218** ⬆️ 3

**Selected Answer:** B
While in Tokyo you can just deselect the security_admin role and click update. But according to the ServiceNow Administration Fundamentals - Tokyo booklet, page 13 states "the elevated role will end when you log out." For testing I would suggest B

---

**👤 hgro** ⬆️ 2

In Vancover it is provided: This role grants the user elevated privileges to all resources controlled by the role for the remainder of the session. When the user logs out, the elevated privileges are terminated with the session but may be reestablished at the next login.

https://docs.servicenow.com/bundle/vancouver-platform-security/page/administer/security/task/t_ElevateToAPrivilegedRole.html

---

**👤 tafidwro** ⬆️ 2

Quote from Prompt to use the Elevated Role:
Elevate role

Elevate a role by adding privileges, which end when you log out. Learn more

AVAILABLE ROLES

security_admin
Grant modification access to High Security Settings, allow user to modify the Access Control List

---

**👤 NunoOSilva** ⬆️ 1

"Elevate a role by adding privileges, which end when you log out"

San Diego

---

**👤 uwbe** ⬆️ 4

Could this question be outdated? At least in Utah I would go User Menu -> Elevate Role and switch on/off security_admin

*...and 6 more comments*


</details>

---

## Question 20

### What type of field allows you to look up values from one other table?

### Answer Choices:
**A.** Reference

**B.** Verity

**C.** Options

**D.** Selections

**E.** Dot walk

**F.** Lookup



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (88%), 8%

</details>

<details>
<summary><strong>💬 View Discussion (22 comments)</strong></summary>

**👤 bb369** ⬆️ 12

Answer is A: Reference
https://docs.servicenow.com/bundle/rome-platform-administration/page/administer/reference-pages/reference/r_FieldTypes.html

---

**👤 Barkles** ⬆️ 7

**Selected Answer:** A
Answer is A https://docs.servicenow.com/bundle/paris-platform-administration/page/administer/field-administration/concept/c_ReferenceField.html

---

**👤 skibumvermont** ⬆️ 1

A is the correct answer

https://www.servicenow.com/docs/csh?topicname=r_FieldTypes.html&version=latest

---

**👤 JinDG** ⬆️ 1

The Suggested Answer Reference linke is a microsoft link LMAO what the F is this.

---

**👤 Cplatt** ⬆️ 1

**Selected Answer:** F
The reference link is definitely off. However, when thoroughly researching this I believe the answer would be F. There is a filed classified as "Lookup" that only looks up data from "one" table. The question is confusing because it's not specifying the scenario. I'd put my bets on F since these questions are very particular.

https://www.servicenow.com/community/developer-articles/use-of-look-up-select-box-variable-on-service-catalog/ta-p/2330117

---

**👤 ofmk301** ⬆️ 2

A - why is there a link to a Microsoft page as (wrong) reference in the provided answer. Did the mix up questions from another exam ?

---

**👤 Mariina_003_B** ⬆️ 2

**Selected Answer:** A
A. Reference

A Reference field in ServiceNow is used to establish a relationship between two tables. It enables you to select a record from another table, and the reference field displays the values from the referenced table. This functionality allows for data consistency and linking records across different tables.

---

**👤 LovMi21** ⬆️ 1

**Selected Answer:** A
https://docs.servicenow.com/en-US/bundle/rome-platform-administration/page/administer/reference-pages/reference/r_FieldTypes.html#:~:text=Table%201.%20Field%20types%20%20%20%20Field,color%20decl%20...%20%2030%20more%20rows%20

---

**👤 Nefeli_k** ⬆️ 1

Reference!

---

**👤 Lilymich** ⬆️ 2

the answer is A

*...and 12 more comments*


</details>

---

## Question 21

### Which module would you use to create a new automation of business logic such as approvals, tasks, and notifications?

### Answer Choices:
**A.** Process Automation > Flow Designer

**B.** Process Automation > Flow Administration

**C.** Process Automation > Workflow Editor

**D.** Process Automation > Process Flow

**E.** Process Automation > Active Flows



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 kcheemal007** ⬆️ 1

**Selected Answer:** A
A is still a valid answer, but workflow design studio is replacing this functionality.

---

**👤 tonystark01** ⬆️ 1

Workflow editor is also an option but it does not comes under Process automation application.

---

**👤 Quero** ⬆️ 1

**Selected Answer:** A
A is the correct answer.

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is correct



</details>

---

## Question 22

### A department manager asks an analyst to build some reports. Where do you recommend the analyst start?

### Answer Choices:
**A.** Report Dashboard > Create New

**B.** Reports > Getting Started

**C.** Performance Analytics > Reports

**D.** Self-Service > Reports

**E.** Reports > Create New



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** E
**Community voting results:** E (60%), B (40%)

</details>

<details>
<summary><strong>💬 View Discussion (17 comments)</strong></summary>

**👤 gedgy** ⬆️ 1

**Selected Answer:** E
E takes you to create a report. B takes you to a ServiceNow document. Remember, you are an admin in this scenario.

---

**👤 Cplatt** ⬆️ 4

In a real world scenario where the individual has never built reports before B. Reports>Getting Started would be the best course of action because the individual would need to learn how to build reports. These questions aren't really clear with the scenario based ones such as this one. Coming from real world experiences I always suggest that a person learn about reporting before trying to feel their way through understanding how to build a report. My guess would be E is the expected answer even though it's not practical if someone has never built a report before. I wish ServiceNow would eliminate questions like this.

---

**👤 shub606** ⬆️ 1

Answer should be E

---

**👤 Atul1711** ⬆️ 1

**Selected Answer:** E
E should be the answer.

---

**👤 wer3ball** ⬆️ 1

E. 

https://docs.servicenow.com/bundle/washingtondc-now-intelligence/page/use/reporting/task/t_CreateYourOwnReport.html

---

**👤 Foodiepanda** ⬆️ 1

**Selected Answer:** E
This seems to be logical

---

**👤 Mariina_003_B** ⬆️ 1

**Selected Answer:** E
E. Reports > Create New

This option allows the analyst to create a new report directly from the Reports module. They can configure and customize the report based on their requirements.

---

**👤 Joe15** ⬆️ 1

**Selected Answer:** E
Correct is E.
B takes you to docs (wiki) page.

---

**👤 ggageagegwseg** ⬆️ 2

useless ass website

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** B
If the analyst is new they probably should read the documentation to learn how to create reports, otherwise, why is that link useful? 

Once the analyst knows how to create a new report there is no reason for the manager to have the analyst review the documentation.

*...and 7 more comments*


</details>

---

## Question 23

### What are the steps for applying an update set to an instance?

### Answer Choices:
**A.** Retrieve, Preview, Commit

**B.** Specify, Transform, Apply

**C.** Retrieve, Assess, Apply

**D.** Get, Test, Push

**E.** Pull, Review, Push



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 6d50eb7** ⬆️ 1

**Selected Answer:** A
Import the Update Set in the Target Instance:
In the target instance, navigate to "System Update Sets" and click on "Retrieved Update Sets."
Click on "Import Update Set from XML," choose the XML file you transferred, and import it.

Preview and Commit the Update Set:
Once imported, preview the Update Set to see the changes that will be applied.
If everything looks correct, commit the Update Set to apply the changes to the target instance.

Verify the Changes:
After committing the Update Set, verify that the changes have been applied as expected in the target instance.

---

**👤 Lacitroger** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 tonystark01** ⬆️ 1

A for sure

---

**👤 Granddude** ⬆️ 1

https://docs.servicenow.com/bundle/rome-application-development/page/build/system-update-sets/hier-update-sets/concept/us-hier-overview.html#us-%20hier-overview does in fact state that A is the correct answer

---

**👤 Granddude** ⬆️ 1

What are the steps for applying an update set to an instance?

 A. Retrieve, Preview, Commit
 B. Specify, Transform, Apply
 C. Retrieve, Assess, Apply
 D. Get, Test, Push
 E. Pull, Review, Push

Does in fact state that A is the correct answer.

---

**👤 Adsilva** ⬆️ 2

A note the other previous question was about 'retrieved', and this question 119 is about 'applying'.



</details>

---

## Question 24

### Which of the following is true of Service Catalog Items in relation to the Service Catalog?

### Answer Choices:
**A.** They run behind the scenes.

**B.** They are the building blocks.

**C.** They are optional.

**D.** They provide options.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (76%), D (24%)

</details>

<details>
<summary><strong>💬 View Discussion (20 comments)</strong></summary>

**👤 WUPHF** ⬆️ 33

What kind of question is this?

---

**👤 clausi** ⬆️ 1

**Selected Answer:** B
**Selected Answer:** B

---

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** B
Correct answer

---

**👤 servicenowExamtopics** ⬆️ 4

**Selected Answer:** B
Service Catalog Items are considered the fundamental components or building blocks of the Service Catalog. They represent individual services or products that can be requested by users.

---

**👤 HarleyR** ⬆️ 4

I verified that the correct answer is B per the SNAF book

---

**👤 047a2d5** ⬆️ 3

B is 100% correct, I check it by myself in the material, which mention that items are building blocks of the service catalog include: Hardware, software, service etc....

---

**👤 abhishek.pal84** ⬆️ 2

Doesn't make any sense.

---

**👤 PerksOfRecreation** ⬆️ 4

**Selected Answer:** D
This question is kind of weird. As it is phrased, I would think rather of the variable Items as the building block and the service catalog Items as providing option

---

**👤 reika1914** ⬆️ 3

**Selected Answer:** B
B. They are the building blocks.

Service Catalog Items are the building blocks of the Service Catalog. They represent the individual services or products that users can request through the catalog. Each catalog item defines a specific service or product, along with its associated form, workflow, and approval process.

---

**👤 shabbs** ⬆️ 1

Is anyone sure, this question is using the correct statement?

*...and 10 more comments*


</details>

---

## Question 25

### When importing spreadsheet data into ServiceNow, in which step does the data get written to the receiving table?

### Answer Choices:
**A.** Run Transform

**B.** Run Import

**C.** Import Dataset

**D.** Execute Transform

**E.** Schedule Transform



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (84%), Other

</details>

<details>
<summary><strong>💬 View Discussion (23 comments)</strong></summary>

**👤 Kflo** ⬆️ 1

**Selected Answer:** A
"When all fields are matched properly, click Transform in the related links to begin transforming data onto the destination table."
Section 4.3 Import Data > Transform Maps

---

**👤 skibumvermont** ⬆️ 1

**Selected Answer:** A
https://www.servicenow.com/docs/csh?topicname=t_RunTransform.html&version=latest

---

**👤 aper** ⬆️ 1

**Selected Answer:** B
B.
"To import the data set you uploaded now, select Run Import." 
https://docs.servicenow.com/bundle/utah-application-development/page/build/app-engine-studio/task/import-map-data-existing-table.html

---

**👤 wer3ball** ⬆️ 1

B. 
Import the file or schedule the data import
Choose what to do with your data import you just created and mapped.
To import the data set you uploaded now, select Run Import.
To set up a future import of the data, select Schedule an import.
https://docs.servicenow.com/bundle/utah-application-development/page/build/app-engine-studio/task/import-map-data-existing-table.html

---

**👤 6d50eb7** ⬆️ 2

A.
https://developer.servicenow.com/dev.do#!/learn/learning-plans/utah/new_to_servicenow/app_store_learnv2_importingdata_utah_run_a_transform_map

---

**👤 rsilva85_BR** ⬆️ 1

**Selected Answer:** D
Answer is D. https://docs.servicenow.com/en-US/bundle/tokyo-service-management-for-the-enterprise/page/product/facilities-service-management/task/t_RunTransform.html.... these documentation shows item 6, after entering Run Transform, you need to CLICK the EXECUTE BUTTON to indeed execute the transformation.

---

**👤 tonystark01** ⬆️ 1

Receiving table = Target table so answer is A

---

**👤 PinkAndBlack** ⬆️ 1

**Selected Answer:** B
I should go for B. Please check 
https://docs.servicenow.com/bundle/utah-integrate-applications/page/administer/import-sets/task/t_RunImport.html

---

**👤 KevFun** ⬆️ 2

It is B, see https://docs.servicenow.com/bundle/utah-application-development/page/build/app-engine-studio/task/import-map-data-existing-table.html

---

**👤 sunnygohappy** ⬆️ 2

ChatGPT agrees to A.

*...and 13 more comments*


</details>

---

## Question 26

### What would you do, on a list, if you wanted to show the records in groups, based on the column category? (Choose two.)

### Answer Choices:
**A.** On list Context Menu, select Group By > Category

**B.** On the Filter Menu, select Group By > Category

**C.** Click Group On icon, select Category

**D.** On Navigator Filter, type tablename.group.category and press enter

**E.** On the Category column title, click Context menu > Group By Category



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AE
**Community voting results:** AE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 Cnu99999** ⬆️ 1

the answer is E and NOT A. you have to chose the column on which to group by where it will say "Gorup by <column name>. There is no generic context menu that just says group by where then later you can select the column.

---

**👤 Mikias** ⬆️ 1

AE correct

---

**👤 Vinnyzworld** ⬆️ 1

**Selected Answer:** AE
A and E

---

**👤 trye_indicators** ⬆️ 1

**Selected Answer:** AE
A, E are correct



</details>

---

## Question 27

### Which collaboration tool is available from the banner, using the bubble icon?
//IMG//

### Answer Choices:
**A.** Now Messenger

**B.** Agent Chat

**C.** Connect Chat

**D.** Collaborate Now

**E.** Live Feed



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 CB528** ⬆️ 4

its called sidebar in Utah release

---

**👤 Markster** ⬆️ 4

Important: Connect is not supported in Next Experience. If you turn on Next Experience, Connect functionality will no longer work. See Next Experience. https://docs.servicenow.com/en-US/bundle/sandiego-servicenow-platform/page/use/collaboration/concept/c_Connect.html



</details>

---

## Question 28

### On the knowledge base record, which tab would you use to define which users are not able to write articles to the knowledge base?

### Answer Choices:
**A.** Can Contribute

**B.** Cannot Author

**C.** Cannot Contribute

**D.** Cannot Write

**E.** Read Only



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (76%), 12%, 12%

</details>

<details>
<summary><strong>💬 View Discussion (17 comments)</strong></summary>

**👤 NikaDMars** ⬆️ 1

**Selected Answer:** A
In the default Knowledge Base form, only Can Read and Can Contribute tabs are visible by default. However, 
"Cannot Read and Cannot Contribute can be added by configuring related lists on a form." 
(by ServiceNow Administration Fundamentals (Xanadu))

This means Cannot Contribute is a valid feature, but it isn’t shown unless an admin explicitly adds it to the form layout.
So in practice, if you’re just using out-of-the-box (OOTB) ServiceNow without customizing the form - you’d use the "Can Contribute" tab and exclude users by not adding them there.

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
C is the correct answer, double checked on the Washington PDI instance.

---

**👤 Kflo** ⬆️ 1

**Selected Answer:** C
C

https://www.servicenow.com/docs/bundle/xanadu-servicenow-platform/page/product/knowledge-management/task/t_SelectUserCriteria.html

Note: If the related lists aren't visible, configure the Knowledge form to display them. (Cannot Contribute and Cannot Read)

---

**👤 skibumvermont** ⬆️ 2

**Selected Answer:** C
Answer for Washington DC release is A
https://www.servicenow.com/docs/csh?topicname=user-access-knowledge.html&version=latest

---

**👤 AJE68** ⬆️ 1

**Selected Answer:** A
There is only a "Can Contrubute" tab on the knowledge base record. So answer A might be the only suitable option.

---

**👤 PeterK0** ⬆️ 2

for some reason in both work and personal development instances i have only can contribute and can read tabs... is there any way to reveal the other options or they are not available since tokyo version ?

---

**👤 NunoOSilva** ⬆️ 2

**Selected Answer:** C
User criteria definitions
User criteria Result
Cannot Contribute Cannot contribute (that is can't create, modify, or retire) knowledge articles within a knowledge base. The Cannot Contribute user criteria is available only for knowledge bases.
Can Contribute Can contribute (that is can view, create, modify, or retire) knowledge articles within a knowledge base. The Can Contribute user criteria is available only for knowledge bases.
Cannot Read 
At the knowledge base level, cannot view knowledge articles within a knowledge base.

At the knowledge article level, cannot view a knowledge article.

Can Read 
At the knowledge base level, can view knowledge articles within a knowledge base.

At the knowledge article level, can view a knowledge article.

---

**👤 jQuerin** ⬆️ 1

**Selected Answer:** C
just checked it on my personal instance, C is correct

---

**👤 MacGyver4th** ⬆️ 3

**Selected Answer:** C
This explains why the answer is C
https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/knowledge-management/concept/user-access-knowledge.html

---

**👤 Spiderman5000** ⬆️ 2

**Selected Answer:** C
"Cannot Contribute" is hidden by default. Configure - Related Lists to show it.

*...and 7 more comments*


</details>

---

## Question 29

### Which features allow you to update multiple records at one time? (Choose two.)

### Answer Choices:
**A.** List Editor

**B.** Field Update Action

**C.** Bulk Record Update

**D.** Data Remediation Dashboard

**E.** Update Selected Action



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AE
**Community voting results:** AE (94%), 6%

</details>

<details>
<summary><strong>💬 View Discussion (14 comments)</strong></summary>

**👤 bb369** ⬆️ 11

Isn't answer AE?

---

**👤 Bog2022** ⬆️ 5

**Selected Answer:** AE
I'm voting AE

---

**👤 naab143** ⬆️ 2

A. List Editor

E. Update Selected Action

Both the List Editor and Update Selected Action functionalities in ServiceNow enable users to update multiple records simultaneously. The List Editor provides a spreadsheet-like interface where you can modify multiple records at once, while the Update Selected Action allows you to perform bulk updates on selected records from a list or a report.

---

**👤 ddiggler** ⬆️ 3

It is A and E and I'm not talking about A and E the TV channel.
It's a good resource, BUT, fix the answers Examtopics.

---

**👤 Holy_97** ⬆️ 1

**Selected Answer:** AB
AB is correct.
B can be done by selecting multiple rows > right click on column (field) header > select Update selected > edit any fiels > click button Update 
E is wrong because the option must be selecting "Actions on selected rows" pulldown but it only acts with the whole record, not the fields

---

**👤 PeterK0** ⬆️ 1

does update selected action mean action on selected rows ?

---

**👤 Sam_jsy** ⬆️ 1

**Selected Answer:** AE
answer is A & E

---

**👤 Mariina_003_B** ⬆️ 2

**Selected Answer:** AE
The features that allow you to update multiple records at one time in ServiceNow are:

A. List Editor
E. Update Selected Action

---

**👤 MacGyver4th** ⬆️ 2

**Selected Answer:** AE
A:
https://docs.servicenow.com/en-US/bundle/utah-platform-user-interface/page/use/using-lists/task/t_EditMultRecUsingListEditor.html

E:
https://docs.servicenow.com/bundle/utah-platform-user-interface/page/use/using-lists/task/t_EditMultiRecUsingEditingForm.html

---

**👤 bagoly** ⬆️ 2

**Selected Answer:** AE
AE is correct

*...and 4 more comments*


</details>

---

## Question 30

### Categories in the knowledge base, by default, can be created and edited by which knowledge workers? (Choose two.)

### Answer Choices:
**A.** Knowledge Authors

**B.** Knowledge Contributors

**C.** Knowledge Controller

**D.** Knowledge Managers

**E.** Knowledge Category Managers

**F.** Knowledge Submitters

**G.** Knowledge Owners

**H.** Knowledge Taxonomy Owner



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BD
**Community voting results:** BD (89%), 11%

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 BlackTECH** ⬆️ 1

**Selected Answer:** DG
Knowledge owners and Knowledge managers is the correct answer

---

**👤 Frost312321** ⬆️ 1

**Selected Answer:** BD
From the ebook, chapter 5.1: Knowledge mangers can define knowledge categories to pre-populate the list of available categories, and knowledge contributors can select categories, and add or edit categories, if enabled, for a Knowledge base.

B for sure, D is questionable but the only other answer which would fit

---

**👤 Dewoodma** ⬆️ 1

Options B and D are the only valid roles listed, however, only Knowledge Managers may modify knowledge base categories. KCS Contributors may create and validate knowledge articles in their area as assigned only. See all versions of SNow document on Knowledge Management Roles: https://docs.servicenow.com/bundle/washingtondc-servicenow-platform/page/product/knowledge-management/reference/r_KnowledgeRoles.html

---

**👤 MACCA81** ⬆️ 3

I think this question is incorrect. There's really only one correct answer and it's D. In the SN Administration Fundamentals - Vancouver, page 224 there's a Knowledge Management RACI Chart at the bottom of the page. In there the Action called Modify Knowledge Base: Categories, article validity, icon, workflow, etc - This action can be performed only by the admin, knowledge_admin and knowledge_manager. 
Knowledge Author, Knowledge Controller, Knowledge Category Manager, Knowledge Submitters, Knowledge Owners and Knowledge Taxonomy Owner, are not SN knowledge roles at all, based on the RACI. They're not mentioned there. 
Knowledge Contributor - as per the RACI - can create, update, modify, and view knowledge ARTICLES, but has NO permissions to modify Knowledge Categories as the question here asks. Hence really only Admin, Knowledge Admin and Knowledge Manager can modify a knowledge Category, and of the 3 only Knowledge Manager exists in the answer choices.

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** BD
Correct answer BD

---

**👤 Alok_kumar_kashyap** ⬆️ 2

Correct Answer is B,D

---

**👤 Alok_kumar_kashyap** ⬆️ 1

A:
https://docs.servicenow.com/en-US/bundle/utah-platform-user-interface/page/use/using-lists/task/t_EditMultRecUsingListEditor.html

E:
https://docs.servicenow.com/bundle/utah-platform-user-interface/page/use/using-lists/task/t_EditMultiRecUsingEditingForm.html

---

**👤 Ghouse1** ⬆️ 1

https://docs.servicenow.com/bundle/sandiego-servicenow-platform/page/product/knowledge-management/reference/r_KnowledgeRoles.html

---

**👤 bagoly** ⬆️ 1

**Selected Answer:** BD
BD correct

---

**👤 trye_indicators** ⬆️ 4

**Selected Answer:** BD
Knowledge contributors can create and edit knowledge articles within a knowledge base to share information across your organization.
https://docs.servicenow.com/bundle/sandiego-servicenow-platform/page/product/knowledge-management/task/create-knowledge-article.html

As a knowledge manager, you can assign other managers, define category structures, configure which users can read and contribute articles, move and pin articles, and modify most fields on the Knowledge Base form. You can also approve the publishing or retiring of knowledge articles in those knowledge bases.
https://docs.servicenow.com/bundle/sandiego-servicenow-platform/page/product/knowledge-management/concept/c_KnowledgeManager.html



</details>

---

## Question 31

### Which collaboration tool opens a sidebar and allows you to create new conversations with other ServiceNow users?

### Answer Choices:
**A.** Skype Now

**B.** Collaborate Now

**C.** Agent Messenger

**D.** Agent Chat

**E.** Connect Chat



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** E
**Community voting results:** E (67%), D (33%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 chicocheco** ⬆️ 12

Outdated. Replaced by Sidebar chat.

---

**👤 Ilovetests** ⬆️ 5

This is outdated. Connect Chat is going away and can't be used with the Next Experience Unified Navigation starting with SanDiego

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** E
In Washington it's called Sidebar, question is outdated

---

**👤 ArisGk** ⬆️ 2

**Selected Answer:** E
Connect Chat opens a sidebar and allows you to create new conversations with other ServiceNow users.

---

**👤 ee1039c** ⬆️ 1

**Selected Answer:** E
Connect Chat opens a sidebar and allows you to create new conversations with other ServiceNow users.
Agent Chat is used for real-time communication between agents and users but does not open a sidebar for creating new conversations.

---

**👤 BlackCarpet** ⬆️ 1

**Selected Answer:** E
Connected Chat

---

**👤 9e0bd16** ⬆️ 1

**Selected Answer:** E
it is E

---

**👤 naab143** ⬆️ 1

E. Connect Chat

---

**👤 Mihaf92** ⬆️ 4

it is E

---

**👤 NatusyaZira** ⬆️ 3

**Selected Answer:** D
D is correct: The ServiceNow® Conversational Interfaces features enable you to install, manage, and monitor your virtual and live agents to support your users through chat. Agent Chat enables live agents to interact directly with end users through chat and Sidebar enhances the ability for agents and subject matter experts to collaborate. Agent Chat and Sidebar were enhanced and updated in the Utah release.

*...and 1 more comments*


</details>

---

## Question 32

### What module would you use if you wanted to view a list of all of the fields on the Incident table? (Choose two.)

### Answer Choices:
**A.** Tables & Columns

**B.** Dictionary

**C.** Data Class Manager

**D.** Dictionary Dashboard

**E.** Database View

**F.** Schema



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AB
**Community voting results:** AB (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 Bog2022** ⬆️ 12

**Selected Answer:** AB
Schema is not a module

---

**👤 Sarvisetti** ⬆️ 2

AB is correct as there is no module named as Schema

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** AB
The details for fields are viewed using "Tables and Columns" or Dictionary modules.

---

**👤 Ghouse1** ⬆️ 2

Administrators can use these tools for viewing and modifying the database structure:

Tables module: Provides a list of all tables in the database.
Tables & Columns module: Provides a list of all existing tables, with columns, column attributes, and indexes.
Schema map: Provides a graphical representation of the relationships between tables.
Data dictionary tables: Contains additional information that defines database elements.

---

**👤 Markster** ⬆️ 3

**Selected Answer:** AB
A & B are correct imho

---

**👤 libelle88** ⬆️ 3

No "Schema" module in ServiceNow --> A B

---

**👤 akinz** ⬆️ 1

If it is schema Map , then the ans are correct.

---

**👤 Remo878** ⬆️ 2

provided ans is right

---

**👤 ANCANC** ⬆️ 4

A and B are correct



</details>

---

## Question 33

### What component causes a flow to run after a record has been created or updated?

### Answer Choices:
**A.** Date-based trigger

**B.** On-change trigger

**C.** Record-based trigger

**D.** Application-based trigger

**E.** Updated-date trigger



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (97%), 3%

</details>

<details>
<summary><strong>💬 View Discussion (18 comments)</strong></summary>

**👤 mona_hmmd** ⬆️ 8

**Selected Answer:** C
I think it is "Record-based Trigger", at the beginning of the article: https://docs.servicenow.com/bundle/rome-servicenow-platform/page/administer/flow-designer/reference/flow-triggers.html

it is mentioned the following:
"Use record triggers to start a flow when a record is created or updated".

---

**👤 s_KASKA** ⬆️ 1

**Selected Answer:** C
C is the right answer.

---

**👤 sithupraji** ⬆️ 1

Use record triggers to start a flow when a record is created or updated.

---

**👤 sithupraji** ⬆️ 1

Answer C

---

**👤 PinkAndBlack** ⬆️ 2

**Selected Answer:** C
The correct answer is C: https://docs.servicenow.com/bundle/utah-build-workflows/page/administer/flow-designer/reference/flow-triggers.html

---

**👤 Oluwalana54** ⬆️ 4

**Selected Answer:** C
We have 3 types of triggers, record (created or updated); date, and application Based triggers

---

**👤 Spiderman5000** ⬆️ 3

**Selected Answer:** C
Even the linked article in the "correct" answer states: "Use record triggers to start a flow when a record is created or updated."

---

**👤 Ranaway** ⬆️ 1

C looks like a correct answer

---

**👤 ainkechik** ⬆️ 1

**Selected Answer:** C
its either Scheduled-based, Application-based or Record-based trigger. So for this question the correct answer should be C

---

**👤 Bubu770** ⬆️ 2

Who set this CSA Questions is beyond this world, and then he wants us to pay for like horrible mistakes

*...and 8 more comments*


</details>

---

## Question 34

### What type of field is Boolean and appears as a check box?

### Answer Choices:
**A.** Yes/No

**B.** True/False

**C.** On/Off

**D.** Binary

**E.** 0/1



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 ba14741** ⬆️ 1

**Selected Answer:** B
true/false

---

**👤 Lavel** ⬆️ 1

**Selected Answer:** B
TRUE/FALSE

---

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** B
TRUE/FALSE

---

**👤 sephereth** ⬆️ 1

**Selected Answer:** B
B is the right answer



</details>

---

## Question 35

### Table Access Control rules are processed in the following order:

### Answer Choices:
**A.** any table name (wildcard), parent table name, table name

**B.** table name, parent table name, any table name (wildcard)

**C.** parent table name, table name, any table name (wildcard)

**D.** any table name (wildcard), table name, parent table name



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (14 comments)</strong></summary>

**👤 Remo878** ⬆️ 24

able ACL rules are processed in the following order:
Match the table name. For example, incident.
Match the parent table name. For example, task.
Match any table name (*). For example, *.

---

**👤 Ruby_2022** ⬆️ 14

most specific to most generously

---

**👤 wnstn75** ⬆️ 1

**Selected Answer:** B
1.ACL name matches table ? 
2.ACL name matches parent table ? 
3.ACL name matches *?

---

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** B
Correct answer

---

**👤 Kflo** ⬆️ 1

**Selected Answer:** B
https://www.servicenow.com/community/itsm-articles/acl/ta-p/2308303#:~:text=Table%20ACL%20Rule,evaluates%20the%20field%20ACL%20rules.

---

**👤 naab143** ⬆️ 2

C. parent table name, table name, any table name (wildcard)

Table Access Control (TAC) rules in ServiceNow are processed in the following order:

First, ServiceNow checks for TAC rules associated with the parent table name.
Then, it checks for TAC rules associated with the table name itself.
Finally, if no specific rules are found for the parent table or table name, any TAC rules associated with a wildcard (any table name) are applied.

---

**👤 Willmonzani** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Amit12345** ⬆️ 1

**Selected Answer:** B
B is correct!

---

**👤 saiakhilpalle** ⬆️ 1

B is correct

*...and 4 more comments*


</details>

---

## Question 36

### Which module is used to access the knowledge bases which are available to you?

### Answer Choices:
**A.** Knowledge > Home

**B.** Self Service > Knowledge

**C.** Knowledge > All

**D.** Knowledge > Knowledge Bases

**E.** Knowledge > Overview



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (19 comments)</strong></summary>

**👤 Ibeauo** ⬆️ 7

**Selected Answer:** B
I vote for B! I have checked on my Developer Instance, answer D is not correct, because

Knowledge > Knowledge Base 

doesn't exist, like OtterSpace has mentioned, available is 

Knowledge > Administration > Knowledge Bases (yes, plural)

---

**👤 gedgy** ⬆️ 3

**Selected Answer:** B
Per Washington DC version this is: Knowledge - Homepage - OR - Self-service - Knowledge

---

**👤 Certs_Hub** ⬆️ 1

Self service > Knowledge is the correct answer.

---

**👤 rcn8278** ⬆️ 1

Correct answer is B

---

**👤 MacGyver4th** ⬆️ 3

**Selected Answer:** B
As an admin, wouldn't you have access to all knowledge bases?

I think this question is asked from the perspective of a non-admin user. In that case, the answer would be B.

---

**👤 evoken** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Spiderman5000** ⬆️ 1

**Selected Answer:** B
"B" is correct as a user. "D" is correct if you wish to administer the articles.

---

**👤 Ranaway** ⬆️ 1

B should be the correct one

---

**👤 Faithye** ⬆️ 1

B is the right answer

---

**👤 foolishsoul4556** ⬆️ 1

**Selected Answer:** B
B is correct, D would have been correct if the question mentioned how to see all existing Knowledge bases within the system.

*...and 9 more comments*


</details>

---

## Question 37

### A customer requests the following data quality measures be added:
✑ Incident numbers should be read only, on all lists and forms, for all users.
✑ Short Description field should be mandatory, on all records, across all applications, on Insert.
Which type of policy would you use to meet this requirement?

### Answer Choices:
**A.** Data Quality Policy

**B.** Dictionary Design Policy

**C.** UI Data Policy

**D.** UI Policy

**E.** Field Criteria Policy

**F.** Data Policy



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** F
**Community voting results:** F (79%), D (21%)

</details>

<details>
<summary><strong>💬 View Discussion (21 comments)</strong></summary>

**👤 Mayur444** ⬆️ 2

Answer shud be D

---

**👤 Mayur444** ⬆️ 1

it is D

---

**👤 AJE68** ⬆️ 1

**Selected Answer:** F
Data Policies are similar to UI policies, but UI Policies only apply to data entered on a form through a standard browser. Since UI Policies can also manage the visibility of fields on a form, you may want to augment UI Policies with Data Policies.

---

**👤 gedgy** ⬆️ 1

**Selected Answer:** D
In short UI policies are frontend and data policies are backend.

---

**👤 PeterK0** ⬆️ 3

**Selected Answer:** D
Data Policies run regardless of how data is entered into ServiceNow, while UI Policies are used for form interactions.

That's why the correct answer is D.

---

**👤 CodingAmo** ⬆️ 4

**Selected Answer:** F
The second requirement says "on Insert". You can insert data from outside sources, not just on the form. So you need a data policy to do that.

---

**👤 rcn8278** ⬆️ 2

Correct answer is F

---

**👤 tonystark01** ⬆️ 2

Data policies enable you to enforce data consistency by setting mandatory and read-only states for fields.

Data policies are similar to UI policies, but UI policies only apply to data entered on a form through the standard browser. Data policies can apply rules to all data entered into the system, including data brought in through import sets or web services and data entered through the mobile UI.

---

**👤 Desired** ⬆️ 2

**Selected Answer:** F
It should be Data policy. The UI Policy is only applied on data on a form.

---

**👤 Mahe12** ⬆️ 3

Its UI Policy. Read the question carefully. 
Making fields to Read-only and mandatory comes under UI Policy

*...and 11 more comments*


</details>

---

## Question 38

### What type of user (persona) has clearly defined paths and workflows in the platform and have one or more roles (ie itil and approver_user)?

### Answer Choices:
**A.** Workflow User

**B.** Request Fulfiller

**C.** ITSM User

**D.** Approving Manager

**E.** Service Desk User

**F.** Process User



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** F
**Community voting results:** F (95%), 5%

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 Dela803** ⬆️ 8

Answer is F. 
Fulfiller/Process Users have random paths and workflows in the platform and have one or more roles, including itil and approver_user roles. They can access all functionality based on assigned roles.

---

**👤 NikaDMars** ⬆️ 1

**Selected Answer:** F
Process User is a defined persona in ServiceNow. These users are typically responsible for ITSM-related tasks such as Incident, Problem, and Change management. They:
* Have structured workflows defined by their assigned roles (e.g., itil, approver_user)
* Can access full functionality based on their roles
* Fulfill day-to-day operational processes

---

**👤 40a783b** ⬆️ 3

**Selected Answer:** F
F is correct answer. search in e-book

---

**👤 ddiggler** ⬆️ 2

F. Process Users have clearly defined paths and workflows in the platform and have one or more roles, including the itil and approver_user roles. They can access all functionality based on assigned roles.

---

**👤 CodingAmo** ⬆️ 1

**Selected Answer:** F
The Approving manager doesn't have to have any roles.

---

**👤 ThuHall** ⬆️ 1

**Selected Answer:** F
Process User

---

**👤 LovMi21** ⬆️ 1

**Selected Answer:** D
https://docs.servicenow.com/bundle/utah-now-intelligence/page/use/dashboards/task/request-bi-service.html

---

**👤 EAMTL** ⬆️ 3

**Selected Answer:** F
F according to SNAF Tokyo doc - page 74

---

**👤 bagoly** ⬆️ 3

**Selected Answer:** F
F is correct

---

**👤 mitashki** ⬆️ 4

**Selected Answer:** F
F is corrent

*...and 1 more comments*


</details>

---

## Question 39

### Which module would you use to customize your instances banner image, text and colors?

### Answer Choices:
**A.** System UI > UI Pages > Branding

**B.** Service Portal > Portals > Branding

**C.** System Properties > Basic Configuration UI16

**D.** System Properties > Branding

**E.** Homepage Admin > Pages > Branding



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
C - checked in PDI

---

**👤 SeekerShan** ⬆️ 2

**Selected Answer:** C
checked. it is C

---

**👤 naab143** ⬆️ 1

D. System Properties > Branding

---

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** C
U16!! C

---

**👤 YMijoe** ⬆️ 1

**Selected Answer:** C
I agree that "System Properties > Basic Configuration UI16" is correct

---

**👤 Mohammedhz** ⬆️ 1

Correct



</details>

---

## Question 40

### Which database provides a logical model of your company infrastructure by identifying, controlling, maintaining and verifying CIs that exist?

### Answer Choices:
**A.** IMDB

**B.** ITSM

**C.** CSDM

**D.** CMDB

**E.** LDAP



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Philippvs** ⬆️ 2

**Selected Answer:** D
https://www.servicenow.com/products/it-operations-management/what-is-cmdb.html

---

**👤 Ilovetests** ⬆️ 1

**Selected Answer:** D
D is correct



</details>

---

## Question 41

### Which module is used as the first step for importing data?

### Answer Choices:
**A.** Coalesce Data

**B.** Transform Data

**C.** Import Data

**D.** Load Data



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (22 comments)</strong></summary>

**👤 Spiderman5000** ⬆️ 21

**Selected Answer:** D
Type "Import Data" in the All menu. Nothing. Type "Load Data" in the All menu, bingo!

---

**👤 Mayur444** ⬆️ 1

D is the right one

---

**👤 ElifSurucu** ⬆️ 1

**Selected Answer:** D
1. Load data into a staging table
2. Create a Transform Map
3. Run a Transform to move data from the staging table to the target table
4. Check the data integrity

---

**👤 wer3ball** ⬆️ 1

D. Import Data

https://developer.servicenow.com/dev.do#!/learn/learning-plans/utah/new_to_servicenow/app_store_learnv2_importingdata_utah_data_import_process

---

**👤 Certs_Hub** ⬆️ 1

C "Import Data" is correct answer as per chatgpt. Below explanation.

In ServiceNow, the module used as the first step for importing data is:

B. Transform Data

Explanation:

Transform Data (Option B): The transformation process is often the initial step when importing data into ServiceNow. During this step, data is mapped and transformed to match the structure of the target tables in the ServiceNow instance.
The other options do play roles in the data import process:

Coalesce Data (Option A): Coalescing is often done during the transformation process to combine or merge data from different sources.

Import Data (Option C): After transformation, the import process is executed to load the data into the target tables.

Load Data (Option D): While not a specific module or step, the loading of data is part of the overall import process and typically follows the transformation and mapping steps.

---

**👤 Adam2023** ⬆️ 2

**Selected Answer:** D
There is no Import Data module, hence Load Data is correct.

---

**👤 ThuHall** ⬆️ 1

**Selected Answer:** D
D. Load Data

---

**👤 jaaaybourne** ⬆️ 1

**Selected Answer:** D
Load data is correct

---

**👤 uwbe** ⬆️ 2

**Selected Answer:** D
at least in Utah the menu item is All -> System Import Sets -> Load Data

---

**👤 Lilymich** ⬆️ 1

Load data

*...and 12 more comments*


</details>

---

## Question 42

### When you need to orchestrate business processes across services with little technical user knowledge, which utility would you use?

### Answer Choices:
**A.** Flow Manager

**B.** Flow Designer

**C.** Flow Editor

**D.** Workflow Editor

**E.** Workflow Designer



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (82%), D (18%)

</details>

<details>
<summary><strong>💬 View Discussion (25 comments)</strong></summary>

**👤 Cooper96** ⬆️ 23

Sorry for asking, but wondering why the rate of wrong answers are so high? It's pretty confusing showing totally wrong answers.

---

**👤 Barkles** ⬆️ 8

**Selected Answer:** B
Flow designer

---

**👤 73301d1** ⬆️ 1

**Selected Answer:** B
My answer will be flow designer
don't mix between Flow Designer and WorkFlow Editor

---

**👤 benarji** ⬆️ 2

B.
 Flow Designer is typically the most suitable utility. Flow Designer is designed to allow users to automate processes and create workflows through a user-friendly, low-code interface.

---

**👤 BrandyD** ⬆️ 2

Its B. Flow Designer

---

**👤 Mariina_003_B** ⬆️ 3

**Selected Answer:** B
The correct answer is:

B. Flow Designer

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** B
Flow Designer

---

**👤 buchibabe** ⬆️ 2

Definitely B - Page 236 of the ServiceNow Administration Fundamentals - Tokyo Edition. The question is spelt out verbatim.

---

**👤 uwbe** ⬆️ 1

**Selected Answer:** B
Flow Designer

---

**👤 Melll1972** ⬆️ 2

**Selected Answer:** B
Flow Designer.

*...and 15 more comments*


</details>

---

## Question 43

### When a user reports that they are not able to see modules on the application navigator, what can you do, to see what modules are visible to them?

### Answer Choices:
**A.** Look up their password, so you can login with their account

**B.** Initiate a Connect Chat session

**C.** Install the Bomgar plug-in

**D.** Impersonate the user

**E.** Launch a NowChat window



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (18 comments)</strong></summary>

**👤 George_Ch** ⬆️ 21

**Selected Answer:** D
Someone is joking right?

---

**👤 andresnow** ⬆️ 13

Wow the amount of questions that are wrong is amazing. Starting to not trust this website at all.

---

**👤 Simdiddy** ⬆️ 1

OMG the answer is definitely D!!
We are cooked people...we are cooked!!

---

**👤 shub606** ⬆️ 1

correct answer is D

---

**👤 3f8d6b1** ⬆️ 7

Guys, paying > 60 bucks for this, sorry to say, increasing bullshit after Question 80, is simply unacceptable.

---

**👤 jaaaybourne** ⬆️ 2

Lmao what is initiating a connect chat session going to do? Connect Chat is deprecated anyway

---

**👤 Lilymich** ⬆️ 1

wait isnt the answer D

---

**👤 FastAndy** ⬆️ 1

D is right

---

**👤 vrwjjr85** ⬆️ 4

B is CLEARLY incorrect. Initiating a chat session wont do much to solve the problem. Therefore D is the correct answer. It will be easier to diagnose why the user cant see modules

---

**👤 Spiderman5000** ⬆️ 4

**Selected Answer:** D
Ha! I laughed at this one. Clearly the answer is "D". I think whoever wrote the questions and answers is having a little fun with us.

*...and 8 more comments*


</details>

---

## Question 44

### What is a key difference between Reporting and Performance Analytics?

### Answer Choices:
**A.** Performance Analytics contains snapshots of data taken over time; Reporting shows only the data as it is, at the moment the report is run.

**B.** Performance Analytics can show trends; Reports cannot.

**C.** Reports can be run on a scheduled basis; Performance Analytics cannot.

**D.** Performance Analytics data can be published to Dashboards; Reports cannot.

**E.** Performance Analytics shows KPIs; Reporting does not.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 Adsilva** ⬆️ 11

**Selected Answer:** A
Correct is A 
https://docs.servicenow.com/bundle/rome-now-intelligence/page/use/performance-analytics/concept/pa-vs-reporting-pa.html

---

**👤 BrandyD** ⬆️ 1

https://docs.servicenow.com/bundle/washingtondc-now-intelligence/page/use/performance-analytics/concept/pa-vs-reporting-pa.html

---

**👤 Manny1218** ⬆️ 1

Both A and B is correct. Page 273 and 274 verifies both answers. ServiceNow Administration Fundamentals - Tokyo

---

**👤 NatusyaZira** ⬆️ 1

**Selected Answer:** A
Correct answer is A: https://docs.servicenow.com/en-US/bundle/vancouver-now-intelligence/page/use/performance-analytics/concept/pa-vs-reporting-pa.html

---

**👤 tonystark01** ⬆️ 1

A for sure

---

**👤 shizuka0323** ⬆️ 1

I think Answer is B.
https://www.scnsoft.com/blog/servicenow-performance-analytics-and-reporting

---

**👤 Bayoubeot** ⬆️ 2

**Selected Answer:** A
Reports can show trends I would think

---

**👤 Vinnyzworld** ⬆️ 1

Guess it could be both A & B

---

**👤 LUKE1992** ⬆️ 2

**Selected Answer:** A
I also think it is A.



</details>

---

## Question 45

### While showing a customer their incident form, they ask to change the Priority values to display their internal terminology P1, P2, P3, P4. They want it to be consistent across all Tasks. How would you do that?
Right click on Priority and select what?

### Answer Choices:
**A.** Configure Lists

**B.** Show Options

**C.** Configure Task

**D.** Show Choices

**E.** Show Choice List

**F.** Configure Options



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** E
**Community voting results:** E (94%), 6%

</details>

<details>
<summary><strong>💬 View Discussion (14 comments)</strong></summary>

**👤 dnbly** ⬆️ 6

**Selected Answer:** E
Correct would be "Configure Choices" or "Show Choice List", as Configure Choices isn't an option only E can be correct

---

**👤 NikaDMars** ⬆️ 1

**Selected Answer:** E
When a customer wants to change the labels of a choice field (like Priority) — for example, from "1 - Critical" to "P1" — you need to access the list of possible values (choices) defined for that field.

You do this by:
1) Opening a record with the field.
2) Right-clicking on the field label (e.g., "Priority").
3) Selecting “Show Choice List” — this displays the defined choices (value/label pairs).
4) Editing the labels or adding new ones.

---

**👤 Mayur444** ⬆️ 1

E is correct

---

**👤 GGSFSD** ⬆️ 2

**Selected Answer:** E
E is accurate

---

**👤 tonystark01** ⬆️ 2

E is accurate

---

**👤 Philippvs** ⬆️ 4

**Selected Answer:** E
I tested and it is "Show choice list".

---

**👤 icen45** ⬆️ 1

**Selected Answer:** F
E is correct

---

**👤 bagoly** ⬆️ 1

**Selected Answer:** E
E is correct here, "Configure Choices" is not among the answers

---

**👤 trye_indicators** ⬆️ 2

"Configure choice" is the right answer, but it is not provided and closest one is F
F. Configure Options

---

**👤 mitashki** ⬆️ 1

**Selected Answer:** E
There is no "Configure options"!

*...and 4 more comments*


</details>

---

## Question 46

### What is the platform name for the User table?

### Answer Choices:
**A.** u_users

**B.** sys_users

**C.** x_users

**D.** sys_user



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (99%), 1%

</details>

<details>
<summary><strong>💬 View Discussion (17 comments)</strong></summary>

**👤 encqizo** ⬆️ 43

It should be D sys_user, no sys_users in SNOW

---

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** D
Correct answer

---

**👤 servicenowExamtopics** ⬆️ 1

**Selected Answer:** D
The platform name for the User table in ServiceNow is sys_user. This table stores information about the users in the system.

---

**👤 yanchok** ⬆️ 1

**Selected Answer:** D
sys_user

---

**👤 floelynn** ⬆️ 1

**Selected Answer:** D
sys_user

---

**👤 Swecabo** ⬆️ 1

D is Correct: User records are stored in the Users [sys_user] table.

---

**👤 deadinside** ⬆️ 1

**Selected Answer:** D
bas hai

---

**👤 da4de64** ⬆️ 2

**Selected Answer:** D
D is correct

---

**👤 saivarun_bodla** ⬆️ 1

d is the corred answer

---

**👤 DeCharlie** ⬆️ 1

**Selected Answer:** D
sys_user

*...and 7 more comments*


</details>

---

## Question 47

### Your customer would like to create a new template to notify users who are affected by network outages at their site. Which module would you use to create a new notification?

### Answer Choices:
**A.** System Notification > Email > Notifications

**B.** Administration > Notification Overview

**C.** System Properties > Email > Settings

**D.** User Preferences > Email > Notifications

**E.** Click Gear > Notifications > New



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 naab143** ⬆️ 1

To create a new notification template to notify users affected by network outages at their site in ServiceNow, you would typically use:

B. Administration > Notification Overview

This module provides an overview of all notifications in the system and allows administrators to create, edit, and manage notification templates. From here, you can create a new notification template specifically tailored to notify users affected by network outages.

---

**👤 PeterK0** ⬆️ 1

**Selected Answer:** A
I use different path and always remove filter set on the list , but yeah it is A.

---

**👤 Seb2014** ⬆️ 2

**Selected Answer:** A
A:
https://docs.servicenow.com/bundle/utah-platform-administration/page/administer/notification/task/t_CreateANotification.html

---

**👤 YMijoe** ⬆️ 1

**Selected Answer:** A
I agree that "System Notification > Email > Notifications" is correct.



</details>

---

## Question 48

### When designing a flow, how do you reference data from a record, in that flow?

### Answer Choices:
**A.** Drag the table icon onto the flow definition

**B.** Use the condition builder to specify the desired values

**C.** Specify the source table on the data pill related list

**D.** Drag the data pill onto the flow definition

**E.** Add the table reference using the slush bucket



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** D
D DATA PILL

---

**👤 alokji** ⬆️ 1

D is the correct answer



</details>

---

## Question 49

### On the Reports page, what sections allow you to see which reports are visible to different audiences? (Choose four.)

### Answer Choices:
**A.** Group

**B.** Department

**C.** My reports

**D.** Team

**E.** Dashboards

**F.** Global

**G.** Admin

**H.** Analytics



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACFI
**Community voting results:** ACFI (100%)

</details>

<details>
<summary><strong>💬 View Discussion (15 comments)</strong></summary>

**👤 OtterSpace** ⬆️ 14

**Selected Answer:** ACFI
There are only 4 categories in the View/run module my reports, group, global, and all

---

**👤 paultenorio23** ⬆️ 4

**Selected Answer:** ACFI
In Vancouver latest release, it is "My Reports, Group, Global, and All"
It should be AFCI.

---

**👤 GoldBear** ⬆️ 3

**Selected Answer:** ACFI
My Reports, Group, Global, All

---

**👤 alokji** ⬆️ 2

ACFI is the correct answer.

---

**👤 PinkAndBlack** ⬆️ 2

**Selected Answer:** ACFI
See https://docs.servicenow.com/bundle/utah-now-intelligence/page/use/reporting/task/t_RunAReport.html

---

**👤 uwbe** ⬆️ 1

**Selected Answer:** ACFI
another outdated question. For Utah its ACFI

---

**👤 pielouis** ⬆️ 1

**Selected Answer:** ACFI
ACFI are correct

---

**👤 JonesSalkvist** ⬆️ 1

Would the answer not be:

A. Group
B. Department
D. Team
J. Company

---

**👤 FastAndy** ⬆️ 1

**Selected Answer:** ACFI
checked it w/ the page

---

**👤 Philippvs** ⬆️ 1

**Selected Answer:** ACFI
I just tested in the Reports View.

*...and 5 more comments*


</details>

---

## Question 50

### Which tool is used to define relationships between fields in an import set table and a target table?

### Answer Choices:
**A.** Transform Schema

**B.** Schema Map

**C.** Dictionary Map

**D.** Transform Map

**E.** Field Transformer

**F.** Import Designer



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 skibumvermont** ⬆️ 1

**Selected Answer:** D
https://www.servicenow.com/docs/csh?topicname=c_ImportSetsKeyConcepts.html&version=latest

---

**👤 naab143** ⬆️ 1

The tool used to define relationships between fields in an import set table and a target table in ServiceNow is:

D. Transform Map

---

**👤 buchibabe** ⬆️ 1

D is correct. Page 391 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 NokoNice** ⬆️ 1

D is the right answer.



</details>

---

## Question 51

### Which ServiceNow capability provides assistance to help users obtain information, make decisions, and perform common work tasks via a messaging interface?

### Answer Choices:
**A.** Agent Workspace

**B.** Chat bot

**C.** Virtual Agent

**D.** Knowledge Chat

**E.** Now Support



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Lavel** ⬆️ 2

**Selected Answer:** C
C is correct

---

**👤 buchibabe** ⬆️ 2

C is correct. page 259 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 HimanPatel** ⬆️ 1

C is correct.



</details>

---

## Question 52

### Which feature allows you to automate business logic for a particular application or process such as approvals, tasks notifications, and record operations?

### Answer Choices:
**A.** Flows

**B.** Action Sequences

**C.** Action Sets

**D.** Task Flows

**E.** Flow Diagrams



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** A
A FLOws

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** A
Flows is the correct answer.

---

**👤 Nasman096** ⬆️ 1

**Selected Answer:** A
Workflow



</details>

---

## Question 53

### From a form, what would you click to add additional fields to the form? (Choose two.)

### Answer Choices:
**A.** Context Menu > Form > Layout

**B.** Context Menu > Configure > Form Layout

**C.** Context Menu > Configure > Form Design

**D.** Right click on header > Add > Field

**E.** Context Menu > Form > Designer

**F.** Right click on header > Configure > UX Dashboard



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BC
**Community voting results:** BC (100%)

</details>

<details>
<summary><strong>💬 View Discussion (13 comments)</strong></summary>

**👤 jjn9306** ⬆️ 1

**Selected Answer:** BC
The second half of these questions have the majority of answers WRONG

---

**👤 Susira** ⬆️ 1

BC... Most of the questions got wrong answweres

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** BC
Form Layout and Form Design are the correct answers.

---

**👤 ehuehifush** ⬆️ 1

ummm ikke veldig cash money

---

**👤 lallenstein** ⬆️ 2

Why are there not more incorrect answers? Boring.

---

**👤 Luchito** ⬆️ 2

**Selected Answer:** BC
I just confirmed this on the test dev instance. These are the only options to available to complete the task as described in the question specifically.

---

**👤 tejareddy** ⬆️ 1

B and C are corect.

---

**👤 Spiderman5000** ⬆️ 2

**Selected Answer:** BC
Maybe in Rome the answers are "B, D", but in Tokyo, "B, C".

---

**👤 Philippvs** ⬆️ 1

**Selected Answer:** BC
https://docs.servicenow.com/bundle/tokyo-it-service-management/page/product/change-management/task/t_CreateCustomField.html

---

**👤 Philippvs** ⬆️ 1

**Selected Answer:** BC
Det riktige svaret er B og C.

*...and 3 more comments*


</details>

---

## Question 54

### What is the name of the table relationship, where two or more tables are related in a bi-directional relationship, so that the related records are visible from both tables in a related list?

### Answer Choices:
**A.** Database View

**B.** Many to Many

**C.** One to Many

**D.** Extended



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 tejareddy** ⬆️ 1

B correct

---

**👤 NokoNice** ⬆️ 1

B is correct

---

**👤 RedWater** ⬆️ 3

B is the right answer



</details>

---

## Question 55

### On a Form header, what is the three bar icon called?

### Answer Choices:
**A.** Pancake icon

**B.** Additional Actions or Context Menu

**C.** Hamburger icon

**D.** Cake icon



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 Frost312321** ⬆️ 1

**Selected Answer:** B
From the ebook, section 3.1: List Context (or control) menus, (also sometimes called Additional Actions when in a form) can be accessed from lists, columns, or on records by using right-click menus which provide different levels of controls:

---

**👤 rcn8278** ⬆️ 1

C is the correct answer.

---

**👤 PinkAndBlack** ⬆️ 3

**Selected Answer:** B
Check here: https://docs.servicenow.com/bundle/utah-platform-user-interface/page/use/using-forms/concept/c_FormContextMenu.html

---

**👤 NokoNice** ⬆️ 1

B is the correct answer.

---

**👤 anku15** ⬆️ 2

C is the correct answer. In ServiceNow, the three-bar icon on a Form header is commonly referred to as the "hamburger menu" icon.

---

**👤 evoken** ⬆️ 3

B, but using using the icon we all know to be the hamburger.



</details>

---

## Question 56

### Group records are stored in which table?

### Answer Choices:
**A.** Group [sn_user_group]

**B.** Group [sys_user_group]

**C.** Group [s_sys_group]

**D.** Group [u_sys_group]



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Lavel** ⬆️ 1

**Selected Answer:** B
B is correct.

---

**👤 Foodiepanda** ⬆️ 1

**Selected Answer:** B
Group [sys_user_group] is correct answer

---

**👤 NokoNice** ⬆️ 1

B is the right answer



</details>

---

## Question 57

### A REQ number in the Service Catalog represents`¦

### Answer Choices:
**A.** the order number.

**B.** the stage.

**C.** the task to complete.

**D.** the individual item in the order.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 floelynn** ⬆️ 2

**Selected Answer:** A
REQ is the order number. multiple RITM (Requested Item) can be under one REQ

---

**👤 40a783b** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 HNTrinh** ⬆️ 1

**Selected Answer:** A
A is correct.

---

**👤 saiakhilpalle** ⬆️ 1

A is correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Bonche** ⬆️ 1

A is the correct answer

---

**👤 S_040** ⬆️ 1

Correct

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 58

### What function do you use to add buttons, links, and context menu items on forms and lists?

### Answer Choices:
**A.** UI Policies

**B.** UI Settings

**C.** UI Actions

**D.** UI Config



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Lavel** ⬆️ 2

**Selected Answer:** C
UI Actions is right

---

**👤 Dekoreh** ⬆️ 2

**Selected Answer:** C
C is correct



</details>

---

## Question 59

### On a Business Rule, the When setting determines at what point the rule executes. What are the options for specifying that timing?

### Answer Choices:
**A.** Before, After, Async, Display

**B.** Prior to, Synchronous, on Update

**C.** Insert, Update, Delete, Query

**D.** Before, Synchronous, Scheduled Job, View



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (83%), C (17%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 benarji** ⬆️ 1

A. Before, After, Display, Async.

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** A
A is the correct answer for "timing"
C is the "action" the rule will perform

---

**👤 Manny1218** ⬆️ 1

**Selected Answer:** A
In the "ServiceNow Administration Fundamentals - Tokyo" booklet, see pages, 438. It states Before, After, Async and Display

---

**👤 alokji** ⬆️ 1

A. Before, After, Async, Display is the correct answer

efore After the user submits the form but before any action is taken on the record in the database.
After After the user submits the form and after any action is taken on the record in the database.
Async When the scheduler runs the scheduled job created from the business rule. The system creates a scheduled job from the business rule after the user submits the form and after any action is taken on the record in the database.
Note: Newly created business rules will run during upgrades.
Display Before the form is presented to the user, just after the data is read from the database.

---

**👤 KevFun** ⬆️ 2

The ones who gave C as answer: https://docs.servicenow.com/bundle/tokyo-application-development/page/script/business-rules/concept/c_BusinessRules.html
 Read again the section 'Time the business rule should run'.

---

**👤 SamIV444** ⬆️ 2

A is the correct answer.

---

**👤 Tech_Baddie509** ⬆️ 3

The answer is A. Scroll down to the section title "Time the business rule should run" the answers will be there https://docs.servicenow.com/bundle/tokyo-application-development/page/script/business-rules/concept/c_BusinessRules.html

---

**👤 Shakir2023** ⬆️ 2

**Selected Answer:** A
Question related to "Time the business rule should run" and not "Database operation that the system takes on the record".

---

**👤 RGGG** ⬆️ 2

A is correct, i tested it.

---

**👤 alpena53** ⬆️ 1

**Selected Answer:** C
https://docs.servicenow.com/bundle/tokyo-application-development/page/script/business-rules/concept/c_BusinessRules.html

*...and 2 more comments*


</details>

---

## Question 60

### What are different types of Data Sources, which may be imported into ServiceNow? (Choose four.)

### Answer Choices:
**A.** Local Sources (i.e. XML, CSV, Excel)

**B.** Implementation Spoke

**C.** DataHub

**D.** JDBC Connection

**E.** Network Server

**F.** LDAP Connection



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ADEF
**Community voting results:** ADEF (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 GoldBear** ⬆️ 2

**Selected Answer:** ADEF
Data Sources are records in ServiceNow that contain information regarding an Import Set data source. You can import data from a local source (e.g. , XML, CSV, Excel ) or from a network server by providing a path and authentication information. A data source can come from a file, a Lightweight Directory Access Protocol (LDAP) connection, or a Java Database Connect ivi ty (JDBC) connection.

---

**👤 Manny1218** ⬆️ 3

**Selected Answer:** ADEF
From the ServiceNow Administration Fundamentals - Tokyo, page 389, it states Local source, Network server, LDAP, and Java (JDBC)

---

**👤 cluelessdood** ⬆️ 3

**Selected Answer:** ADEF
Taken from the training:

An Import Set is used to import data from various data sources and then map that data into ServiceNow tables.

You can import data from:
• A local source file (i.e. XML, CSV, Excel)
• A network server by providing a path and authentication information
• An LDAP connection
• A JDBC connection

---

**👤 Dekoreh** ⬆️ 1

**Selected Answer:** ADEF
Based on SN ebook.

---

**👤 dnbly** ⬆️ 1

Maybe an issue with the way the answer is written for this one as Data Stream Integration Hub is a Utah feature and would be correct however it is written as DataHub (which doesn't exist). Network server should probably reference REST API as that is where the data actually get's imported from after the API retrieves it from the server but is technically correct despite not mentioning REST.

---

**👤 buchibabe** ⬆️ 3

ADEF - Page 389 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 al3xunder** ⬆️ 2

ACDF

A. Local Sources (i.e. XML, CSV, Excel): ServiceNow allows data to be imported from files in various formats, including XML, CSV, and Excel files. These files can be stored locally on the user's computer or on a network drive.

D. JDBC Connection: ServiceNow also supports the import of data from databases using JDBC (Java Database Connectivity) connections. This enables data to be imported from a wide range of database types, such as Oracle, MySQL, and SQL Server.

F. LDAP Connection: ServiceNow can also import data from LDAP (Lightweight Directory Access Protocol) directories, which are commonly used to store user and organizational data. This allows user and group data to be imported into ServiceNow from an LDAP directory.

C. DataHub: ServiceNow's DataHub allows data to be imported from external sources, such as cloud-based applications, third-party databases, and other enterprise systems. This enables ServiceNow to act as a central hub for data integration across an organization.

---

**👤 Sallex** ⬆️ 1

**Selected Answer:** ADEF
https://docs.servicenow.com/bundle/tokyo-platform-administration/page/administer/import-sets/concept/c_ImportDataUsingImportSets.html

---

**👤 Philippvs** ⬆️ 1

**Selected Answer:** ADEF
https://docs.servicenow.com/bundle/tokyo-platform-administration/page/administer/import-sets/concept/c_ImportDataUsingImportSets.html

---

**👤 Ghouse1** ⬆️ 3

Local source, network server, LDAP, JDBC

*...and 2 more comments*


</details>

---

## Question 61

### What are the components that make up a filter condition? (Choose three.)

### Answer Choices:
**A.** Operator

**B.** Match Criteria

**C.** Value

**D.** Column

**E.** Field



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACE
**Community voting results:** ACE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 wer3ball** ⬆️ 1

A C E
https://docs.servicenow.com/bundle/utah-platform-user-interface/page/use/common-ui-elements/concept/c_ConditionBuilder.html

---

**👤 Philippvs** ⬆️ 2

**Selected Answer:** ACE
https://docs.servicenow.com/en-US/bundle/tokyo-platform-user-interface/page/use/using-lists/concept/c_Filters.html



</details>

---

## Question 62

### When impersonating a user for testing purposes, what is the best way to return the instance, logged in with your user account?

### Answer Choices:
**A.** Turn your computer off and on again

**B.** Clear browser cache

**C.** End Impersonation

**D.** Log out and back in



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (80%), D (20%)

</details>

<details>
<summary><strong>💬 View Discussion (17 comments)</strong></summary>

**👤 MaDSnow** ⬆️ 1

C. End Impersonation

---

**👤 ddiggler** ⬆️ 3

C. You only have to logout and back in if your using Elevated role such as security_admin

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** C
End impersonation

---

**👤 JonesSalkvist** ⬆️ 1

**Selected Answer:** C
It's end impersonation

---

**👤 Jana08** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** C
C is coreect

---

**👤 YMijoe** ⬆️ 2

**Selected Answer:** C
I agree that "Ending the impersonation" is the correct answer.

---

**👤 it0m_man** ⬆️ 1

**Selected Answer:** C
C is the correct answer.

---

**👤 timothy_moore** ⬆️ 1

Answer is C

---

**👤 it0m_man** ⬆️ 1

Answer is C

*...and 7 more comments*


</details>

---

## Question 63

### What controls the publishing and retiring process for knowledge articles?

### Answer Choices:
**A.** Approval Policies

**B.** Approval Definitions

**C.** Workflow Designer

**D.** Workflows

**E.** State Lifecycle



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 GoldBear** ⬆️ 1

**Selected Answer:** D
https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/knowledge-management/reference/r_KnowledgeWorkflows.html

---

**👤 Vishy123** ⬆️ 1

D is correct answer

---

**👤 RedWater** ⬆️ 3

"The publishing and retirement processes for a knowledge article are controlled by workflows defined for the knowledge base that the article belongs to."



</details>

---

## Question 64

### What type of query allows you to filter list data using normal words, instead of the condition builder?

### Answer Choices:
**A.** Natural Language Query

**B.** Alexa Query

**C.** Machine Learning Query

**D.** Predictive Intelligence Query

**E.** Auto-suggest Query



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 s_KASKA** ⬆️ 1

**Selected Answer:** A
The right answer is A. Natural query language (NQL) allows you to type in your query without building any condition.

---

**👤 timothy_moore** ⬆️ 3

Natural Language Query only exists in Utah release



</details>

---

## Question 65

### Tables may have a One to Many relationships. From the Service Catalog, what are examples of tables having a one to many relationships? (Choose three.)

### Answer Choices:
**A.** One Approval can have many Requests

**B.** One Request can have many Requested Items

**C.** One Requested Item can have many Approvals

**D.** One Requested Item can have many Catalog Tasks

**E.** One Cart can have many Requests



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BCD
**Community voting results:** BCD (69%), BDE (31%)

</details>

<details>
<summary><strong>💬 View Discussion (13 comments)</strong></summary>

**👤 BlackCarpet** ⬆️ 1

**Selected Answer:** BDE
BDE is the correct answer

---

**👤 047a2d5** ⬆️ 1

If you looking to the requested item you will see below the request catalog tasks, and approvers and Group approvals, that's means ensure C is correct, each requested item can have many approvals.

---

**👤 DDD1377** ⬆️ 2

BCD. The cart has not been submitted yet to even create the request number, so how can there be more than one request once submitted?

---

**👤 gedgy** ⬆️ 2

BDE...I am not sure how C is relevant. Perhaps if it said "One request can have many approvals", meaning the cart had multiple items which required many approvals.

---

**👤 lica09** ⬆️ 3

**Selected Answer:** BDE
How C is correct ? i dont understand . and what about option E ? one cart can definitely have many request .

---

**👤 san85** ⬆️ 1

**Selected Answer:** BCD
BCD is correct

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** BCD
BCD are the correct choice.

---

**👤 Sallex** ⬆️ 1

**Selected Answer:** BCD
should be BCD

---

**👤 lu_ming** ⬆️ 2

**Selected Answer:** BCD
I think B, C, D is the correct answer

---

**👤 Vishy123** ⬆️ 1

BCD is correct

*...and 3 more comments*


</details>

---

## Question 66

### On a filter condition, which component is always a choice list?

### Answer Choices:
**A.** Operator

**B.** Filter Criteria

**C.** Operation

**D.** Match Criteria



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 Dela803** ⬆️ 9

Answer is A. Operator is a choice list based on the field type. For example: is, is not, is one of, greater than, is different, is empty string, on, after, starts with etc

---

**👤 luishuallpa** ⬆️ 1

Hi, So correct answer for exam is C, but the correct answer is A, right??

---

**👤 ddiggler** ⬆️ 4

The incorrect answers are frustrating. This is just blatant.

---

**👤 BrandyD** ⬆️ 1

Its A - Operator . See - https://docs.servicenow.com/bundle/washingtondc-platform-user-interface/page/use/common-ui-elements/concept/c_ConditionBuilder.html

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
Answer is A

---

**👤 Spiderman5000** ⬆️ 2

**Selected Answer:** A
Answer is "A". Even the "correct" answer linked document states Operator (not Operation).

Operator: a choice list based on the field type. For example, in the Incident [incident] table, the greater than operator does not apply to the Active field but it does apply to the Priority field.

---

**👤 Vishy123** ⬆️ 1

A is correct

---

**👤 spradeep233** ⬆️ 1

A is the answer

---

**👤 Ranaway** ⬆️ 1

Operation is not a valid answer, so it should be A

---

**👤 Nasman096** ⬆️ 3

**Selected Answer:** A
https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/use/using-lists/concept/c_Filters.html

*...and 1 more comments*


</details>

---

## Question 67

### A Role is defined as what?

### Answer Choices:
**A.** A collection of permissions

**B.** A set of user access policies

**C.** A Persona in a workflow

**D.** A set of access control rules



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 daniuja** ⬆️ 10

**Selected Answer:** A
A - is the correct answer according to SN fundamentals training. "A role is a collection of permissions in the Now platform".

---

**👤 Dela803** ⬆️ 6

Answer is A. 
A Role is a collection of permissions in the now platform to grant access to applications and other parts of the platform.

---

**👤 DDD1377** ⬆️ 1

A is correct

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** A
SNAF - page 76 "A role is a collection of permissions."

---

**👤 val7223** ⬆️ 1

**Selected Answer:** A
A Role is defined as what?
A. A collection of permissions

The response is A , update the questionnaire.please

---

**👤 Vishy123** ⬆️ 1

A is correct

---

**👤 Ranaway** ⬆️ 1

it must be A, role is not ACL



</details>

---

## Question 68

### Which would NOT appear in the History section of the Application Navigator?

### Answer Choices:
**A.** Records

**B.** UI Pages

**C.** Lists

**D.** Forms



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 S_040** ⬆️ 6

Records are not tracked

---

**👤 gbongain** ⬆️ 1

A has to be the correct one. If you search for a UI page, list or form, then when you go to history those will appear. The record is the table's entry and is not seen (tracked) individually but as part of the many entries in the table.

---

**👤 Mikias** ⬆️ 1

D is correct

---

**👤 HNTrinh** ⬆️ 1

**Selected Answer:** B
UI pages and other non-standard interfaces not tracked

---

**👤 KIKI_AI** ⬆️ 3

**Selected Answer:** B
B is correct

---

**👤 Bonche** ⬆️ 1

B is the correct answer

---

**👤 Ausias18** ⬆️ 1

B is correct

---

**👤 bharatns** ⬆️ 4

Correct. UI pages and other non-standard interfaces not tracked



</details>

---

## Question 69

### A user is complaining that they are seeing a blank page, when they click Create New, from your custom Inventory application. You have confirmed that they can see the Inventory application, and the Create New module on the application navigator. What could be the cause of this issue?

### Answer Choices:
**A.** Create New module has a broken link

**B.** Known intermittent issue with UI15

**C.** User should be using Chrome instead of Explorer for their browser

**D.** User has read role, but not the write role on the Inventory table

**E.** User session has timed out



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (57%), E (43%)

</details>

<details>
<summary><strong>💬 View Discussion (13 comments)</strong></summary>

**👤 NikaDMars** ⬆️ 1

**Selected Answer:** D
In ServiceNow, when a user clicks "Create New" on a module, the platform tries to open a form for creating a new record in the associated table. If the user does not have the write role (permission) for that table, the form cannot be rendered, resulting in a blank page.

In this scenario:
* The user can see the application and module (they have read access).
* But they can't create a new record, because they lack write permissions on the Inventory table.

---

**👤 Mayur444** ⬆️ 1

D is correct

---

**👤 ddiggler** ⬆️ 1

D is correct

---

**👤 ddiggler** ⬆️ 1

There would most likely be an error displayed indicating the user session did timeout.

---

**👤 tp_nah** ⬆️ 1

D is correct

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** D
D is the closest thing to a correct answer.

---

**👤 dnbly** ⬆️ 4

**Selected Answer:** E
I would prefer to vote for none of the above but I have to vote for E as it is not quite accurate anymore but the wording is ok. Some other CSA answers are incorrect because the wording uses the wrong terms and D says "write" instead of "create", which makes this question literally incorrect.

---

**👤 Jana08** ⬆️ 2

**Selected Answer:** D
D is correct, user will forced to login if their session times out - E makes no sense.

---

**👤 Spiderman5000** ⬆️ 2

**Selected Answer:** E
Is there a "none of the above" choice? "E" isn't correct anymore. And "D" is not quite accurate. The user would need the ability to "create", not "write". Not sure why ServiceNow decided to stray from CRUD. Maybe they like CRWD better?

---

**👤 Ranaway** ⬆️ 1

E doesn't make any sense. D is correct

*...and 3 more comments*


</details>

---

## Question 70

### What resource can you use to view details of the tables and configuration items (CIs) associated with a particular use case?

### Answer Choices:
**A.** Scenario Dashboard

**B.** CI Use Case Modeler

**C.** CMDB Use Case Modeler

**D.** Common Service Data Model (CSDM) product view



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (50%), C (50%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 zakiyy** ⬆️ 1

**Selected Answer:** C
Should be C . The CMDB Use Case Modeler is specifically designed to help you view details of tables and CIs, and how they relate to business use cases (like Incident Management, Change Management, etc.).

CSDM Product View refers to a high-level reference model or framework — it's a documentation resource, not an interactive tool.

---

**👤 DDD1377** ⬆️ 1

D is correct

---

**👤 FOJ** ⬆️ 1

**Selected Answer:** D
Common Service Data Model (CSDM) product view is the resource used to view details of the tables and configuration items (CIs) associated with a particular use case in ServiceNow. The CSDM provides a framework for organizing, defining, and visualizing an organization's services and related data. It helps to provide a common language and structure to facilitate communication between IT and business stakeholders, as well as to ensure that data is consistent and accurate across the organization.



</details>

---

## Question 71

### A manager wants to view a snapshot of month-end Sales performance data, as compared to Sales targets. In addition, the manager wants to be able to see those monthly numbers trended over time, and forecasted into the future. What capability do you suggest for this manager?

### Answer Choices:
**A.** Scheduled Reports, a custom snapshot table, and a Trend report

**B.** Scheduled Reports and Excel

**C.** Scheduled Reports, a custom snapshot table, and a Projection report

**D.** Performance Analytics

**E.** Key Performance Indicators



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (86%), 14%

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 NikaDMars** ⬆️ 1

**Selected Answer:** D
Performance Analytics (PA) is the right tool for this scenario because it enables:
+ Snapshots of historical performance data
+ Trend analysis over time
+ Forecasting and projections into the future
+ Comparison of actual performance against targets (goals/KPIs)

PA goes beyond standard reporting by allowing real-time dashboards, interactive scorecards, and time-series data analysis, which is ideal for management-level performance tracking like sales progress.

---

**👤 32f51ec** ⬆️ 1

**Selected Answer:** D
Okay the people choosing B are deadass being boomers about this question.

---

**👤 DDD1377** ⬆️ 1

D, PA captures data overtime and is used for trending.

---

**👤 benarji** ⬆️ 1

D. Monthly numbers trended over time, and forecasted into the future can be reached only via Performance Analytics.

---

**👤 oswaldorusb** ⬆️ 1

**Selected Answer:** D
D is correct. PA

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** D
Monthly numbers trended over time, and forecasted into the future can be reached only via Performance Analytics.

---

**👤 Vinoth9289** ⬆️ 2

**Selected Answer:** D
D is correct https://docs.servicenow.com/en-US/bundle/vancouver-now-intelligence/page/use/performance-analytics/concept/pa-snapshots.html

---

**👤 judyt35** ⬆️ 1

**Selected Answer:** B
What is the correct answer? Shouldn't be "B"?

---

**👤 Kev06** ⬆️ 1

Snapshot - isn't this part of reporting functionality? Not sure why the Answer is D



</details>

---

## Question 72

### What are advantages of using Flow Designer? (Choose three.)

### Answer Choices:
**A.** Supports advanced developers

**B.** Enables complicated scripting

**C.** Reduces technical debt

**D.** Less manual scripting

**E.** Smooth integration with 3rd party systems



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** CDE
**Community voting results:** CDE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Dela803** ⬆️ 6

Correct answer is C, D & E

---

**👤 morbidmama** ⬆️ 1

https://www.servicenow.com/community/workflow-automation-forum/what-are-advantages-of-using-flow-designer/td-p/2760662

---

**👤 BrandyD** ⬆️ 1

I agree with C, D and E - https://www.servicenow.com/products/platform-flow-designer.html?state=seamless

---

**👤 BenBose** ⬆️ 3

I think the answer given is correct A,D & E. The impact on technical debt will vary on a case by case basis hence it is possible for Flow Designer to have a positive or negative impact on technical debt depending on what is being developed and what compromises are being conceded when opting for a Flow Designer solution.

---

**👤 Vishy123** ⬆️ 1

CDE correct

---

**👤 oliver_k** ⬆️ 1

**Selected Answer:** CDE
Correct c,d,e

---

**👤 ALDADI** ⬆️ 1

**Selected Answer:** CDE
Correct answer is C, D & E

---

**👤 Ghouse1** ⬆️ 3

**Selected Answer:** CDE
The answer is C, D, and E



</details>

---

## Question 73

### Your customer requires that they be able to monitor which users are performing impersonations in their instance. What would you do to meet that requirement?

### Answer Choices:
**A.** Add the role Log Write [sn_log_write] to the Impersonator Group

**B.** Create user update set for impersonation tracking

**C.** Activate the glide.sys.log_impersonation prop

**D.** From User icon, select Elevate Roles

**E.** On the Impersonator role record, right click and select Create Log



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Manny1218** ⬆️ 2

**Selected Answer:** C
Read the ServiceNow Administration Fundamentals - Tokyo booklet, page 12. States, the sys_property glide.sys.log_impersonation needs to be added and set to true in order to see events

---

**👤 ALDADI** ⬆️ 2

**Selected Answer:** C
C is correct

---

**👤 Nasman096** ⬆️ 2

**Selected Answer:** C
Impersonation logging for interactive sessions
Interactive sessions are performed through the user interface (UI). Enable or disable impersonation logging for interactive sessions using the glide.sys.log_impersonation property.

If you enable impersonation logging for interactive sessions by setting glide.sys.log_impersonation to true, all interactive sessions are recorded in the impersonate log.



</details>

---

## Question 74

### When a flow runs an action, it generates a runtime value, which stays the same for the duration of the flow. What is the name of this runtime value?

### Answer Choices:
**A.** Trigger runtime value

**B.** Sequence runtime value

**C.** Starting runtime value

**D.** Data pill runtime value

**E.** Input runtime value



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Ghouse1** ⬆️ 5

low designers use action result data pills to provide input for other flows, actions, or subflows. Flow designers can use the sequence value in the data pill name to ensure that they select the correct data pill as an input value. When a flow runs an action, it generates the data pill runtime value, which remains the same for the duration of the flow. For example, if a data pill for [Trigger->Incident record] gets populated with incident record values at the start of a flow, the data pill preserves these values for the rest of the flow.

---

**👤 buchibabe** ⬆️ 2

D is correct. Page 235 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 Ranaway** ⬆️ 1

D is the correct answer



</details>

---

## Question 75

### The wait time for end users is based on the round-trip between the client and the server. What activities are included in the round-trips?

### Answer Choices:
**A.** Request + Response

**B.** Save + Update

**C.** Write + Read

**D.** Submit + Query

**E.** Insert + Verify



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 buchibabe** ⬆️ 2

A is correct. Page 433 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 YMijoe** ⬆️ 1

**Selected Answer:** A
I agree that "Request + Response" is correct.



</details>

---

## Question 76

### When importing data, what happens to imported rows, if no coalesce field is specified?

### Answer Choices:
**A.** All rows are rejected from the import, as coalesce field is required.

**B.** All rows are treated as new records. No existing records are updated.

**C.** Duplicate rows are rejected from the import.

**D.** All rows are treated as new records, but errors will be flagged in the import log.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Dela803** ⬆️ 10

Correct answer is B. If no coalesce is defined, all imported rows are treated as new records. No existing records are updated.

---

**👤 MaDSnow** ⬆️ 1

Correct Answer is B

---

**👤 buchibabe** ⬆️ 2

B is correct. Page 392 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 1

B is correct

---

**👤 Vishy123** ⬆️ 1

B is correct answer

---

**👤 Ranaway** ⬆️ 1

Answer is B. If no coalesce is defined, all imported rows are treated as new records

---

**👤 Ghouse1** ⬆️ 4

**Selected Answer:** B
If no coalesce is defined, all imported rows are treated as new records. No existing records are updated.

---

**👤 bagoly** ⬆️ 4

**Selected Answer:** B
B is correct



</details>

---

## Question 77

### What is the most common role that has access to almost all platform features, functions, and data?

### Answer Choices:
**A.** Security Admin [security_admin]

**B.** Sys Admin [sys_admin]

**C.** Admin [sn_admin]

**D.** System Administrator [admin]

**E.** Base Admin [base_admin]



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 PappyFox** ⬆️ 1

D is correct

---

**👤 Spiderman5000** ⬆️ 4

**Selected Answer:** D
OOB there is no "sn_admin" role, but there is an "admin" role.

---

**👤 Ranaway** ⬆️ 1

D is correct. C doesn't exist

---

**👤 Nasman096** ⬆️ 1

**Selected Answer:** D
admin: A user with the admin (administrator) role in ServiceNow has permission to perform almost all functions. The term "admin" usually includes both the global admin and the admin for a given application.

---

**👤 Ilovetests** ⬆️ 2

**Selected Answer:** D
D is correct
The name of the role is 'admin' and the description for this role is "The System Administrator role. This role has access to all system features, functions, and data, regardless of security constraints. ''Grant this privilege carefully.'' If you have sensitive information, such as HR records, that you need to protect, you must create a custom '''admin '''role for that area and train a person authorized to see those records to act as the administrator"

---

**👤 Ghouse1** ⬆️ 1

**Selected Answer:** D
System Administrator [admin]

---

**👤 Magic_Carpet** ⬆️ 1

Is C the correct answer?



</details>

---

## Question 78

### What feature do you use to specify which users are able to access a Service Catalog Item?

### Answer Choices:
**A.** Can Read Role

**B.** Catalog User Role

**C.** Can Order Tab

**D.** User Criteria



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 ba14741** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 wer3ball** ⬆️ 1

D. It's under Catalog Security: User Criteria for Items and Categories

---

**👤 PappyFox** ⬆️ 2

D is correct



</details>

---

## Question 79

### Which one of the following statements is a recommendation from ServiceNow about Update Sets?

### Answer Choices:
**A.** Avoid using the Default Update set as an Update Set for moving customizations from instance to instance

**B.** Before moving customizations from instance to instance with Update Sets, ensure that both instances are different versions

**C.** Use the Baseline Update Set to store the contents of items after they are changed the first time

**D.** Once an Update Set is closed as ג€Completeג€, change it back to ג€In Progressג€ until it is applied to another instance



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** A
Correct answer

---

**👤 floelynn** ⬆️ 2

**Selected Answer:** A
The default update set captures changes made to the instance without adding those changes to any user-created update sets. It is recommended to avoid using the Default Update Set for moving customizations between instances. Instead, use a named Update Set (user-created).

---

**👤 rahu4l2321** ⬆️ 1

A is correct answer.

---

**👤 c5c4d3d** ⬆️ 3

A is the correct answer
D. is not the correct answer because:
Mark an update set as Complete only when it is ready to migrate. Once an update set is complete, do not change it back to In progress. Instead, create another update set for the rest of the changes, and be sure to commit them together in the order that they were created

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 saiakhilpalle** ⬆️ 1

A is correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** A
Create custom update set

---

**👤 Bonche** ⬆️ 1

A is the correct answer

---

**👤 S_040** ⬆️ 1

Correct

*...and 2 more comments*


</details>

---

## Question 80

### Which component of a table contains a piece of data for one record?

### Answer Choices:
**A.** Factor

**B.** Field

**C.** Datapoint

**D.** Element

**E.** Item



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 benarji** ⬆️ 1

The correct answer is B. Field. The columns in the tables are called fields. A field contains a specific piece of information within a record.

---

**👤 buchibabe** ⬆️ 1

B is correct. Page 343 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 1

B is correct



</details>

---

## Question 81

### What type of field has a drop down list, from which you can pick from pre-defined options?

### Answer Choices:
**A.** Choice

**B.** Picker

**C.** Drop down

**D.** Option



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 ba14741** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 buchibabe** ⬆️ 1

a is correct. Page 163 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 RangerPM** ⬆️ 1

A is the only one that is in the list of available options.
https://docs.servicenow.com/bundle/vancouver-platform-administration/page/administer/reference-pages/reference/r_FieldTypes.html

---

**👤 krissyk** ⬆️ 1

A - Choice

https://support.servicenow.com/kb?id=kb_article_view&sysparm_article=KB0538947#:~:text=Description,pre%2Ddefined%20set%20of%20options.

---

**👤 PappyFox** ⬆️ 1

A-choice is correct

---

**👤 evoken** ⬆️ 1

All wrong? The answer is Select Box.



</details>

---

## Question 82

### User records are stored in which table?

### Answer Choices:
**A.** User [sys_user]

**B.** User [sn_user]

**C.** User [u_sys_user]

**D.** User [s_user]



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 wer3ball** ⬆️ 1

A. Its under section "Users & Groups"

---

**👤 PappyFox** ⬆️ 1

sys_user - is correct



</details>

---

## Question 83

### What ServiceNow feature can be triggered by events, and is used to inform users about activities or updates in ServiceNow?

### Answer Choices:
**A.** Notifications

**B.** Alerts

**C.** Texts

**D.** Events

**E.** Emails



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 bagoly** ⬆️ 6

**Selected Answer:** A
It's A

---

**👤 DDD1377** ⬆️ 1

A is correct. Notifications can be triggered by events.

---

**👤 ba14741** ⬆️ 1

**Selected Answer:** A
It's A

---

**👤 buchibabe** ⬆️ 4

a is correct. Page 292 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 Jman64** ⬆️ 1

It's A

---

**👤 Vishy123** ⬆️ 1

A is correct

---

**👤 icen45** ⬆️ 2

**Selected Answer:** A
It should be Notification.

---

**👤 icen45** ⬆️ 1

It should be Notification.

---

**👤 Ghouse1** ⬆️ 4

**Selected Answer:** A
Notifications



</details>

---

## Question 84

### Which ServiceNow capability allows you to provide knowledge articles, via a conversational messaging interface?

### Answer Choices:
**A.** Agent Assist

**B.** Virtual Agent

**C.** Now Messenger

**D.** Connect Agent



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (60%), A (40%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 wer3ball** ⬆️ 5

Virtual Agent is the answer

https://docs.servicenow.com/bundle/washingtondc-release-notes/page/release-notes/now-platform-capabilities/virtual-agent-rn.html#:~:text=The%20ServiceNow%C2%AE%20Virtual%20Agent,to%20perform%20common%20work%20tasks.

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
B is the conversational option

---

**👤 96d187b** ⬆️ 1

B Virtual Agent. 

Bit frustrated with the lack of correct responses I'm seeing — agent assist is a feature that allows an agent to find related articles/incidents etc. to help resolve the case. Not conversational whatsoever

---

**👤 JR83** ⬆️ 2

**Selected Answer:** B
Virtual Agent is conversational due to NLU and will present KB articles to end users by utilizing AI search capabilities.

---

**👤 es_222** ⬆️ 3

**Selected Answer:** B
Virtual Agent: Increase deflections by empowering your employees and customers to serve themselves using a friendly messaging interface, featuring pre-built conversations powered by artificial intelligence.

---

**👤 bcc5e85** ⬆️ 1

in the Washington version , the answer is Virtual Agent

---

**👤 Austin98** ⬆️ 1

**Selected Answer:** A
A is the correct answer: Page 48 of the ServiceNow Administration Fundamentals - Vancouver edition: Lab step 2: Select the Agent Assist Icon to display the Agent Assist panel, then select and read the "Mobile Event App for Staff- Error 555" article.
Lab Step 3: Select attached to share this information with the Caller

---

**👤 BrooksieOwl** ⬆️ 3

I think the answer is B. Agent assist provides agents with automatic search results that show possible solutions for records they open. Configure Agent assist in Configurable Workspace by configuring a record page with a Glide form data source and a Contextual Sidebar. For more information, see Agent assist Overview on the ServiceNow Developer Site.Benefits of Virtual Agent
Redefine your support experience
Provide intelligent exchanges powered by generative AI and automated resolutions to common issues.
Provide self-service anytime
Let users get resolutions in seconds. No need to wait for an available live agent.
See immediate business value
Get instant ROI with guided setup, prebuilt conversations, integrations, and configuration tools.
Increase deflections and agent productivity
Route issues to a chatbot automatically. Reduce call volumes and increase efficiency.

---

**👤 MACCA81** ⬆️ 3

**Selected Answer:** A
A is the correct answer: Page 48 of the ServiceNow Administration Fundamentals - Vancouver edition: Lab step 2: Select the Agent Assist Icon to display the Agent Assist panel, then select and read the "Mobile Event App for Staff- Error 555" article.
Lab Step 3: Select attached to share this information with the Caller

---

**👤 MACCA81** ⬆️ 2

A is the correct answer: Page 48 of the ServiceNow Administration Fundamentals - Vancouver edition: Lab step 2: Select the Agent Assist Icon to display the Agent Assist panel, then select and read the "Mobile Event App for Staff- Error 555" article.
Lab Step 3: Select attached to share this information with the Caller.

*...and 1 more comments*


</details>

---

## Question 85

### Which role can manage multiple knowledge bases?

### Answer Choices:
**A.** knowledge_base_admin

**B.** kb_admin

**C.** sn_kb_admin

**D.** knowledge_admin



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 benarji** ⬆️ 1

D. knowledge_admin role: Administrators and those with the knowledge_admin role have the ability to manage multiple Knowledge Bases.

---

**👤 wer3ball** ⬆️ 1

D. Answer is under "What is Knowledge Management" section

---

**👤 RangerPM** ⬆️ 1

https://docs.servicenow.com/bundle/vancouver-servicenow-platform/page/product/knowledge-management/reference/r_KnowledgeRoles.html

---

**👤 PappyFox** ⬆️ 2

https://docs.servicenow.com/en-US/bundle/sandiego-servicenow-platform/page/product/knowledge-management/reference/r_KnowledgeRoles.html
knowledge-admin



</details>

---

## Question 86

### Which statement correctly describes the differences between a Client Script and a Business Rule?

### Answer Choices:
**A.** A Client Script executes before a record is loaded and a Business Rule executes after a record is loaded

**B.** A Client Script executes on the server and a Business Rule executes on the client

**C.** A Client Script executes on the client and a Business Rule executes on the server

**D.** A Client Script executes before a record is loaded and a Business Rule executes after a record is updated



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. A Client Script executes on the client and a Business Rule executes on the server

---

**👤 buchibabe** ⬆️ 1

C is correct. Pages 437 & 438 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 1

C. A Client Script executes on the client and a Business Rule executes on the server - is correct



</details>

---

## Question 87

### What are benefits of assigning work tasks to a group, rather than to an individual? (Choose four.)

### Answer Choices:
**A.** Group members can choose their tasks from My Groups Work

**B.** Groups can assign tasks to users based on on-call schedules

**C.** Site support members can pick tasks, based on Location

**D.** Groups can assign tasks to users based on skills

**E.** Group members can avoid tasks, which are nearing SLA breach

**F.** Groups can assign tasks to users based on availability



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABDF
**Community voting results:** ABDF (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** ABDF
C is a limited scope, just for site support, the question is wider, about all groups. 
E is a non-answer, in best practice that should not happen.

---

**👤 benarji** ⬆️ 2

Ans: ABDF

---

**👤 Joe15** ⬆️ 2

**Selected Answer:** ABDF
ABDF is correct!

---

**👤 Manny1218** ⬆️ 2

**Selected Answer:** ABDF
C and E are also true but aren't realistic. The commonsense answers would be ABDF.

---

**👤 anjanc** ⬆️ 1

Any thoughts about E?

---

**👤 Mihaf92** ⬆️ 1

isn't ABCF

---

**👤 PappyFox** ⬆️ 1

PappyFox says it's ABDF!

---

**👤 davgmane** ⬆️ 2

**Selected Answer:** ABDF
answer is ABDF

---

**👤 al3xunder** ⬆️ 3

Correct answer is ABDF



</details>

---

## Question 88

### What ServiceNow feature allows you to include data from a secondary related table on a report?

### Answer Choices:
**A.** SQL

**B.** Dot Walking

**C.** Outer Join

**D.** Joins



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
Dot walking allows you to pull date from another table

---

**👤 PappyFox** ⬆️ 2

B- is correct



</details>

---

## Question 89

### What attributes can you manage, using System Properties > Basic Configuration UI16? (Choose five.)

### Answer Choices:
**A.** Browser tab title

**B.** Module text color

**C.** Preferred browser

**D.** Base theme

**E.** Font style

**F.** Animation style

**G.** Header background color

**H.** Banner Image



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABDGH
**Community voting results:** ABDGH (90%), 10%

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 DezeO** ⬆️ 1

**Selected Answer:** ADFGH
The correct answer is ADFGH.
Browser Tab Title: Set the title that appears in the browser tab.
Base Theme: Choose the base theme for the interface.
Animation Style: Select the animation style for transitions.
Header Background Color: Customize the background color of the header.
Banner Image: Upload a banner image to display in the header

---

**👤 davgmane** ⬆️ 2

**Selected Answer:** ABDGH
this is correct

---

**👤 YMijoe** ⬆️ 4

**Selected Answer:** ABDGH
I agree that "Browser tab title", "Module text color", "Base theme", "Header background color", and "Banner image" are correct.

---

**👤 bagoly** ⬆️ 3

**Selected Answer:** ABDGH
correct



</details>

---

## Question 90

### Which of the following is used to initiate a flow?

### Answer Choices:
**A.** A Trigger

**B.** Core Action

**C.** A spoke

**D.** An Event



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 tk1978** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 kisharogers** ⬆️ 1

A. The trigger specifies the conditions that start running the flow. When the trigger conditions are met, the system starts running the flow. 
 
https://docs.servicenow.com/bundle/sandiego-application-development/page/administer/flow-designer/reference/flow-triggers.html

---

**👤 julianamendo** ⬆️ 2

**Selected Answer:** A
A is the correct answer

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 ppremy** ⬆️ 1

A is the answer. A trigger is used to initiate a flow.

---

**👤 saiakhilpalle** ⬆️ 1

A is correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 DANX1** ⬆️ 1

A. https://developer.servicenow.com/dev.do#!/learn/courses/tokyo/app_store_learnv2_aescreateappfromscratch_tokyo_create_an_app_from_scratch_with_app_engine_studio/app_store_learnv2_aescreateappfromscratch_tokyo_automate_processes_with_flows/app_store_learnv2_aescreateappfromscratch_tokyo_flow_editor

---

**👤 Bonche** ⬆️ 1

A is the correct answer

*...and 2 more comments*


</details>

---

## Question 91

### Which field (or fields) is used as a unique key during imports?

### Answer Choices:
**A.** Match Fields

**B.** Coalesce Fields

**C.** Key Fields

**D.** Sys IDs



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 buchibabe** ⬆️ 4

B is correct. Page 392 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 YMijoe** ⬆️ 2

**Selected Answer:** B
I agree that "Coalesce Fields" is correct.



</details>

---

## Question 92

### You are asked to create an option in the Service Catalog, which will allow a user to click Get Help and describe the issue they are having. These forms should create incident records, which are automatically routed to the Service Desk. Which method would you use?

### Answer Choices:
**A.** Create Record Producer

**B.** Create Catalog Item

**C.** Create Order Guide

**D.** Create Content Item



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 CodingAmo** ⬆️ 1

A
Record producers are a type of Catalog Item that creates task based records e.g. incident. Not all Catalog Items are Record producers, so B is incorrect.

---

**👤 krissyk** ⬆️ 1

A
https://docs.servicenow.com/en-US/bundle/utah-it-service-management/page/product/incident-management/task/t_CreateARecordProducer.html

---

**👤 evoken** ⬆️ 3

**Selected Answer:** A
Sigh. It’s A.

---

**👤 Ranaway** ⬆️ 1

Record Producer for incidents and it allows non-itil to use that

---

**👤 Nasman096** ⬆️ 4

**Selected Answer:** A
Populate record producer data and redirect users
You can enter data for all records created by the record producer, and redirect an end user to a particular page after the record producer is submitted.

---

**👤 S_883** ⬆️ 2

**Selected Answer:** A
a is correct

---

**👤 foolishsoul4556** ⬆️ 3

**Selected Answer:** A
A is correct. . Catalog Item will end up in opening a request and reqeust item and attaches a workflow and catalog task, approvals Where as a Record producer simply insert a record in the selected table.

---

**👤 Asiwaju1** ⬆️ 2

A is the answer

---

**👤 bagoly** ⬆️ 3

**Selected Answer:** A
A is correct here



</details>

---

## Question 93

### What is the result of the order in which access controls are evaluated?

### Answer Choices:
**A.** Ensures user has access to the fields in a table, before considering their access to the table

**B.** Ensures user can get to work as quickly as possible

**C.** Ensures user has access to the application, before evaluating access to a module within the application

**D.** Ensures user has access to a table, before evaluating access to a field in the table



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (83%), A (17%)

</details>

<details>
<summary><strong>💬 View Discussion (19 comments)</strong></summary>

**👤 evoken** ⬆️ 9

**Selected Answer:** D
In ServiceNow, when a user tries to access a table or field, the access is evaluated first at the table level, and then at the field level. - chatgpt

---

**👤 76c8608** ⬆️ 1

**Selected Answer:** A
Field then table (specific to general)

---

**👤 4419016** ⬆️ 1

**Selected Answer:** A
from more specific to general, A is the correct answer , field first, then table.

---

**👤 JinDG** ⬆️ 1

**Selected Answer:** D
Anwser is D People are dumb..

---

**👤 jjn9306** ⬆️ 4

**Selected Answer:** D
ACL evalutes access to table first (specific to general) then it evaluates field (specific to general)

---

**👤 a0739f2** ⬆️ 1

It's A, rules are evaluated specific to general. This is in the SN textbook.

---

**👤 lica09** ⬆️ 1

**Selected Answer:** D
First at the Table-level (Most specific to most general), then at the Field-level (most specific to most general

---

**👤 Sudharshan77** ⬆️ 1

for the exam should we give a or d, which option is going to fetch marks for us

---

**👤 [Removed]** ⬆️ 4

It's A. As ServiceNow evaluates from specific to general. 
As seen in page 319 in the SNAF eBook.

---

**👤 buchibabe** ⬆️ 2

D is correct. Page 429 of the ServiceNow Administration Fundamentals - Tokyo Edition.

*...and 9 more comments*


</details>

---

## Question 94

### Which tool graphically displays an infrastructure view for a configuration item (CI) and its relationship with other CIs?

### Answer Choices:
**A.** Schema Map

**B.** Dependency View

**C.** Dependency Map

**D.** Database View



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 benarji** ⬆️ 1

B. In ServiceNow, a "Dependency View" is a graphical representation of the dependencies between Configuration Items (CIs) in an IT infrastructure. A dependency view provides a visual representation of the relationships between CIs and the services

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** B
B. Dependency View

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** B
The latest link is: https://docs.servicenow.com/bundle/vancouver-servicenow-platform/page/product/business-service-management-map-ng/concept/c_BusinesssServiceManagementMaps.html

---

**👤 buchibabe** ⬆️ 1

B is correct. Page 427 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 evoken** ⬆️ 2

**Selected Answer:** B
In ServiceNow, a "Dependency View" is a graphical representation of the dependencies between Configuration Items (CIs) in an IT infrastructure. A dependency view provides a visual representation of the relationships between CIs and the services they support, enabling IT teams to quickly understand the impact of changes, troubleshoot issues, and plan for future improvements.- chatgpt

---

**👤 Vishy123** ⬆️ 1

B is correct answer

---

**👤 smeagel** ⬆️ 1

**Selected Answer:** B
correct

---

**👤 Philippvs** ⬆️ 1

Regarding the current version:
https://docs.servicenow.com/bundle/tokyo-servicenow-platform/page/product/business-service-management-map-ng/concept/c_BusinesssServiceManagementMaps.html

---

**👤 RedWater** ⬆️ 1

https://docs.servicenow.com/bundle/quebec-servicenow-platform/page/product/business-service-management-map-ng/concept/c_BusinesssServiceManagementMaps.html



</details>

---

## Question 95

### What are examples of Core tables in the ServiceNow platform?

### Answer Choices:
**A.** Configuration, Connect, Chat

**B.** Team, Party, Awards

**C.** User, Task, Incident

**D.** Work, Caller, Timecard



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (13 comments)</strong></summary>

**👤 Magic_Carpet** ⬆️ 7

Shouldn't the answer be C?

---

**👤 MaDSnow** ⬆️ 1

Answer is C

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
User, Task, Incident are core tables in ServiceNow

---

**👤 CodingAmo** ⬆️ 1

**Selected Answer:** C
Connect and Chat are not tables in SN.

---

**👤 uwbe** ⬆️ 2

**Selected Answer:** C
This is an ambiguous question and therefore should not be in the exam I believe. It looks like there are two definitions of the core table: 1.) tables, which are base tables and doesn't have a parent. In this case c) would be wrong as Incident is a child of task
2.) Core tables are tables that come with the base system. In this case c) is the correct answer

---

**👤 evoken** ⬆️ 1

**Selected Answer:** C
It’s C. It seems like the answers were chosen at random.

---

**👤 Ibeauo** ⬆️ 1

**Selected Answer:** C
It's clearly answer C, I don't know why anyone here chose answer A. Is it to make us confused?

Answer C 'User, Task, Incident' is the only correct answer from my point of view, this is also mentioned again and again in the ServiceNow courses.

---

**👤 smeagel** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Ranaway** ⬆️ 1

I am voting for C

---

**👤 Philippvs** ⬆️ 1

**Selected Answer:** C
Here shows the answer as C: User, task, incident.
https://www.servicenow.com/community/it-service-management-forum/difference-between-quot-core-quot-table-and-quot-base-quot-table/m-p/574302?lightbox-message-images-574305=28676i8D5F17943CB8AEAC

*...and 3 more comments*


</details>

---

## Question 96

### Which tab on the knowledge base record, would you use to identify the sets of users who are able to read articles in that knowledge base?

### Answer Choices:
**A.** Access List

**B.** Can Access

**C.** Accessible to

**D.** Can Read



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - can read

---

**👤 wer3ball** ⬆️ 1

D. "knowledge bases use user criteria records to determine which sets of users can read or contribute...." its under "Knowledge Security and Visibility: User Criteria" section

---

**👤 PappyFox** ⬆️ 1

D. is correct



</details>

---

## Question 97

### What are the main components of the Form Design interface? (Choose three.)

### Answer Choices:
**A.** Field Layout

**B.** Page Header

**C.** Field Navigator

**D.** Field Picker

**E.** Form Layout



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BCE
**Community voting results:** BCE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 f5003a6** ⬆️ 2

**Selected Answer:** BCE
B. Page Header
C. Field Navigator
E. Form Layout

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** BCE
https://docs.servicenow.com/bundle/vancouver-platform-administration/page/administer/form-administration/concept/c_FormDesign.html

---

**👤 buchibabe** ⬆️ 3

BCE is correct. Page 168 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 YMijoe** ⬆️ 3

**Selected Answer:** BCE
I agree that "Page Header", "Field Navigator", and "Form Layout" are correct.



</details>

---

## Question 98

### What is used to determine user access to knowledge bases or a knowledge article?

### Answer Choices:
**A.** sn_kb_read, sn_article_read

**B.** Privacy Settings

**C.** Read Access Flag

**D.** User Criteria



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Ginfante** ⬆️ 2

**Selected Answer:** D
https://docs.servicenow.com/bundle/sandiego-servicenow-platform/page/product/knowledge-management/concept/diagnose-knowledge-user-criteria.html

---

**👤 PappyFox** ⬆️ 1

D. is correct



</details>

---

## Question 99

### What are the three key tables in an enterprise CMDB? (Choose three.)

### Answer Choices:
**A.** cmdb

**B.** sn_cmdb_bak

**C.** cmdb_rel_ci

**D.** sn_cmdb

**E.** cmdb_bak

**F.** cmdb_ci

**G.** sn_cmdb_ci



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACF
**Community voting results:** ACF (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 f5003a6** ⬆️ 3

**Selected Answer:** ACF
A. cmdb
C. cmdb_rel_ci
F. cmdb_ci

---

**👤 buchibabe** ⬆️ 2

ACF is correct. Page 407 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 missmam** ⬆️ 4

**Selected Answer:** ACF
ACF is correct



</details>

---

## Question 100

### What is the best practice related to using the Default Update Set for moving customizations between instances?

### Answer Choices:
**A.** Merge Default update sets before moving between instances

**B.** Submit Default update set to application repository

**C.** You should not use the Default Update sets for moving between instances

**D.** Keep Default update set to maximum of 20 records, for troubleshooting purposes



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 f5003a6** ⬆️ 2

**Selected Answer:** C
C. You should not use the Default Update sets for moving between instances

---

**👤 buchibabe** ⬆️ 3

C is correct. Page 454 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 evoken** ⬆️ 3

**Selected Answer:** C
C; the default update set is a catch all for admin only, not for migrating changes.

---

**👤 missmam** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Ghouse1** ⬆️ 4

**Selected Answer:** C
admins should use named update sets for moving customizations between instances.

---

**👤 bagoly** ⬆️ 4

**Selected Answer:** C
C is correct

---

**👤 RedWater** ⬆️ 3

Maybe C instead?



</details>

---

## Question 101

### For Administrators creating new Service Catalog items, what is a characteristic they should know about Service Catalog variables?

### Answer Choices:
**A.** Service Catalog variables can only be used in Record Producers

**B.** Service Catalog variables can only be used in Order Guides

**C.** Service Catalog variables cannot affect the order price

**D.** Service Catalog variables are global by default



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (79%), C (21%)

</details>

<details>
<summary><strong>💬 View Discussion (23 comments)</strong></summary>

**👤 dalvisa** ⬆️ 1

**Selected Answer:** D
Correct answer

---

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** D
Correct answer

---

**👤 153df45** ⬆️ 1

**Selected Answer:** D
Variables are Global by default. They can define available options and affect the order price.

---

**👤 roopsrich** ⬆️ 1

**Selected Answer:** C
Catalog variables are not global by default

---

**👤 KV_CTS** ⬆️ 2

**Selected Answer:** C
A. Service Catalog variables can only be used in Record Producers - No, vriables are used in Record Producers, Order Guides, and Catalog Items
B. Service Catalog variables can only be used in Order Guides - No, variables can also be used in Catalog Items and Record Producers, not just Order Guides
C. Service Catalog variables cannot affect the order price - Yes, variables collect user input, but do not change pricing unless scripted separately
D. Service Catalog variables are global by default - No, variables are scoped to the specific Catalog Item unless manually set as global (a checkbox needs to be selected on variable creation form)

---

**👤 GoldBear** ⬆️ 4

**Selected Answer:** D
D is the correct answer.

---

**👤 julianamendo** ⬆️ 2

**Selected Answer:** D
D is the correct answer

---

**👤 est_1995** ⬆️ 2

**Selected Answer:** D
D is the best answer

---

**👤 HNTrinh** ⬆️ 2

**Selected Answer:** D
Service Catalog variables may affect the price. SC variable is global by default. the definition is in the SN fundamental book.

---

**👤 pielouis** ⬆️ 2

**Selected Answer:** C
In general, Service Catalog variables do not directly affect the order price. Service Catalog variables are primarily used for collecting user input and customizing the behavior of catalog items or processes. The order price is typically determined by factors such as the pricing rules, configuration items, or pricing models associated with the catalog items or services being ordered.

*...and 13 more comments*


</details>

---

## Question 102

### On what part of the ServiceNow instance, would you find the option to Impersonate User?

### Answer Choices:
**A.** Module

**B.** Content Frame

**C.** Application Navigator

**D.** Banner



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 BrooksieOwl** ⬆️ 5

Q277 is a duplicate of Q190 but they have different answers. Q190 answer is Banner and Q277 answer is User Menu. Neither gives these options in the other question. Both answers are right, however. Open the User menu by clicking your user avatar in the ServiceNow banner. Select the Impersonate user menu item. In the Select a user to impersonate dialog, enter the name of the user to impersonate in the Search for a user field. Select the user from the list and click the Impersonate user button.

---

**👤 f5003a6** ⬆️ 2

**Selected Answer:** D
D. Banner

---

**👤 PappyFox** ⬆️ 1

D. is correct



</details>

---

## Question 103

### Which application is used primarily to load data into ServiceNow?

### Answer Choices:
**A.** Import Hub

**B.** System Import Sets

**C.** Data Import Configuration

**D.** Import Management



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 ba14741** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 PappyFox** ⬆️ 1

B. is correct



</details>

---

## Question 104

### If a knowledge base has no access details specified, what users are able to read articles in that knowledge base?

### Answer Choices:
**A.** itil users

**B.** Any user with an article's permalink

**C.** Any active user

**D.** No users

**E.** Users with kb_user role



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 PappyFox** ⬆️ 1

**Selected Answer:** C
Any active user

---

**👤 YMijoe** ⬆️ 3

**Selected Answer:** C
I agree that "Any active user" is correct.

---

**👤 akaki** ⬆️ 2

**Selected Answer:** C
https://docs.servicenow.com/bundle/tokyo-servicenow-platform/page/product/knowledge-management/concept/user-access-knowledge.html



</details>

---

## Question 105

### How would you define an Access Control, to allow a user with itil role to have permission to create incident records?

### Answer Choices:
**A.** Name: incident.None; Operation: create; Role: itil

**B.** Name: incident.Any; Operation: write; Permission: itil

**C.** Name: incident:*; Permission: write; Role: itil

**D.** Name: incident.None; Permission: create; Role: itil

**E.** Name: incident:*;Operation: write; Permission: itil



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Qwertydry** ⬆️ 3

**Selected Answer:** A
Option A is Correct. Operation only arrives not Permission.

---

**👤 PappyFox** ⬆️ 3

A is correct



</details>

---

## Question 106

### What Service Catalog feature do you use to organize items into logical groups?

### Answer Choices:
**A.** Categories

**B.** Variable Sets

**C.** Sections

**D.** Catalog items



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** A
A. Categories

---

**👤 buchibabe** ⬆️ 2

A is correct. Page 204 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 2

A is correct



</details>

---

## Question 107

### When creating a new notification, what must you define? (Choose three.)

### Answer Choices:
**A.** The associated knowledge base

**B.** Settings for handing inactive user accounts

**C.** Under what conditions is the notification sent

**D.** Who receives the notification

**E.** What the content of the notification



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** CDE
**Community voting results:** CDE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** CDE
Who will receive - D, When will receive? - C and What will receive? - E.

---

**👤 ba14741** ⬆️ 1

**Selected Answer:** CDE
It's CDE

---

**👤 wer3ball** ⬆️ 1

CDE

Under 6.2 Creating Notifications

---

**👤 YMijoe** ⬆️ 3

**Selected Answer:** CDE
I agree that C,D,E are correct.



</details>

---

## Question 108

### The ServiceNow platform supports a wide variety of plug-and-play applications. You can choose from the included workflows of build your own workflow: Which of these workflows are included in the platform? (Choose three.)

### Answer Choices:
**A.** Federal Workflows

**B.** Customer Workflows

**C.** Infrastructure Workflows

**D.** Manufacturing Workflows

**E.** Employee Workflows

**F.** IT Workflows



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BEF
**Community voting results:** BEF (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 ba14741** ⬆️ 1

**Selected Answer:** BEF
It's BEF

---

**👤 Vinnyzworld** ⬆️ 3

**Selected Answer:** BEF
BEF is correct.
The service now products are broadly categorized into these workflow

---

**👤 bagoly** ⬆️ 1

**Selected Answer:** BEF
correct



</details>

---

## Question 109

### An IT manager is responsible for the Network and Hardware assignment groups, each group contains 5 team members. These team members are working on many tasks, but the manager cannot see any tasks on the Service Desk > My Groups Work list. What could explain this?

### Answer Choices:
**A.** The Assignment Group manager field is empty.

**B.** The manager does not have the itil role.

**C.** The manager is not a member of the Service Desk group.

**D.** The manager is not a member of the Network and Hardware groups.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (83%), B (17%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 Ibeauo** ⬆️ 6

There is the same question on examtopics with a picture where you can see the filter, but there is one more answer which is not displayed in this questions. The same question you could find here (question 87): 

https://www.examtopics.com/discussions/servicenow/view/77737-exam-csa-topic-1-question-87-discussion/

There the most voted answer with 60% is "The Service Desk > My Groups Work list shows active work tasks that are not yet assigned." And this answer is not displayed here. So I would vote for D in this question.

---

**👤 Raxon** ⬆️ 2

"My Groups Work" is not accessible without ITIL as it is under Service Desk > My Groups Work. Even before the rewrite. The answer has always been Network & Hardware group.

---

**👤 ba14741** ⬆️ 1

**Selected Answer:** D
It's D

---

**👤 MACCA81** ⬆️ 1

**Selected Answer:** D
The automatic filter for the Service -> Desk My Groups Work module is set to show Active tasks, that are not assigned to anyone and are for the groups you’re a member of. Since we know there are active incidents for the Network and Hardware groups, and there’s no option in the answers to indicate all tasks are assigned to people, then the only option left is that the manager is not a member of the Network and Hardware groups (answer D) 
Answer B is NOT correct because access to Apps and Modules is controlled by roles, and you have to have either the itil role or the sn_change_task role to access the Service Desk Module. Assuming the manager does not have the itil role, means has at least sn_change_task role to get to My groups work. In this case will see any Change related tasks for the 2 groups. (incidents will be hidden from him, but change records and tasks at least should be visible).

---

**👤 dnbly** ⬆️ 3

**Selected Answer:** D
Looks like they rewrote the question as it was confusing. Definitely much easier to understand and answer now.

---

**👤 Ibeauo** ⬆️ 3

**Selected Answer:** D
Someone of you might choose for B ('The manager does not have the itil role', in my point of view B could not be correct, because, if you don't have the ITIL role, you would not see My Groups work, it will not appear.

---

**👤 Vishy123** ⬆️ 2

D is correct answer

---

**👤 99Sekhar** ⬆️ 3

B is wrong. The manager clearly has ITIL role if he can see "Service Desk > My Groups Work list."
The correct answer is D> Once he is a member of the groups, he can see the work assigned to the group that is unassigned

---

**👤 S_883** ⬆️ 2

**Selected Answer:** D
it should be D

---

**👤 foolishsoul4556** ⬆️ 2

**Selected Answer:** B
Is it not B?



</details>

---

## Question 110

### You have been asked to configure a form so an employee could order tablet and select the standard accessory options to purchase with it. These standard options are: carrying case, screen cleaner, tablet stand, and screen protector. What approach would you take? (Choose three.)

### Answer Choices:
**A.** Create Catalog Item for the Tablet, and add a variable set to the form, for the accessory options.

**B.** Create a Record producer, and on the form, add a check box variable for each accessory option.

**C.** On Shopping Cart configuration, select option lo show the Add Accessories button.

**D.** Create one Catalog item for each: tablet, carrying case, screen cleaner, tablet stand, and screen protector.

**E.** Create Catalog Item for the tablet, and on the form, add a check box variable for each accessory option.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ADE
**Community voting results:** ADE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 bagoly** ⬆️ 9

**Selected Answer:** ADE
Record producer should not be used for ordering catalog items, so I think the correct answer is: A, D, E

---

**👤 connal308876** ⬆️ 1

Honestly, E is the only answer needed.

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** ADE
It's A, D and E

---

**👤 dnbly** ⬆️ 1

**Selected Answer:** ADE
ADE is about as correct as this can get in the absence of Order Guide as an option.

---

**👤 PappyFox** ⬆️ 2

**Selected Answer:** ADE
ADE for moisture

---

**👤 evoken** ⬆️ 4

What in the world. You would create an order guide. All answers are wrong

---

**👤 Ibeauo** ⬆️ 2

**Selected Answer:** ADE
Option B should not be chosen in any case, because a 'Record Producer' is defined as following:

'A record producer is a specific type of catalog item that allows end users to create task-based records, such as incident records, from the service catalog. The look and feel of a record producer is similar to that of a catalog item. But the record producer generates a task record such as incident, instead of a requested item.'

So I would vote for ADE



</details>

---

## Question 111

### Which ServiceNow utility provides a modern interactive graphical interface to visualize configuration items and their relationships?

### Answer Choices:
**A.** Dependency View

**B.** CI Class Map

**C.** Business Service Map

**D.** CSDM Schema



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
Dependency view, text book answer

---

**👤 PappyFox** ⬆️ 1

A is correct



</details>

---

## Question 112

### Which one of the following statements applies to a set of fields when they are coalesced during an import?

### Answer Choices:
**A.** If a match is found using the coalesce fields, the existing record is updated with the information being imported

**B.** If a match is not found using the coalesce fields, the system does not create a Transform Map

**C.** If a match is found using the coalesce fields, the system creates a new record

**D.** If a match is not found using the coalesce fields, the existing record is updated with the information being imported



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (26 comments)</strong></summary>

**👤 Raxon** ⬆️ 5

**Selected Answer:** A
If an existing record with a matching value in the target table is found, that record is updated. If no matching record is found, then a new record is created in the target table.

https://docs.servicenow.com/bundle/utah-integrate-applications/page/administer/import-sets/concept/c_ImportSetCoalesce.html

---

**👤 Maka777** ⬆️ 1

**Selected Answer:** A
Correct

---

**👤 CarlaWI** ⬆️ 1

**Selected Answer:** A
Explanation I wrote was rejected by the website. :-(

---

**👤 SaiExamtopic** ⬆️ 2

A is correct

---

**👤 julianamendo** ⬆️ 2

**Selected Answer:** A
A is correct

---

**👤 AMITKSI** ⬆️ 3

**Selected Answer:** A
A is correct

---

**👤 rohit55555** ⬆️ 2

A is correct answer

---

**👤 shravan3** ⬆️ 2

A is correct

---

**👤 HNTrinh** ⬆️ 2

**Selected Answer:** A
A is correct answer

---

**👤 Mahindra_Reddie** ⬆️ 2

**Selected Answer:** A
A is correct answer

*...and 16 more comments*


</details>

---

## Question 113

### Which one of the following statements is true about Column Context Menus?

### Answer Choices:
**A.** It displays actions such as creating quick reports, configuring the list, and exporting data

**B.** It displays actions related to filtering options, assigning tags, and search

**C.** It displays actions related to viewing and filtering the entire list

**D.** It displays actions such as view form, view related task, and add relationship



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 floelynn** ⬆️ 2

**Selected Answer:** A
Column Context Menus a.k.a Column Options – found on the header of each field (the 3 dots on the left of field header)
B – Assigning tags is on “Actions on selected rows” tab and search is the tab with magnifying glass. Both is on the top right corner.
C- This is actually true but A is more precise. So A is the answer
D – these actions are on each row/record, on the “i” (info) icon not on Column Context Menu

---

**👤 chrisklesar1234** ⬆️ 1

DEFINITELY A

---

**👤 Mikias** ⬆️ 1

A is correct

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** A
A is the correct answer.

---

**👤 Jana08** ⬆️ 2

**Selected Answer:** A
A is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 saiakhilpalle** ⬆️ 1

A is correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Bonche** ⬆️ 1

A is the correct answer

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 114

### What is the definition of a group?

### Answer Choices:
**A.** A collection of subject matter experts

**B.** A team of users

**C.** An escalation pod

**D.** A collection of users

**E.** A department



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D as users can be part of different teams, so a SN group can a collection of users from various teams/departments

---

**👤 buchibabe** ⬆️ 1

D is correct. Page 75 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 1

D is correct



</details>

---

## Question 115

### On the Reports page, what sections allow you to see which reports are visible to different audiences? (Choose four.)

### Answer Choices:
**A.** Group

**B.** Department

**C.** My reports

**D.** Team

**E.** Global

**F.** All



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACEF

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 PappyFox** ⬆️ 7

ACEF are correct

---

**👤 f5003a6** ⬆️ 3

Group, My Reports, All and Global

---

**👤 wer3ball** ⬆️ 1

The real answer is on Report Visibility Controls

A, C, E, F

---

**👤 buchibabe** ⬆️ 3

ACEF is correct. Page 266 of the ServiceNow Administration Fundamentals - Tokyo Edition.



</details>

---

## Question 116

### On a filter condition, there is an element, which is based on the table, the user access rights, and columns on the table. What is this element called?

### Answer Choices:
**A.** Label

**B.** Column

**C.** Data Element

**D.** Field

**E.** Attribute



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 servicepow** ⬆️ 2

**Selected Answer:** D
D is correct

---

**👤 PappyFox** ⬆️ 1

D. is correct



</details>

---

## Question 117

### You have been asked to create a way for users to order a new iPhone, but only if they get two levels of approval. The approvers and users should be automatically notified at each approval level. What feature would you use to manage the approvals and notifications?

### Answer Choices:
**A.** Approval Chains

**B.** Flows

**C.** Approver Delegates

**D.** Parent-Child Approvers

**E.** Approval Criteria



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** B
B. Flows

---

**👤 Sazeka** ⬆️ 2

B is correct:
Resolution
Creating the Knowledge simple approval workflow

1. In the Application Navigator filter text, type workflow to find the Workflow application menu.

2. Select the Workflow Editor module. The Workflow Editor always opens in an adjacent browser tab.

3. Click the New button.
https://support.servicenow.com/kb?id=kb_article_view&sysparm_article=KB0538552

---

**👤 PappyFox** ⬆️ 1

B. is correct



</details>

---

## Question 118

### Groups are stored in what table?

### Answer Choices:
**A.** User Group [user_groups]

**B.** Groups [sys_user_groups]

**C.** Group [sn_sys_user_group]

**D.** Group [sys_user_group]

**E.** User Groups [sn_user_groups]



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 f5003a6** ⬆️ 2

**Selected Answer:** D
D. Group [sys_user_group]

---

**👤 buchibabe** ⬆️ 3

D is correct. Page 75 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 1

D. is correct



</details>

---

## Question 119

### When managing tags, you can adjust who is able to see it. What are the visibility options? (Choose three.)

### Answer Choices:
**A.** Groups and Users

**B.** Me

**C.** Roles and Permissions

**D.** Everyone

**E.** Admins



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABD
**Community voting results:** ABD (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 f5003a6** ⬆️ 3

**Selected Answer:** ABD
A. Groups and Users
B. Me
D. Everyone

---

**👤 Qwertydry** ⬆️ 2

**Selected Answer:** ABD
Yeah the Answer is A,B,D.

---

**👤 PappyFox** ⬆️ 3

ABD are correct

---

**👤 YMijoe** ⬆️ 2

**Selected Answer:** ABD
I agree that "Groups and Users", "Me", and "Everyone" are correct.



</details>

---

## Question 120

### What module enables an administrator to define destinations for imported data on any ServiceNow table?

### Answer Choices:
**A.** Field Transform

**B.** Schema Map

**C.** Transform Map

**D.** Import Map



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 benarji** ⬆️ 1

C. Transform Map
The Transform Maps module enables an administrator to define destinations for imported data on any tables.

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. Transform Map

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** C
c is correct

---

**👤 yo_mamma** ⬆️ 2

C is correct
https://docs.servicenow.com/bundle/utah-employee-service-management/page/product/human-resources/reference/transform-maps-navex.html



</details>

---

## Question 121

### On the Form header, which icon do you use to access form templates?

### Answer Choices:
**A.** Stamp

**B.** Pages

**C.** More Options (...)

**D.** Paperclip



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (83%), A (17%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. More Options (...)

---

**👤 Manny1218** ⬆️ 2

**Selected Answer:** C
Check out page 33 in the Bonus section 5.2, of the ServiceNow Administration Fundamentals - Tokyo. It states, "Click the More options icon (...) from the form header, then Toggle Template Bar to work with templates."

---

**👤 Kubis** ⬆️ 1

**Selected Answer:** C
C - https://www.youtube.com/watch?v=C6ujM7xaVMQ

---

**👤 Dekoreh** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 studyGUYd** ⬆️ 2

Answer is C

https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/use/using-forms/concept/c_UI16FormHeader.html

---

**👤 halflife7** ⬆️ 1

**Selected Answer:** A
STAMP is correct ignore these , wrong answers....

---

**👤 PappyFox** ⬆️ 3

C. is correct



</details>

---

## Question 122

### When using the Data Pill Picker, use which keys to dot-walk to fields in other tables?

### Answer Choices:
**A.** Plus, Minus

**B.** Ctrl <, Ctrl >

**C.** Arrows

**D.** Ctrl C, Ctrl V

**E.** Shift F4, Shift F5



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 f5003a6** ⬆️ 2

**Selected Answer:** C
C. Arrows

---

**👤 buchibabe** ⬆️ 1

C is correct. Page 235 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 2

C. is correct



</details>

---

## Question 123

### In what order are Access Controls evaluated?

### Answer Choices:
**A.** Field-level - most specific to most general; then Table-level - most specific to most general

**B.** Field-level - most general to most specific; then Row-level - most specific to most general

**C.** Table-level - most specific to most general; then Field-level - most specific to most general

**D.** Table-level - most specific to most general, then Row-level - most specific to most general



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 smeagel** ⬆️ 6

**Selected Answer:** C
Answer is C. First table lvl after field lvl

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. Table-level - most specific to most general; then Field-level - most specific to most general

---

**👤 ddiggler** ⬆️ 1

It is C

---

**👤 MACCA81** ⬆️ 1

**Selected Answer:** C
C is correct. Access controls are always evaluated table and field, and always most specific to most general (

---

**👤 Mathias92** ⬆️ 1

1.Match the table and field name: x_58872_needit_needit.short_description
2. Match the parent table and field name: task.short_description
3. Match any table with the field name: *.short_description
4. Match the table and any field (wildcard): x_58872_needit_needit.*
5. Match the parent table and any field (wildcard): task.*
6. Match any table (wildcard) and any field (wildcard): *.*

---

**👤 buchibabe** ⬆️ 2

C is correct. Page 429 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 tangoalpha** ⬆️ 1

**Selected Answer:** C
From the CSA prep course

61. In what order are access controls evaluated?

First at the Table-level (Most specific to most general), then at the Field-level (most specific to most general)

---

**👤 Vishy123** ⬆️ 2

c is correct

---

**👤 Egba1** ⬆️ 2

C is the right answer

---

**👤 ALDADI** ⬆️ 3

**Selected Answer:** C
Answer is C



</details>

---

## Question 124

### Which ServiceNow products can be used to discover and populate the CMDB? (Choose two.)

### Answer Choices:
**A.** Discovery

**B.** IntegrationHub ETL

**C.** Finder

**D.** CMDB Plug-in

**E.** CMDB Integration Dashboard



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AB
**Community voting results:** AB (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 Kcaher** ⬆️ 17

A and B is the Correct Answer.
https://docs.servicenow.com/bundle/sandiego-servicenow-platform/page/product/configuration-management/concept/c_OptionsToPopulateCMDB.html

You can populate the CMDB by using Discovery, by using the IntegrationHub ETL or Import Sets to import and integrate data from a third-party source, by integrating with an external CMDB, or by manually creating CIs.

---

**👤 floelynn** ⬆️ 2

**Selected Answer:** AB
You can populate the CMDB by using Discovery, by using the IntegrationHub ETL or Import Sets to import and integrate data from a third-party source, by integrating with an external CMDB, or by manually creating CIs.

---

**👤 chrisklesar1234** ⬆️ 1

AB FOR SURE

---

**👤 TimmyLam** ⬆️ 1

**Selected Answer:** AB
A and B

---

**👤 BrandyD** ⬆️ 4

A and B
https://docs.servicenow.com/bundle/washingtondc-servicenow-platform/page/product/configuration-management/concept/c_OptionsToPopulateCMDB.html

---

**👤 julianamendo** ⬆️ 2

**Selected Answer:** AB
AB is the correct answer.

---

**👤 Prem009** ⬆️ 2

A &B are correct

---

**👤 saiakhilpalle** ⬆️ 2

A and B are correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** AB
A B is correct

---

**👤 Bonche** ⬆️ 1

A, B is the correct answers

*...and 2 more comments*


</details>

---

## Question 125

### What instance resource allows you to access guided tours, information about actions, and instructions on how to use inputs and outputs in your flow?

### Answer Choices:
**A.** Docs

**B.** Community

**C.** Help Panel (question mark icon)

**D.** Wiki



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 benarji** ⬆️ 1

C. Help Panel (question mark icon)

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. Help Panel (question mark icon)

---

**👤 wer3ball** ⬆️ 1

C. Section "Flow Components: Data"

---

**👤 PappyFox** ⬆️ 1

C. is correct



</details>

---

## Question 126

### The Report Designer contains different sections for configuring your report. Which section is used to specify grouping and calculations to be run against the data?

### Answer Choices:
**A.** Style

**B.** Group by

**C.** Configure

**D.** Format

**E.** Data



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 benarji** ⬆️ 2

C.
https://developer.servicenow.com/dev.do#!/learn/courses/washingtondc/app_store_learnv2_reportanalytics_washingtondc_reporting_and_analytics/app_store_learnv2_reportanalytics_washingtondc_data_visualization/app_store_learnv2_reportanalytics_washingtondc_viewrun_reports

---

**👤 Dekoreh** ⬆️ 2

**Selected Answer:** C
Based on SN ebook pg 267

---

**👤 al3xunder** ⬆️ 2

Answer is C. https://developer.servicenow.com/dev.do#!/learn/courses/tokyo/app_store_learnv2_reportanalytics_tokyo_reporting_and_analytics/app_store_learnv2_reportanalytics_tokyo_data_visualization/app_store_learnv2_reportanalytics_tokyo_configuring_reports

---

**👤 agm43** ⬆️ 1

Answer is C. Configure.
https://docs.servicenow.com/bundle/tokyo-now-intelligence/page/use/reporting/reference/aggregation-reporting.html

---

**👤 agm43** ⬆️ 2

There are only 4 sections and they are DATA, TYPE, CONFIGURE, and STYLE. And Group by do not include calculations Aggregation does. Both Group by and Aggregation are both under the Configure section therefore making C. Configure the answer.

---

**👤 agm43** ⬆️ 1

The answer is the Configure section. The data section does not provide any of what the question is asking. Group by is not a section its a subsection under Configure.

---

**👤 RGGG** ⬆️ 1

Answer is B, I checked it on my instance

---

**👤 ellitetka** ⬆️ 1

I think B. Group by should be the correct answer...

---

**👤 ellitetka** ⬆️ 1

I think B. Grouop by should be the correct answer...



</details>

---

## Question 127

### What icon do you use to change the icon and color on a Favorite?

### Answer Choices:
**A.** Clock

**B.** Pencil

**C.** Triangle

**D.** Star



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (75%), D (25%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 gedgy** ⬆️ 3

**Selected Answer:** B
Star selects the favorite and pencil edits the favorite

---

**👤 5ec5b4b** ⬆️ 1

**Selected Answer:** D
Is this not by using the star Icon and clicking on "more". There I saw the color and icon options.

---

**👤 PappyFox** ⬆️ 2

B is correct



</details>

---

## Question 128

### You have heard about a new application released by ServiceNow. You want to try it out, to see if it might be useful for your company's ServiceNow implementation. What would be the best way to get hands-on experience with the new application?

### Answer Choices:
**A.** Search the wiki for the sales demo request form

**B.** Check the latest release notes at docs servicenow.com

**C.** Activate the application plug in, on your personal dev instance

**D.** Activate the application plug in, on your company's production instance.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
C - you want hands on experience

---

**👤 Mihaf92** ⬆️ 1

I think it A

---

**👤 alokji** ⬆️ 1

Answer is B:
Check the latest release notes at docs servicenow.com

---

**👤 PappyFox** ⬆️ 3

C is correct



</details>

---

## Question 129

### When looking at a long list of records, you want to quickly filter, to show only those which have Category of Hardware. How might you do that?

### Answer Choices:
**A.** On the Category column header, right click and select Show > Hardware

**B.** Right click on magnifier, type Hardware and click enter

**C.** On the list, locate and right click on the value Hardware, select Show Matching

**D.** On Breadcrumb, click > icon, type Hardware and click enter

**E.** Click Funnel icon, type Hardware and click enter



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Austin98** ⬆️ 2

**Selected Answer:** C
Show matching

---

**👤 Cloudrookie** ⬆️ 1

**Selected Answer:** C
Answer is C

---

**👤 PappyFox** ⬆️ 1

C is correct



</details>

---

## Question 130

### When looking at a long list of records, you want to quickly filter, to show only those which have Short Description containing email. How might you do that?

### Answer Choices:
**A.** Click List Magnifier to expand column search, on Short Description, type email, click enter

**B.** On Search box, select text, type email, click enter

**C.** Click List Magnifier to expand column search, on Short Description, type *email, click enter

**D.** Click List Magnifier to expand column search, on Short Description, type %email, click enter



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (89%), 11%

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 Mehamood** ⬆️ 5

**Selected Answer:** C
%email - returns records with short description ending with email
*email - returns records with short description that contains word email

---

**👤 gedgy** ⬆️ 1

**Selected Answer:** D
A, C, and D are all correct. It just depends on whether you are looking for email as a standalone word or combined with another word. But all (A, C, D) are correct. The question indicates it wants you to pull any records where the word email is contained. In that case (again technically) D is the most correct.

---

**👤 MACCA81** ⬆️ 3

**Selected Answer:** C
It's C. With the short description you can't do "show matching" as in the previous question, you need to search for records that have the word "e-mail" in the short description. and this is done by searching "*e-mail" (wildcard e-mail) in the Short Description column.

---

**👤 PappyFox** ⬆️ 2

C is correct



</details>

---

## Question 131

### When importing spreadsheet data into ServiceNow, what is the first step in the process?

### Answer Choices:
**A.** Run Data Scrubber

**B.** Set Coalesce

**C.** Select Import Set

**D.** Load Data

**E.** Define Data Source



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (56%), C (25%), E (19%)

</details>

<details>
<summary><strong>💬 View Discussion (14 comments)</strong></summary>

**👤 NikaDMars** ⬆️ 1

**Selected Answer:** D
To start a spreadsheet import, always begin with Load Data to stage your file in an Import Set table before any mapping or transformation steps.

---

**👤 Chuleta311** ⬆️ 2

**Selected Answer:** D
Load data: Navigate to System Import Sets -> Load

---

**👤 699ed72** ⬆️ 4

1. Load data: Navigate to System Import Sets -> Load

---

**👤 047a2d5** ⬆️ 1

I am with E option, this is the write one, the first step in the process of importing data is Define data source, if you go to Navigation bar and type import data set, you see the module is data source this define where the data come " fetch the data from excel to this staging source.

---

**👤 JinDG** ⬆️ 3

**Selected Answer:** E
The Anwser is Not C because you can also create import Set instead of Select one so there is no way either of those two anwsers were correct.
It is also not D since another question(Q.322) with the excat question text doesn't have the option'Load Data', and I am pretty sure you dont load data from the beginning the data is loaded after you hit submit. So the only anwser seems right would be E "Define Data Source"

---

**👤 gedgy** ⬆️ 3

**Selected Answer:** D
Gonna have to opt for Load Data. If I already have a spreadsheet where the data is defined, what do I do next? I type Load Data in the app navigator. What next? I create an import set. What next? I choose the source of my data. Clearly the answer is D. If I wanted to start before I knew what the data was then I would select E. However, since the question states that I already have that in the form of a spreadsheet then the answer is D.

---

**👤 Raxon** ⬆️ 1

**Selected Answer:** D
Answer is D. 

"Import Sets" Is under 
"System Import Sets > Advanced > Import Sets" 
Which if you create a new instance is "Empty" & has no way to Load Data. Import Sets only shows you the imported data sources.

Under All menu - The "Integration - Imports" drop down is called "System Import Sets" Load Data is the Module that begins an import to ServiceNow. 

After filling out the form & clicking submit it states "Next Steps...." & provides the following links. 

Import Sets - Go to the import Sets for this data load.
Loaded Data.
Create Transformation Map. 
Import Logs.

---

**👤 1595** ⬆️ 2

Right Ans IS option E

---

**👤 BrooksieOwl** ⬆️ 2

Bit naughty this one and I'd imagine these options wouldnt be the same. If you browse, you have to select "Select Import Set" before you select "Load Data" but you could also type Load Data and you could select that without selecting "Select Import Set"

---

**👤 MACCA81** ⬆️ 3

**Selected Answer:** C
I would go with C here. The Module you use to load data is indeed called "Load Data" module, but once you open it, the first thing it asks you to do is select (or create a new ) Import Set table, then you need to choose a data source, and only after you do these 2 things you can click "Submit" which Loads the data....

*...and 4 more comments*


</details>

---

## Question 132

### Tables may be set up with Many to Many relationships. What is a classic example of a scenario where the tables would have many to many relationships?

### Answer Choices:
**A.** Vendors can sell multiple products; and products can be sold by multiple vendors.

**B.** A Task can trigger many Workflows; and a Workflow can trigger many Tasks.

**C.** Requests can contain many Items; and Items can be any item from the catalog.

**D.** A Configuration Item can belong to multiple Classes, and Classes can contain multiple Configuration Items.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 missmam** ⬆️ 5

**Selected Answer:** A
option A is the example used in the Data schema overview lesson under Additional SN fundamentals content for Sys Admins & Developers.

---

**👤 dnbly** ⬆️ 2

**Selected Answer:** A
Can't be B as this would 1) cause a loop but also 2) it would be the workflow generating the task that triggers another (sub)flow, not the task itself. Workflow-to-task is one-to-many.

---

**👤 Sallex** ⬆️ 4

**Selected Answer:** A
A should be the correct answer

---

**👤 rittersportler** ⬆️ 4

**Selected Answer:** A
A should be the correct answer. Workflows and tasks are not two-way (i.e. a task can trigger a workflow, but this workflow will not trigger the same task - otherwise you have a loop...)



</details>

---

## Question 133

### What section on a task record would you use to see the most recent update made to a record?

### Answer Choices:
**A.** Audit Log

**B.** Timeline

**C.** Activity

**D.** Journal



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
C - this is where you see the full list of activities

---

**👤 PappyFox** ⬆️ 2

C is correct



</details>

---

## Question 134

### The Employee On-boarding team has asked for a way for managers to order computers, monitors, business cards, and cell phones for new employees. How would you proceed to meet this requirement?

### Answer Choices:
**A.** Create Requested Item

**B.** Create Record Producer

**C.** Create On-boarding Bot

**D.** Create Order Guide



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - create order guide

---

**👤 PappyFox** ⬆️ 2

D. is correct



</details>

---

## Question 135

### When using the Load Data and Transform Map process, what is the Mapping Assist used for?

### Answer Choices:
**A.** Mapping fields using the Import Log

**B.** Mapping fields using Transform History

**C.** Mapping fields using an SLA

**D.** Mapping fields using a Field Map



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 julianamendo** ⬆️ 2

**Selected Answer:** D
D is the correct answer.

---

**👤 saiakhilpalle** ⬆️ 1

D is correct

---

**👤 KIKI_AI** ⬆️ 2

**Selected Answer:** D
D is correct

---

**👤 Bonche** ⬆️ 1

D is the correct answer

---

**👤 trye_indicators** ⬆️ 3

**Selected Answer:** D
https://docs.servicenow.com/bundle/sandiego-platform-administration/page/script/server-scripting/concept/c_MappingOptions.html

---

**👤 bharatns** ⬆️ 2

correct



</details>

---

## Question 136

### On the CI Dependency View, what enables you to trace from an infrastructure item, like a Server, to the Services that are dependent on that Server?

### Answer Choices:
**A.** Automapping Utility

**B.** Relationships

**C.** Service Tracer

**D.** Transform Map



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
B - you can see the dependencies

---

**👤 sri_ravi** ⬆️ 1

B is correct

---

**👤 PappyFox** ⬆️ 1

B is correct



</details>

---

## Question 137

### From a related list, what would a user click to personalize the layout of the columns?

### Answer Choices:
**A.** Gear

**B.** Context Menu

**C.** Pencil

**D.** Magnifier



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

A is correct

---

**👤 f5003a6** ⬆️ 3

**Selected Answer:** A
A. Gear

---

**👤 davgmane** ⬆️ 2

**Selected Answer:** A
A is correct

---

**👤 alaro** ⬆️ 2

A is the correct answer

---

**👤 PappyFox** ⬆️ 3

**Selected Answer:** A
Correct answer is A



</details>

---

## Question 138

### What is the language used for scripting in ServiceNow?

### Answer Choices:
**A.** C++

**B.** JavaScript

**C.** PHP

**D.** Python



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

B is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** B
B. JavaScript

---

**👤 alaro** ⬆️ 1

B is the correct answer

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** B
B IS CORRECT



</details>

---

## Question 139

### What are examples of UI Actions, relating to Lists? (Choose four.)

### Answer Choices:
**A.** List Links

**B.** List Choices

**C.** List Buttons

**D.** List Override

**E.** List Context Menu

**F.** List Control



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABCE
**Community voting results:** ABCE (89%), 5%

</details>

<details>
<summary><strong>💬 View Discussion (14 comments)</strong></summary>

**👤 ALDADI** ⬆️ 6

**Selected Answer:** ABCE
List links - List Choice - List button - List Style - List Context menus

---

**👤 153df45** ⬆️ 1

**Selected Answer:** ABCE
A. List Links
B. List Choices
C. List Buttons
E. List Context Menu

---

**👤 sri_ravi** ⬆️ 1

ABCE correct options

---

**👤 f5003a6** ⬆️ 2

**Selected Answer:** ABCE
A. List Links
B. List Choices
C. List Buttons
E. List Context Menu

---

**👤 tata23** ⬆️ 1

ABCE is correct links buttons context menu

---

**👤 Kubis** ⬆️ 1

**Selected Answer:** ABCE
ABCE is correct, you can find it SNAF book page 435: 
https://evantage.gilmoreglobal.com/reader/books/SN-SNAF-T010-PG-E/pageid/435

---

**👤 hananel** ⬆️ 4

**Selected Answer:** ABCE
ABCE are the right answers, check page 436 of the ebook to confirm

---

**👤 Spiderman5000** ⬆️ 1

**Selected Answer:** ABCE
I am changing my answer :-)
"A,B,C,D,E" are all correct. "F" is wrong. The list itself is not a UI Action.

---

**👤 Spiderman5000** ⬆️ 1

**Selected Answer:** ACDE
"F" is wrong. The list itself is not a UI Action. "B" is sort of correct, but not, since a UI Action is a single choice, not plural "choices".

---

**👤 rittersportler** ⬆️ 2

**Selected Answer:** ABCE
Confusing question as it uses plurals which are not part of the UI action > New record.

*...and 4 more comments*


</details>

---

## Question 140

### A Service Catalog project will involve building 80 catalog items. For each of the catalog items, the following fields will be mandatory on the forms:
• Requested for
• Requested by
• Approving manager
• Delivery instructions

All of the other variables will be specific to the individual catalog item. What features would you use when designing the catalog item form?

### Answer Choices:
**A.** Create a Variable Set Template, then apply to all of the catalog items.

**B.** Create one Variable Set for the four variables, then add that variable set to each of the 80 catalog items.

**C.** Create a Record Producer that contains the four fields; then add to the record producer related list on the Catalog Items.

**D.** Create a Flow Designer Action, with Variable Set Data Pill, then apply flow to all of the 80 catalog items.

**E.** Create an Order Guide, which includes all variables; then copy and hide variables as needed.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

B is correct

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** B
Variable set is correct.

---

**👤 Dekoreh** ⬆️ 2

**Selected Answer:** B
B is correct based on the definition of variable set



</details>

---

## Question 141

### A task worker asks how they can monitor any updates occurring to records assigned to him, like responses from customers. What do you suggest?

### Answer Choices:
**A.** On My Work list, select the Activity Stream icon to show a frame with live updates

**B.** Click on the eyeglass icon to expand the Monitor frame

**C.** Open an Agent workspace tab for each record he wants to monitor

**D.** Select Service Desk > My Work Dashboard



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

A is correct

---

**👤 alaro** ⬆️ 1

A is the correct answer

---

**👤 PappyFox** ⬆️ 1

A is correct



</details>

---

## Question 142

### What access does a user need to be able to import articles to a knowledge base?

### Answer Choices:
**A.** sn_knowledge_import

**B.** sn_knowledge_contribute

**C.** Can contribute

**D.** Can import



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (83%), A (17%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 PappyFox** ⬆️ 5

C is correct

---

**👤 wnstn75** ⬆️ 2

**Selected Answer:** C
https://www.servicenow.com/community/training-and-certifications/that-access-does-a-user-need-to-be-able-to-import-articles-to-a/td-p/3134613

---

**👤 Kflo** ⬆️ 3

C. What ‘’access” not “role”

---

**👤 SnowNick** ⬆️ 3

**Selected Answer:** C
C is correct:
https://www.servicenow.com/docs/bundle/xanadu-servicenow-platform/page/product/knowledge-management/task/import-word-platform.html

---

**👤 aper** ⬆️ 1

**Selected Answer:** A
In ServiceNow, the sn_knowledge_import role is specifically used to manage and facilitate the importing of knowledge articles into the Knowledge Base.

---

**👤 sri_ravi** ⬆️ 1

A is correct



</details>

---

## Question 143

### When importing data from a spreadsheet, which step defines where the incoming data columns will be written in the receiving table?

### Answer Choices:
**A.** Schedule Transform

**B.** Field Matching

**C.** Select Data Source

**D.** Create Transform Map

**E.** Load Data



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (67%), B (33%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 FOJ** ⬆️ 5

**Selected Answer:** B
Answer is B. 
Field Matching is the step that defines where the incoming data columns will be written in the receiving table when importing data from a spreadsheet. During the field matching step, you map the columns in the source file to the fields in the target table. This allows you to specify which columns in the source file correspond to which fields in the target table, ensuring that the data is imported correctly.

---

**👤 Nexfo** ⬆️ 3

**Selected Answer:** D
Filed Mapping is what you do in Create Transform Map

The B answer says Filed Matching not Filed Mapping...

---

**👤 sri_ravi** ⬆️ 1

B is correct

---

**👤 connal308876** ⬆️ 3

D is correct. Field match was never considered a step. However Create Transform Map, is considered a step in the process.

---

**👤 a00e073** ⬆️ 2

**Selected Answer:** D
Questions is on which "step", so i would go with: D.Create Transform Map. At transform map you can then do field matching but the "step" is D.

---

**👤 MACCA81** ⬆️ 2

Can I say NONE of the above? In reality you can create a transform map record by just specifying the source and target tables and clicking submit. You need to do the firld mapping before you transform. But it's Field MAPPING not Field Matching and it's done in the Transform Map (so it has to be there) through Auto Field Map Matching or Mapping Assist links. Very confusing question...

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** D
D - Create Transform Map is the correct answer. 
B - Field Matching: For Event Management, when creating Event Rule to bind non-host CI using CI field matching, if there are multiple CI records having same name.

---

**👤 hc1222** ⬆️ 1

B is correct

---

**👤 Mahe12** ⬆️ 3

**Selected Answer:** D
Answer should be Field Mapping. But we dont have that option. 
So its D because Transform Map is a set of field maps.

---

**👤 KevFun** ⬆️ 1

Wouldn't it be B if it was spelled correctly to Field Mapping? Note that not only the questions could be spelled incorrectly, I recognize that many answers are spelled incorrectly with or without purpose.

*...and 2 more comments*


</details>

---

## Question 144

### To apply a UI Policy to all views, which field should be set to true in its definition record?

### Answer Choices:
**A.** Global

**B.** Reverse if false

**C.** On load

**D.** Inherit



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

A is correct

---

**👤 karlouioanna** ⬆️ 1

To apply a UI Policy to all views, set the Global setting to true.

---

**👤 YMijoe** ⬆️ 2

**Selected Answer:** A
Per my PDI, "if the 'Global' checkbox is selected, the UI Policy applies to all form views; otherwise the UI Policy is view-specific".

---

**👤 99Sekhar** ⬆️ 2

Answer is A. If UI policy applies to extended table then answer is D. So here answer is A.

---

**👤 99Sekhar** ⬆️ 2

Answer is A.
Global [Advanced view] Option for specifying whether the UI policy applies to all form views. If this check box is cleared, the UI policy is view-specific.
By default, the Global UI policy applies to all form views. However, a UI policy can be specific to a view. For example, you can define a UI policy for only the itil view of a form. Use the View field to accomplish this.

---

**👤 smeagel** ⬆️ 2

**Selected Answer:** A
Answer is A

---

**👤 Vishy123** ⬆️ 2

A : Global
https://docs.servicenow.com/bundle/sandiego-platform-administration/page/administer/form-administration/task/t_CreateAUIPolicy.html

---

**👤 jjjrrrferere** ⬆️ 2

**Selected Answer:** A
yes global makes the most sense

---

**👤 ALDADI** ⬆️ 3

**Selected Answer:** A
Global



</details>

---

## Question 145

### What are the steps for importing data using an import set?

### Answer Choices:
**A.** Select source file; Run automap; Transform data; Clean up target table

**B.** Identity source; Import transform map; Run transformer, Verify import

**C.** Setup LDAP; Test map; Create update set; Run import; Apply update set

**D.** Load the data; Create transform map; Transform data; Clean up import table



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - contains the major steps.

---

**👤 sri_ravi** ⬆️ 1

D is correct

---

**👤 PappyFox** ⬆️ 2

D. https://docs.servicenow.com/bundle/tokyo-platform-administration/page/administer/import-sets/concept/c_ImportDataUsingImportSets.html



</details>

---

## Question 146

### Which one of the following statements describes the contents of the Configuration Management Database (CMDB)?

### Answer Choices:
**A.** The CMDB contains data about tangible and intangible business assets

**B.** The CMDB contains the Business Rules that direct the intangible, configurable assets used by a company

**C.** The CMDB archives all Service Management PaaS equipment metadata and usage statistics

**D.** The CMDB contains ITIL process data pertaining to configuration items



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** A
Answer

---

**👤 julianamendo** ⬆️ 2

**Selected Answer:** A
A is the correct answer.

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Bonche** ⬆️ 2

A is the correct answer

---

**👤 trye_indicators** ⬆️ 3

The configurations are stored in a configuration management database (ServiceNow CMDB) which consists of entities, called Configuration Items (CI), that are part of your environment. A CI may be:

A physical entity, such as a computer or router
A logical entity, such as an instance of a database
Conceptual, such as a Requisition Service

https://docs.servicenow.com/bundle/sandiego-servicenow-platform/page/product/configuration-management/concept/cnfig-mgmt-and-cmdb.html

---

**👤 Markster** ⬆️ 1

Per the SN Fundamentals book, Configuration Items can be tangible or intangible devices or applications in the CMDB such as firewalls, computers, email services, and services.

---

**👤 bharatns** ⬆️ 1

Shouldn't it be "D. The CMDB contains ITIL process data pertaining to configuration items"?



</details>

---

## Question 147

### Which type of scripts run in the browser?

### Answer Choices:
**A.** Script Include Scripts

**B.** Access Control Scripts

**C.** Business Rule Scripts

**D.** UI Policies and Client Scripts



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - both UI Policies and Client Scripts run on the Client side.

---

**👤 sri_ravi** ⬆️ 1

D is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** D
D. UI Policies and Client Scripts

---

**👤 PappyFox** ⬆️ 2

**Selected Answer:** D
Correct answer is D



</details>

---

## Question 148

### Which modules can you use to create a new table? (Choose two.)

### Answer Choices:
**A.** Dictionary

**B.** Schema Map

**C.** Tables

**D.** Tables & Columns



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** CD
**Community voting results:** CD (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 Vishy123** ⬆️ 7

CD is correct

---

**👤 studyGUYd** ⬆️ 5

I just hopped in my instance and tested.

Tables

Tables & Columns

Dictionary 

Are all valid ways to find the button to create a new table (Utah)

---

**👤 Schoolboz** ⬆️ 1

**Selected Answer:** CD
It cannot be A because you cannot create a table in Dictionary.

---

**👤 sri_ravi** ⬆️ 1

CD is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** CD
C. Tables
D. Tables & Columns

---

**👤 davgmane** ⬆️ 2

**Selected Answer:** CD
CD is the answer

---

**👤 99Sekhar** ⬆️ 2

AC table creation



</details>

---

## Question 149

### Which one of the following describes the primary operations performed against tables in the ServiceNow platform?

### Answer Choices:
**A.** Create, Read, Upload, Delete

**B.** Capture, Rate, Write, Develop

**C.** Create, Rate, Update, Delete

**D.** Create, Read, Write, Delete



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (71%), C (19%), 10%

</details>

<details>
<summary><strong>💬 View Discussion (14 comments)</strong></summary>

**👤 Manny1218** ⬆️ 6

**Selected Answer:** D
Please read page 367 from the ServiceNow Administration Fundamentals - Tokyo booklet.
It states Create, Delete, Read, Write. 

There is no Upload. 

There is an update, as stated in answer "C" but "C" also contains "Rate" which is also incorrect. 

Trick question!

---

**👤 Nexfo** ⬆️ 1

**Selected Answer:** D
Create, Read, Write, Delete
To make it easier to remember they acronym CRUD was introduced, where U stands for Update.

But such choice is not available in AC rules, you have WRITE only.

CRUD is for ease of remembering the basic operations, but it NOT the real option available.

---

**👤 sri_ravi** ⬆️ 1

D is correct

---

**👤 SamusSamus** ⬆️ 2

**Selected Answer:** D
The correct answer is D. In ServiceNow a CRUD is named as a CRWD, Create, Read, Write and Delete, be careful and take your time to understand the question

---

**👤 sally41** ⬆️ 1

**Selected Answer:** D
D is the right answer.

---

**👤 RoupT** ⬆️ 2

**Selected Answer:** D
Correct Answer is D

Choice A has "Upload" this is incorrect
Choice C has "Rate" this is incorrect

---

**👤 RoupT** ⬆️ 3

**Selected Answer:** D
The answer is D. Please read each choice in the question carefully. CRUD / CRWD are used interchangeably. 
Create / Read / Update or Write / Delete
The screen shots proof is here

https://www.servicenow.com/community/developer-forum/crud-or-crwd/m-p/2583304

---

**👤 studyGUYd** ⬆️ 2

"CRUD", but the W for Write can replace the U for Update.

---

**👤 tangoalpha** ⬆️ 1

**Selected Answer:** C
https://docs.servicenow.com/bundle/utah-it-service-management/page/product/configurable-workforce-optimization-itsm/reference/components-installed-worforce-optimization-itsm.html

---

**👤 davgmane** ⬆️ 1

Create, Read, Update, Delete is CRUD

*...and 4 more comments*


</details>

---

## Question 150

### How is a user defined in ServiceNow?

### Answer Choices:
**A.** A user is a record stored in the Profile [sys_user_profile] table

**B.** A user is a record stored in the User [sys_user] table

**C.** A user is a record stored in the User Preference [sys_user_preference] table

**D.** A user is a field in the LDAP integration



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

B is the answer

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** B
B is the correct answer



</details>

---

## Question 151

### Which ServiceNow utility gives a Service Desk agent the ability to trace from a Service having an issue, to see which CIs supporting that service have active issues?

### Answer Choices:
**A.** CI Dependency View

**B.** Event Management Homepage

**C.** Service Dashboard

**D.** CI Health Dashboard



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (65%), D (35%)

</details>

<details>
<summary><strong>💬 View Discussion (18 comments)</strong></summary>

**👤 Manny1218** ⬆️ 6

**Selected Answer:** A
According to the ServiceNow Administration Fundamentals - Tokyo, page 427, it states, Dependency Views indicate if a CI has any active, pending issues."

I'd select A on the Exam

---

**👤 tata23** ⬆️ 5

Answer is A per SN Fundamentals book p.361-362

---

**👤 aakash101** ⬆️ 1

**Selected Answer:** D
https://www.servicenow.com/docs/bundle/washingtondc-servicenow-platform/page/product/configuration-management/concept/cmdb-health-dashboards.html

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
A - dependency view

The CI Health Dashboard is used to look at things accuracy/completeness of CI information.

---

**👤 Ratatoeskr** ⬆️ 1

**Selected Answer:** A
Dependency views shows the related services down to the affected cis

---

**👤 sri_ravi** ⬆️ 1

A is the answer

---

**👤 BlackCarpet** ⬆️ 1

**Selected Answer:** A
Answer is A

---

**👤 connal308876** ⬆️ 2

This is tough. I leaning toward A because the question is relates to what a Service Desk user would use; not an Administrator.

---

**👤 AlexA1457** ⬆️ 2

**Selected Answer:** A
According to the ServiceNow Administration Fundamentals - "it states, Dependency Views indicate if a CI has any active, pending issues."

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** D
https://docs.servicenow.com/bundle/washingtondc-servicenow-platform/page/product/configuration-management/task/t_ViewCIHealth.html

*...and 8 more comments*


</details>

---

## Question 152

### Which feature enables business process owners to organize Flow Designer content into unified and digitized cross-enterprise processes via a digitized task board interface?

### Answer Choices:
**A.** Flow Designer

**B.** Workflow Editor

**C.** Process Workflow Designer

**D.** Process Automation Designer



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

D is the answer

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** D
D. Process Automation Designer

---

**👤 buchibabe** ⬆️ 2

D is correct. Page 237 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 yo_mamma** ⬆️ 3

D is correct
https://docs.servicenow.com/bundle/utah-build-workflows/page/administer/process-automation-designer/concept/process-automation-designer.html



</details>

---

## Question 153

### An IT user calls the service desk because they need to work on task records. All they can see is Self Service on their homepage when they login to the ServiceNow instance. What issue could explain this? (Choose two.)

### Answer Choices:
**A.** Their user account does not have itil role

**B.** Their user account was not approved by their manager

**C.** Their user account is not logged in properly

**D.** Their user account failed LDAP authentication

**E.** Their user account does not belong to any groups, which contain the itil role



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AE
**Community voting results:** AE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

AE is correct answer

---

**👤 connal308876** ⬆️ 1

CE in my opinion. ITIL role should be given to the group; not the user account. If they are not logged on properly, is more of a likely scenario or they are still Impersonating someone and forgot.

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** AE
A. Their user account does not have itil role
E. Their user account does not belong to any groups, which contain the itil role

---

**👤 PappyFox** ⬆️ 4

AE are correct



</details>

---

## Question 154

### On a related list, which buttons are commonly used for managing the records on the list? (Choose three.)

### Answer Choices:
**A.** Add

**B.** Edit

**C.** Publish

**D.** Manage

**E.** New



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABE
**Community voting results:** ABE (93%), 7%

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

ABE correct answer

---

**👤 jjn9306** ⬆️ 1

**Selected Answer:** ABE
D. is not correct. In Utah, there are no UI Actions named "manage", but there are "Add" UI Actions

---

**👤 f5003a6** ⬆️ 2

**Selected Answer:** ABE
A. Add
B. Edit
E. New

---

**👤 servicepow** ⬆️ 1

**Selected Answer:** ABE
ABE correct answer

---

**👤 PappyFox** ⬆️ 2

**Selected Answer:** ABE
Correct answer is ABE

---

**👤 Sallex** ⬆️ 4

**Selected Answer:** ABE
Should Be ABE

---

**👤 evoken** ⬆️ 2

**Selected Answer:** ABE
Can only see Add,Edit and New

---

**👤 Philippvs** ⬆️ 1

**Selected Answer:** BE
What I have found so far mentioned about Edit, Collection, Save, New and Submit.
https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/use/using-forms/task/t_SelectRelatedRecords.html

---

**👤 smeagel** ⬆️ 2

**Selected Answer:** ABE
ABE should be correct



</details>

---

## Question 155

### A customer requests the following data quality measures be added:
• Incident numbers should be read only, on all lists and forms, for all users.
• Short Description field should be mandatory, on all records, across all applications, on Insert.

Which type of policy would you use to meet this requirement?

### Answer Choices:
**A.** Data Quality Policy

**B.** Dictionary Design Policy

**C.** Data Policy

**D.** Field Criteria Policy



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

C is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. Data Policy

---

**👤 Kev06** ⬆️ 1

shouldn't this be D - field criteria policy?

---

**👤 PappyFox** ⬆️ 3

**Selected Answer:** C
C is correct



</details>

---

## Question 156

### On what part of the ServiceNow instance, would you find the option to access applications, like Incident Management?

### Answer Choices:
**A.** Application Navigator

**B.** Service Desk Homepage

**C.** Self Service Module

**D.** Favorites



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

A is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** A
A. Application Navigator

---

**👤 ba14741** ⬆️ 1

**Selected Answer:** A
It's A

---

**👤 PappyFox** ⬆️ 4

**Selected Answer:** A
Answer is A

---

**👤 SofiHAn** ⬆️ 3

**Selected Answer:** A
Answer should be A

---

**👤 mraiman** ⬆️ 3

**Selected Answer:** A
A is correct

---

**👤 Sallex** ⬆️ 3

**Selected Answer:** A
Answer is A

---

**👤 evoken** ⬆️ 1

Answer is A.

---

**👤 Philippvs** ⬆️ 2

**Selected Answer:** A
The Self-Service is an application and this is why it's in the Application Navigator.

---

**👤 SemperJulio7** ⬆️ 1

Answer is A. Self-Service is not a Module. It is an application.



</details>

---

## Question 157

### In what order should filter elements be specified?

### Answer Choices:
**A.** Field, Operator, then Value

**B.** Field, Operator, then Condition

**C.** Operator, Condition, then Value

**D.** Value, Operator, then Field



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 julianamendo** ⬆️ 2

**Selected Answer:** A
A is the correct answer.

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Potelter** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Jana08** ⬆️ 1

**Selected Answer:** A
A is correct:

https://docs.servicenow.com/bundle/utah-platform-user-interface/page/use/using-lists/task/t_CreatingFilters.html

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 anku15** ⬆️ 1

**Selected Answer:** A
A is correct!

---

**👤 saiakhilpalle** ⬆️ 1

A is correct

---

**👤 Bonche** ⬆️ 1

A is the correct answer

---

**👤 bharatns** ⬆️ 3

A is correct



</details>

---

## Question 158

### What catalog tool would you use to create a catalog item or record producer?

### Answer Choices:
**A.** Catalog Builder

**B.** Workflow Designer

**C.** Catalog Designer

**D.** Catalog Formatter



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

A is correct

---

**👤 f5003a6** ⬆️ 2

**Selected Answer:** A
A. Catalog Builder

---

**👤 PappyFox** ⬆️ 3

**Selected Answer:** A
A is the correct answer



</details>

---

## Question 159

### On a form, which type of field has this icon which can be clicked, to see a preview of the associated record?

//IMG//

### Answer Choices:
**A.** Lookup

**B.** Preview

**C.** Reference

**D.** Snapshot

**E.** Quickview

**F.** Drilldown



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (83%), B (17%)

</details>

<details>
<summary><strong>💬 View Discussion (16 comments)</strong></summary>

**👤 Jana_GREG** ⬆️ 5

**Selected Answer:** C
The icon is preview but the question is "what type of field > Reference field is the answer

---

**👤 adyrip** ⬆️ 2

**Selected Answer:** C
C - Reference field. It's a trick question, the button is preview but there is no Preview Field Type in ServiceNow. So the only logical answer is Reference.

---

**👤 sri_ravi** ⬆️ 2

C is correct

---

**👤 f5003a6** ⬆️ 2

**Selected Answer:** C
C. Reference

---

**👤 krissyk** ⬆️ 2

C

https://docs.servicenow.com/en-US/bundle/utah-platform-administration/page/administer/field-administration/concept/c_ReferenceIcon.html

---

**👤 stevefido** ⬆️ 1

I can confirm that B is the answer.

---

**👤 StarWarsTrader** ⬆️ 1

**Selected Answer:** B
B "Preview this Record" is the correct answer. Tested on Utah

---

**👤 PappyFox** ⬆️ 1

C is the correct answer

---

**👤 yo_mamma** ⬆️ 1

C is correct

---

**👤 SamIV444** ⬆️ 1

**Selected Answer:** C
Answer is C

*...and 6 more comments*


</details>

---

## Question 160

### While on an Incident record, how would you add a Tag for "Special Handling" to the record?

### Answer Choices:
**A.** Click on the Context menu, select Add Tag, type Special Handling, press enter

**B.** Click on the More options (...) icon, click Add Tag, type Special Handling, press enter

**C.** On the Tag field, select Special Handling from the choice list

**D.** On the Special Handling field, check the box



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

B is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** B
B. Click on the More options (...) icon, click Add Tag, type Special Handling, press enter

---

**👤 Kubis** ⬆️ 1

**Selected Answer:** B
Just tested it in my instance - B is correct.

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** B
B is the correct answer

---

**👤 al3xunder** ⬆️ 2

B is the right answer

---

**👤 Shakir2023** ⬆️ 1

I think option A will more fit here.



</details>

---

## Question 161

### What feature allows you to limit who is able to contribute or read knowledge within a knowledge base?

### Answer Choices:
**A.** Roles

**B.** Groups

**C.** User Criteria

**D.** Categories



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

C is correct

---

**👤 PappyFox** ⬆️ 2

**Selected Answer:** C
C is the correct answer

---

**👤 yo_mamma** ⬆️ 2

C is correct
https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/knowledge-management/concept/diagnose-knowledge-user-criteria.html



</details>

---

## Question 162

### When moving a homepage or dashboard between instances, what must you remember?

### Answer Choices:
**A.** Manually add them to the update set

**B.** They cannot be moved via update set

**C.** They are automatically added to the update set

**D.** Create a separate update set for them



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (75%), B (25%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 fb9b779** ⬆️ 1

**Selected Answer:** A
A is correct. B is not correct- they are not *automatically* captured in an update set like other development items but still can be moved via update set if they are manually added.

---

**👤 dfec26e** ⬆️ 1

**Selected Answer:** B
B- don’t be fooled they ain’t part of a update set

---

**👤 sri_ravi** ⬆️ 1

A is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** A
A. Manually add them to the update set

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** A
A is the correct answer



</details>

---

## Question 163

### What is the platform name for the Group table?

### Answer Choices:
**A.** sys_groups

**B.** group

**C.** sys_user_group

**D.** sys_group



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

C is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. sys_user_group

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** C
C is the correct answer



</details>

---

## Question 164

### Many actions are included with flow designer, what are some frequently used core actions? (Choose four.)

### Answer Choices:
**A.** Look for Update

**B.** Create Record

**C.** Ask for Approval

**D.** Look Up Record

**E.** Wait for Condition

**F.** Wait for Match



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BCDE
**Community voting results:** BCDE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 imshop** ⬆️ 1

**Selected Answer:** BCDE
easy, must-know

---

**👤 sri_ravi** ⬆️ 1

BCDE correct

---

**👤 f5003a6** ⬆️ 2

**Selected Answer:** BCDE
B. Create Record
C. Ask for Approval
D. Look Up Record
E. Wait for Condition

---

**👤 PappyFox** ⬆️ 2

**Selected Answer:** BCDE
BCDE is the correct answer

---

**👤 yo_mamma** ⬆️ 1

BCDE is correct 
https://docs.servicenow.com/bundle/utah-build-workflows/page/administer/flow-designer/concept/flow-actions.html



</details>

---

## Question 165

### What role enables someone to authorize a request, with no other permissions on the platform?

### Answer Choices:
**A.** Approval Group [approval_group]

**B.** Authorize [authorize_user]

**C.** Reviewer [reviewer_user]

**D.** Verification [verify_user]

**E.** Approver [approver_user]



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** E
**Community voting results:** E (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

E is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** E
E. Approver [approver_user]

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** E
E is the correct answer

---

**👤 yo_mamma** ⬆️ 1

E is correct



</details>

---

## Question 166

### On the knowledge base record, which tab would you use to define which users are not able to write articles to the knowledge base?

### Answer Choices:
**A.** Can Contribute

**B.** Cannot Author

**C.** Can Read

**D.** Can Write

**E.** Can Author



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (54%), A (46%)

</details>

<details>
<summary><strong>💬 View Discussion (18 comments)</strong></summary>

**👤 jjjrrrferere** ⬆️ 14

The question is confusing, it seems like the correct answer would be "cannot contribute" which is not an option. If you *only* select "can read" then inherently those users can only read and not contribute. So technically this may be the correct answer 
https://docs.servicenow.com/bundle/quebec-servicenow-platform/page/product/knowledge-management/task/t_SelectUserCriteria.html

---

**👤 wnstn75** ⬆️ 1

**Selected Answer:** A
I think it's A. I did 2 tests. 
Knowledge > Administration > Knowledge Bases.=> open a record. 
1.Trying to add a role to both 'can read' and 'can contribute'. It worked. 
 So I think just setting 'can read' does not have full control to remove a write access. 
2.On 'Can contribute', I see we can add filter options like 'OR users does not contain xx'. 
 So this at least a way to let a user/users are NOT able to write. 
So my conclusion, either 'able to'/'not able to' is controlled in the same tab 'can contribute' for writing access. Just my opinion at the moment.

---

**👤 e780f6d** ⬆️ 2

**Selected Answer:** C
The answer choices are wrong. The answer is "cannot contribute" and it is not listed

---

**👤 574738b** ⬆️ 1

**Selected Answer:** C
CAN READ means they can’t contribute

---

**👤 Carissima** ⬆️ 1

**Selected Answer:** C
Can contribute refers to users that CAN actually contribute to the article, so the answer C because can read CANNOT contribute to the attribute, they can only read

---

**👤 Nexfo** ⬆️ 1

**Selected Answer:** C
Cannot Contribute would be the perfect answer.
For this question, if we add them to Can Read, this will limit their ability to create new articles (I think).
Anyway, it;s the closest thing to correct answer

---

**👤 Kflo** ⬆️ 1

**Selected Answer:** A
A - Can Contrbiture (because Cannot Contribute is not in the choices)

If you define can contribute, it means only the users in the criteria can write, the rest CANNOT.

---

**👤 sri_ravi** ⬆️ 1

Ans is B The "Cannot Author" tab is specifically designed to restrict users from writing or authoring articles in the knowledge base. The "Can Read" tab, on the other hand, only allows users to read the articles without making any changes.

---

**👤 ADILKS** ⬆️ 2

**Selected Answer:** C
Can Read: Users can read knowledge articles in the knowledge base.
Cannot Read: Users can't read knowledge articles in the knowledge base.
Can Contribute: Users can create, modify, and retire knowledge articles in a knowledge base. Contribute access to a knowledge base also provides read access to all articles in the knowledge base.
Cannot Contribute: Users can't create, modify, retire, or read knowledge articles in the knowledge base.

---

**👤 Fantastikaspiration** ⬆️ 1

Answer is A. On P.223 in the Vancouver study guide: User Criteria records are accessed from the Can read or Can contribute related lists.

*...and 8 more comments*


</details>

---

## Question 167

### What types of entities can receive task assignments, in ServiceNow? (Choose two.)

### Answer Choices:
**A.** Users

**B.** Departments

**C.** Groups

**D.** Teams



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AC
**Community voting results:** AC (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

ans is AC

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** AC
A. Users
C. Groups

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** AC
AC is the correct answer

---

**👤 yo_mamma** ⬆️ 1

AC is correct



</details>

---

## Question 168

### Which statement is true about business rules?

### Answer Choices:
**A.** A business rule must run before a database action occurs

**B.** A business rule can be a piece of Javascript

**C.** A business rule must not run before a database action occurs

**D.** A business rule monitors fields on a form



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (89%), 11%

</details>

<details>
<summary><strong>💬 View Discussion (19 comments)</strong></summary>

**👤 fauxmaux** ⬆️ 28

B is the correct answer,
BR can be set to run BEFORE OR AFTER the database action has occurred.
BR don't depends from form action. Other actions may have an impact on the table.
BR stdby, but WHEN the operation is to be performed is set.

---

**👤 Oluwalana54** ⬆️ 5

**Selected Answer:** B
The wording of the answer is very important. A and C seem to have been correct if the MUST is not in the wording.

---

**👤 armr** ⬆️ 2

**Selected Answer:** B
A business rule can run either before or after a database action and does not solely pertain to a form (can be applied to other SNow elements). It most definitely can be a script (js) on the server side. So, B is correct.

---

**👤 Aalu61** ⬆️ 1

A is the correct answer

---

**👤 KDingle** ⬆️ 1

The question says what must be true of BRs. A is true, but not necessarily the only truth about BRs. so A is acceptable.

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** B
B is the correct answer.

---

**👤 evoken** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 mrnow** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 PLiza2** ⬆️ 1

from: https://docs.servicenow.com/bundle/tokyo-application-development/page/script/business-rules/concept/c_BusinessRules.html
Before: After the user submits the form but before any action is taken on the record in the database.
After: After the user submits the form and after any action is taken on the record in the database.

---

**👤 4learning2u** ⬆️ 1

**Selected Answer:** A
A lot of answers here seem to be indicating A as the correct answer, but they selected B as the correct answer! The correct answer should be A.

*...and 9 more comments*


</details>

---

## Question 169

### The Report Designer contains different sections for configuring your report. Which section is used to adjust the look of your report, including colors, titles and legend layout?

### Answer Choices:
**A.** Layout

**B.** Format

**C.** Configure

**D.** Style



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

D is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** D
D. Style

---

**👤 buchibabe** ⬆️ 2

D is correct. Page 267 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** D
D is the correct answer

---

**👤 yo_mamma** ⬆️ 2

D is correct
https://docs.servicenow.com/bundle/utah-now-intelligence/page/use/reporting/task/t_CreateYourOwnReport.html

---

**👤 Alex_Lima84** ⬆️ 4

Style is the correct answer. You may change the Chart title and colors.

---

**👤 dflexin** ⬆️ 1

This is not right, it should be C



</details>

---

## Question 170

### How would you distinguish between a Base Class table and a Parent Class table?

### Answer Choices:
**A.** Extended tables are always extended from Parent tables. Extended tables are usually extended from Base tables.

**B.** Extended tables can be extended from Parent tables or Base tables; but they cannot be extended from both.

**C.** Base Class tables always have tables extended from them. Parent tables do not have tables extended from them.

**D.** Base Class table is not extended from another table, Parent class tables may be extended from another table.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

D is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** D
D. Base Class table is not extended from another table, Parent class tables may be extended from another table.

---

**👤 PappyFox** ⬆️ 2

**Selected Answer:** D
D is the correct answer

---

**👤 yo_mamma** ⬆️ 1

D is correct

---

**👤 memope** ⬆️ 3

From the Table extensions and classes in Servicenow Product documentation 
"A table that extends another table is called a child class, and the table it extends is the parent class. A table can be both a parent and child class both extending and providing extensions for other tables. A parent class that is not an extension of another table is called a base class."



</details>

---

## Question 171

### When a custom table is created, which access control rules are automatically created? (Choose four.)

### Answer Choices:
**A.** create

**B.** delete

**C.** execute

**D.** update

**E.** read

**F.** write



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABEF
**Community voting results:** ABEF (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Nexfo** ⬆️ 2

**Selected Answer:** ABEF
Nobody votes for CRUD here? :)
CRWD is the correct combo of course!

---

**👤 Kflo** ⬆️ 1

**Selected Answer:** ABEF
SNAF Book Washington
Topic: System Created Access Controls


CRUD is something else - specific to access control restriction

---

**👤 sri_ravi** ⬆️ 1

ABEF correct

---

**👤 f5003a6** ⬆️ 2

**Selected Answer:** ABEF
A. create
B. delete
E. read
F. write

---

**👤 5852843** ⬆️ 3

I feel like this is a bit of a trick question. Write = allows users to update records in a table. The acronym CRUD uses update vs delete. I think update is also a correct answer.

---

**👤 EricSon** ⬆️ 1

**Selected Answer:** ABEF
https://docs.servicenow.com/bundle/vancouver-platform-security/page/administer/contextual-security/concept/acl-rule-types.html

---

**👤 buchibabe** ⬆️ 3

ABEF is correct. Page 367 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** ABEF
ABEF is the correct answer



</details>

---

## Question 172

### Which banner icon do you use to change your personal system settings, like your instance color scheme?

### Answer Choices:
**A.** Magnifier

**B.** Question mark

**C.** Gear

**D.** Chat bubbles



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

C is correct

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. Gear

---

**👤 MACCA81** ⬆️ 2

**Selected Answer:** C
C is correct from the answers provided, but technically, in the latest versions of SN, the gear icon is no longer on the banner. It appears in the User Profile settings (when you click on the user profile pic)

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** C
c is the correct answer

---

**👤 yo_mamma** ⬆️ 1

C is correct



</details>

---

## Question 173

### Which banner icon do you use to change your personal system settings, like your instance color scheme?

### Answer Choices:
**A.** Magnifier

**B.** Question mark

**C.** Gear

**D.** Chat bubbles



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

C is correct

---

**👤 MACCA81** ⬆️ 2

**Selected Answer:** C
C is correct from the answers provided, but technically, in the latest versions of SN, the gear icon is no longer on the banner. It appears in the User Profile settings (when you click on the user profile pic)

---

**👤 yo_mamma** ⬆️ 1

Question is repeated / C is correct



</details>

---

## Question 174

### When building an extended table from a base table, which fields do you need to create? (Choose two.)

### Answer Choices:
**A.** The fields that are not in the base table.

**B.** The mandatory fields for the base table.

**C.** The fields that are specific to the extended table.

**D.** The reference fields for the base table.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AC
**Community voting results:** AC (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

AC is correct

---

**👤 ADILKS** ⬆️ 1

**Selected Answer:** AC
AC is the correct answer

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** AC
AC is the correct answer

---

**👤 evoken** ⬆️ 2

**Selected Answer:** AC
Only And C make sense

---

**👤 Spiderman5000** ⬆️ 4

**Selected Answer:** AC
You do not need to create a reference field for the parent table.

---

**👤 Vishy123** ⬆️ 4

**Selected Answer:** AC
AC looks correct



</details>

---

## Question 175

### While showing a customer their Incident form, they ask to change the Priority field title to display their internal terminology PValue. How would you do that? (Choose two.)

### Answer Choices:
**A.** Right click on Priority and select Configure Label

**B.** Right click on Priority and select Configure Dictionary

**C.** Right click on Priority and select Configure Display Settings

**D.** Right click on Priority and select Configure Column



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AB
**Community voting results:** AB (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

AB is correct

---

**👤 connal308876** ⬆️ 1

In Washington, there is NO configure label.....fyi

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** AB
A. Right click on Priority and select Configure Label
B. Right click on Priority and select Configure Dictionary

---

**👤 Dekoreh** ⬆️ 1

AB is very correct

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** AB
AB is the correct answer



</details>

---

## Question 176

### As administrator, what must you do to access features of High Security Settings?

### Answer Choices:
**A.** Impersonate Security Admin

**B.** Select Elevate Roles

**C.** Add security_admin role to your user account

**D.** Use System Administration > Elevate Roles module



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

B is correct

---

**👤 Kubis** ⬆️ 1

**Selected Answer:** B
B - You will click on user menu in top right corner and click Elevate rolo, click checbox security_admin

---

**👤 PappyFox** ⬆️ 4

**Selected Answer:** B
B is the correct answer

---

**👤 yo_mamma** ⬆️ 3

B is correct

---

**👤 tonko8585** ⬆️ 1

There is no such role as security_admin. I believe it is B

---

**👤 RGGG** ⬆️ 2

B is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** B
Answer is B

---

**👤 Ibeauo** ⬆️ 2

Why on some question I can comment and on other I can't? If I would like to open a new discussion for another question where a discussion is not started yet, I got the message 'Custom discussion creation is under maintenance.'. 

There is a question (Question #252 Topic 1) regarding 'Create, Read, Update/Write, Delete' which confused me a lot. But I couldn't open a discussion because of mentioned error message.

---

**👤 Rido4good** ⬆️ 2

B should be the correct answer. You are suppose to elevate role

---

**👤 rittersportler** ⬆️ 2

Unclear question. According to the documentation, the security admin role is not automatically inherited by the admin and need to be assigned specific users. They then need to elevate their user roles to security_admin. So both B and C can be correct.... 
https://docs.servicenow.com/bundle/tokyo-platform-security/page/administer/security/concept/c_HighSecuritySettings.html

*...and 2 more comments*


</details>

---

## Question 177

### What section on the notes tab, shows the history of the work documented on the record?

### Answer Choices:
**A.** Audit Log

**B.** Timeline

**C.** Journal

**D.** Diary

**E.** Activity



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** E
**Community voting results:** E (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

E is correct

---

**👤 benarji** ⬆️ 1

E. An Activity Log (also known as an Activity Diary or a Job Activity Log) is a written record of how you spend your time. By keeping an Activity Log for a few days, you can build up an accurate picture of what you do during the day, and how you invest your time.

---

**👤 studyGUYd** ⬆️ 1

E

Activity Stream

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** E
E is the correct answer



</details>

---

## Question 178

### How would you navigate to the Schema map for a table?

### Answer Choices:
**A.** System Definition > Tables; Select Table; Go to Related links and click Show Schema Map

**B.** System Dictionary > Show Schema Map; Select Table

**C.** System Definition > Show Schema Map; Select Table

**D.** System Definition > Dictionary; Select Table; Go to Related links and click Show Schema Map



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

A is correct

---

**👤 benarji** ⬆️ 1

Answer: A
System Definition->Select Table->View Schema Map

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** A
A is the correct answer

---

**👤 yo_mamma** ⬆️ 1

Most likely answer is A
https://docs.servicenow.com/bundle/utah-application-development/page/administer/table-administration/task/t_GenerateASchemaMap.html



</details>

---

## Question 179

### Which of the following are a type of client scripts supported in ServiceNow? (Choose four.)

### Answer Choices:
**A.** onSubmit

**B.** onUpdate

**C.** onCellEdit

**D.** onLoad

**E.** onEdit

**F.** onChange

**G.** onSave



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACDF
**Community voting results:** ACDF (100%)

</details>

<details>
<summary><strong>💬 View Discussion (13 comments)</strong></summary>

**👤 Nicknameonetwo** ⬆️ 11

ACDF CORRECT
onSubmit when a form is submitted. 
onLoad execute script logic when forms are loaded
onCellEdit Client Script is for lists rather than form 
onChange when a particular field's value changes

https://developer.servicenow.com/dev.do#!/learn/courses/quebec/app_store_learnv2_scripting_quebec_scripting_in_servicenow/app_store_learnv2_scripting_quebec_client_side_scripting/app_store_learnv2_scripting_quebec_client_script_types

---

**👤 showboat_zw** ⬆️ 1

**Selected Answer:** ACDF
acdf is correct

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** ACDF
acdf is correct

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** ACDF
A,C,D,F is correct

---

**👤 BeepBopBoop** ⬆️ 1

Confirmed, ACDF

---

**👤 PradeepGen** ⬆️ 1

**Selected Answer:** ACDF
ACDF is correct

---

**👤 saiakhilpalle** ⬆️ 1

ACDF is correct option

---

**👤 Ranaway** ⬆️ 3

ACDF
Client Scripts are a specific kind of client-side script, which execute on forms (or in the case of onCellEdit Client Scripts, on the list view) of a record. Client Scripts come in four basic types: onLoad, onChange, onSubmit, and onCellEdit. Each type runs under different conditions, and often has a different use than the others.

---

**👤 Bonche** ⬆️ 2

A, C, D, F is the correct answers

---

**👤 som_420** ⬆️ 1

Given answer is correct

*...and 3 more comments*


</details>

---

## Question 180

### Which is the base table of the configuration management database hierarchy?

### Answer Choices:
**A.** cmdb_ci

**B.** cmdb

**C.** cmdb_rel_ci

**D.** ucmdb



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (68%), A (32%)

</details>

<details>
<summary><strong>💬 View Discussion (20 comments)</strong></summary>

**👤 chatterbox13** ⬆️ 6

**Selected Answer:** B
It's B: From SNAF Vancouver Manual, page 192: Three key CMDB tables are Base Configuration Item [cmdb], Configuration Item [cmdb_ci] which contains CI data, and CI Relationship [cmdb_rel_ci] which contains CI relationship data.

---

**👤 aakash101** ⬆️ 1

**Selected Answer:** A
cmdb – This is a broader table used to track CMDB metadata, not the base for CIs.

---

**👤 aakash101** ⬆️ 1

**Selected Answer:** A
https://www.servicenow.com/docs/bundle/yokohama-servicenow-platform/page/product/configuration-management/reference/cmdb-tables-details.html

---

**👤 sri_ravi** ⬆️ 1

B is correct

---

**👤 MACCA81** ⬆️ 3

**Selected Answer:** B
It's B

---

**👤 manikanta_gnbv** ⬆️ 2

**Selected Answer:** B
cmdb is the base table

---

**👤 al3xunder** ⬆️ 3

**Selected Answer:** B
CMDB is correct.

---

**👤 GoldBear** ⬆️ 2

**Selected Answer:** B
B - cmdb, has no parent.

---

**👤 gotbooboos** ⬆️ 2

Its B, cmdb. Open cmdb_ci table in ServiceNow you will see it extends cmdb table.

---

**👤 EricSon** ⬆️ 1

**Selected Answer:** A
From ChatGPT - The "cmdb_ci" table, also known as the Configuration Item (CI) table, is the base table in the CMDB hierarchy. It represents the top-level table that contains common fields and attributes for all Configuration Items. It serves as the foundation for all CI classes in the CMDB, such as servers, databases, applications, network devices, and more. Each CI class in the CMDB hierarchy has its own corresponding table that extends the "cmdb_ci" table.

*...and 10 more comments*


</details>

---

## Question 181

### Which best describes a field in a ServiceNow table?

### Answer Choices:
**A.** A field is a table row.

**B.** A field is an item that appears in a menu list.

**C.** A field is a table cell that stores data.

**D.** A field is a record in a table.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (83%), D (18%)

</details>

<details>
<summary><strong>💬 View Discussion (22 comments)</strong></summary>

**👤 f5003a6** ⬆️ 7

**Selected Answer:** C
C. A field is a table cell that stores data.
Record is a raw on table. so D is not correct.

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
C - a field is a cell in the table

---

**👤 sri_ravi** ⬆️ 1

c is correct

---

**👤 7c9877a** ⬆️ 1

**Selected Answer:** C
It's C. A row is a record.

---

**👤 gedgy** ⬆️ 1

**Selected Answer:** D
The answer is D per the following ServiceNow Doc link. Technically, C and D are both correct but ServiceNow uses the word "record". In the world of databases of course a field is a piece of data in a cell. Yet, I will remind you that ServiceNow likes the word record. I will also say that the term in all the ServiceNow training uses the phrase 'everything in ServiceNow is a record in a table'.

https://docs.servicenow.com/en-US/bundle/tokyo-platform-user-interface/page/use/using-forms/concept/c_FormFields.html

---

**👤 lica09** ⬆️ 3

**Selected Answer:** C
Record is a row in a table.

---

**👤 manikanta_gnbv** ⬆️ 2

**Selected Answer:** C
it's c not d

---

**👤 davrak2000** ⬆️ 4

**Selected Answer:** C
C is the correct answer. D would be a record is a row. A field is a column!!!!!!

---

**👤 davrak2000** ⬆️ 3

**Selected Answer:** C
C is the correct answer. D would be a record!!!!!!

---

**👤 EricSon** ⬆️ 2

**Selected Answer:** C
"A field represents an individual item of data on a record."
So it is a data, not a record.

*...and 12 more comments*


</details>

---

## Question 182

### What are examples of UI Actions relating to forms? (Choose three.)

### Answer Choices:
**A.** Form Columns

**B.** Form View

**C.** Form Buttons

**D.** Form Context Menu

**E.** Form Links



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** CDE
**Community voting results:** CDE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 sri_ravi** ⬆️ 1

CDE correct answer

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** CDE
C. Form Buttons
D. Form Context Menu
E. Form Links

---

**👤 manikanta_gnbv** ⬆️ 1

**Selected Answer:** CDE
100 % CDE

---

**👤 buchibabe** ⬆️ 2

CDE is correct. Page 436 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 Dekoreh** ⬆️ 2

CDE is correct

---

**👤 PappyFox** ⬆️ 2

**Selected Answer:** CDE
CDE is the correct answer



</details>

---

## Question 183

### Here is an example of the criteria set for a knowledge base:
• Companies: ACME North America
• Departments: HR
• Groups: ACME Managers
• Match All: Yes

In this example, what users would have access to this knowledge base?

### Answer Choices:
**A.** Members of the ACME Manager group, who are also members of HR Department and part of ACME North America

**B.** Employees of ACME North America, who are members of HR Department or the ACME Managers group

**C.** Users which are members of either ACME North America, or HR Department, or ACME Managers group

**D.** Members of the ACME Managers group, and HR department, regardless of geography



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 Kflo** ⬆️ 2

**Selected Answer:** A
A - match all = yes
so ALSO and AND are the hint

---

**👤 manikanta_gnbv** ⬆️ 2

**Selected Answer:** A
it's A

---

**👤 dnbly** ⬆️ 2

None of the above? Seems to be a common thing on these questions. A is close but the group name is specified as Mangers but the answer says Manager so.....

---

**👤 PappyFox** ⬆️ 4

**Selected Answer:** A
A is the correct answer



</details>

---

## Question 184

### In Flow Designer, where is the data from an action stored so it can be used in subsequent actions in the flow?

### Answer Choices:
**A.** Data Pill

**B.** Data Element

**C.** Data Trigger

**D.** Field Value

**E.** Field Icon



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 PappyFox** ⬆️ 1

**Selected Answer:** A
A is the correct answer



</details>

---

## Question 185

### A customer has asked for the following updates to a form:
• Make Resolution code Mandatory, when state is changed to Resolved
• Hide Major Incident check box, unless logged in user has Major Incident Manager role

What type of rule(s) would you use to implement this requirement?

### Answer Choices:
**A.** Form Constraint

**B.** UI Design

**C.** Field Limiter

**D.** UI Policy

**E.** Dictionary Design



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 manikanta_gnbv** ⬆️ 1

**Selected Answer:** D
D is the answer

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** D
D is the correct answer



</details>

---

## Question 186

### What setting allows users to view a Knowledge Base article even if they are not logged in?

### Answer Choices:
**A.** The Public setting

**B.** The View All setting

**C.** The ESS role

**D.** The Allow All role



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 malarag** ⬆️ 1

A is correct

---

**👤 MACCA81** ⬆️ 1

This Question is really confusing. I wonder if it's from an outdated version ServiceNow or I just don't understand it, but in the Vancouver CSA fundamentals book it's clearly stated that "If there's no user criteria specified in the "Can Read" related list then the Knowledge Base becomes open to the public." Same is the information in the ServiceNow forums: "To make a Knowledge base public navigate to Knowledge > Administration > Knowledge Bases. Select the knowledge base you want to make public. Go to the Can Read section or tab and remove any entries from the list. That way everyone gets read permission on all articles in that knowledge base." 
I could not find any information anywhere about a specific "Public setting". 
Nevertheless, I would probably have to go for answer A as the other ones are even more meaningless.

---

**👤 PappyFox** ⬆️ 1

**Selected Answer:** A
A is the correct answer

---

**👤 evoken** ⬆️ 3

The setting that allows users to view a Knowledge Base article even if they are not logged in is called "Public Knowledge Base". This setting can be enabled or disabled by ServiceNow administrators in the Knowledge Base Service Portal application.



</details>

---

## Question 187

### When adding a related list to a form, you choose the related list from the list collector. What is an example of a related list you might see on the list collector? (Choose three.)

### Answer Choices:
**A.** Problem==Parent

**B.** HR Case->Parent

**C.** Catalog Task->Parent

**D.** Outage->Task number

**E.** Release Phase==Parent



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BCD
**Community voting results:** BCD (67%), BCE (33%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 73301d1** ⬆️ 1

**Selected Answer:** BCE
Option Analysis Valid?
A. Problem == Parent Problem typically is a parent table, not a child. There's no standard reference in Problem pointing to another table via Parent. ❌ Unlikely to appear
B. HR Case → Parent ✅ sn_hr_core_case (HR Case) has a Parent reference field. This is used when nesting HR cases or associating them with a parent item. ✅ Correct
C. Catalog Task → Parent ✅ sc_task has a Parent reference field, pointing to sc_req_item. This is how tasks are tied to requested items. ✅ Correct
D. Outage → Task number Task number is not a reference field, it's a display field (number). The actual reference is likely Task or Affected CI. ❌ Incorrect
E. Release Phase == Parent ✅ rm_release_phase (Release Phase) has a Parent field that links back to rm_release. This is used in project/release planning. ✅ Correct

---

**👤 PopularOutcast** ⬆️ 2

Chatgpt says ACE for Washington DC release
A. Problem==Parent
C. Catalog Task -> Parent
E. Release Phase==Parent

---

**👤 40a783b** ⬆️ 2

**Selected Answer:** BCD
A. Problem==Parent - Problem alone re. list is not given but Problem Task
B. HR Case->Parent - correct
C. Catalog Task->Parent - correct
D. Outage->Task number - correct
E. Release Phase==Parent - == / ->

---

**👤 047a2d5** ⬆️ 2

Chat GPT choose ACD, and I am with in : 
These options indicate the relationships between the records. For example:

Problem->Parent: Shows related problems where the current record is the parent.
Catalog Task->Parent: Displays catalog tasks related to the parent record.
Outage->Task number: Lists outages associated with a specific task number.

---

**👤 047a2d5** ⬆️ 2

I see in the related list : Problem->Parent but the operator == is not their anymore, I see also outage-> Task number and Catalog Task-> Parent so for me CD I see them but HR Case not their anymore and for Release Phase-> Parent is their but the operator == no. so for me I am sure with CD



</details>

---

## Question 188

### How is the ServiceNow platform set up so that Administrators can easily configure their instances to send an email at the end of an upgrade?

### Answer Choices:
**A.** Administrators can update the email notification named "System Upgraded" in the System Logs module by adding the appropriate User to receive it.

**B.** Administrators can update the email notification named "System Upgraded" in the Notifications module by adding the appropriate User to receive it.

**C.** Administrators can write a Client Script to send out an email to the Administrator when an Update is complete.

**D.** Administrators can write a Business Rule to send out an email to the Administrator when an Update is complete.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 faiz10001** ⬆️ 6

**Selected Answer:** B
checked in Notification module, when to send-> send when(event is fired)->event name(system.upgraded)

---

**👤 fb9b779** ⬆️ 1

**Selected Answer:** B
Correct answer B: key word here is "easy" which would be to use a module rather than creating business rules.

---

**👤 MACCA81** ⬆️ 3

**Selected Answer:** B
Even not knowing for sure if there's a "System Upgraded" notification out of the box, I would go with B - First of all notifications are available in System Notification -> E-mail -> Notifications (so that rules out A), second of all why would the administrator want to send a notification for a completed upgrade (through a Client Script or a Business Rule) to himself? That rules out C and D. So B is the only plausible one.
But to confirm my logic I also went in and checked in my out of the box personal development instance and there is indeed a "System Upgraded" notification in System Notification -> E-mail -> Notifications

---

**👤 Manny1218** ⬆️ 3

**Selected Answer:** B
There is a "System Upgraded" notification. Head to System Notification -> Notification and type "*upgrade" in the name, you will see the "System Upgraded" Notification. 

For the Exam I would choose A

---

**👤 PappyFox** ⬆️ 3

D is correct "system upgraded" is not an out of box notification

---

**👤 yo_mamma** ⬆️ 2

Most likely answer is D
https://docs.servicenow.com/bundle/utah-application-development/page/script/business-rules/concept/c_BusinessRules.html

---

**👤 ellitetka** ⬆️ 2

I thick D is the correct answer



</details>

---

## Question 189

### A customer wants to be able to identify and track components of their infrastructure that support their eCommerce service. What ServiceNow products could support this requirement? (Choose three.)

### Answer Choices:
**A.** Asset Management

**B.** Discovery

**C.** Configuration Management (CMDB)

**D.** Service Mapping

**E.** Performance Analytics



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BCD
**Community voting results:** BCD (66%), ABC (34%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 ALDADI** ⬆️ 6

**Selected Answer:** BCD
Asset Management is out of scope for CSA.

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** BCD
BCD - Correct. 

C - correct, CMDB for obvious reasons
B- per Washington doc, Discovery scans the network to inventory devices
D- per Washington doc, Service Mapping or Top-Down Discovery augments the CDMB with IT relationships and dependencies.

---

**👤 al3xunder** ⬆️ 3

**Selected Answer:** BCD
https://www.servicenow.com/blogs/2023/steps-better-infrastructure-visibility.html#Adopt%20A%20Phased%20Plan%20of%20Attack

---

**👤 GoldenDisciple2** ⬆️ 2

**Selected Answer:** ABC
ABC makes the most since to me based off of google searches, comments, and ServiceNow documentation.

---

**👤 GoldBear** ⬆️ 4

**Selected Answer:** ABC
A - Asset Management is the first Application in ServiceNow that I learned about prior to studying for CSA. That is where items are identified.

B - Discovery is used to Track Components

C - CMDB is the base for the entire system.

D - Service Mapping - works hand in hand with Discovery.
https://www.servicenow.com/lpwhp/application-context-infastructure.html

---

**👤 Mihaf92** ⬆️ 1

it is ABD

---

**👤 dnbly** ⬆️ 2

**Selected Answer:** BCD
A is correct in practice but I don't believe it's actually a topic from the CSA course. BCD are correct and definitely covered topics.

---

**👤 HarisBMRE** ⬆️ 4

**Selected Answer:** ABC
"Their infrastructure" is handled with "Asset Management" 
"Discovery" can collect the assets
CMDB stores the asset information

---

**👤 it0m_man** ⬆️ 3

**Selected Answer:** BCD
BCD is Correct.

---

**👤 missmam** ⬆️ 4

**Selected Answer:** BCD
BCD seems more appropriate



</details>

---

## Question 190

### Which type of tables may be extended by other tables, but do not extend another table?

### Answer Choices:
**A.** Base Tables

**B.** Core Tables

**C.** Extended Tables

**D.** Custom Tables



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 PLiza2** ⬆️ 18

the wording of the question does not make sense. It should probably be: Which type of tables may be extended by other tables, but are not extended from another table? Or 'do not extend from another table'.

---

**👤 bharatns** ⬆️ 5

Base tables are those tables which are extended, but itself is not an extended table.

---

**👤 roopsrich** ⬆️ 1

**Selected Answer:** A
base tables like task

---

**👤 zzzhang98** ⬆️ 1

1. base class table: not extended from another table
2. Parent class table: maybe extended from another table

---

**👤 Holy_97** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 manu18** ⬆️ 1

B IS CORRECT ANS

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** A
A is correct.

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 MacGyver4th** ⬆️ 4

This helped me:
https://www.servicenow.com/community/itsm-forum/difference-between-quot-core-quot-table-and-quot-base-quot-table/m-p/574302

---

**👤 Bonche** ⬆️ 1

A is the correct answer

*...and 1 more comments*


</details>

---

## Question 191

### For your implementation, the following tables are extended from each other:
• Incident table is extended from Task table.
• Super Incident table is extended from Incident table.

In this situation, which table(s) are Parent, Child and Base tables? (Choose five.)

### Answer Choices:
**A.** Super Incident table is a Parent table

**B.** Incident table is a Child table

**C.** Super Incident table is a Base table

**D.** Incident table is a Base table

**E.** Task table is a Child table

**F.** Incident table is a Parent table

**G.** Super Incident table is a Child table

**H.** Task table is a Parent table



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BFGHI
**Community voting results:** BFGHI (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 f5003a6** ⬆️ 2

**Selected Answer:** BFGHI
B. Incident table is a Child table
F. Incident table is a Parent table
G. Super Incident table is a Child table
H. Task table is a Parent table
I. Task table is a Base table

---

**👤 manikanta_gnbv** ⬆️ 2

**Selected Answer:** BFGHI
the answewr is BFGHI

---

**👤 Raxon** ⬆️ 1

Someone explain this one. Why is it choose 5 and not 3? 

Wouldn't it be : 
B. Incident Table is a Child Table 
C. Super Incident Table is a Base Table
H. Task Table is a Parent Table

???



</details>

---

## Question 192

### What capability allows users to create dashboards with widgets to visualize data over time in order to identify areas of improvement?

### Answer Choices:
**A.** Scheduled Reports

**B.** Performance Analytics

**C.** Analytics Reports

**D.** Reporting



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** B
B. Performance Analytics

---

**👤 yo_mamma** ⬆️ 2

B is correct
https://docs.servicenow.com/bundle/utah-now-intelligence/page/use/performance-analytics/concept/pa-overview.html



</details>

---

## Question 193

### Which type of ServiceNow script runs on the web browser?

### Answer Choices:
**A.** Server script

**B.** Database script

**C.** Client script

**D.** Local script



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. Client script

---

**👤 Certs_Hub** ⬆️ 1

**Selected Answer:** C
C correct.

---

**👤 yo_mamma** ⬆️ 3

C is correct
https://docs.servicenow.com/bundle/utah-application-development/page/script/client-scripts/concept/client-scripts.html



</details>

---

## Question 194

### When selecting the Target table for an import, which tables can you select? (Choose three.)

### Answer Choices:
**A.** Tables outside of ServiceNow

**B.** Tables within the global scope

**C.** Related tables, using Dot Walk

**D.** Tables which allow write access to other applications

**E.** Tables within the existing application scope



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BDE
**Community voting results:** BDE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 MACCA81** ⬆️ 1

**Selected Answer:** BDE
Yes, the answer is BDF, found on page 176 of the SA fundamentals - Vancouver edition

---

**👤 Certs_Hub** ⬆️ 1

**Selected Answer:** BDE
BDE is correct

---

**👤 buchibabe** ⬆️ 3

BDE is correct. Page 391 of the ServiceNow Administration Fundamentals - Tokyo Edition.



</details>

---

## Question 195

### On Access Control Definitions, what are ways you can set the permissions on a Table? (Choose three.)

### Answer Choices:
**A.** Conditional Expressions

**B.** Roles

**C.** CRUD

**D.** Script that sets the answer variable to true or false

**E.** Groups



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABD
**Community voting results:** ABD (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** ABD
As per Washington docs

---

**👤 Sallex** ⬆️ 1

Update for the Utah release, basically same thing as what rittersportler post but the URL change

https://docs.servicenow.com/en-US/bundle/utah-platform-security/page/administer/contextual-security/concept/access-control-rules.html

---

**👤 rittersportler** ⬆️ 2

Documentation: https://docs.servicenow.com/bundle/tokyo-platform-security/page/administer/contextual-security/concept/access-control-rules.html



</details>

---

## Question 196

### What tool is used to import data from various data sources, and map that data into ServiceNow tables?

### Answer Choices:
**A.** Transform Set

**B.** Data Pack

**C.** Update Set

**D.** Import Set



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Certs_Hub** ⬆️ 2

**Selected Answer:** D
D is correct as per chatgpt.

---

**👤 buchibabe** ⬆️ 2

D is correct. Page 389 of the ServiceNow Administration Fundamentals - Tokyo Edition.



</details>

---

## Question 197

### When you set a policy that is applied to all data entered into the platform (UI, Import Sets, or Web Services), where does this policy run by default?

### Answer Choices:
**A.** Client

**B.** Network

**C.** Browser

**D.** Server



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 val7223** ⬆️ 5

**Selected Answer:** D
D. Server.

When you set a policy that applies to all data entered into the platform, such as through the UI (User Interface), Import Sets, or Web Services, the policy is typically enforced on the server side. The server is responsible for processing and validating the data that is being entered into the platform. By running the policy on the server, the platform can ensure consistent enforcement of the policy rules regardless of the entry point (UI, Import Sets, or Web Services) used to submit the data. This helps maintain data integrity and security across the platform.

---

**👤 benarji** ⬆️ 3

D. Server: When you set a policy that applies to all data entered into the platform, such as through the UI (User Interface), Import Sets, or Web Services, the policy is typically enforced on the server side. The server is responsible for processing and validating the data that is being entered into the platform. By running the policy on the server, the platform can ensure consistent enforcement of the policy rules regardless of the entry point (UI, Import Sets, or Web Services) used to submit the data. This helps maintain data integrity and security across the platform.

---

**👤 Certs_Hub** ⬆️ 2

**Selected Answer:** D
D server as per chatgpt.

---

**👤 al3xunder** ⬆️ 2

**Selected Answer:** D
SERVER

---

**👤 varuns03** ⬆️ 1

**Selected Answer:** D
As explained here, data policy runs only on server
https://docs.servicenow.com/en-US/bundle/utah-platform-administration/page/administer/field-administration/concept/c_DataPolicy.html

---

**👤 Shakir2023** ⬆️ 1

**Selected Answer:** D
Server

---

**👤 evoken** ⬆️ 1

**Selected Answer:** D
D data policy.

---

**👤 Spiderman5000** ⬆️ 2

**Selected Answer:** D
A data policy is applied at the Server. Consider trying to create an Incident using REST API. The server would need to reject the HTTP request if a mandatory field is empty.

---

**👤 missmam** ⬆️ 3

**Selected Answer:** D
Server is correct

---

**👤 Philippvs** ⬆️ 4

**Selected Answer:** D
https://docs.servicenow.com/en-US/bundle/tokyo-platform-administration/page/administer/field-administration/concept/c_DataPolicy.html

*...and 2 more comments*


</details>

---

## Question 198

### On what part of the ServiceNow instance, would you find the option to Impersonate User?

### Answer Choices:
**A.** User Menu

**B.** Content Fame

**C.** Application Navigator

**D.** Module



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 BrooksieOwl** ⬆️ 1

Q277 is a duplicate of Q190 but they have different answers. Q190 answer is Banner and Q277 answer is User Menu. Neither gives these options in the other question. Both answers are right, however. Open the User menu by clicking your user avatar in the ServiceNow banner. Select the Impersonate user menu item. In the Select a user to impersonate dialog, enter the name of the user to impersonate in the Search for a user field. Select the user from the list and click the Impersonate user button.

---

**👤 RGGG** ⬆️ 4

**Selected Answer:** A
A is correct



</details>

---

## Question 199

### How would you describe the relationship between the Incident and Task table?

### Answer Choices:
**A.** Incident table has a one to many relationship with the Task table

**B.** Incident table is extended from Task table

**C.** Incident table is related to the Task table via the INC number

**D.** Incident table has a many to many relationship with the Task table

**E.** Incident table is a database view of the Task table



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (76%), A (24%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 ssadique** ⬆️ 6

**Selected Answer:** B
The correct answer is B. Incident table is extended from Task table.

Explanation:
In ServiceNow, the Incident table is a child table that extends the Task table. This means that the Incident table inherits fields and functionality from the Task table, which is the parent table for many other tables like Change, Problem, and Request.

A. Incident table has a one to many relationship with the Task table: This is incorrect because Incident is a type of Task, not a separate table with a one-to-many relationship.
B. Incident table is extended from Task table: This is correct. The Incident table inherits from the Task table, meaning incidents are a type of task.

---

**👤 SeekerShan** ⬆️ 2

**Selected Answer:** B
B is correct

---

**👤 9e0bd16** ⬆️ 2

**Selected Answer:** B
it is b

---

**👤 Trymybest** ⬆️ 3

**Selected Answer:** B
Answer- B - The Incident table is extended from the task table, which basically means that it inherits all of the fields, rules and policies from task.

---

**👤 gedgy** ⬆️ 3

Answer is B because it describes the fundamental relationship between the two tables.

---

**👤 9125fbf** ⬆️ 1

**Selected Answer:** A
Answer is A, one to many relationship describing one incident to having many tasks for a single incident

---

**👤 chrisRY** ⬆️ 1

It's a easy to read over relationship. It's A

---

**👤 SNPCSA** ⬆️ 1

In question it clearly says - relationship between..
Option A describes relationship.

---

**👤 SNPCSA** ⬆️ 1

**Selected Answer:** A

---

**👤 6d50eb7** ⬆️ 1

**Selected Answer:** A
definitely A

*...and 1 more comments*


</details>

---

## Question 200

### Which flow components allow you to specify when a flow should be run?

### Answer Choices:
**A.** Trigger and Condition Pill

**B.** Condition and Table

**C.** Trigger Criteria and Clock

**D.** Trigger and Condition

**E.** Scope and Trigger Condition



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Certs_Hub** ⬆️ 1

**Selected Answer:** D
D per chatgpt.

---

**👤 buchibabe** ⬆️ 2

D is correct. Page 233 of the ServiceNow Administration Fundamentals - Tokyo Edition.



</details>

---

## Question 201

### Which of the following statement describes the purpose of an Order Guide?

### Answer Choices:
**A.** Order Guides restrict the number of items in an order to only one item per request

**B.** Order Guide provide a list of guidelines for Administrators on how to set up item variables

**C.** Order Guide provide the ability to order multiple, related items as one request

**D.** Order Guides take the user directly to the checkout without prompting for information



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** C
Answer is

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** C
C is correct.

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 sifot** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Jana08** ⬆️ 3

**Selected Answer:** C
C is correct:
https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/service-catalog-management/concept/c_ServiceCatalogOrderGuides.html

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 Bonche** ⬆️ 1

C is the correct answer

---

**👤 bharatns** ⬆️ 3

correct



</details>

---

## Question 202

### Which feature helps to automatically allocate a critical, high-priority, service request to the appropriate assignment group or team member?

### Answer Choices:
**A.** Assignment Rule

**B.** User Policy

**C.** Predictive Intelligence

**D.** UI Policy



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (83%), C (17%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 amt_123** ⬆️ 1

**Selected Answer:** C
C is the right one

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
A - as this is part of the CSA official book. You have labs on it. 
C- Predictive Intelligence is barely mentioned.

---

**👤 Certs_Hub** ⬆️ 2

**Selected Answer:** A
In ServiceNow, Assignment Rules are used to automatically allocate tasks, incidents, or service requests to the appropriate assignment group or team member based on specified criteria. These rules help streamline the assignment process and ensure that critical or high-priority items are directed to the appropriate individuals or groups for prompt resolution.

---

**👤 5852843** ⬆️ 3

A is the correct answer. The Assignment rules module allows you to automatically set a value in the assigned_to and assignment_group fields when a set of conditions occurs.

An assignment rule must also meet these additional criteria to run:
The task record has been created or updated. Assignment rules do not apply to unsaved changes on a form.
The task record must be unassigned. The record cannot have an existing value for either the assigned_to or assignment_group fields. Assignment rules cannot overwrite existing assignments (including assignments set by a default value or a previously run assignment rule).
The assignment rule is the first rule that matches the table and conditions. If more than one assignment rule matches the conditions, only the rule with the lowest order value runs.

---

**👤 EricSon** ⬆️ 1

**Selected Answer:** A
Since the criteria have been predefined as mentioned in the question, so it should be "Assignment Rule".

---

**👤 jmm18** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 buchibabe** ⬆️ 1

C is correct. Page 314 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 Muskan_1809** ⬆️ 1

Can anyone please confirm whether its A or C?

---

**👤 spradeep233** ⬆️ 1

A IS CORRECT



</details>

---

## Question 203

### The Report Designer contains different sections for configuring your report. Which section is used to specify the name of the report, and the table or data source for the report?

### Answer Choices:
**A.** Type

**B.** Properties

**C.** Configure

**D.** Sources

**E.** Data



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** E
**Community voting results:** E (83%), D (17%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Kflo** ⬆️ 1

**Selected Answer:** E
E the first section/tab

---

**👤 DDD1377** ⬆️ 2

E. There is Data, Type, Configure and Style when creating a report. The first step the define name and data source is in the Data section.

---

**👤 MACCA81** ⬆️ 4

**Selected Answer:** E
E is the correct answer. The Report designer contains 4 sections:
Data - where the name of the report is specified and the data source
Type - Where you choose the report type (like pie chart, Bar chart, Pareto, etc)
Configure - where you can configure the report like what each section of the pie chart would represent, what would the axis on the Bar chart be, etc. You can also group data by a specific field or run calculations against the data
Style - This is where you make your report pretty. You can change the report labels and colours, etc.

---

**👤 manikanta_gnbv** ⬆️ 1

**Selected Answer:** E
it is data

---

**👤 Raxon** ⬆️ 2

**Selected Answer:** E
https://developer.servicenow.com/dev.do#!/learn/learning-plans/vancouver/new_to_servicenow/app_store_learnv2_reportanalytics_vancouver_report_designer

Report Designer has four tabs:

Data: data source for the report <-- "Report Name" 
Type: visualization of the report such as bar, pie, or histogram
Configure: report configuration based on report type
Style: report appearance

---

**👤 buchibabe** ⬆️ 3

E is correct - Page 267 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 jomom54** ⬆️ 2

**Selected Answer:** E
Its E I checked

---

**👤 studyGUYd** ⬆️ 2

**Selected Answer:** D
Answer is Data

Open your PDI, navigate to Reports > View/Run, click Create a report, "Report name" is the first mandatory field in the Data tab section.



</details>

---

## Question 204

### //IMG//
You are editing a new incident record and would like the "Save" button to be located on the Form header. Which action would need to be taken for that button to appear?

### Answer Choices:
**A.** Context Menu > Form Design > add the “Save” button.

**B.** All > System Properties > UI Properties > Turn on the “glide.ui.advanced” property.

**C.** All > System Properties > UI Properties > Turn on the “Save” button.

**D.** Context Meru > Form Layout > add the “Save” button.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (63%), B (19%), Other

</details>

<details>
<summary><strong>💬 View Discussion (24 comments)</strong></summary>

**👤 NikaDMars** ⬆️ 1

**Selected Answer:** C
C. All > System Properties > UI Properties > Turn on the “Save” button.
To make the Save button appear on the form header, you need to enable the visibility of the button via a system property in UI Properties. This controls whether the Save button is shown on forms globally.


Regarding the rest of options:
* Adding the button in Form Layout (option D) manages which buttons can appear but does not guarantee visibility if the system property disables the button globally.
* The glide.ui.advanced property (option B) enables other advanced UI features but not specifically the Save button.
* Form Design (option A) is used for field layout, not header buttons.

---

**👤 wnstn75** ⬆️ 1

**Selected Answer:** B
Testing update 'glide.ui.advanced' to 'true', 'save' button comes up at form header.

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
I initially picked B. 

C is the most correct answer, although B is also true. At least in the Washington PDI, you get the option to "Show Save, Insert, Insert and Stay" Buttons. It is also the glide property, but in the tool options you see it as Save Button.

---

**👤 Kflo** ⬆️ 2

**Selected Answer:** C
C - try all the options..

---

**👤 SnowNick** ⬆️ 3

**Selected Answer:** B
There is no "Turn on the save button" option.

The option is called "Show "Save", "Insert" and "Insert and Stay" buttons on form", and if you check the property name, it is called "glide.ui.advanced”. 

So answer is option B.

---

**👤 aper** ⬆️ 1

**Selected Answer:** B
Answer is B.
In All > System Properties > UI Properties, check the box "Show "Save", "Insert" and "Insert " and Stay" buttons on forms, which turns on the "glide.ui.advanced" property.

---

**👤 DDD1377** ⬆️ 2

C is the correct answer. You cannot add the SAVE button in the form header using Form Layout or Form Design.

---

**👤 Mehamood** ⬆️ 2

**Selected Answer:** C
You can only do it by checking on Show "Save", "Insert" and "Insert and Stay" buttons on forms option. You can try it in your PDI

---

**👤 Macramento** ⬆️ 1

C... just tested it in the PDI, vancouver release.

---

**👤 ElifSurucu** ⬆️ 1

**Selected Answer:** D
Form Layout: This is the area where you can configure the arrangement and placement of fields, buttons, and other elements on a form.
Therefore, option D is the correct action to take to ensure that the "Save" button appears in the Form header when editing a new incident record in ServiceNow.

*...and 14 more comments*


</details>

---

## Question 205

### Which features ensures data consistency while importing data using import sets and web services?

### Answer Choices:
**A.** UI Policy

**B.** Data Policy

**C.** Business Rule

**D.** Client Script

**E.** CSDM



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Ginfante** ⬆️ 1

**Selected Answer:** B
Data Policies are applied to all data entered into the platform: form (UI), Import Sets or Web Services.

---

**👤 M_Rami** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 NowNoob** ⬆️ 1

page 435, CSA Book (tokyo2022 version)



</details>

---

## Question 206

### When using Flow Designer, what is the Flow Execution initiated by?

### Answer Choices:
**A.** A flow logic

**B.** An existing subflow

**C.** An execution data pill

**D.** A trigger



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Certs_Hub** ⬆️ 2

**Selected Answer:** D
In ServiceNow's Flow Designer, the flow execution is initiated by a trigger. Triggers are events or conditions that start the execution of a flow. They define when a flow should be activated based on specific criteria, such as the creation of a record, a scheduled time, or an external event.

---

**👤 it0m_man** ⬆️ 3

**Selected Answer:** D
D is correct.



</details>

---

## Question 207

### What is the name of the string that displays filter criteria?

### Answer Choices:
**A.** Breadcrumb

**B.** Choice

**C.** Menu

**D.** Topic



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Certs_Hub** ⬆️ 3

**Selected Answer:** A
In ServiceNow, the string that displays filter criteria is commonly referred to as the "Breadcrumb." It shows the current filter conditions applied to a list or a module, making it easy for users to understand the context of the displayed data.

---

**👤 it0m_man** ⬆️ 3

**Selected Answer:** A
A is the right answer.



</details>

---

## Question 208

### Which system property is added and set to true in order to see impersonation events in the System Log?

### Answer Choices:
**A.** glide.sys.all_impersonation

**B.** glide.user_setting

**C.** glide.impersonation_setting

**D.** glide.sys.log_impersonation

**E.** glide.sys.admin_login



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 rittersportler** ⬆️ 8

**Selected Answer:** D
https://support.servicenow.com/kb?id=kb_article_view&sysparm_article=KB0717055

---

**👤 MACCA81** ⬆️ 2

**Selected Answer:** D
"Impersonation can be Enabled or Disabled for interactive sessions using the glide.sys.log_impersonation property" - taken from the SA Fundamentals E-book, Vancouver edition

---

**👤 Certs_Hub** ⬆️ 1

**Selected Answer:** D
D. glide.sys.log_impersonation

Setting this property to true allows the system to log events related to user impersonation in the system log.

---

**👤 suryansha_95** ⬆️ 2

Correct is D.

Impersonation logs
Impersonations are logged in the system log.

Log impersonations for either interactive (UI) or non-interactive sessions.

Impersonation logging for interactive sessions
Interactive sessions are performed through the user interface (UI). Enable or disable impersonation logging for interactive sessions using the glide.sys.log_impersonation property.

If you enable impersonation logging for interactive sessions by setting glide.sys.log_impersonation to true, all interactive sessions are recorded in the log.

---

**👤 val7223** ⬆️ 1

**Selected Answer:** D
the reponse trueis property with the following values:
Name: glide.sys.log_impersonation
Type: true/false
Value: true
Save

---

**👤 it0m_man** ⬆️ 2

**Selected Answer:** D
D is Correct!

---

**👤 GHOTI** ⬆️ 2

**Selected Answer:** D
D is correct

---

**👤 Vishy123** ⬆️ 2

D is correct

---

**👤 spradeep233** ⬆️ 2

D is correct



</details>

---

## Question 209

### What process allows users to create, categorize, review, approve and browse important information in a centralized location that is shared by the entire organization?

### Answer Choices:
**A.** Self Service Management

**B.** Knowledge Management

**C.** Business Information Management

**D.** Information Portal Management

**E.** Knowledge-Centered Management



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Certs_Hub** ⬆️ 1

**Selected Answer:** B
Knowledge Management in ServiceNow allows users to create, categorize, review, approve, and browse important information in a centralized location that is shared by the entire organization. It involves creating a knowledge base where information, such as articles, FAQs, and other documentation, can be stored, managed, and accessed. This helps in sharing and leveraging collective knowledge within the organization.

---

**👤 6d50eb7** ⬆️ 1

**Selected Answer:** B
The question is what "process", only knowledge management is deemed as a process out of all the choices.

---

**👤 Vishy123** ⬆️ 2

B is correct



</details>

---

## Question 210

### A colleague wants to rearrange the columns on their My Work List. Once the user has navigated to the list, where should they navigate to select and arrange the columns?

### Answer Choices:
**A.** Right click on any column header, Context Menu > Configure > List Layout

**B.** Click List Context Menu > Configure > List Layout

**C.** Click List Context Menu > Personalize List

**D.** Click Personalize List



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (86%), 14%

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 Spiderman5000** ⬆️ 6

**Selected Answer:** D
"A" is correct if you wish to change the layout for all. "D" is correct (in Tokyo) if you want to personalize.

---

**👤 Cplatt** ⬆️ 2

This is a bit tricky because the entire time I'm thinking from an admin perspective and my colleague would also be an admin so Option A would be a logical choice for more control and dot walking capabilities if they need it. D is also correct. I guess the term "colleague" is where this can be tricky. I hope the test marks D as the correct answer.

---

**👤 MACCA81** ⬆️ 3

**Selected Answer:** D
Correct answer is D - This is the "Gear" icon on the right-hand side of the list header, and it's called Personalize List (hover over the gear icon and you'll see the name).
Answers A is incorrect as through this option you don't change your personal view of the list but the the list view for everyone, and only admins and specialized admins can do that.
B and C are not possible at all as on the List context menu there are NO Configure or Personalize List options.

---

**👤 6d50eb7** ⬆️ 2

**Selected Answer:** D
Click the gear to re-arrange the columns

---

**👤 EricSon** ⬆️ 2

**Selected Answer:** C
I just tried - it should be the gear button, then rearrange the columns in the personalised list.

---

**👤 Philippvs** ⬆️ 1

**Selected Answer:** D
It’s most likely.

---

**👤 rittersportler** ⬆️ 2

I agree with Spiderman5000, Vshy123, smeagel. The wording is a bit confusing, but I am sure they mean the "gear icon" with "Personalize list". It becomes visible when you hover over it with the mouse.

---

**👤 Vishy123** ⬆️ 3

D is correct

---

**👤 smeagel** ⬆️ 4

D should be correct.
Configure - list layout would change it for all workers.
And normal workers (ITIL User) dont even have the configure option

---

**👤 spradeep233** ⬆️ 2

A IS CORRECT ONLY

*...and 1 more comments*


</details>

---

## Question 211

### You are looking at a list of Active Incidents. You want to exclude Incidents with the state of Resolved. How might you do that?

### Answer Choices:
**A.** On Search, select State, type not Resolved, press enter

**B.** On State column title, right-click, select Filter Out > Resolved

**C.** On the list of records, locate and right-click on the Resolved value, select Filter Out

**D.** On the list of records, locate and right-click on the Resolved value, select Exclude

**E.** Click Funnel icon, click AND, Select Resolved, is Not, State, click Run



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 GoldBear** ⬆️ 1

**Selected Answer:** C
C - is the correct answer.

---

**👤 FATMASAAL2** ⬆️ 1

**Selected Answer:** C
The answer should be C, I just tested it

---

**👤 varuns03** ⬆️ 2

**Selected Answer:** C
The answer should be C

---

**👤 Shakir2023** ⬆️ 1

**Selected Answer:** C
E, could work if the steps sequence was correct which is not the case here in my point of view.

---

**👤 PetterRE** ⬆️ 2

**Selected Answer:** C
Answer is C, D is wrong, exclude doesnt exist in lists. On E state must be selected before you can select resolved.

---

**👤 Ibeauo** ⬆️ 3

**Selected Answer:** C
I have tested on PDI, for me answer C and E works well, BUT the order of answer is not correct in my point of view. If it would be E, the order should be 'Click Funnel icon, enter State (in Field), is not, (select) Resolved, click Run'. 

It is annoying that questions and answers sometimes are not written precisely. For this question I would choose C.

---

**👤 missmam** ⬆️ 4

**Selected Answer:** C
C is correct

---

**👤 Vishy123** ⬆️ 3

sorry, i think C is correct answer

---

**👤 Vishy123** ⬆️ 2

d is correct

---

**👤 spradeep233** ⬆️ 3

answer is c

*...and 1 more comments*


</details>

---

## Question 212

### Which tool is used to have conversations with logged-in users in real-time?

### Answer Choices:
**A.** Connect Chat

**B.** Now Messenger

**C.** User Presence

**D.** Comments



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (93%), 7%

</details>

<details>
<summary><strong>💬 View Discussion (20 comments)</strong></summary>

**👤 julianamendo** ⬆️ 15

**Selected Answer:** A
A is correct but this question is deprecated since connect chat no longer exists. Now we have sidebar discussions, which is not the same.

---

**👤 tk1978** ⬆️ 1

**Selected Answer:** A
Its been depreciated, but is correct for previous versions

---

**👤 PopularOutcast** ⬆️ 2

DEPRECATED

---

**👤 mabobora** ⬆️ 2

deprecated

---

**👤 brownbear324** ⬆️ 1

**Selected Answer:** A
User Presence is not a "tool" like the question asks. Connect Chat is a tool. A is correct.

---

**👤 Nabuto** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 BenBose** ⬆️ 3

**Selected Answer:** A
ChatGPT - Connect Chat is a real-time messaging tool that enables users to chat with individuals and groups, quickly share files, and collaborate on any record by connecting with the right people instantly. It is not supported in Next Experience1. Agent Chat is the newer version of Connect Chat in ServiceNow. One of the first differences you’ll notice is that Agent Chat is not available in the standard ServiceNow user interface. Agents receive chats as well as other work items within Agent Workspace, which is a newer and more modern user interface (UI)2.

---

**👤 FastAndy** ⬆️ 1

**Selected Answer:** A
According to SNow

---

**👤 CSA_R** ⬆️ 1

A -Connect Chat is a correct Answer 
https://docs.servicenow.com/en-US/bundle/utah-servicenow-platform/page/use/collaboration/concept/c_Collaboration.html

---

**👤 CSA_R** ⬆️ 1

https://docs.servicenow.com/en-US/bundle/utah-servicenow-platform/page/use/collaboration/concept/c_Collaboration.html

*...and 10 more comments*


</details>

---

## Question 213

### Roles can inherit permissions from other roles. Which role inherits all of the permissions of the catalog role, and the user_criteria_admin role, plus has permissions to create Items and Services?

### Answer Choices:
**A.** Sys Admin [sys_admin]

**B.** Catalog Admin [catalog_admin]

**C.** Catalog Author [sn_catalog_write]

**D.** Item Admin [sn_item_admin]



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 MACCA81** ⬆️ 1

**Selected Answer:** B
B is correct. The Admin role would have been correct as well if it was called admin, as the System administrator role is called in SN. There's no sys_admin role. The other 2 are incorrect

---

**👤 buchibabe** ⬆️ 1

B is correct - Page 77 of the ServiceNow Administration Fundamentals - Tokyo Edition.



</details>

---

## Question 214

### What component of the ServiceNow infrastructure defines every table and field in the system?

### Answer Choices:
**A.** Schema

**B.** Field Map

**C.** Table Class Manager

**D.** Dictionary

**E.** Data Atlas



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Joe15** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 alaro** ⬆️ 1

D is correct



</details>

---

## Question 215

### Which data consistency settings can be achieved using UI Policy? (Choose three.)

### Answer Choices:
**A.** Setting fields to accept the data in an expected format

**B.** Setting fields to accept the data with ‘n’ number of characters

**C.** Setting fields hidden

**D.** Settings fields read-only

**E.** Setting fields mandatory



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** CDE
**Community voting results:** CDE (80%), ABD (20%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 Spiderman5000** ⬆️ 5

**Selected Answer:** CDE
UI Policy does not control data format

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** CDE
From the Xanadu docs: 
This example demonstrates how to implement the following controls.
 Make a Close Notes field mandatory.
 Hide the Opened by field.
 Make the Priority, Severity, and Urgency fields read-only.
 Run a client script that displays an alert message.

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** CDE
CDE are the correct choice. Look at the SNAF

---

**👤 RogerW** ⬆️ 1

A, B, D
Under the Advance, Run Scripts, you should be able to disable the save button when the data is not in the expected format or the number of characters is not equal to "n".

---

**👤 buchibabe** ⬆️ 1

CDE is correct - Page 434 of the ServiceNow Administration Fundamentals - Tokyo Edition.

---

**👤 dnbly** ⬆️ 1

The problem with this question is that BCDE are all correct. My instance at work has some UI policies to enforce specific lengths of specific data entry (as they are mandatory reference numbers) and the other 3 are basic to do too.

---

**👤 BenBose** ⬆️ 3

**Selected Answer:** ABD
C & E do not impact data consistency

---

**👤 alaro** ⬆️ 1

CDE is correct

---

**👤 evoken** ⬆️ 2

**Selected Answer:** CDE
CDE is correct.

---

**👤 Vishy123** ⬆️ 2

CDE is correct

*...and 2 more comments*


</details>

---

## Question 216

### A customer wants to use a client script to validate things on a form in order to make sure the submission makes sense. What type of client script would you recommend to meet this requirement?

### Answer Choices:
**A.** onSubmit()

**B.** onSubmission()

**C.** onUpdate()

**D.** onLoad()



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
A - as onSubmission() doesn't exist

---

**👤 ArisGk** ⬆️ 1

**Selected Answer:** A
A correct

---

**👤 alaro** ⬆️ 1

A is absolutely correct



</details>

---

## Question 217

### //IMG//
An order from the Service Catalog has been placed. Two records in the Platform are created as a result. Which two records are associated with this newly ordered item? (Choose two.)

### Answer Choices:
**A.** A record of sc_req_item table

**B.** A record of sc_task

**C.** An incident record

**D.** A change record

**E.** A problem record



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AB
**Community voting results:** AB (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** AB
A. A record of sc_req_item table
B. A record of sc_task

---

**👤 alaro** ⬆️ 1

A and B



</details>

---

## Question 218

### What action will allow you to personalize layouts of columns in a list?

### Answer Choices:
**A.** Context Menu > View > Personalize

**B.** Click Gear Icon > Personalize window options > Select the appropriate columns

**C.** Select the column to be personalized and right at the header > Choose the options to personalize

**D.** Select the column to be personalized > Click Edit icon (Pencil) > Choose the options to personalize



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 MACCA81** ⬆️ 2

**Selected Answer:** B
This question is the same as question #288, just the wording of the question and the correct answer is different. In Question 288 they referred to clicking on Personalize List and here they refer to clicking on the Gear icon, but it's the same thing. Personalize list is the lable of the Gear icon (when you hover over the icon you can see it)

---

**👤 dnbly** ⬆️ 1

B is correct but would be better stated as Click Gear Icon > Select the appropriate columns > Ok

---

**👤 alaro** ⬆️ 1

B is correct



</details>

---

## Question 219

### An order for new office equipment has been placed through the Service Catalog. How would you view the lists of requests after the orders have been placed?

### Answer Choices:
**A.** All > Tables and Columns > Tasks

**B.** In the Navigation Filter, type “requests.list" and press the Enter key

**C.** All > Service Catalog > Requests

**D.** All > Service Catalog > Open Records > Items



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (54%), C (46%)

</details>

<details>
<summary><strong>💬 View Discussion (29 comments)</strong></summary>

**👤 smeagel** ⬆️ 8

there is no correct answer here.
Should be Service Catalog -> open records -> requests

---

**👤 6d50eb7** ⬆️ 6

**Selected Answer:** C
The question is how would you view the lists of REQUESTS, not items. Therefore the answer is C.

---

**👤 IT_Bank** ⬆️ 2

**Selected Answer:** D
Answer is D 100%. Tested in my PDI. All --> Service Catalog --> Open Records --> Items has a field called "Requests" which, you guessed it, has REQ values.
Its not C because All --> Service Catalog --> Requests does not exist.

---

**👤 2ace220** ⬆️ 1

**Selected Answer:** C
List of Request (REQ) is different from the list of requested item (RITM)
REQ = ALL> SERVICE CATALOG > REQUEST
RITM = ALL > SERVICE CATALOG > OPEN RECORD > ITEM

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
The question mentions Requests and I initially picked C. But that path doesn't exist in the Navigator, D is the only viable path and you can see a list of Items, in the list view you can also see Requests.

Very poorly phrased question.

---

**👤 Kflo** ⬆️ 1

**Selected Answer:** D
D - this is how you get to open RITMs

it's not C - who looks at REQ unless its an order guide or bundle requests with multiple RITMs

---

**👤 Schoolboz** ⬆️ 1

**Selected Answer:** C
it is C. The question asked for a list of requests, not of items.

---

**👤 PappyFox** ⬆️ 2

**Selected Answer:** C
Service Catalog > Requests is the standard navigation path to view a list of requests placed through the Service Catalog. It displays all Request [sc_request] records, including details like request numbers, requesters, and statuses.

---

**👤 aper** ⬆️ 1

All->Service Catalog->Open Records ->Requests. So the answer is E. None of the above.

---

**👤 96d187b** ⬆️ 2

The correct answer is C. All > Service Catalog > Requests.

When an order is placed through the Service Catalog, it generates a request record in the Request [sc_request] table. This record can be viewed under the Service Catalog application in the navigator. 

For those saying D — This navigates to catalog items, not the requests.

*...and 19 more comments*


</details>

---

## Question 220

### //IMG//
Which path would you take to access the table configuration page from a form?

### Answer Choices:
**A.** The Form Context menu > View > Show Table

**B.** The Form Context menu > View > Table

**C.** The Form Context menu > Configure > Dictionary

**D.** The Form Context menu > Configure > Table



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 GoldBear** ⬆️ 1

**Selected Answer:** D
D is correct.



</details>

---

## Question 221

### Which admin role is required to make changes to High Security Settings?

### Answer Choices:
**A.** high_sec_admin

**B.** sn_acl_admin

**C.** admin

**D.** security_admin



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 Vishy123** ⬆️ 5

D is correct



</details>

---

## Question 222

### What is the most common role that has access to almost all platform features, functions, and data?

### Answer Choices:
**A.** Super User [sn_super_user]

**B.** Security Admin [securty_admin]

**C.** System Administrator [admin]

**D.** Base Admin [base_admin]

**E.** System Manager [sys_manager]



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** C
Answer is C



</details>

---

## Question 223

### As it relates to ServiceNow reporting, which of the following statements describes what a metric can do?

### Answer Choices:
**A.** A metric is a report gauge used on homepages to display real-time data

**B.** A metric is a time measurement used to report the effectiveness of workflows and SLAs

**C.** A metric is used to measure and evaluate the effectiveness of IT service management processes

**D.** A metric is a comparative measurement used to report the effectiveness of flows and SLAs.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (94%), 6%

</details>

<details>
<summary><strong>💬 View Discussion (22 comments)</strong></summary>

**👤 hatanaoki** ⬆️ 10

**Selected Answer:** C
The correct answer to this question is definitely C. I broke up with my girlfriend. It is hard. I am already 36 years old. If I don't pass this exam, I won't have a future, so I want to do my best.

---

**👤 SNOW_SAGAR** ⬆️ 1

**Selected Answer:** B
The correct answer is:

B. A metric is a time measurement used to report the effectiveness of workflows and SLAs

Explanation:
In ServiceNow, a metric:

Tracks and records the amount of time an item (like an incident, change, or task) spends in a particular state or with a certain value.

Is commonly used to measure SLA (Service Level Agreement) compliance and workflow efficiency.

Helps in analyzing how long it takes for a record to progress through various stages, e.g., how long an incident remains in "New" state.

Why the other options are incorrect:
A. Incorrect – That's describing a gauge, not a metric.

C. Close but vague – Doesn't emphasize the time-based nature of a metric.

D. Incorrect – The term "comparative measurement" is misleading; metrics focus on time-based tracking.

---

**👤 soscandia** ⬆️ 1

C is correct. A metric is created to evaluate fields changes in tasks.

---

**👤 lezica** ⬆️ 1

the definition in SN Docs

---

**👤 skibumvermont** ⬆️ 2

A metric measures and evaluates the effectiveness of IT service management processes.

For example, a metric could measure the effectiveness of the incident resolution process by calculating how long it takes to resolve an incident.
For example, a metric could measure the effectiveness of the incident resolution process by calculating how long it takes to resolve an incident.

---

**👤 shub606** ⬆️ 2

c is correct answer

---

**👤 Slayer1985** ⬆️ 2

C is correct

---

**👤 SaiExamtopic** ⬆️ 2

C is Correct

---

**👤 julianamendo** ⬆️ 2

**Selected Answer:** C
C is correct. A metric is created to evaluate fields changes in tasks.

---

**👤 shravan3** ⬆️ 2

c. a metric is used to measure and evaluate IT service

*...and 12 more comments*


</details>

---

## Question 224

### Which of the following concepts are associated with the ServiceNow CMDB? (Choose four.)

### Answer Choices:
**A.** Service Processes

**B.** User Permissions

**C.** Tables and Fields

**D.** A Database

**E.** The Dependency View



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACDE
**Community voting results:** ACDE (91%), 9%

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 Jana08** ⬆️ 5

**Selected Answer:** ACDE
A, C, D, E:

https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/configuration-management/concept/cnfig-mgmt-and-cmdb.html

---

**👤 dalvisa** ⬆️ 1

**Selected Answer:** ACD
Correct answer

---

**👤 thpr31** ⬆️ 1

**Selected Answer:** ACDE
ACDEが正解です

---

**👤 AselaGaja** ⬆️ 1

**Selected Answer:** ACDE
correct answer

---

**👤 reika1914** ⬆️ 2

**Selected Answer:** ACDE
the correct choices are A, C, D, and E.

---

**👤 Mihaf92** ⬆️ 3

isn't BCDE ?

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** ACDE
ACDE is correct.

---

**👤 Dekoreh** ⬆️ 1

A, C, D, E is very correct

---

**👤 Bonche** ⬆️ 4

A, C, D, E is the correct answers



</details>

---

## Question 225

### When moving multiple update sets at one time, what might you do to facilitate the move?

### Answer Choices:
**A.** Preview

**B.** Batch

**C.** List

**D.** Map



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 JollyRgr** ⬆️ 1

There is really no excuse for D being marked as correct. Do these get edited/corrected?

---

**👤 Cplatt** ⬆️ 1

**Selected Answer:** B
B is correct. You'd batch update sets to move several to a different instance.

---

**👤 akhil371** ⬆️ 1

B Is the correct answer

---

**👤 Joe15** ⬆️ 1

**Selected Answer:** B
B is the correct answer

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** B
B - Batch is the correct answer.

---

**👤 yo_mamma** ⬆️ 1

B is correct
https://docs.servicenow.com/bundle/utah-application-development/page/build/system-update-sets/hier-update-sets/concept/us-hier-overview.html

---

**👤 Sallex** ⬆️ 1

**Selected Answer:** B
B is the correct answer

---

**👤 evoken** ⬆️ 4

**Selected Answer:** B
Answer is B



</details>

---

## Question 226

### What do you click when you have made modifications to your report, and you want to see the results without saving?

### Answer Choices:
**A.** Preview

**B.** Test

**C.** Run

**D.** Try It

**E.** Execute



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 benarji** ⬆️ 1

C. Run 
Run the report and it displays the information based on the updates made.

---

**👤 Cplatt** ⬆️ 1

**Selected Answer:** C
Definitely "C". You Run the report and it displays the information based on the updates made.

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. Run

---

**👤 ddiggler** ⬆️ 1

It is C

---

**👤 Joe15** ⬆️ 1

**Selected Answer:** C
The correct answer is C - Run

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** C
The correct answer is C - Run

---

**👤 MR1992** ⬆️ 1

**Selected Answer:** C
C run is the correct

---

**👤 Emuti** ⬆️ 1

C run is the correct answer

---

**👤 Melby** ⬆️ 1

**Selected Answer:** C
Run is correct

---

**👤 Melby** ⬆️ 1

Run is correct

*...and 2 more comments*


</details>

---

## Question 227

### Which framework can automatically populate values for the Priority and Category fields based on the Short description field value?

### Answer Choices:
**A.** Predictive Intelligence

**B.** Assignment Rule

**C.** CSDM

**D.** Action

**E.** UI Policy



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 f5003a6** ⬆️ 1

**Selected Answer:** A
A. Predictive Intelligence

---

**👤 lica09** ⬆️ 1

**Selected Answer:** A
https://servicenow.read.inkling.com/a/b/27b45d8d82aa4679a2f9d05e3bb66423/p/f2c05c1f58174425b99cea4b0694b9f3

---

**👤 MACCA81** ⬆️ 1

**Selected Answer:** A
Predictive Intelligence uses machine learning algorithms to set field values during record creation.

---

**👤 manikanta_gnbv** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Joe15** ⬆️ 1

**Selected Answer:** A
A is the correct answer

---

**👤 Emuti** ⬆️ 1

A is the correct answer

---

**👤 FFlores** ⬆️ 2

**Selected Answer:** A
A its the correct opton, predictive intelligence do that on fields.

---

**👤 yo_mamma** ⬆️ 3

A is correct
https://docs.servicenow.com/bundle/utah-intelligent-experiences/page/administer/predictive-intelligence/concept/using-predictive-intelligence.html

---

**👤 ellitetka** ⬆️ 1

Not UI Action?

---

**👤 H_Grobler** ⬆️ 2

Correct answer is A:
The Predictive Intelligence classification framework enables you to use machine-learning algorithms to set field values during record creation, such as setting the incident category based on the short description.

*...and 1 more comments*


</details>

---

## Question 228

### Which testing framework is used to test ServiceNow Applications?

### Answer Choices:
**A.** Test Driven Framework (TDF)

**B.** Junit

**C.** Selenium

**D.** Automated Test Framework (ATF)



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 ADILKS** ⬆️ 1

**Selected Answer:** D
D is the correct answer

---

**👤 yo_mamma** ⬆️ 2

D is correct
https://docs.servicenow.com/bundle/sandiego-application-development/page/administer/auto-test-framework/concept/automated-test-framework.html



</details>

---

## Question 229

### Which allows the creation of a task-based record from Service Catalog?

### Answer Choices:
**A.** Assignment Rule

**B.** Flow Designer

**C.** UI Builder

**D.** Record Producers



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - to create a task record (INC, CHG, etc) from the Service Catalog you use a Record Producer

---

**👤 yo_mamma** ⬆️ 4

D is correct
https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/service-catalog-management/concept/c_RecordProducer.html



</details>

---

## Question 230

### What module do you use to access the reports that are available to you?

### Answer Choices:
**A.** Self-Service > My Reports

**B.** Self-Service > My Dashboards

**C.** Reports > View / Run

**D.** Reports > Homepage

**E.** Reports > Overview



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 benarji** ⬆️ 1

C: View a list of reports and create reports from the Reports list.

---

**👤 M_Rami** ⬆️ 2

**Selected Answer:** C
I have just tested it, it is C

---

**👤 yo_mamma** ⬆️ 1

C is correct
https://docs.servicenow.com/bundle/utah-now-intelligence/page/use/reporting/reference/r_ViewTheReportsList.html



</details>

---

## Question 231

### Security rules are defined to restrict the permissions of users from viewing and interacting with data. What are these security rules called?

### Answer Choices:
**A.** CRUD Rules

**B.** Access Control Rules

**C.** Role Assignment Rules

**D.** Scripted User Rules

**E.** User Authentication Rules



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 yo_mamma** ⬆️ 5

B is correct
https://docs.servicenow.com/bundle/utah-platform-security/page/administer/contextual-security/concept/access-control-rules.html

---

**👤 buchibabe** ⬆️ 4

B is correct - Page 364 of the ServiceNow Administration Fundamentals - Tokyo Edition.



</details>

---

## Question 232

### A new employee joins the IT department and needs to perform work assigned to Network and Hardware groups. How would you set up their access? (Choose three.)

### Answer Choices:
**A.** Add User Account to Hardware group

**B.** Add User Account to IT Knowledgebase

**C.** Create User Account

**D.** Add User Account to itil group

**E.** Add User Account to Network group

**F.** Add User Account to ACL



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACE
**Community voting results:** ACE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 amt_123** ⬆️ 1

**Selected Answer:** ACE
ACE is correct

---

**👤 Mihaf92** ⬆️ 1

i think ABE

---

**👤 dnbly** ⬆️ 4

**Selected Answer:** ACE
C - NEW employee will need an account setting up otherwise they can't access anything ever. If your work Instance use LDAP or Azure Sync or some other method then good for you but these are not configured OOB and the question doesn't reference them. Either way, the very first step is account setup regardless of means.
AE - best practice is to assign Role(s) to the Group (especially for dependencies) so ITIL should have been added to these already therefore adding the new user to either Group will allow them to inherit ITIL Role.

Creating a Group specifically to assign the ITIL Role would not be best practice as it could be removed independently of other access that requires it to be in place. This means that adding to an ITIL Group would be incorrect (as would adding the Role directly).

---

**👤 RoupT** ⬆️ 2

ADE is the answer. 
 I never create user accounts in SN. They are always sent vis feed through LDAP. 
The question is about access, and with an ITIL role, this user can't see anything. The question is poorly worded.

---

**👤 tangoalpha** ⬆️ 1

darsy2001 is right. ITIL is a role

https://docs.servicenow.com/bundle/utah-platform-administration/page/administer/roles/reference/r_BaseSystemRoles.html

---

**👤 studyGUYd** ⬆️ 2

Y'all... itil isn't a group

---

**👤 Emuti** ⬆️ 1

Hardware gp, Network gp is correct but with Itil > it is a role also user already has an account so creating user account doesn't make no sense.

---

**👤 datauser** ⬆️ 3

A. Add User Account to Hardware group - CORRECT
B. Add User Account to IT Knowledgebase - NOT RELATED
C. Create User Account - NOT RELATED - Questions is about access. Also, in most cases account will be created through a feed such as AD/LDAP but most often groups need to be manually added, either in ServiceNow or AD
D. Add User Account to itil group - CORRECT - There can be a group with the ITIL Role
E. Add User Account to Network group - CORRECT
F. Add User Account to ACL - NOT RELATED


Correct Answer: A, D, E

---

**👤 al3xunder** ⬆️ 3

I think it's ADE



</details>

---

## Question 233

### The customer has asked that you change the default layout of the Task list. They would like these columns, in this order:

• Number
• Task Type
• Parent
• Short Description
• Assignment Group
• Assignee
• Updated

After navigating to the list, where would you click, to meet this requirement?

### Answer Choices:
**A.** Click List Context Menu > Personalize List

**B.** Click List Context Menu > Configure > Columns

**C.** Right click List Gear icon > Configure > Columns

**D.** Right click on any column header, Context Menu > Configure > List Layout



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 gedgy** ⬆️ 1

This appears to be an outdated question. At the moment in (Washington DC) there is no List Layout in the Configure menu after right-clicking on Any Column Header. Only the Gear icon has this option.

---

**👤 MACCA81** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 yo_mamma** ⬆️ 3

D is correct
https://docs.servicenow.com/bundle/utah-platform-administration/page/administer/list-administration/task/t_ConfigureTheListLayout.html



</details>

---

## Question 234

### Which ServiceNow capability allows you to provide knowledge articles, via a conversational messaging interface?

### Answer Choices:
**A.** Agent Assist

**B.** Virtual Agent

**C.** Now Messenger

**D.** Instance Chat



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
B - Virtual agent is a conversational platform.

---

**👤 johnye** ⬆️ 2

Agent Assist - Isn't a conversational platform. 

Virtual Agent - is a conversational platform that helps users obtain information, make decisions, and perform common work tasks within a messaging interface. This includes information stored in the Knowledge Base, Service Catalog, and more. 

Answer is B.

---

**👤 ee7bb78** ⬆️ 1

Can anyone please confirm if the answer is A or B?

---

**👤 BrooksieOwl** ⬆️ 3

This is a duplicate of Q174, however, I think Q174 community vote is wrong and it is in fact Virtual Agent.



</details>

---

## Question 235

### What is a formatter? Select one of the following.

### Answer Choices:
**A.** A formatter allows you to configure applications on your instance

**B.** A formatter is a form element used to display information that is not a field in the record

**C.** A formatter allows you to populate fields automatically

**D.** A formatter is a set of conditions applied to a table to help find and work with data



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 9e0bd16** ⬆️ 1

**Selected Answer:** B
b is correct

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** B
Updated link that explains the basics of a Formatter.
https://docs.servicenow.com/bundle/vancouver-platform-administration/page/administer/form-administration/concept/c_Formatters.html

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** B
B is correct.

---

**👤 evoken** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 anku15** ⬆️ 2

B is correct, it includes Activity Stream and Work Notes.

---

**👤 saiakhilpalle** ⬆️ 1

B is correct

---

**👤 Bonche** ⬆️ 1

B is the correct answer

---

**👤 Cheerfad** ⬆️ 1

**Selected Answer:** B
correct

---

**👤 siopau** ⬆️ 3

correct
https://docs.servicenow.com/bundle/rome-platform-administration/page/administer/form-administration/concept/c_Formatters.html

---

**👤 bharatns** ⬆️ 2

B is correct



</details>

---

## Question 236

### On the Form header, which element you to access form templates?

### Answer Choices:
**A.** Stamp

**B.** More Options (...)

**C.** Pages

**D.** Paperclip



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
B- More options. There is no stamp and the paperclip is for attachments

---

**👤 sri_ravi** ⬆️ 1

A is correct

---

**👤 yo_mamma** ⬆️ 4

B is correct
https://docs.servicenow.com/bundle/utah-platform-user-interface/page/administer/workspace/task/use-more-form-options.html



</details>

---

## Question 237

### What is the definition of a group?

### Answer Choices:
**A.** A collection of subject matter experts

**B.** A department

**C.** An escalation pod

**D.** A collection of users

**E.** A collection of tasks



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - Correct, a group is a collection of users. They don't have to be from the same department, team, etc. Duplicate question.

---

**👤 BrooksieOwl** ⬆️ 2

Duplicate of Q200

---

**👤 yo_mamma** ⬆️ 2

D is correct
https://docs.servicenow.com/bundle/sandiego-platform-administration/page/administer/roles/concept/c_UserAdministration.html



</details>

---

## Question 238

### Which ServiceNow utility provides a modern interactive graphical interface to visualize configuration items and their relationships?

### Answer Choices:
**A.** Flow Design

**B.** CI Class Map

**C.** Dependency View

**D.** Business Service Map



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 ddiggler** ⬆️ 1

It is C for sure

---

**👤 GoldBear** ⬆️ 1

**Selected Answer:** C
Dependency view is the correct answer.

---

**👤 choseneagle297** ⬆️ 1

**Selected Answer:** C
answer is C

---

**👤 Emuti** ⬆️ 1

C dependency view is the correct answer

---

**👤 n_jweb** ⬆️ 1

**Selected Answer:** C
Answer is C

---

**👤 Mattchutammy** ⬆️ 1

**Selected Answer:** C
C is the correct answer

---

**👤 unicorn365** ⬆️ 2

The answer is C, Dependency View.
From ServiceNow documentation:
The maps generated by Dependency Views are based on D3 and Angular technology, providing a modern interactive graphical interface to visualize configuration items and their relationships.
https://docs.servicenow.com/bundle/tokyo-servicenow-platform/page/product/business-service-management-map-ng/task/t_AccessNGBSM.html

---

**👤 KITELinz** ⬆️ 1

**Selected Answer:** C
Should be C

---

**👤 Melby** ⬆️ 1

**Selected Answer:** C
should be C

---

**👤 yo_mamma** ⬆️ 1

should be C
https://docs.servicenow.com/bundle/utah-employee-service-management/page/product/human-resources/reference/transform-maps-navex.html

*...and 1 more comments*


</details>

---

## Question 239

### What icon do you use to change the label on a Favorite?

### Answer Choices:
**A.** Star

**B.** Clock

**C.** Triangle

**D.** Pencil



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Certs_Hub** ⬆️ 1

**Selected Answer:** D
Pencil is correct.

---

**👤 yo_mamma** ⬆️ 2

D is correct



</details>

---

## Question 240

### Which tool is used to define relationships between fields in an import set table and a target table?

### Answer Choices:
**A.** Schema Map

**B.** Field Transformer

**C.** Transform Map

**D.** Transform Schema



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 GoldBear** ⬆️ 1

**Selected Answer:** C
C - Transform Map.

---

**👤 yo_mamma** ⬆️ 3

C is correct
https://docs.servicenow.com/bundle/utah-employee-service-management/page/product/human-resources/reference/transform-maps-navex.html



</details>

---

## Question 241

### When moving a homepage or dashboard between instances, what must you remember?

### Answer Choices:
**A.** Download both as PDF and XML files

**B.** They cannot be moved via update set

**C.** The Platform will automatically add them to the update set

**D.** Manually add them to the update set



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (67%), B (33%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 aakash101** ⬆️ 1

**Selected Answer:** D
Some might think B, but no. The reason is Dashboard can actually be ofladed to an updat set, so it can be moved via updated set.

---

**👤 dfec26e** ⬆️ 1

**Selected Answer:** B
Dont be fooled... Dashboards amd homepages are not included

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D- correct as you have to add them manually. There is another question where they ask what is not included as part of a standard update set and homepages are not.

---

**👤 yo_mamma** ⬆️ 2

Possibly D
https://docs.servicenow.com/bundle/utah-platform-user-interface/page/administer/homepage-administration/task/t_AddAHomepageToAnUpdateSetAndApp.html



</details>

---

## Question 242

### Which allows the creation of a task-based record from Service Catalog?

### Answer Choices:
**A.** Record Producers

**B.** UI Builder

**C.** Assignment Rule

**D.** UI Actions

**E.** Flow Designer



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 GoldenDisciple2** ⬆️ 1

**Selected Answer:** A
UI Actions means nothing in this context. Answer is A.

---

**👤 choseneagle297** ⬆️ 1

**Selected Answer:** A
answer is A

---

**👤 Dekoreh** ⬆️ 1

**Selected Answer:** A
A is the correct answer

---

**👤 varuns03** ⬆️ 1

A is the answer

---

**👤 Emuti** ⬆️ 1

A is the correct answer

---

**👤 Honoured** ⬆️ 2

**Selected Answer:** A
IT IS A

---

**👤 KITELinz** ⬆️ 1

**Selected Answer:** A
common sense...

---

**👤 yo_mamma** ⬆️ 1

A is correct
https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/service-catalog-management/concept/c_RecordProducer.html

---

**👤 alaro** ⬆️ 1

I think the answer is A



</details>

---

## Question 243

### On the knowledge base record, which tab would you use to define which users are able to write articles to the knowledge base?

### Answer Choices:
**A.** Can Contribute

**B.** Cannot Author

**C.** Can Read

**D.** Can Write

**E.** Can Author



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 Chanukagalagamage** ⬆️ 1

**Selected Answer:** A
A is the answer

---

**👤 shruthiexam12** ⬆️ 1

A is right

---

**👤 choseneagle297** ⬆️ 1

**Selected Answer:** A
answer is A

---

**👤 Dekoreh** ⬆️ 1

**Selected Answer:** A
Cn contribute is the correct answer

---

**👤 choseneagle297** ⬆️ 1

**Selected Answer:** A
answer is A

---

**👤 varuns03** ⬆️ 1

A is the correct answer

---

**👤 Emuti** ⬆️ 1

A is the correct answer

---

**👤 StarWarsTrader** ⬆️ 1

**Selected Answer:** A
its "A"

---

**👤 KITELinz** ⬆️ 1

**Selected Answer:** A
A is right

---

**👤 yo_mamma** ⬆️ 2

A is correct
https://docs.servicenow.com/bundle/utah-servicenow-platform/page/product/knowledge-management/task/t_SelectUserCriteria.html

*...and 1 more comments*


</details>

---

## Question 244

### Which type of scripts run in the browser?

### Answer Choices:
**A.** UI Policies

**B.** Script Include Scripts

**C.** Access Control Scripts

**D.** Business Rule Scripts



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 GoldBear** ⬆️ 1

**Selected Answer:** A
A- UI Policies are run on the client - SNAF page 434.



</details>

---

## Question 245

### What enables you to trace the connection from an infrastructure item, like a Server, to the Services that are dependent on that Server?

### Answer Choices:
**A.** Automapping Utility

**B.** Relationships

**C.** Service Tracer

**D.** Transform Map



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 Certs_Hub** ⬆️ 2

**Selected Answer:** B
B. Relationships

ServiceNow allows you to establish relationships between different configuration items, such as servers and services. By defining relationships, you can create a map of dependencies, and this helps in understanding the impact of changes and tracing connections between various elements in your infrastructure.



</details>

---

## Question 246

### When searching using the App Navigator search field, what can be returned? (Choose four.)

### Answer Choices:
**A.** Names of Applications and Modules

**B.** Names of Modules

**C.** Names of Applications

**D.** Favorites

**E.** History Records

**F.** Titles of Dashboard Gauges



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABCD
**Community voting results:** ABCD (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 Ekayy** ⬆️ 9

A., B., & C. literally say the same thing...lol

---

**👤 chicocheco** ⬆️ 5

To be more precise, to search in Favorites you have to open the Favorites menu although from Utah, it's not necessary anymore.

---

**👤 9e0bd16** ⬆️ 1

**Selected Answer:** ABCD
abcd are correct

---

**👤 Osatohanme** ⬆️ 1

ABCD correct

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** ABCD
ABCD is correct

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** ABCD
A,B,C,D is correct

---

**👤 AMER1995** ⬆️ 1

**Selected Answer:** ABCD
CORRECT

---

**👤 saiakhilpalle** ⬆️ 2

ABCD are correct

---

**👤 4learning2u** ⬆️ 1

**Selected Answer:** ABCD
A, B, C, and D are the correct answers

---

**👤 Bonche** ⬆️ 1

A, B, C, D is the correct answers

*...and 1 more comments*


</details>

---

## Question 247

### What section on a task record is used to see the most recent updates made to a record?

### Answer Choices:
**A.** Timeline

**B.** Related List

**C.** Activity Stream

**D.** Audit Log



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 Certs_Hub** ⬆️ 2

**Selected Answer:** C
C is correct.



</details>

---

## Question 248

### While using the CMDB, what do you call the component that needs to be managed in order to deliver services?

### Answer Choices:
**A.** Configuration Item

**B.** Asset

**C.** Catalog Items

**D.** Data Plow

**E.** Service Offerings



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 GilSAtx** ⬆️ 2

**Selected Answer:** A
Answer is A Configuration item



</details>

---

## Question 249

### What is the first step in the process to import spreadsheet data into ServiceNow?

### Answer Choices:
**A.** Select Import Set

**B.** Run Data Scrubber

**C.** Define Data Source

**D.** Create import Set

**E.** Set Coalesce



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (50%), C (50%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 che98008** ⬆️ 6

This is different then question 216. I guess if they give the option “define data source,” then that takes priority over “load data”?

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
Initially I opted for C, but next question #323 kind of answers this. First step is Create Import Set.

---

**👤 Azharudeen4545** ⬆️ 1

**Selected Answer:** C
First you have to define the data source

---

**👤 40a783b** ⬆️ 3

This question is a bit tricky. If you go to the module "Load Data", you have first the option to select an existing import set or to create a new one. After that you select the "Source of the import", which could be a fiel, in this case a spreadsheet or a data source.

---

**👤 bern_111** ⬆️ 3

**Selected Answer:** D
The question already defines the data source that is a spreadsheet, the next step should be create import set

---

**👤 f5003a6** ⬆️ 1

**Selected Answer:** C
C. Define Data Source

---

**👤 8ef56fb** ⬆️ 2

**Selected Answer:** C
Defining the data source is essential as it specifies where the data is coming from, which in this case would be the spreadsheet. This step sets the foundation for the data import process by identifying the source file or connection that will provide the data to be imported into ServiceNow.

---

**👤 Certs_Hub** ⬆️ 2

**Selected Answer:** C
C. Define Data Source

Defining the data source involves specifying the source from which you will be importing data. This is typically the first step before creating an Import Set or performing other import-related tasks.

---

**👤 6d50eb7** ⬆️ 2

**Selected Answer:** D
Create an Import Set:
Define Import Set Table and Transform Map:
Load Data into Import Set:
Transform Data:
Validate and Correct Errors:
Preview Import Sets:
Execute Transform:
Review Target Table Records:



</details>

---

## Question 250

### What are the steps for importing data using an import set?

### Answer Choices:
**A.** Create Import Set; Create transform map; Transform data; Clean up import table

**B.** Create a Transform Map, Load Data, Transform Data, Run Transform Map Script

**C.** Identify source; Import transform map; Run transformer; Verify import

**D.** Select source file; Run AutoMap; Transform data; Clean up target table



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** A
A is the Way

---

**👤 Manny1218** ⬆️ 1

**Selected Answer:** A
I agree with A, Create Import Set, then Transform map, then Transform data and lastly clean up.



</details>

---

## Question 251

### If users would like to locate and assign a task to themselves in the Platform, what action could they perform from the list view to make the assignment? (Choose two.)

### Answer Choices:
**A.** Select the record using the check box, then select the Person icon

**B.** Select the record using the check box then select the Assign To Me UI action on the List Header

**C.** Double click on the Assigned to value, type the name of the user, and select the green check

**D.** Right click on the Task number and select the Assign to me option in the menu

**E.** Select the Task number, and select the Assign to me UI action on the form



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** CD
**Community voting results:** CD (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 deng27** ⬆️ 7

**Selected Answer:** CD
C and D are the correct answers.

---

**👤 6d50eb7** ⬆️ 1

**Selected Answer:** CD
C and D are correct

---

**👤 [Removed]** ⬆️ 2

Is 323 and 325 answer correct?



</details>

---

## Question 252

### What module enables an administrator to define destinations for imported data on any ServiceNow table?

### Answer Choices:
**A.** Load Data

**B.** Field Transform

**C.** Schema Map

**D.** Transform Map



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 BrooksieOwl** ⬆️ 1

Duplicate of Q206

---

**👤 Manny1218** ⬆️ 1

**Selected Answer:** D
I agree with D, Transform Map



</details>

---

## Question 253

### Which ServiceNow capability provides assistance to help users obtain information, make decisions, and perform common work tasks via a messaging interface?

### Answer Choices:
**A.** Now Support

**B.** ServiceNow Messenger

**C.** Agent Workspace

**D.** Virtual Agent

**E.** Knowledge Chat



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - is a messaging interface that allows users to get info.



</details>

---

## Question 254

### What instance resource allows you to access guided tours, information about actions, and instructions on how to use inputs and outputs in your flow?

### Answer Choices:
**A.** Now Support

**B.** Community

**C.** Docs

**D.** Help Panel (question mark icon)



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - correct as it's an instance resource. Now Support, Community, Docs are not instance resources.

---

**👤 BrooksieOwl** ⬆️ 1

Duplicate of 210



</details>

---

## Question 255

### When importing data from a spreadsheet, which step defines where the incoming data columns will be written in the receiving table?

### Answer Choices:
**A.** Select Data Source

**B.** Schedule Transform

**C.** Load Data

**D.** Create Transform Map

**E.** Field Alignment



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - Transform Map. Field Alignment isn't an option.

---

**👤 BrooksieOwl** ⬆️ 2

Duplicate of Q227. Answer should be Field Mapping. But we dont have that option.
So its D because Transform Map is a set of field maps



</details>

---

## Question 256

### On a form header, what icon would you click to access Template features?

### Answer Choices:
**A.** Context Menu

**B.** Paper clip

**C.** Stamp

**D.** More options (...)



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Cplatt** ⬆️ 1

**Selected Answer:** D
D is the correct answer there is no "stamp" on a form

---

**👤 4ce9f72** ⬆️ 2

**Selected Answer:** D
D is correct, there is no "Stamp"

---

**👤 ddiggler** ⬆️ 1

D is correct answer to Toggle Template Bar

---

**👤 karlouioanna** ⬆️ 1

Correct is D

---

**👤 CMPB** ⬆️ 2

**Selected Answer:** D
correct answer is D

---

**👤 chrisRY** ⬆️ 1

D is the correct answer

---

**👤 SNPCSA** ⬆️ 1

D , is the correct answer

---

**👤 SakeerValappil** ⬆️ 1

Its D more options (...)



</details>

---

## Question 257

### Which technique is used to get information from a series of referenced fields from different tables?

### Answer Choices:
**A.** Table-Walking

**B.** Sys_ID Pulling

**C.** Dot-Walking

**D.** Record-Hopping



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (86%), 14%

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 Nexfo** ⬆️ 1

**Selected Answer:** C
dot-walking, C is correct, by definition of this activity :)

---

**👤 gprieto** ⬆️ 1

**Selected Answer:** C
c is correct.

---

**👤 9e0bd16** ⬆️ 1

**Selected Answer:** D
d is correct

---

**👤 David_Wu** ⬆️ 1

https://docs.servicenow.com/bundle/vancouver-platform-user-interface/page/use/navigation/reference/dot-walking-examples.html

---

**👤 Mikias** ⬆️ 1

C is correct

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** C
C is correct.

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Jana08** ⬆️ 1

**Selected Answer:** C
Dot-Walking:
https://docs.servicenow.com/bundle/utah-platform-user-interface/page/use/navigation/reference/dot-walking-examples.html

---

**👤 saiakhilpalle** ⬆️ 2

C is correct

---

**👤 4learning2u** ⬆️ 1

**Selected Answer:** C
dot-walking, C is correct

*...and 2 more comments*


</details>

---

## Question 258

### Access Control rules are applied to a specific table, like the Incident table. What is the object name for a rule that applies to the entire Incident table (all rows and fields)?

### Answer Choices:
**A.** incident.*

**B.** incident.all

**C.** incident.!

**D.** incident.None



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 DeCharlie** ⬆️ 1

**Selected Answer:** D
incident.None includes all fields and column (entire table) then comes the part where any specific field are not to be included which is done by include.field here comes to play the incident.* (wildcard) that includes all the fields except one defined by include.field.
thus correct option will be incident.None

---

**👤 MACCA81** ⬆️ 2

**Selected Answer:** D
incident.* rule applies to all fields on the table that are NOT controlled by a specific field rule. i.e. if there are no field rules on any of the field it can represent the whole table, but as long as there's even one specific field rule it will represent all the other fields in the table but NOT this one. Hence A cannot be the correct answer.
incident.None rule represents access to the table as a whole. It's the correct answer.

---

**👤 chrisRY** ⬆️ 1

Answer: table.--None-- applies to the entire table - Pg 158 Vancouver SNAF 
Table. --*-- Represents all other fields in the table

---

**👤 Kiekie** ⬆️ 2

D. incident.None
The answer is D. As seen in SNAF Page 368



</details>

---

## Question 259

### On a list, what does each row show?

### Answer Choices:
**A.** A filter

**B.** A record

**C.** A table

**D.** A field



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 90bc085** ⬆️ 1

**Selected Answer:** B
its B for sure

---

**👤 22b3b87** ⬆️ 1

B should be the right



</details>

---

## Question 260

### Which action enables personalization in a form for the admin role, only?

### Answer Choices:
**A.** Navigate to sys_form_properties.list and set the property glide.ui.enable_personalize_form.admin to true.

**B.** Navigate to Context Menu > Configure > Form Layout and select ‘Enable Personalization’ and Enter the ‘admin’ role.

**C.** Navigate to Context Menu > Configure > Table and add the role ‘Admin’ in the ‘Available User’ list box.

**D.** Navigate to sys_properties.list find the property glide.ui.personalize_form.role and set the Value to admin.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 f5003a6** ⬆️ 5

**Selected Answer:** D
https://docs.servicenow.com/bundle/washingtondc-platform-administration/page/administer/form-administration/concept/c_FormPersonalizationAdmin.html



</details>

---

## Question 261

### Which element is used to track items not saved with a field, in a record?

### Answer Choices:
**A.** Sidebar

**B.** List Editor

**C.** Activity formatter

**D.** Dictionary



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Kflo** ⬆️ 1

**Selected Answer:** C
C - Activity stream is an example of a formatter

---

**👤 JinDG** ⬆️ 3

**Selected Answer:** C
The wording is kinda confuzing..

---

**👤 BrandyD** ⬆️ 1

It is C
https://docs.servicenow.com/bundle/washingtondc-platform-administration/page/administer/form-administration/concept/c_ActivityFormatter.html



</details>

---

## Question 262

### What does ServiceNow recommend as a best practice regarding data imports?

### Answer Choices:
**A.** Adjust your Transform maps, after the data is loaded into the target table.

**B.** Use extremely large Import Sets, instead of multiple large Import Sets.

**C.** Create a new Import set table for each new data load.

**D.** Plan time before your import to remove obsolete or inaccurate data.

**E.** Monitor data quality and clean imported data, using the Data Scrub Workspace.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
D - listed in the CSA book Washington as good practice

---

**👤 benarji** ⬆️ 1

D. Ensure that your import completes successfully and that the resulting data matches your expectations.

---

**👤 denono** ⬆️ 1

https://developer.servicenow.com/dev.do#!/learn/learning-plans/utah/new_to_servicenow/app_store_learnv2_importingdata_utah_preparing_data_for_import



</details>

---

## Question 263

### When an administrator sets a policy that is applied to all data entered into the Platform (UI, Import Sets, or Web Services), where does this policy run by default?

### Answer Choices:
**A.** Network

**B.** Server

**C.** Client

**D.** Browser



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
Something that applies to data inputs via forms, integrations, etc must run on the server side.



</details>

---

## Question 264

### If a user is on an Incident form and is alerted when they change the value of the Priority field, which type of script executes in the Platform?

### Answer Choices:
**A.** A server script

**B.** A client script

**C.** A fix script

**D.** A business rule



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (86%), 14%

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 gedgy** ⬆️ 3

**Selected Answer:** B
Key words "on a form"

---

**👤 bern_111** ⬆️ 3

**Selected Answer:** B
Client script

---

**👤 ShuL1** ⬆️ 1

B, Client Script onChange

---

**👤 barti5** ⬆️ 1

**Selected Answer:** A
Answer: A - Server Script.



</details>

---

## Question 265

### Which feature can be used to categorize a set of records from a list and make them visible to other users?

### Answer Choices:
**A.** Tags

**B.** History

**C.** Favorites

**D.** Activity Formatter



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
Tags can help with categorizing a list of records and you can set visibility for the tags



</details>

---

## Question 271

### Which role(s) are required to impersonate a user? (Choose two.)

### Answer Choices:
**A.** security_admin

**B.** sys_admin

**C.** impersonator

**D.** sys_user

**E.** admin



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** CE
**Community voting results:** CE (78%), BD (22%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** CE
CE - as per the book. 
A - you could impersonate a user with the security_admin role, but it is not required, you can do that with the normal admin.

---

**👤 Mayur444** ⬆️ 2

CE are correct

---

**👤 kristiyan_nikolov** ⬆️ 2

**Selected Answer:** CE
sys_user is a table, there is no such thing as sys_admin in SNOW, 
the correct answers are C - Impersonator so you can impersonate people if you are not admin and admin - if you are system administrator and you can 
impersonate people as a sys admin.

---

**👤 JuanSP** ⬆️ 2

**Selected Answer:** CE
C and E. No sys_admin role in ServiceNow

---

**👤 02532a3** ⬆️ 3

I see this as C and E as per current info.

---

**👤 9e0bd16** ⬆️ 2

**Selected Answer:** CE
CE is for me

---

**👤 a375085** ⬆️ 2

**Selected Answer:** BD
B and D is correct



</details>

---

## Question 272

### Which set of steps is used to import spreadsheet data into a ServiceNow table?

### Answer Choices:
**A.** Select Data Source, Schedule Transform

**B.** Load Data, Create Transform Map, Run Transform

**C.** Define Data Source, Select Transform Map, Run Transform

**D.** Select Import Set, Select Transform Map, Run Transform



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 JuanSP** ⬆️ 1

**Selected Answer:** B
B is the right one

---

**👤 DarksideSunrised** ⬆️ 3

**Selected Answer:** B
You have a spreadsheet:
1) You need to Load it
2) Create Transform map based on data
3) Run transformation

---

**👤 TimmyLam** ⬆️ 1

I believe the answer is D
1. Select your import set (create new or select existing table)
2. Create new or use existing transform map
3. Run the transform

---

**👤 Cplatt** ⬆️ 2

**Selected Answer:** B
Based on a previous question the answer to this would be "B"

---

**👤 bern_111** ⬆️ 3

**Selected Answer:** B
B should be the correct answer



</details>

---

## Question 273

### What contains the configuration changes made in an instance (i.e. changes in a form) and helps to implement the changes from the Dev environment to another environment?

### Answer Choices:
**A.** Transform maps

**B.** Update sets

**C.** Import sets

**D.** System dictionaries



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 amt_123** ⬆️ 1

**Selected Answer:** B
• B. Update sets



</details>

---

## Question 274

### An administrator creates “customer_table_admin” and “customer_table_user” roles for the newly created "Customer Table”. Which ACL rule would grant access to all rows and all fields to both the customer_table_admin and customer_table_user roles?

### Answer Choices:
**A.** customer.none

**B.** customer.all

**C.** customer.field

**D.** customer.*



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (60%), D (40%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 7c9877a** ⬆️ 1

**Selected Answer:** A
additional reference in ServiceNow Administrator Fundamentals eBook Page 60
The None access Control (which appears as just sys_user) granted all rows and all fields to both the table_admin and table_user roles.

---

**👤 szycha002** ⬆️ 1

**Selected Answer:** A
Best Practice:
- when creating .* Access Controls, also create a None Access Control, because only None grants access to records
- when writing an ACL that mostly grants access, use only None
- when writing an ACL that mostly denies access, use None and .*

---

**👤 gedgy** ⬆️ 2

Completely agree with kanafuka225. The table.* is allowing access to both of these new roles to all rows and fields as specified. Conversely, table.none is restricting all row and field access to these two roles.

---

**👤 SuiR** ⬆️ 1

**Selected Answer:** A
none will be used to grant all accesses to the record

---

**👤 Prepper003** ⬆️ 3

**Selected Answer:** A
.none is the table level which grants access to all fields and rows.

---

**👤 kanafuka225** ⬆️ 4

**Selected Answer:** D
The customer .* ACL rule would grant access to all rows and all fields to both the customer_table_admin and customer_table_user roles because it uses a wildcard (*) to match any operation on the customer table. The other options are either too restrictive or too broad.
References1: Access control list rules - Product Documentation: Utah - ServiceNow4: Access Controls - ServiceNow Elite



</details>

---

## Question 275

### Which tables are available by default in a ServiceNow instance? (Choose three.)

### Answer Choices:
**A.** User

**B.** Issue

**C.** Incident

**D.** Project

**E.** Task

**F.** Item



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACE
**Community voting results:** ACE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 bern_111** ⬆️ 4

**Selected Answer:** ACE
A,C,E is the correct answer.

---

**👤 Prepper003** ⬆️ 3

**Selected Answer:** ACE
A,C,E is the correct answer.

---

**👤 kanafuka225** ⬆️ 3

**Selected Answer:** ACE
user,incident,task



</details>

---

## Question 276

### In a Knowledge Base record, where can an administrator find the User Criteria for who can read the articles?

### Answer Choices:
**A.** From the Accessible to tab

**B.** From the Available to tab

**C.** From the Can Access tab

**D.** From the Can Read tab



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 Raxon** ⬆️ 2

**Selected Answer:** D
Correct - D
Under Knowledge -> Administration -> Knowledge Bases. 

The knowledge records shows Can Read as a tab.



</details>

---

## Question 277

### What action helps to see which modules are visible to a user?

### Answer Choices:
**A.** Install the Bomgar plug-in

**B.** Initiate a Connect Chat session

**C.** Launch a NowChat window

**D.** Impersonate the user



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
Impersonation is an action that allows you to view what is accessible to the user

---

**👤 PappyFox** ⬆️ 1

Bomgar....



</details>

---

## Question 278

### What are the different Notification methods that can be used to alert users when events that concern them have occurred? (Choose three.)

### Answer Choices:
**A.** Meeting Invitation

**B.** Email

**C.** Browser Pop ups

**D.** Messenger

**E.** SMS



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABE
**Community voting results:** ABE (65%), BCE (24%), 12%

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 weet123** ⬆️ 5

**Selected Answer:** ABE
Email, SMS, Meeting Invitation is the correct answer. Check the book

---

**👤 adyrip** ⬆️ 2

**Selected Answer:** ABE
Initially I selected BCE, as Meeting Invitation makes little sense as a Notification from SN. That is done via the calendar app. 

But in the book, Washington, there are 3 methods listed: Email, SMS, Meeting Invitation

---

**👤 Kflo** ⬆️ 1

**Selected Answer:** ABE
as per the book

---

**👤 ee1039c** ⬆️ 2

**Selected Answer:** BCE
I'm beginning to question the validity of the answers on this site. How in the world is a meeting invitation considered a notification???

---

**👤 BlackCarpet** ⬆️ 2

**Selected Answer:** BCE
BCE is the correct answer

---

**👤 zoan** ⬆️ 4

SN Admin Fundamentals Vancouver:
pg. 324
"A notification is a tool for alerting users when events that concern them have occurred through the following methods: 

Email
SMS
Meeting Invitation

---

**👤 Raxon** ⬆️ 2

**Selected Answer:** BDE
https://docs.servicenow.com/bundle/washingtondc-platform-administration/page/administer/notification/reference/notifications.html

---

**👤 TimmyLam** ⬆️ 3

**Selected Answer:** ABE
Literally read it out of the SNOW Admin knowledge book



</details>

---

## Question 279

### Which one of the following statements best describes the purpose of an Update Set?

### Answer Choices:
**A.** An Update Set allows administrators to group a series of changes into a named set and then move this set as a unit to other systems

**B.** By default, an Update Set includes customizations, Business Rules, and homepages

**C.** An Update Set is a group of customizations that is moved from Production to Development

**D.** By default, the changes included in an Update Set are visible only in the instance to which they are applied



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 julianamendo** ⬆️ 1

**Selected Answer:** A
A is the correct answer.

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Dekoreh** ⬆️ 2

**Selected Answer:** A
A is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 saiakhilpalle** ⬆️ 2

A is correct

---

**👤 Bonche** ⬆️ 1

A is the correct answer

---

**👤 som_420** ⬆️ 3

correct

---

**👤 siopau** ⬆️ 3

correct
https://docs.servicenow.com/bundle/rome-application-development/page/build/system-update-sets/concept/system-update-sets.html

---

**👤 bharatns** ⬆️ 1

correct



</details>

---

## Question 280

### What are the steps to apply an update set retrieved from another instance?

### Answer Choices:
**A.** Load the Update Set > Import the Update Set > Install

**B.** Verity Update Set is Complete, Retrieve, Preview, Commit

**C.** Verify Update Set is Complete, Test Connection, Apply

**D.** Import the Update Set > Verify the Update Set is complete > Link the Update Set > Publish



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
Textbook definition Retrieve/Preview/Commit.



</details>

---

## Question 281

### After finishing your work on High Security Settings, what is a possible way to return to normal admin security levels?

### Answer Choices:
**A.** Select Global Update Set

**B.** Log out and back in

**C.** End Impersonation

**D.** Select Normal role

**E.** Use System Administration > Normal Security module



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 skepraxis** ⬆️ 1

I agree with connal308876.

---

**👤 connal308876** ⬆️ 2

In Washington, you can select "Elevate Role" again, and deselect the checkbox. Then click the Update button.

---

**👤 8d6fb83** ⬆️ 4

**Selected Answer:** B
By default, you do not have elevated privilege roles upon login. You must manually elevate to the privilege of the role. An elevated privilege role lasts only for the duration of your user session. Session timeout or logout removes the role.

---

**👤 kanafuka225** ⬆️ 2

**Selected Answer:** B
B: Login out and back in



</details>

---

## Question 282

### Which feature can be used to give users the choice to easily populate the most-used fields for a specific table?

### Answer Choices:
**A.** Tags

**B.** Formatter

**C.** Template

**D.** Reference type fields



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 8d6fb83** ⬆️ 2

**Selected Answer:** C
C: Template is correct



</details>

---

## Question 283

### What protects applications by restricting access to data in another application, in the same instance?

### Answer Choices:
**A.** Admin Center

**B.** Access Control Rules

**C.** Assignment Rules

**D.** Application Scope



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
Application scope protects app data.



</details>

---

## Question 284

### Which module in the Service Catalog application is used to create a new catalog item?

### Answer Choices:
**A.** Configuration Items

**B.** Maintain Items

**C.** Content Items

**D.** Catalog Items



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 DDD1377** ⬆️ 2

B is the correct answer

---

**👤 cvazquez** ⬆️ 2

**Selected Answer:** B
B is the right answer

---

**👤 shub606** ⬆️ 3

B is correct answer

---

**👤 Prepper003** ⬆️ 2

**Selected Answer:** B
It is maintain items. You can check in PDI

---

**👤 359c17a** ⬆️ 2

**Selected Answer:** B
B is correct

---

**👤 8d6fb83** ⬆️ 2

**Selected Answer:** B
B. Maintain Items is correct



</details>

---

## Question 285

### What helps to define the structure of a catalog item form that is displayed to the customer?

### Answer Choices:
**A.** Variables

**B.** Catalog Description

**C.** Catalog Definition

**D.** Order Guides



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 40a783b** ⬆️ 1

**Selected Answer:** A
A correct



</details>

---

## Question 286

### Which displays the list of activities, or history, on a form?

### Answer Choices:
**A.** Formatter

**B.** Dictionary

**C.** Sidebar

**D.** History Menu



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Latyfah** ⬆️ 6

**Selected Answer:** A
Formatter is the correct answer.

The history menu only shows the history of your activities on the platform, not the form.

---

**👤 9e0bd16** ⬆️ 1

**Selected Answer:** A
a is correct

---

**👤 AVK301096** ⬆️ 2

Formatter



</details>

---

## Question 287

### Where should an administrator navigate to add the "Save" button in the form header?

### Answer Choices:
**A.** Context Menu > Form Design

**B.** All > System Properties > UI Action Settings

**C.** All > System Properties > UI Properties

**D.** Context Menu > Form Layout



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 Latyfah** ⬆️ 6

**Selected Answer:** C
C is the correct answer.

You need to go to System Properties > UI Properties to enable the save, update and insert and stay option (true/false box) from the glide.ui.advance property.

---

**👤 359c17a** ⬆️ 3

**Selected Answer:** C
C is correct, tried it in my PDI



</details>

---

## Question 288

### What are the options that can be set to determine when a Business Rule executes? (Choose four.)

### Answer Choices:
**A.** Async

**B.** Load

**C.** Change

**D.** Display

**E.** After

**F.** Before

**G.** Submit

**H.** Click



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ADEF
**Community voting results:** ADEF (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 257884d** ⬆️ 5

According to the vancouver SNAF e-book, the answer is ADEF

---

**👤 Cosmo87** ⬆️ 1

i cheked in my vancouver version the correct answers is ADEF

---

**👤 shub606** ⬆️ 2

answer should be - ADEF

---

**👤 Cplatt** ⬆️ 3

**Selected Answer:** ADEF
I checked in my PDI and the correct answer is ADEF

---

**👤 8d6fb83** ⬆️ 4

**Selected Answer:** ADEF
In business rule definition the "when" field contains before, after, async and display options (ADEF)



</details>

---

## Question 289

### Which of the following can be customized through the Basic Configuration UI 16 module? (Choose three.)

### Answer Choices:
**A.** Banner Image

**B.** Record Number Format

**C.** Browser Tab Title

**D.** System Date Format

**E.** Form Header Size



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACD
**Community voting results:** ACD (63%), ACE (38%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 chelu** ⬆️ 1

**Selected Answer:** ACD
https://www.servicenow.com/docs/bundle/yokohama-platform-user-interface/page/administer/navigation-and-ui/task/t_ConfigureLogoColorsSysDfltsUI16.html

---

**👤 Deshpamo** ⬆️ 3

**Selected Answer:** ACE
The Basic Configuration UI 16 module in ServiceNow allows administrators to make simple branding and UI customizations without requiring deep technical knowledge. Based on its typical functionalities, the three customizable options from your list are:

A. Banner Image – ✅ Can be customized via the module to change the system's branding.
C. Browser Tab Title – ✅ This can be configured to display a custom title in the browser tab.
E. Form Header Size – ✅ Allows changes to the appearance of form headers.

The incorrect options:
B. Record Number Format – ❌ This is controlled by numbering rules, not the Basic Configuration UI 16 module.
D. System Date Format – ❌ Managed in system properties, not via the Basic Configuration UI 16 module.

Final answer: A, C, and E.

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** ACD
ACD is the correct answer.

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** ACD
A, C, D is correct

---

**👤 Dekoreh** ⬆️ 1

**Selected Answer:** ACD
correct answer

---

**👤 RaviRama** ⬆️ 3

ACD -
https://docs.servicenow.com/ja-JP/bundle/rome-platform-user-interface/page/administer/navigation-and-ui/task/t_ConfigureLogoColorsSysDfltsUI16.html

---

**👤 saiakhilpalle** ⬆️ 1

ACD are correct

---

**👤 4learning2u** ⬆️ 1

**Selected Answer:** ACD
A, C, and D are the correct answers

---

**👤 Bonche** ⬆️ 1

A, C, D is the correct answers

---

**👤 som_420** ⬆️ 2

correct

*...and 1 more comments*


</details>

---

## Question 290

### Which access control rule applies to every field in the incident table?

### Answer Choices:
**A.** incident<=>*

**B.** incident.all

**C.** incident.*

**D.** incident||*



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
Wildcard applies to every field on a record, without a table.field rule - extract from the CSA Book.



</details>

---

## Question 291

### What is the purpose of an application scope?

### Answer Choices:
**A.** Scope determines which parts of an application are available for use by other applications In ServiceNow

**B.** Scope prevents applications from changing their own tables and business logic

**C.** Scope determines the functionality of core services in the platform

**D.** Scope determines all applications access each other's tables and business logic



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 manojabc** ⬆️ 1

**Selected Answer:** A
A is correct



</details>

---

## Question 292

### On a Catalog Task record, how would an administrator change the priority choice options?

### Answer Choices:
**A.** In the Context menu of the form, select Configure Options

**B.** Right-click the Priority Label > select Configure Lists

**C.** Right-click the Priority Label > select Configure Choices

**D.** Open the Data Dictionary > select Values



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
Tested in PDI



</details>

---

## Question 293

### Which sequence of actions applies an update set to an instance?

### Answer Choices:
**A.** Preview, Save, Commit

**B.** Preview, Retrieve, Commit

**C.** Retrieve, Commit, Preview

**D.** Retrieve, Preview, Commit



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
Basic steps per CSA book



</details>

---

## Question 294

### A customer asks you to make the following updates to a form:

1. When Caller field is populated, automatically look up caller location and load it in the Location field.
2. Add notice at the top of the page when the record is submitted.

What type of script would you use to meet this requirement?

### Answer Choices:
**A.** UI script

**B.** Server script

**C.** JavaScript

**D.** Client script



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** D
CSA book uses these exact examples to explain what a Client Script is.



</details>

---

## Question 295

### Where do Ul Policies run?

### Answer Choices:
**A.** Client-side (browser)

**B.** Knowledge Base

**C.** Server-side (database)

**D.** Within server-side scripts



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
User Interface policies run on the Client side.



</details>

---

## Question 296

### You are creating a catalog item for ordering a new desktop computer. The computers have these options:


Color: Black or Silver -

RAM: 32 MB or 64MB -

Keyboard: Standard or Ergonomic -

Monitor: 24 inch or 32 inch -

How would you add these options to the catalog item form?

### Answer Choices:
**A.** Add choices

**B.** Add variables

**C.** Add UI options

**D.** Add fields



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
As per CSA book chapter on Catalog Items/Variables



</details>

---

## Question 297

### Which form element displays the list of activities, or history, on a task form?

### Answer Choices:
**A.** Action Formatter

**B.** Activity Formatter

**C.** Action Stream

**D.** Sidebar



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** B
Activity Formatter shows all the activities in the task form



</details>

---

## Question 298

### What is the function of user impersonation?

### Answer Choices:
**A.** Testing and visibility

**B.** Activate verbose logging

**C.** View custom perspectives

**D.** Unlock Application master list



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 Parth012** ⬆️ 1

A. is correct but why is C. wrong?

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** A
A is correct.

---

**👤 segunal** ⬆️ 4

I know A is correct but can someone please explain why view custom perspectives is wrong?

---

**👤 sifot** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 saiakhilpalle** ⬆️ 1

A is correct

---

**👤 Bonche** ⬆️ 1

A is the correct answer

---

**👤 som_420** ⬆️ 1

correct



</details>

---

## Question 299

### Your customer wants to update a notification so it is sent to the Caller's Manager.

Which action supports this requirement?

### Answer Choices:
**A.** On the 'Who will receive' tab on the Notification record add the Caller field, then dot walk to the Caller's Manager field to add it.

**B.** On the Notification record create a flow and include a notification in the flow for "All receivers".

**C.** On the 'Send to' tab on the Notification record, add the Caller field, then dot walk to the Caller’s Manager field to add it.

**D.** On the 'Send to' tab on the Notification record, set "Who will receive" to Subscribable.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 IT_Bank** ⬆️ 1

**Selected Answer:** A
The correct answer is A. Verified.

https://www.servicenow.com/community/training-and-certifications/csa-exam-question-doubt-can-someone-helop-me-answering-this/m-p/3166078

 ff063b3 is a degenerate giving false answers.

---

**👤 182f6be** ⬆️ 1

**Selected Answer:** A
There's no "Send to" tab anymore

---

**👤 ff063b3** ⬆️ 3

**Selected Answer:** A
In current ServiceNow patch answer A is correct.

My wife's boyfriend told me this as a fact



</details>

---

## Question 300

### Which action enables a user to view and specify date and time formats in their instance?

### Answer Choices:
**A.** Select the User menu > Preferences > Time Settings > Toggle display time zone

**B.** Ask the user to adjust the time zone on their personal computer

**C.** Use the system properties to correct the instance's time zone

**D.** Select the User menu > Preferences > Language & Region > Set date and time format and time zone

**E.** Create a UI Script to set the default timezone



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 d6c6f7b** ⬆️ 1

**Selected Answer:** C
The answer is C. 
All-> system properties -> System Configuration

Regarding D, there's no Language & Region. The only way using the preferences is to go to display and change the time there.



</details>

---

## Question 301

### A manager is complaining that they can't get the data they need on a report, because the data resides in two different tables. This data is used for many different reports in their department. You have checked to see if dot-walking will meet the requirement, and it is not possible. What else might you try to help this manager?

### Answer Choices:
**A.** Export the tables to spreadsheet

**B.** Create a Database View

**C.** Create a Report Template

**D.** Create a Report Source

**E.** Create a custom table



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 73301d1** ⬆️ 1

**Selected Answer:** B
A Database View in ServiceNow allows you to join data from multiple tables (e.g., Incident + Task SLA). It acts like a virtual table that can be used in reporting.
Perfect for scenarios where dot-walking can’t access the necessary related records.



</details>

---

## Question 302

### What information does the System Dictionary contain?

### Answer Choices:
**A.** The human-readable labels and language settings

**B.** The definition for each table and column

**C.** The information on how tables relate to each other

**D.** The language dictionary used for spell checking



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 GoldBear** ⬆️ 2

**Selected Answer:** B
The system dictionary is a table, called Dictionary Entry [sys_dictionary], that contains details for each table and the definition for every column on each table in an instance.

https://docs.servicenow.com/bundle/vancouver-platform-administration/page/administer/data-dictionary-tables/concept/c_SystemDictionary.html

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** B
B is correct.

---

**👤 in_da_cloud** ⬆️ 1

B is correct!
Insert -> Saves the item as a new record and returns to the previously viewed page.
Update -> Click Submit or Update to save changes and return to the previously viewed page.
source:
https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/use/using-forms/task/t_EditingInForms.html

---

**👤 evoken** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Oluwalana54** ⬆️ 2

**Selected Answer:** B
B is the accurate answer

---

**👤 saiakhilpalle** ⬆️ 1

B is correct

---

**👤 4learning2u** ⬆️ 1

**Selected Answer:** B
B is your answer

---

**👤 Bonche** ⬆️ 1

B is the correct answer

---

**👤 VishalMoon** ⬆️ 1

**Selected Answer:** B
Both Insert and update will close the form. Insert and Stay or Save options will stay on the form

---

**👤 som_420** ⬆️ 2

correct

*...and 1 more comments*


</details>

---

## Question 303

### Which storefront is a single location for accessing pre-built spokes, to quickly integrate with third party services to build and share content?

### Answer Choices:
**A.** ServiceNow Store

**B.** Integration Spoke Store

**C.** Spoke Store

**D.** ServiceNow Spoke Store



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (60%), B (20%), D (20%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 73301d1** ⬆️ 1

**Selected Answer:** B
The IntegrationHub Store is a centralized location within ServiceNow where you can:
>Access pre-built spokes (ready-made connectors)
>Quickly integrate with third-party services (e.g., Slack, Microsoft Teams, Zoom, AWS, etc.)
>Download, install, and manage spokes for Flow Designer
>Share and reuse integration content

---

**👤 574738b** ⬆️ 1

**Selected Answer:** A
NO such thing as a ServiceNow spoke store.

---

**👤 IT_Bank** ⬆️ 1

**Selected Answer:** A
The answer is A. There is no such thing as a ServiceNow Spoke Store dfec26e is a degenerate providing wrong answers.

---

**👤 dfec26e** ⬆️ 1

**Selected Answer:** D
Servicenow does have a spoke store integrated do the answer is right

---

**👤 dfec26e** ⬆️ 1

**Selected Answer:** A
Isnt the answer A?



</details>

---

## Question 304

### Which actions would you take to edit the title on an application menu? (Choose two.)

### Answer Choices:
**A.** Select the Application name on the Navigator, then select the pencil icon

**B.** Select System Definition > Dictionary > Application

**C.** Select System Definitions > Application Menus then select the application

**D.** Select Configuration > Applications, then select the application

**E.** Select the form for the Application, then right-click, Configure > Form Designer



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AC
**Community voting results:** CD (100%)

</details>

<details>
<summary><strong>💬 View Discussion (1 comments)</strong></summary>

**👤 73301d1** ⬆️ 1

**Selected Answer:** CD
edit the title of an application menu in ServiceNow, you can use either of the following valid methods:

✅ C. System Definition > Application Menus
Go to All → System Definition → Application Menus

This shows a list of all application menus.

Open the record and update the "Title" field.

✅ D. Configuration > Applications
Go to All → Configuration → Applications

This module is used in scoped application development.

Selecting the application allows you to update metadata, including the title and description.



</details>

---

## Question 305

### When working on a form, what is the difference between Insert and Update operations?

### Answer Choices:
**A.** Insert creates a new record and Update saves changes, both remain on the form

**B.** Insert creates a new record and Update saves changes, both exit the form

**C.** Insert saves changes and exits the form, Update saves changes and remains on the form

**D.** Insert saves changes and remains on the form, Update saves changes and exits the form



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (99%), 1%

</details>

<details>
<summary><strong>💬 View Discussion (62 comments)</strong></summary>

**👤 Jo98778** ⬆️ 28

**Selected Answer:** B
From the actual ServiceNow fundamentals course transcript:

""Some records—for example, surveys and catalog items—allow you to copy records through Insert and Insert and Stay. Check for these options on the form context menu if you want to create a copy of the current record. Insert copies the record and redisplays the list. Insert and Stay copies the record and keeps it open so that you can work on the new copy immediately."

Kristen: Tell me about it! If I want to make a copy of the record and close the form, I make my changes, and then just select Insert. I'm good to go. So simple! "

The suggested answer is WRONG and should be changed, unless they are deliberately trying to get people to fail the test.

---

**👤 encqizo** ⬆️ 23

The correct answer is B, Insert, Insert and Stay, update, save

---

**👤 showboat_zw** ⬆️ 1

B is correct .if the choice is insert and stay ,D is correct.

---

**👤 9e0bd16** ⬆️ 1

**Selected Answer:** B
b is correct

---

**👤 nlaksdmrqe** ⬆️ 1

**Selected Answer:** B
The correct answer is B, I have gone to my dev instance, performed both save and update operations on a form. Insert creates a new record and Update saves changes, both exit the form

---

**👤 40a783b** ⬆️ 1

**Selected Answer:** B
B is correct answer. 
What is a difference between insert and save if D should be the right answer?

---

**👤 Prabhu81** ⬆️ 1

D is right answer

---

**👤 Dzwg53** ⬆️ 2

Correct answer is B as Insert and Stay allows for you to stay on the form, if you select Insert it will exit the form.

---

**👤 D_Maj** ⬆️ 3

The answer is B for sure. If answer is D then What is the use of insert and stay ?

---

**👤 David_Wu** ⬆️ 1

**Selected Answer:** B
B is correct for sure...

*...and 52 more comments*


</details>

---

## Question 306

### The display sequence is controlled in a Service Catalog Item using which of the following?

### Answer Choices:
**A.** The Default Value field in the Catalog Item form

**B.** The Sequence field in the Catalog Item form

**C.** The Order field in the Variable form

**D.** The Choice field in the Variable form



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (16 comments)</strong></summary>

**👤 vosegoh** ⬆️ 2

**Selected Answer:** C
C: Correct https://docs.servicenow.com/bundle/sandiego-servicenow-platform/page/product/service-catalog-management/concept/c_DefineVariableSetLayout.html#d1331070e66

https://www.servicenow.com/community/servicenow-impact-forum/best-csa-exam-tips/m-p/2791114

---

**👤 jnk252** ⬆️ 1

**Selected Answer:** C
C is Correct

---

**👤 SaiExamtopic** ⬆️ 1

C is Correct

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 LovMi21** ⬆️ 1

https://support.servicenow.com/kb?id=kb_article_view&sysparm_article=KB0688324

---

**👤 servicepow** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 shravan3** ⬆️ 1

c. order field in the variable
 form

---

**👤 Mahindra_Reddie** ⬆️ 1

**Selected Answer:** C
Is correct answer

---

**👤 Augusto0007** ⬆️ 1

**Selected Answer:** C
C is correct

*...and 6 more comments*


</details>

---

## Question 307

### How is the Event Log different from the Event Registry?

### Answer Choices:
**A.** Event Log contains generated Events, the Event Registry is a table of Event definitions

**B.** Event Log is formatted in the Log style, the Event Registry displays different fields

**C.** Event Log lists Events that were triggered by integrations, the Event Registry lists the Events that were triggered during the day (24-hour period)

**D.** Event Log is the same as the Event Registry



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 FOJ** ⬆️ 8

**Selected Answer:** A
A is partially correct. Event Registry is a feature that allows ServiceNow to capture events from external systems, such as servers, applications, and network devices. The Event Registry provides a way to collect and analyze events from multiple sources in a single location, allowing organizations to identify patterns and trends in their IT infrastructure. The Event Registry can be configured to trigger notifications, alerts, and other actions based on specific events, enabling organizations to proactively manage their IT environment.

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** A
A is correct.

---

**👤 evoken** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 saiakhilpalle** ⬆️ 2

A is correct

---

**👤 4learning2u** ⬆️ 1

**Selected Answer:** A
The correct answer is A

---

**👤 Bonche** ⬆️ 1

A is the correct answer

---

**👤 VishalMoon** ⬆️ 4

**Selected Answer:** A
https://docs.servicenow.com/bundle/sandiego-platform-administration/page/administer/platform-events/event-logs-2.html
https://docs.servicenow.com/bundle/sandiego-platform-administration/page/administer/platform-events/concept/c_EventRegistry.html



</details>

---

## Question 308

### What is a Dictionary Override?

### Answer Choices:
**A.** A Dictionary Override is an incoming customer update in an Update Set which applies to the same objects as a newer local customer update

**B.** A Dictionary Override is the addition, modification, or removal of anything that could have an effect on IT services

**C.** A Dictionary Override is a task within a flow that requests an action before the flow can continue

**D.** A Dictionary Override sets field properties in extended tables



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Thattechgirl** ⬆️ 11

dictionary overrides can be used to set field properties in extended tables in ServiceNow. An extended table is a custom database table that is created by extending an existing base table in the platform. When you create an extended table, you can specify which fields from the base table you want to include in the new table, as well as any additional fields that are specific to the extended table.

You can use dictionary overrides to modify the properties of fields in an extended table in much the same way as you would for fields in a base table. For example, you might use a dictionary override to change the label or description of a field, or to specify a default value or a custom picker for that field.

It is important to note that dictionary overrides apply to the specific field that they are defined for, and do not affect other fields in the same table or in related tables. So if you want to modify the properties of multiple fields in an extended table, you will need to create separate dictionary overrides for each field.

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** D
D is correct.

---

**👤 Dibonddo** ⬆️ 2

A Dictionary Override is a feature of the ServiceNow platform, which is used for IT service management. It allows customers to update or modify the metadata (dictionary) of a specific table in a ServiceNow instance, even if that table has been customized by the instance owner. This allows for greater flexibility in customization and ensures that customer updates can be easily managed and tracked.

In simple terms, a Dictionary Override occurs when a customer updates the metadata of a table in a ServiceNow instance that has already been customized by the instance owner. The customer update is stored separately from the instance owner's customization, and can be easily identified and managed.

---

**👤 Jana08** ⬆️ 3

**Selected Answer:** D
D is correct.

---

**👤 saiakhilpalle** ⬆️ 1

D is correct

---

**👤 4learning2u** ⬆️ 3

**Selected Answer:** D
Dictionary overrides provide the ability to define a field on an extended table differently from the field on the parent table. For example, for a field on the Task [task] table, a dictionary override can change the default value on the Incident [incident] table without affecting the default value on Task [task] or on Change [change]

---

**👤 DANX1** ⬆️ 2

D.-
https://docs.servicenow.com/bundle/tokyo-platform-administration/page/administer/data-dictionary-tables/concept/c_DictionaryOverrides.html

---

**👤 som_420** ⬆️ 4

correct



</details>

---

## Question 309

### Which group of permissions is used to control Application and Module access?

### Answer Choices:
**A.** Access Control Rules

**B.** UI Policies

**C.** Roles

**D.** Assignment Rules



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (9 comments)</strong></summary>

**👤 Dibonddo** ⬆️ 6

C. Roles are used to control application and module access in most systems that use role-based access control (RBAC) as a security model. Roles are assigned to users or groups and define the permissions and access levels that they have within the system. By defining roles with specific permissions, access to applications and modules can be restricted to only those users who need it, while preventing unauthorized access. Access Control Rules, UI Policies, and Assignment Rules are used for other purposes within the system, such as enforcing data security or automating tasks.

---

**👤 David_Wu** ⬆️ 1

**Selected Answer:** C
C is the answer

---

**👤 DelB** ⬆️ 1

https://developer.servicenow.com/dev.do#!/learn/learning-plans/utah/servicenow_application_developer/app_store_learnv2_securingapps_utah_roles

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** C
C is correct.

---

**👤 Jana08** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 som_420** ⬆️ 2

correct

---

**👤 siopau** ⬆️ 2

correct
https://docs.servicenow.com/bundle/rome-platform-administration/page/administer/roles/concept/c_Roles.html

---

**👤 bharatns** ⬆️ 2

Correct. In ServiceNow, roles are used to limit and control access to features in applications and modules



</details>

---

## Question 310

### What is a Record Producer?

### Answer Choices:
**A.** A Record Producer is a type of Catalog Item that is used for Requests, not Services

**B.** A Record Producer creates user records

**C.** A Record Producer is a type of Catalog Item that provides easy ordering by bundling requests

**D.** A Record Producer is a type of a Catalog Item that allows users to create task-based records from the Service Catalog



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Dibonddo** ⬆️ 5

correct answer is D. A Record Producer is a type of Catalog Item that allows users to create task-based records from the Service Catalog.

In the context of IT service management and ServiceNow platform, a Record Producer is a type of Catalog Item that enables users to create new records of a specified table and task-based records from the Service Catalog interface. It is a form-based interface that presents users with fields and options to fill in, which then generates a new record in the designated table with the entered information.

Record Producers are often used to facilitate user self-service and automate the creation of records for various types of requests, such as incidents, change requests, or service requests. They can also be used to bundle multiple requests into a single, easy-to-use interface, making it simpler for users to order multiple services or items at once.

---

**👤 2pagesServiceNowSolutions** ⬆️ 2

D may be the correct answer for the exam but for people wanting to know the actual use. A record producer can be used to insert a record directly from the service catalog into ANY table, task-based or otherwise.

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** D
D is correct.

---

**👤 Oluwalana54** ⬆️ 2

**Selected Answer:** D
D is correct

---

**👤 saiakhilpalle** ⬆️ 1

D is correct

---

**👤 4learning2u** ⬆️ 2

**Selected Answer:** D
D is correct

---

**👤 som_420** ⬆️ 2

correct

---

**👤 siopau** ⬆️ 4

correct
https://docs.servicenow.com/bundle/rome-servicenow-platform/page/product/service-catalog-management/concept/c_RecordProducer.html



</details>

---

## Question 311

### Create Incident, Password Reset, and Report outage: what do these services in the Service Catalog have in common?

### Answer Choices:
**A.** They direct the user to a record producer

**B.** They direct the user to a catalog property

**C.** They direct the user to a catalog UI policy

**D.** They direct the user to a catalog client script



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 Dibonddo** ⬆️ 6

Correct answer is A. They direct the user to a record producer.

The three services in the Service Catalog - Incident, Password Reset, and Report Outage - have in common that they all direct the user to a record producer. A record producer is a form that allows users to create a new record, such as an incident or a password reset request, in the ServiceNow platform. When a user requests any of these services, they are directed to a record producer where they can fill out the required information and submit the request.

While the specific details of each record producer may vary depending on the service being requested, the commonality between them is that they all serve as the starting point for creating a new record in the ServiceNow platform.

---

**👤 roopsrich** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
A IS CORRECT

---

**👤 saiakhilpalle** ⬆️ 3

A is correct

---

**👤 4learning2u** ⬆️ 1

**Selected Answer:** A
A is the correct answer

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 312

### What is the Import Set Table?

### Answer Choices:
**A.** A table where data will be placed, post-transformation

**B.** A table that determines relationships

**C.** A staging area for imported records

**D.** A repository for Update Set information



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 Dibonddo** ⬆️ 8

Correct answer is C. Import Set Table is a staging area for imported records in ServiceNow. It is used to import data from various sources into ServiceNow tables. Import Set Tables provide a temporary holding area for records imported from a data source such as a CSV file, an Excel file, or a database table. Once the records are imported into the Import Set Table, they can be transformed and then moved to the appropriate ServiceNow table. The Import Set Table is typically used in conjunction with Transform Maps, which define how the imported data is transformed and mapped to ServiceNow tables

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C IS CORRECT

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** C
C is correct.

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 Ranaway** ⬆️ 1

C is the correct answer

---

**👤 som_420** ⬆️ 3

correct

---

**👤 siopau** ⬆️ 4

correct
https://docs.servicenow.com/bundle/rome-platform-administration/page/administer/import-sets/concept/c_ImportSetsKeyConcepts.html



</details>

---

## Question 313

### What is a characteristic of importing data into ServiceNow?

### Answer Choices:
**A.** An existing Transform Map can be used one time on the same import set

**B.** Coalesce fields are used only after running Transform

**C.** Any user can manage and set up import sets

**D.** An existing Transform Map can be used multiple times on the same import set



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 Dibonddo** ⬆️ 6

Correct answer is D. An existing Transform Map can be used multiple times on the same import set is a characteristic of importing data into ServiceNow. Transform Maps are used to map data from an import set to fields in a ServiceNow table. Once a Transform Map is created, it can be used multiple times on the same import set or on different import sets that have the same data structure. This makes it easier to import data into ServiceNow, especially if you need to import the same type of data on a regular basis.

---

**👤 som_420** ⬆️ 5

D seems to be correct from the list of available options

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** D
D is correct.

---

**👤 saiakhilpalle** ⬆️ 3

D is correct

---

**👤 DANX1** ⬆️ 2

D is correct



</details>

---

## Question 314

### What module in the Service Catalog application does an Administrator access to begin creating a new item?

### Answer Choices:
**A.** Maintain Categories

**B.** Maintain Items

**C.** Content Items

**D.** Items



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 Kcaher** ⬆️ 5

Admin can define individual catalog items using Service Catalog > Catalog Definition > Maintain Item application. You can create, edit, and copy catalog items.

Maintain items is an application module that list few types of request items like catalog item, record producer, standard change templates, software catalog, hardware catalog etc. All of these are extended from the base sc_cat_item table and used for request fulfillment from your service catalog. They do create new records ie request or RITM from catalog items or a task based record from a record producer or a change from standard template.

---

**👤 sorenft** ⬆️ 1

**Selected Answer:** B
The correct answer is: B. Maintain Items
Why not the others:
A. Maintain Categories: Used to manage and organize catalog categories, not individual items.
C. Content Items: Used for informational items, not for creating standard requestable catalog items.
D. Items: Not a standalone module — "Maintain Items" is the correct full module name.

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 s_KASKA** ⬆️ 1

B is the correct answer.

---

**👤 chrisklesar1234** ⬆️ 1

- Service Catalog > Catalog Definitions > Maintain Items B

---

**👤 sithupraji** ⬆️ 1

B IS RIGHT

---

**👤 Dibonddo** ⬆️ 1

The module that an Administrator would access to begin creating a new item in the Service Catalog application would be "B. Maintain Items".

---

**👤 saiakhilpalle** ⬆️ 1

B is correct

---

**👤 4learning2u** ⬆️ 3

**Selected Answer:** B
B - Service Catalog > Catalog Definitions > Maintain Items

---

**👤 Mohammedhz** ⬆️ 3

Correct



</details>

---

## Question 315

### Which of the following allows a user to edit field values in a list without opening the form?

### Answer Choices:
**A.** Data Editor

**B.** Edit Menu

**C.** List Editor

**D.** Form Designer



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 gprieto** ⬆️ 1

C is correct

---

**👤 evoken** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 saiakhilpalle** ⬆️ 3

C is correct, Just double click on any field to edit.

---

**👤 DANX1** ⬆️ 4

C.-
https://docs.servicenow.com/bundle/tokyo-platform-administration/page/administer/list-administration/reference/r_AdministeringTheListEditor.html

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 316

### Which three Variable Types can be added to a Service Catalog Item?

### Answer Choices:
**A.** True/False, Multiple Choice, and Ordered

**B.** True/False, Checkbox, and Number List

**C.** Number List, Single Line Text, and Reference

**D.** Multiple Choice, Select Box, and Checkbox



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Dibonddo** ⬆️ 8

The answer to this question may vary depending on the specific Service Catalog software being used. However, based on common practices, the three variable types that can be added to a Service Catalog Item are typically:

D. Multiple Choice, Select Box, and Checkbox

These variable types allow users to select options from a predefined list and can be used for a variety of purposes, such as selecting a specific model or version of a product, choosing a department or location, or selecting a priority level. Other variable types that may also be supported by some Service Catalog software include text fields, date/time pickers, and attachments.

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** D
D is correct.

---

**👤 evoken** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 saiakhilpalle** ⬆️ 2

D is correct

---

**👤 4learning2u** ⬆️ 3

**Selected Answer:** D
D seems to be the correct answer
https://docs.servicenow.com/bundle/tokyo-application-development/page/administer/flow-designer/reference/supported-service-catalog-types.html

---

**👤 SC22MarkTopics** ⬆️ 1

**Selected Answer:** D
D is correct.

---

**👤 som_420** ⬆️ 2

Correct



</details>

---

## Question 317

### Reports can be created from which different places in the platform? (Choose two.)

### Answer Choices:
**A.** List column heading

**B.** Metrics module

**C.** Statistics module

**D.** View / Run module



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AD
**Community voting results:** AD (95%), 5%

</details>

<details>
<summary><strong>💬 View Discussion (25 comments)</strong></summary>

**👤 HNTrinh** ⬆️ 7

**Selected Answer:** AD
The correct answers are A and D as per SN fundamental book in Module 5: enable productivity.

---

**👤 jnk252** ⬆️ 1

**Selected Answer:** AD
A and D are correct.

---

**👤 showboat_zw** ⬆️ 1

what is statistics module?

---

**👤 TipoZico** ⬆️ 1

**Selected Answer:** AD
A,D. To approach 100%.

---

**👤 DDD1377** ⬆️ 1

A and D are correct.

---

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** AD
A and d

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** AD
A and D is correct

---

**👤 servicepow** ⬆️ 1

**Selected Answer:** AD
A and D are correct

---

**👤 shravan3** ⬆️ 1

A and D is correct

---

**👤 Mahindra_Reddie** ⬆️ 1

**Selected Answer:** AD
Is correct answer

*...and 15 more comments*


</details>

---

## Question 318

### How are Workflows moved between instances?

### Answer Choices:
**A.** Workflows are moved using Update Sets

**B.** Workflows are moved using Transform Maps

**C.** Workflows are moved using Application Sets

**D.** Workflows cannot be moved between instances



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 csa4u** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Dibonddo** ⬆️ 4

A. Workflows are moved using Update Sets.

Update Sets are used to move customizations and configurations from one ServiceNow instance to another. Workflows, being a type of customization, can be moved between instances using Update Sets. When you create an Update Set, you can add workflows to it, along with any related components such as tables, fields, business rules, and UI pages. Once you have created an Update Set, you can then export it from the source instance and import it into the target instance. This allows you to move workflows and other customizations between instances as needed.

---

**👤 PLiza2** ⬆️ 1

A is correct

---

**👤 PLiza2** ⬆️ 3

The system tracks workflows in update sets differently than other records because workflow information is stored across multiple tables.

Changes made to a workflow version are not added to the update set until the workflow is published, at which point the entire workflow is added into the update set. Update sets store workflows as a single Workflow [wf_workflow] record and only retain the latest version with the update type of Workflow.

https://docs.servicenow.com/bundle/tokyo-servicenow-platform/page/administer/workflow-administration/concept/c_WorkflowMovementWithUpdateSets.html

---

**👤 saiakhilpalle** ⬆️ 1

A might be correct

---

**👤 som_420** ⬆️ 4

correct



</details>

---

## Question 319

### The baseline Service Catalog homepage contains links to which of the following components?

### Answer Choices:
**A.** Record Producers, Order Guides, and Catalog Items

**B.** Order Guides, Item Variables, and flows

**C.** Order Guides, Catalog Items, and flows

**D.** Record Producers, Order Guides, and Item Variables



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 Gambol** ⬆️ 2

**Selected Answer:** A
https://docs.servicenow.com/bundle/rome-servicenow-platform/page/product/service-catalog-management/concept/c_ViewNavSvrCat.html

---

**👤 singh_vik** ⬆️ 3

The baseline Service Catalog homepage typically contains links to Order Guides, Catalog Items, and flows. Therefore, the correct answer is C. Order Guides, Catalog Items, and flows.

Record Producers and Item Variables are also important components of the Service Catalog, but they are not typically listed on the homepage. Instead, they are usually accessed through links within Catalog Items or Order Guides.

---

**👤 saiakhilpalle** ⬆️ 2

A is correct

---

**👤 rpersonal2** ⬆️ 1

**Selected Answer:** A
A is correct.

---

**👤 AWoman** ⬆️ 2

**Selected Answer:** A
correct



</details>

---

## Question 320

### Which of the following statements is true when a new table is created by extending another table?

### Answer Choices:
**A.** The new table archives the parent table and assumed its roles in the database

**B.** The new table inherits all of the Business Rules, Client Scripts, and UI Policies of the parent table, but none of the existing fields

**C.** The new table inherits all of the fields of the parent table and can also contain new fields unique to itself

**D.** The new table inherits all of the fields, but does not inherit Access Control rules, Client Scripts, and UI Policies of the parent table



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Sami1622** ⬆️ 1

C is Correct

---

**👤 Mythilyy** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 sithupraji** ⬆️ 1

C IS CORRECT

---

**👤 saiakhilpalle** ⬆️ 2

C is correct

---

**👤 rpersonal2** ⬆️ 1

**Selected Answer:** C
C is correct.

---

**👤 AWoman** ⬆️ 1

**Selected Answer:** C
correct

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 321

### Where can Admins check which release is running on an ServiceNow instance?

### Answer Choices:
**A.** Memory Stats module

**B.** Stats module

**C.** System.upgraded table

**D.** Transactions log



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (88%), 13%

</details>

<details>
<summary><strong>💬 View Discussion (13 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 dipayan9051** ⬆️ 1

**Selected Answer:** B
Stats.do is the exact command..

---

**👤 csa4u** ⬆️ 1

**Selected Answer:** B
B is correct. Run stats.do to check

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** B
B is correct. System.upgraded table does not exist.

---

**👤 kohliaashish** ⬆️ 1

Run stats.do to check

---

**👤 Nabuto** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Jana08** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 narmada11** ⬆️ 1

B is correct.....quick way

---

**👤 singh_vik** ⬆️ 1

**Selected Answer:** C
Admins can check which release is running on a ServiceNow instance by viewing the "System Definition" > "Upgrade History" > "System Upgrades" list and checking the "Target Version" field of the most recent record in the list. This information is stored in the sys_upgrade_history table.

---

**👤 saiakhilpalle** ⬆️ 1

B is correct

*...and 3 more comments*


</details>

---

## Question 322

### A knowledge article must be which of the following states to display to a user?

### Answer Choices:
**A.** Published

**B.** Drafted

**C.** Retired

**D.** Reviewed



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 sithupraji** ⬆️ 1

Correct Answer is A

---

**👤 csa4u** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 chsai** ⬆️ 1

A is correct

---

**👤 saiakhilpalle** ⬆️ 2

A is correct

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 323

### What is the name of the conversational bot platform that provides assistance to help users obtain information, make decisions, and perform common tasks?

### Answer Choices:
**A.** Answer Agent

**B.** live Feed

**C.** Virtual Agent

**D.** Connect Chat



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C virtual agent

---

**👤 csa4u** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 chsai** ⬆️ 1

option c

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 AWoman** ⬆️ 2

**Selected Answer:** C
virtual agent is correct

---

**👤 som_420** ⬆️ 2

correct



</details>

---

## Question 324

### What is the purpose of a Related List?

### Answer Choices:
**A.** To create a one-to-many relationship

**B.** To dot-walk to a core table

**C.** To present related fields

**D.** To present related records



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 Arun005** ⬆️ 1

**Selected Answer:** D
D is the correct answer

---

**👤 evoken** ⬆️ 1

D is correct.

---

**👤 saiakhilpalle** ⬆️ 1

D is correct

---

**👤 Ranaway** ⬆️ 1

D is the right answer

---

**👤 DANX1** ⬆️ 3

D.-
https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/use/using-forms/concept/c_RelatedLists.html

---

**👤 fauxmaux** ⬆️ 3

Related lists appear on forms and show records in tables that have relationships to the current record.

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 325

### Which one of the following statements describes the purpose of a Service Catalog flow?

### Answer Choices:
**A.** A Service Catalog flow generates three basic components: item variable types, tasks, and approvals

**B.** Although a Service Catalog flow cannot send notifications, the flow drives complex fulfillment processes

**C.** A Service Catalog flow is used to drive complex fulfillment processes and sends notifications to defined users or groups

**D.** A Service Catalog flow generates three basic components: item variable types, tasks, and notifications



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C IS CORRECT

---

**👤 naab143** ⬆️ 4

C. A Service Catalog flow is used to drive complex fulfillment processes and sends notifications to defined users or groups.

Service Catalog flows are designed to automate and orchestrate complex processes related to service fulfillment, such as provisioning resources or handling service requests. Additionally, they can incorporate notifications to inform stakeholders about the progress or completion of these processes.

---

**👤 csa4u** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 lils13** ⬆️ 1

**Selected Answer:** C
correct

---

**👤 AWoman** ⬆️ 1

**Selected Answer:** C
correct



</details>

---

## Question 326

### Which term best describes something that is created, has worked performed upon it, and is eventually moved to a state of closed?

### Answer Choices:
**A.** report

**B.** flow

**C.** event

**D.** task



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 JordiFT** ⬆️ 2

**Selected Answer:** D
D is correct

---

**👤 servicepow** ⬆️ 1

**Selected Answer:** D
D is Correct

---

**👤 Jana08** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 saiakhilpalle** ⬆️ 2

D is correct

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 327

### Which are valid Service Now User Authentication Methods? (Choose three.)

### Answer Choices:
**A.** XML feed

**B.** Local database

**C.** LDAP

**D.** SSO

**E.** FTP authentication



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BCD
**Community voting results:** BCD (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 2

**Selected Answer:** BCD
BCD are correct

---

**👤 servicepow** ⬆️ 2

**Selected Answer:** BCD
BCD are correct

---

**👤 Jana08** ⬆️ 2

**Selected Answer:** BCD
BCD are correct:
https://docs.servicenow.com/bundle/sandiego-platform-security/page/integrate/single-sign-on/concept/c_Authentication.html

---

**👤 saiakhilpalle** ⬆️ 1

BCD are correct

---

**👤 Ausias18** ⬆️ 1

BCD are correct

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 328

### Knowledge Base Search results can be sorted by which of the following? (Choose three.)

### Answer Choices:
**A.** Most recent update

**B.** Popularity

**C.** Relevancy

**D.** Manager assignment

**E.** Number of views



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ACE
**Community voting results:** ACE (80%), ABC (20%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 SNOW_SAGAR** ⬆️ 1

**Selected Answer:** ABC
The correct answers are:

✅ A. Most recent update
✅ B. Popularity
✅ C. Relevancy

---

**👤 shravan3** ⬆️ 1

ACE
popularity
relevency
number of views

---

**👤 Mahindra_Reddie** ⬆️ 1

**Selected Answer:** ACE
Is correct answer

---

**👤 evoken** ⬆️ 1

**Selected Answer:** ACE
A C E is correct

---

**👤 mrnow** ⬆️ 1

**Selected Answer:** ACE
A C E is correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** ACE
A C E is correct

---

**👤 Bonche** ⬆️ 1

A, C, E is the correct answer

---

**👤 S_040** ⬆️ 1

Correct ACE

---

**👤 trye_indicators** ⬆️ 3

Sort the items in a knowledge base or search results list using the Sort by options at the top of the list.
Views: sorts by number of article views.
Newest: sorts by the date created or updated.
Alphabetical: sorts alphabetically by the article Short description field.
Relevancy: sorts by relevance to the search term.

---

**👤 Markster** ⬆️ 1

correct. https://docs.servicenow.com/bundle/quebec-servicenow-platform/page/product/knowledge-management/concept/knowledge-service-portal-search.html

*...and 1 more comments*


</details>

---

## Question 329

### Access Control rules may be defined with which of the following permission requirements? (Choose three.)

### Answer Choices:
**A.** Roles

**B.** Conditional Expressions

**C.** Assignment Rules

**D.** Scripts

**E.** User Criteria

**F.** Groups



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABD
**Community voting results:** ABD (80%), ABE (20%)

</details>

<details>
<summary><strong>💬 View Discussion (13 comments)</strong></summary>

**👤 fauxmaux** ⬆️ 21

ABD
Access Control Rules allow access to the specified resource if all three of these checks evaluate to true:
The user has one of the roles specified in the Role list, or the list is empty.
Conditions in the Condition field evaluate to true, or conditions are empty.
The script in the Script field (advanced) evaluates to true, or sets the variable "answer" to true, or is empty.
The three checks are evaluated independently in the order displayed above.

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** ABD
ABD are correct

---

**👤 s_KASKA** ⬆️ 2

ABD is the right answer.

---

**👤 xenonthezequel** ⬆️ 4

The answer is ABD - it's in the ServiceNow Fundamentals course and here on the Access Control List Rules documentation page: https://docs.servicenow.com/bundle/utah-platform-security/page/administer/contextual-security/concept/access-control-rules.html

---

**👤 sithupraji** ⬆️ 2

Access Control Rules allow access to the specified resource if all three of these checks evaluate to true:
The user has one of the roles specified in the Role list, or the list is empty.
Conditions in the Condition field evaluate to true, or conditions are empty.
The script in the Script field (advanced) evaluates to true, or sets the variable "answer" to true, or is empty.

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** ABD
ABD is correct.

---

**👤 servicepow** ⬆️ 1

**Selected Answer:** ABD
ABD are correct

---

**👤 HNTrinh** ⬆️ 3

**Selected Answer:** ABD
In SN fundamental courses

---

**👤 Jana08** ⬆️ 1

**Selected Answer:** ABD
ABD are correct

---

**👤 FOJ** ⬆️ 1

User criteria is used in Knowledge Base and Service Catalog.

*...and 3 more comments*


</details>

---

## Question 330

### Which section of the ServiceNow UI allows you to perform a global search?

### Answer Choices:
**A.** Application Navigator

**B.** Banner frame

**C.** List pane

**D.** Content frame



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (80%), A (20%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Frost312321** ⬆️ 1

**Selected Answer:** B
Answer B
https://developer.servicenow.com/dev.do#!/learn/learning-plans/washingtondc/new_to_servicenow/app_store_learnv2_buildmyfirstapp_washingtondc_getting_around_in_servicenow

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 PeterK0** ⬆️ 4

is it unified navigation header or banner frame ? i am confused...

---

**👤 Qwertydry** ⬆️ 3

**Selected Answer:** B
Answer: B,
https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/administer/navigation-and-ui/concept/c_NavigationAndTheUserInterface.html

---

**👤 cindy2023** ⬆️ 1

B is correct

---

**👤 _Anni_** ⬆️ 1

B ~1:48 video(zones joined to top banner) https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/administer/navigation-and-ui/concept/c_NavigationAndTheUserInterface.html

---

**👤 saiakhilpalle** ⬆️ 2

B is correct

---

**👤 som_420** ⬆️ 2

correct



</details>

---

## Question 331

### How do you make a list filter available to everyone?

### Answer Choices:
**A.** Make active, assign a name, and save

**B.** Assign a group, set visibility, and save

**C.** Assign a name, set visibility, and save

**D.** Make active, set visibility, and save



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 2zackattack2** ⬆️ 9

C is Correct. 1. You need to give the filter a name "Save as". 2. Set the Visibility. 3. Save

---

**👤 som_420** ⬆️ 7

correct

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 chrisklesar1234** ⬆️ 2

technically, it is Assign a name, set visibility, and save

---

**👤 jkishere** ⬆️ 1

**Selected Answer:** C
C is right

---

**👤 evoken** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 MacGyver4th** ⬆️ 5

**Selected Answer:** C
https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/use/using-lists/task/t_SavingFilters.html#t_SavingFilters

---

**👤 Philippvs** ⬆️ 2

**Selected Answer:** C
After put the name, visibility and save it will be available for everyone.

---

**👤 Thattechgirl** ⬆️ 6

The correct answer is: D. Make active, set visibility, and save

To make a list filter available to everyone in ServiceNow, you need to make it active, set the visibility to "Public," and save the changes. Here are the steps to follow:

Navigate to the list that you want to create the filter for, and click the "Filter" button to open the filter editor.

In the filter editor, select the fields and conditions that you want to use to define the filter, and then click the "Save" button.

In the "Save As" dialog, enter a name for the filter, and then select the "Public" option in the "Visibility" field.

Click the "Save" button to save the filter.

Once you have saved the filter, it will be available to all users who have access to the list, regardless of their role or group membership. You can also use the "Shared" option in the "Visibility" field to make the filter available to specific groups or users, rather than to everyone.

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

*...and 2 more comments*


</details>

---

## Question 332

### What would NOT appear in the Application Navigator if `service` is typed into the filter field?

### Answer Choices:
**A.** Configuration > Business Services

**B.** Self-Service > Knowledge

**C.** Service Portal > Widgets

**D.** Incident > Assigned to me



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (80%), A (20%)

</details>

<details>
<summary><strong>💬 View Discussion (21 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 s_KASKA** ⬆️ 1

D is the right answer. When you search for 'service' all applications and modules that contain 'service' would appear. Incident>assigned to me will not appear because it does not contain service.

---

**👤 Prabhu81** ⬆️ 1

D right Answer

---

**👤 abhishekm6** ⬆️ 1

D is correct, quick way to see all other options have "Service" in it 
Incidents won't show up, Self-Service> My Incidents will show However "Incident" Modules and assigned to me wont be there.

---

**👤 xenonthezequel** ⬆️ 1

The answer is D. If you test in your own PDI and use CTRL/CMD + F to verify, you'll see that "Business Services" does show up when you look up "service" in the Filter Navigator but "Incidents > Assigned to me" does *not*

---

**👤 Arun005** ⬆️ 2

**Selected Answer:** D
D is the correct because 'Configuration > Business Services' DO appear when service is searched in the filter navigator.

---

**👤 Mihaf92** ⬆️ 2

A and D

---

**👤 Testeru1988** ⬆️ 1

D is correct, "Service" is not inside.

---

**👤 OnesR** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 SamIV444** ⬆️ 1

**Selected Answer:** D
D is correct

*...and 11 more comments*


</details>

---

## Question 333

### Which of the following is used to categorize, flag, and locate records?

### Answer Choices:
**A.** Search

**B.** Favorites

**C.** Tags

**D.** Bookmarks



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
c is correct

---

**👤 gprieto** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 DANX1** ⬆️ 4

C.-
https://docs.servicenow.com/bundle/tokyo-platform-user-interface/page/use/common-ui-elements/concept/c_Tags.html

---

**👤 AWoman** ⬆️ 1

**Selected Answer:** C
correct

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 334

### Which tool should be used to populate commonly used fields in a form?

### Answer Choices:
**A.** Template

**B.** Reference Qualifier

**C.** Formatter

**D.** Assignment Rule



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
a is correct

---

**👤 gprieto** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 saiakhilpalle** ⬆️ 2

A is correct

---

**👤 AWoman** ⬆️ 1

**Selected Answer:** A
agreed

---

**👤 S_040** ⬆️ 1

CORRECT

---

**👤 fauxmaux** ⬆️ 4

A
Templates simplify the process of submitting new records by populating fields automatically.

---

**👤 Mohammedhz** ⬆️ 2

Correct:
https://docs.servicenow.com/en-US/bundle/sandiego-platform-administration/page/administer/form-administration/concept/c_Templates.html



</details>

---

## Question 335

### How is a group defined in ServiceNow?

### Answer Choices:
**A.** A group is one record stored in the Group Type [sys_user_group_type] table

**B.** A group is one record stored in the Group [sys_user_group] table

**C.** A group defines a set of users that share the same location

**D.** A group defines a set of users that share the same job title



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** B
B IS CORRECT

---

**👤 CB528** ⬆️ 3

A group is a set of users who share a common purpose

---

**👤 saiakhilpalle** ⬆️ 2

B is correct

---

**👤 santosgj5** ⬆️ 1

**Selected Answer:** B
Correct

---

**👤 S_040** ⬆️ 2

CORRECT

---

**👤 som_420** ⬆️ 2

correct



</details>

---

## Question 336

### What is a role in ServiceNow?

### Answer Choices:
**A.** A role is one record in the Role [user_sys_role] table

**B.** A role is a set of modules for a particular application

**C.** A role is one record in the Role [sys_user_role] table

**D.** A role is a persona used in Live Feed Chat



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (80%), A (20%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 BlackTECH** ⬆️ 1

**Selected Answer:** C
sys_user_role.list is C

---

**👤 famous_pornstar** ⬆️ 1

**Selected Answer:** A
A and C are written the same

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 gprieto** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 chrisklesar1234** ⬆️ 1

try sys_user_role.list

---

**👤 CB528** ⬆️ 1

wrong again-defines what they have access to in SN

---

**👤 Jana08** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 saiakhilpalle** ⬆️ 2

C is correct

---

**👤 S_040** ⬆️ 2

CORRECT

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 337

### What is a Notification?

### Answer Choices:
**A.** A new Knowledge article created by a Business Rule

**B.** A tool for alerting users that events that concern them have occurred

**C.** A message through Connect related to a Change Request

**D.** An email file attachment



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** B
B IS CORRECT

---

**👤 Schyraz** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 saiakhilpalle** ⬆️ 2

B is correct

---

**👤 S_040** ⬆️ 2

CORRECT

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 338

### Which one of the following is NOT a type of Visual Task Board?

### Answer Choices:
**A.** Flexible

**B.** Freeform

**C.** Feature

**D.** Guided boards



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 kcheemal007** ⬆️ 1

**Selected Answer:** C
In XANADU release we know only Freeform Board and Data Driven Board

---

**👤 jnk252** ⬆️ 4

**Selected Answer:** C
In Washington DC release we know only Freeform Board and Data Driven Board

---

**👤 gprieto** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 pielouis** ⬆️ 2

**Selected Answer:** C
Types of Visual Task Boards include Flexible, Freeform, Guided. So that, featured is not a type of VTB

---

**👤 timothy_moore** ⬆️ 4

The VTB types in Tokyo are Freeform Board and Data Driven Board

---

**👤 saiakhilpalle** ⬆️ 1

C is correct, as featured is not a type of VTB

---

**👤 4learning2u** ⬆️ 3

**Selected Answer:** C
Yes, the answer is C. Check this video for more info: https://youtu.be/FzvUt_mClYc

---

**👤 difesaso14** ⬆️ 1

Correct



</details>

---

## Question 339

### What is the path an Administrator could take to view the fulfillment stage task list for an order placed by a user?

### Answer Choices:
**A.** RITM (Number)>REQ (Number)>PROCUREMENT (Number)

**B.** REQ (Number)>RITM (Number)>PROCUREMENT (Number)

**C.** REQ (Number)>RITM (Number)>TASK (Number)

**D.** FULFILLMENT (Number)>RITM (Number)>TASK (Number)



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (14 comments)</strong></summary>

**👤 BidduSnow** ⬆️ 5

REQ contains RITMs and RITM contains TASKs

---

**👤 alsman** ⬆️ 3

'Task' is the parent table of all task-type records in the system. It is the parent of REQ, RITM, and SCTASK.

When you order a catalog item, the system generates a single RITM record for each item in the cart at the time you order. These RITM records are all grouped underneath a parent REQ record.

Depending on your workflow, you might also have SCTASK records that get generated as children of a RITM record

---

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** C
C is it

---

**👤 JUN17** ⬆️ 1

**Selected Answer:** C
C is done

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Mahindra_Reddie** ⬆️ 1

**Selected Answer:** C
Is correct answer

---

**👤 mrnow** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Bonche** ⬆️ 1

C is the correct answer

*...and 4 more comments*


</details>

---

## Question 340

### What is (are) best practice(s) regarding users/groups/roles? (Choose two.)

### Answer Choices:
**A.** You should never assign roles to groups.

**B.** You should assign roles to users.

**C.** You should add users to groups.

**D.** You should assign roles to groups.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** CD
**Community voting results:** CD (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** CD
CD are correct

---

**👤 gprieto** ⬆️ 1

**Selected Answer:** CD
C & D is correct

---

**👤 Hannahwu** ⬆️ 1

why B is not correct

---

**👤 HimanPatel** ⬆️ 1

CD is correct.

---

**👤 saiakhilpalle** ⬆️ 2

CD are correct

---

**👤 chongchen** ⬆️ 1

correct

---

**👤 sephereth** ⬆️ 1

**Selected Answer:** CD
correct interms of best practice

---

**👤 som_420** ⬆️ 2

correct



</details>

---

## Question 341

### What are two ways to generate an Event? (Choose two.)

### Answer Choices:
**A.** Business Rule

**B.** Workflow

**C.** Log entry

**D.** Knowledge article publication



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** AB
**Community voting results:** AB (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 lhughes** ⬆️ 5

This question has questionable wording, a event will be generated by publishing a KB doc but you can create an event using either business rules and workflows yes.

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** AB
AB is correct

---

**👤 Qwertydry** ⬆️ 1

**Selected Answer:** AB
AB Correct.

---

**👤 saiakhilpalle** ⬆️ 1

AB are correct

---

**👤 som_420** ⬆️ 3

correct



</details>

---

## Question 342

### Which core table in the ServiceNow platform provides a series of standard fields used on each of the tables that extend it, such as the Incident [incident] and
Problem [problem] tables?

### Answer Choices:
**A.** Task [task]

**B.** Assignment [assignment]

**C.** Service [service]

**D.** Workflow [workflow]



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (7 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 s_KASKA** ⬆️ 1

The correct answer is A. 
The Incident [Incident] and Problem [Problem] tables extend the base Task [Task] table.

---

**👤 Qwertydry** ⬆️ 1

**Selected Answer:** A
A is correct.

---

**👤 uwbe** ⬆️ 1

A is correct but ambiguous for non-native speakers. As Incident and Problem are core tables themself and task is a base table.

---

**👤 vrwjjr85** ⬆️ 1

A is correct. Keyword here is core

---

**👤 saiakhilpalle** ⬆️ 2

A is correct

---

**👤 som_420** ⬆️ 4

correct



</details>

---

## Question 343

### Which of the following statements describes how data is organized in a table?

### Answer Choices:
**A.** A column is a field in the database and a record is one user

**B.** A column is one field and a record is one row

**C.** A column is one field and a record is one column

**D.** A column contains data from one user and a record is one set of fields



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (70 comments)</strong></summary>

**👤 Jo98778** ⬆️ 35

**Selected Answer:** B
What is up with all the websites giving the wrong answer to these questions.
"
A table is a collection of records in the database. Each record corresponds to a row in a table, and each field on a record corresponds to a column on that table."

https://docs.servicenow.com/bundle/rome-platform-administration/page/administer/table-administration/concept/c_TableAdministration.html

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Mayur444** ⬆️ 1

its is B

---

**👤 yanchok** ⬆️ 1

**Selected Answer:** B
It's b

---

**👤 chrisklesar1234** ⬆️ 1

Row - one record,
Column - value of the record.

so, B

---

**👤 DarksideSunrised** ⬆️ 1

**Selected Answer:** B
Row - one record,
Column - value of the record.

---

**👤 TipoZico** ⬆️ 1

B is the correct answer.

---

**👤 c9f6304** ⬆️ 1

**Selected Answer:** B
Definitely notated incorrectly. A would be true if the user table was specified.

---

**👤 Peter526** ⬆️ 1

**Selected Answer:** B
100% true

---

**👤 Bettylety** ⬆️ 1

B is the correct answer.

*...and 60 more comments*


</details>

---

## Question 344

### What is a sys_id?

### Answer Choices:
**A.** Unique 32-character identifier that is assigned to every record

**B.** A client-side Business Rule

**C.** A server-side Business Rule

**D.** Unique 64-character identifier that is assigned to every record



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (88%), 13%

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 Felix010** ⬆️ 1

**Selected Answer:** A
A unique 32-character GUID (Globally Unique ID), called a Sys ID (sys_id) identifies each record in an instance.
https://www.servicenow.com/docs/bundle/yokohama-platform-administration/page/administer/table-administration/concept/c_UniqueRecordIdentifier.html

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 nikky2** ⬆️ 1

**Selected Answer:** D
I guess D

---

**👤 Maia133** ⬆️ 1

A is the good answer.

---

**👤 PeterK0** ⬆️ 3

**Selected Answer:** A
Every time i work with sys_id, it has length like this : jfodpskemchnfutincgeplacjrksuenc, not like this : jfodpskemchnfutincgeplacjrksuencjfodpskemchnfutincgeplacjrksuenc, so its A) :D

---

**👤 julianamendo** ⬆️ 1

**Selected Answer:** A
A is the correct answer

---

**👤 Kelseydairo** ⬆️ 1

**Selected Answer:** A
A is the correct answer

---

**👤 andycsa** ⬆️ 1

A is correct

---

**👤 saiakhilpalle** ⬆️ 1

A is correct

---

**👤 trye_indicators** ⬆️ 2

A unique 32-character GUID (Globally Unique ID), called a Sys ID (sys_id) identifies each record in an instance. https://docs.servicenow.com/bundle/sandiego-platform-administration/page/administer/table-administration/concept/c_UniqueRecordIdentifier.html

*...and 1 more comments*


</details>

---

## Question 345

### When creating a global custom table named `abc`, what is the table name that is automatically assigned by the platform?

### Answer Choices:
**A.** snc_abc

**B.** abc

**C.** u_abc

**D.** sys_abc



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Jo98778** ⬆️ 5

**Selected Answer:** C
I just want to see the most voted answer so I a leaving a comment to vote and see the most voted answer.

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 julianamendo** ⬆️ 2

**Selected Answer:** C
C is the correct answer

---

**👤 HimanPatel** ⬆️ 1

C is correct

---

**👤 snow_dev_pankaj** ⬆️ 1

c is correct

---

**👤 saiakhilpalle** ⬆️ 3

C is correct, as for custom tables prefix will be u_

---

**👤 sephereth** ⬆️ 3

**Selected Answer:** C
c is correct

---

**👤 som_420** ⬆️ 2

correct



</details>

---

## Question 346

### Access Control rules may provide access security for which of the following database objects?

### Answer Choices:
**A.** For a specific role, group, or user

**B.** For a specific row, column, or table

**C.** For specific groups

**D.** For a specific CMDB Configuration item



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (74%), D (23%), 4%

</details>

<details>
<summary><strong>💬 View Discussion (36 comments)</strong></summary>

**👤 HimanPatel** ⬆️ 16

B. For a specific row, column, or table

While all the options provided are valid for access control rules in ServiceNow, the most commonly associated database objects with access security in ServiceNow are specific rows, columns, or tables. Access control rules can be applied to these elements to define who can view, modify, or interact with the data stored within them.

---

**👤 MacGyver4th** ⬆️ 6

**Selected Answer:** B
https://docs.servicenow.com/bundle/rome-platform-administration/page/administer/contextual-security/concept/access-control-rules.html

---

**👤 Niranjanbasnet** ⬆️ 1

**Selected Answer:** B
Since getting the row restricted can be achieved inside Database table

---

**👤 jwsops** ⬆️ 1

i believe it is D

---

**👤 Mayur444** ⬆️ 1

B is the correct one

---

**👤 shub606** ⬆️ 1

B is Correct Answer

---

**👤 naab143** ⬆️ 1

The correct answer is:

A. For a specific role, group, or user

Access control rules in databases are typically used to manage and restrict access to specific database objects, such as tables, views, or stored procedures, based on the roles, groups, or individual users accessing the database. These rules ensure that only authorized users or groups can perform certain operations on the database objects.

---

**👤 Charity35** ⬆️ 1

I vote that this should be B

---

**👤 David_Wu** ⬆️ 1

**Selected Answer:** B
It's B

---

**👤 Adamkrt** ⬆️ 1

**Selected Answer:** B
clear B

*...and 26 more comments*


</details>

---

## Question 347

### What is the primary application used to load data into ServiceNow?

### Answer Choices:
**A.** Service Level Management

**B.** Configuration

**C.** System Import Sets

**D.** System Update Sets



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Allviy** ⬆️ 2

When we are loading, from the word itself guess it. It is System IMPORT Sets

---

**👤 487531a** ⬆️ 1

c is correct

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 4learning2u** ⬆️ 4

**Selected Answer:** C
System Import Sets is correct

https://developer.servicenow.com/dev.do#!/learn/learning-plans/quebec/new_to_servicenow/app_store_learnv2_importingdata_quebec_load_data

---

**👤 som_420** ⬆️ 2

corect



</details>

---

## Question 348

### Which of the following steps can be used to import new data into ServiceNow from a spreadsheet?

### Answer Choices:
**A.** Select Data Source, Schedule Transform

**B.** Load Data, Create Transform Map, Run Transform

**C.** Define Data Source, Select Transform Map, Run Transform

**D.** Select Import Set, Select Transform Map, Run Transform



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (75%), D (25%)

</details>

<details>
<summary><strong>💬 View Discussion (41 comments)</strong></summary>

**👤 Charity35** ⬆️ 10

This is supposed to help us to study, if the answer is B but the exam is saying right answer is D then it certainly isn't helping, right?

---

**👤 PeterK0** ⬆️ 7

**Selected Answer:** D
I'm surprised how many people think that they have to create new transform map every time when they import new data into servicenow... It is D)

---

**👤 sorenft** ⬆️ 1

**Selected Answer:** B
Answer is B 
Here is the process:
1. Load Data
– Upload the spreadsheet (usually CSV or Excel).
– This creates an Import Set Table.
2. Create Transform Map
– Define how fields in the import set map to fields in the target table.
3. Run Transform
– Apply the transform map to move data from the import set to the target table.

---

**👤 Felix010** ⬆️ 1

**Selected Answer:** B
"from a spreadsheet"
So B is correct

---

**👤 dfec26e** ⬆️ 1

**Selected Answer:** B
Whats the correct answer? This question has been coming up a few times. Please someone explain with evidence

---

**👤 RGR89** ⬆️ 2

**Selected Answer:** B
The correct answer is "B", because the "NEW DATA" in the question.

---

**👤 KamiV69** ⬆️ 1

**Selected Answer:** D
For me, the Correct answer is D because the import set step is missing in B. Do you have any other opinions from your recent experience?

---

**👤 Denmups** ⬆️ 1

**Selected Answer:** D
D is correct; datas are not directly processed; it goes through Import Set

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 showboat_zw** ⬆️ 1

it is no need to create a new transform map each time

*...and 31 more comments*


</details>

---

## Question 349

### Which tool is used for creating dependencies between configuration items in the CMDB?

### Answer Choices:
**A.** CI Relationship Editor

**B.** CMDB Builder

**C.** CI Service Manager

**D.** Cl Class Manager



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (67%), D (33%)

</details>

<details>
<summary><strong>💬 View Discussion (54 comments)</strong></summary>

**👤 MACCA81** ⬆️ 13

A is the correct answer. The CI Relationship Editor is used to create Configuration Item relationship and define which is the base CI class, which is the dependant one and what is the dependency. 
The CI Class Manager is a central location for the CI class hierarchy in the CMDB 0 it displays the entire CI Class hierarchy in a tree-view format and consolidates CI class definitions in a central location

---

**👤 Felix010** ⬆️ 1

**Selected Answer:** A
https://www.servicenow.com/community/itsm-forum/configuration-items-79/m-p/2558844

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
As per point 4.4 in the official CSA book, the relationship editor is used for defining relationships between CIs

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** A
A is correct

---

**👤 skibumvermont** ⬆️ 2

(ChatGPT) The CI Relationship Editor is the tool you would use to create dependencies between Configuration Items (CIs) in the CMDB.

This tool allows you to:
 • Define and establish relationships, including dependency relationships, between individual CIs.
 • Create, modify, and visualize how CIs depend on each other (e.g., a server depending on a database, or an application depending on a server).

By using the CI Relationship Editor, you can ensure that the relationships between different CIs reflect the actual dependencies within your IT infrastructure.

---

**👤 40a783b** ⬆️ 2

**Selected Answer:** A
A is a correct one! 
eBook CSA Fundamentials chapter 4.4 - CI Class Manager Suggested Relationships

quote: User the CI Relationship Editor to create configuration item relationships

---

**👤 chrisklesar1234** ⬆️ 1

CI Relationship Editor - view, create, modify, or delete CI relationships.

---

**👤 AVK301096** ⬆️ 1

**Selected Answer:** A
Use the relationship editor to view, create, modify, or delete CI relationships.

---

**👤 Slayer1985** ⬆️ 1

its D, CI class manager is used

---

**👤 Adamkrt** ⬆️ 1

**Selected Answer:** A
A is correct, CI class manager is not for relations but for CI types

*...and 44 more comments*


</details>

---

## Question 350

### Which term refers to application menus and modules which you may want to access quickly and often?

### Answer Choices:
**A.** Breadcrumb

**B.** Favorite

**C.** Tag

**D.** Bookmark



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (12 comments)</strong></summary>

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** B
B for sure

---

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Mahindra_Reddie** ⬆️ 1

**Selected Answer:** B
Is correct answer

---

**👤 Augusto0007** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 mrnow** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 saiakhilpalle** ⬆️ 1

B is correct

---

**👤 KIKI_AI** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Bonche** ⬆️ 1

B is the correct answer

---

**👤 Ausias18** ⬆️ 1

B is correct

---

**👤 S_040** ⬆️ 1

Correct

*...and 2 more comments*


</details>

---

## Question 351

### What is the difference between a UI Policy and Data Policy?

### Answer Choices:
**A.** Data Policies run when data is entered through the form, by an Import Set, or by web services, while UI Policies are set only by web services

**B.** Data Policies can be converted into UI Policies, but UI Policies cannot be converted into Data Policies

**C.** Data Policies run regardless of how data is entered into ServiceNow, while UI Policies are used for form interactions

**D.** Data Policies run only after UI Policies run successfully



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 jnk252** ⬆️ 2

**Selected Answer:** C
C is correct

---

**👤 JordiFT** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Rur787** ⬆️ 1

C Is Corrrect.

---

**👤 dhruv2405** ⬆️ 1

**Selected Answer:** C
Both B and C are correct.

---

**👤 Mikias** ⬆️ 3

**Selected Answer:** C
Both B and C are correct.

---

**👤 HimanPatel** ⬆️ 1

C
Data Policies run regardless of how data is entered into ServiceNow to enforce data consistency and integrity, while UI Policies are used for form interactions to control the visibility and behavior of form elements based on conditions.

---

**👤 saiakhilpalle** ⬆️ 2

C is correct

---

**👤 katbrosa** ⬆️ 3

c - Data policies enable you to enforce data consistency by setting mandatory and read-only states for fields.

Data policies are similar to UI policies, but UI policies only apply to data entered on a form through the standard browser. Data policies can apply rules to all data entered into the system, including data brought in through import sets or web services and data entered through the mobile UI.



</details>

---

## Question 352

### Which one of the following is an accurate list of changes that are captured in an Update Set?

### Answer Choices:
**A.** Changes made to: tables, forms, schedules, and client scripts

**B.** Changes made to: tables, forms, Business Rules, and data records

**C.** Changes made to: tables, forms, groups, and configuration items (CIs)

**D.** Changes made to: table, forms, views, and fields



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (5 comments)</strong></summary>

**👤 Jo98778** ⬆️ 31

**Selected Answer:** D
Configurations are automatically recorded in update sets. Things like client scripts, business rules, UI Actions, etc. Things that are considered data, like users, groups, group associates (records on sys_user_grmember) don't get saved automatically. You can manually add these records to your update set by clicking the UI action in the bottom left corner." https://community.servicenow.com/community?id=community_question&sys_id=b18c761cdb83b344414eeeb5ca96192f

https://community.servicenow.com/community?id=community_question&sys_id=3e5e8feddb9cdbc01dcaf3231f9619e0

A is wrong because Schedule jobs are not included
B. is wrong because transactional data records are not included
C. is wrong because configuration items are not included

D therefore is the most accurate

---

**👤 nikky2** ⬆️ 1

**Selected Answer:** D
D is the correct answer

---

**👤 jnk252** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 cindy2023** ⬆️ 1

I think is B, changes to table, form,business rule and data records .

---

**👤 saiakhilpalle** ⬆️ 2

D is correct



</details>

---

## Question 353

### What are the steps to retrieve an Update Set?

### Answer Choices:
**A.** Verify Update Set is Complete, Retrieve, Preview, Apply

**B.** Verify Update Set is Complete, Test Connection, Apply

**C.** Verify Update Set is Complete, Test Connection, Commit

**D.** Verify Update Set is Complete, Retrieve, Preview, Commit



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (81%), C (19%)

</details>

<details>
<summary><strong>💬 View Discussion (44 comments)</strong></summary>

**👤 Stefcio** ⬆️ 17

Correct answer is D.

---

**👤 nikky2** ⬆️ 1

**Selected Answer:** D
its correct answer

---

**👤 jnk252** ⬆️ 1

**Selected Answer:** D
D is correct

---

**👤 prabhamegha08** ⬆️ 2

Answer is D

---

**👤 oswaldorusb** ⬆️ 2

**Selected Answer:** D
D is the correct one

---

**👤 David_Wu** ⬆️ 2

**Selected Answer:** D
Clearly it's D

---

**👤 Holy_97** ⬆️ 2

**Selected Answer:** D
D is right

---

**👤 cluelessdood** ⬆️ 3

From the training:
To retrieve an update set from a remote instance, in the Production instance, navigate to All > System Update Sets > Update Sources and select the sub-production/development instance with the completed update set you want to commit. 

With remote instances, the update sets are already previewed during the retrieval. The platform does it for you! Click the Retrieve Completed Update Sets button or related link. Any update sets marked as Complete are transferred from the source (sub-production) instance to the target (production) instance. 

Select the Retrieved Update Sets Related list to access the Retrieved update set; you will see it was already previewed. 

Select the Update Set you want to commit and click the Commit Update Set button.

---

**👤 Dekoreh** ⬆️ 3

**Selected Answer:** D
D is correct.
Check SN admin fundamental book pg 465. It is written in bold

---

**👤 LovMi21** ⬆️ 2

**Selected Answer:** C
https://docs.servicenow.com/en-US/bundle/rome-application-development/page/build/system-update-sets/task/t_RetrieveAnUpdateSet.html
Test a connection is a step here.

*...and 34 more comments*


</details>

---

## Question 354

### IntegrationHub enables execution of third-party APIs as a part of a flow. These integrations are referred to as

### Answer Choices:
**A.** an action

**B.** a spoke

**C.** a connection

**D.** an integration step



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (6 comments)</strong></summary>

**👤 Mohammedhz** ⬆️ 6

Correct:

https://docs.servicenow.com/bundle/sandiego-servicenow-platform/page/administer/integrationhub/concept/integrationhub.html

'Integration Hub enables execution of third-party APIs as a part of a flow when a specific event occurs in ServiceNow. These integrations, referred to as spokes'

---

**👤 jnk252** ⬆️ 2

**Selected Answer:** B
B is correct

---

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** B
Spoke is it

---

**👤 kohliaashish** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 melmel** ⬆️ 2

The correct answer is D. an integration step.

Integration steps are the building blocks of IntegrationHub flows. They allow you to execute third-party APIs, call other flows, or perform other actions as part of a flow.

Here are some examples of integration steps:

API Call: Executes a third-party API.
Flow Call: Calls another flow.
Script: Runs a custom script.
Integration steps are referred to as "spokes" in the IntegrationHub documentation. However, the more widely used term is "integration step."

The other options are incorrect:

Option A: Actions are tasks that can be performed in a flow, such as sending an email or creating a record. They are not used to execute third-party APIs.
Option B: Spokes are a type of integration step that allows you to call other flows. However, they are not the only type of integration step.
Option C: Connections are used to connect to third-party APIs. However, they are not used to execute the APIs.

---

**👤 saiakhilpalle** ⬆️ 1

B is correct



</details>

---

## Question 355

### Which of the following protects applications by identifying and restricting access to available files and data?

### Answer Choices:
**A.** Application Configuration

**B.** Verbose Log

**C.** Access Control Rules

**D.** Application Scope



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 jnk252** ⬆️ 3

**Selected Answer:** D
D is correct

---

**👤 TimmyLam** ⬆️ 2

**Selected Answer:** D
Answer is D

---

**👤 TimmyLam** ⬆️ 2

I am cool

---

**👤 sithupraji** ⬆️ 2

Application scoping protects applications by identifying and restricting access to application files and data. D is correct.

---

**👤 saiakhilpalle** ⬆️ 1

D is correct

---

**👤 fauxmaux** ⬆️ 3

corect: D

"Application scoping protects applications by identifying and restricting access to application files and data."

https://docs.servicenow.com/en-US/bundle/sandiego-application-development/page/build/applications/concept/c_ApplicationScope.html

---

**👤 Mattikus51** ⬆️ 1

D Is the correct answer.

---

**👤 Rameshsu** ⬆️ 1

I think C is the correct answer.



</details>

---

## Question 356

### Which one statement correctly describes Access Control rule evaluation?

### Answer Choices:
**A.** Table access rules are evaluated from the general to the specific

**B.** If more than one rule applies to a record, the older rule is evaluated first

**C.** If a row level rule and a field level rule exist, both rules must be true before an operation is allowed

**D.** The role with the most permissions evaluates the rules first.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (80%), 13%, 7%

</details>

<details>
<summary><strong>💬 View Discussion (18 comments)</strong></summary>

**👤 Jorr** ⬆️ 7

**Selected Answer:** C
A is wrong. the rules are evaluated: specific to general so it can't be A.

C is correct

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** C
As per the CSA book if a row level rule and field level rule exist, both rules must be true before an operation is allowed.

---

**👤 TimmyLam** ⬆️ 1

**Selected Answer:** C
I believe the correct answer is C.

---

**👤 ba14741** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Mihaf92** ⬆️ 1

A is outdated now it from specific to general 
C is the right anwser

---

**👤 cluelessdood** ⬆️ 1

I think SN switched from "general-to-specific" to "specific-to-general" somewhere between San Diego and Tokyo.

In San Diego:
This processing order ensures that users gain access to more general objects before gaining access to more specific objects. A user must pass both table and field ACL rules to access a record object.

In Tokyo:
This processing order ensures that users gain access to more specific objects before gaining access to more general objects. A user must pass both table and field ACL rules to access a record object.

In my "Additional ServiceNow Fundamentals Content for System Administrators and Developers course" (San Diego edition), it says:
When a session requests data, the system looks for matching access control rules.
Record ACL rules are processed in the following order:
1. Match the object against table ACL rules - most specific to most general.
2. Match the object against field ACL rules - most specific to most general.

---

**👤 Dekoreh** ⬆️ 1

**Selected Answer:** C
C is the correct answer

---

**👤 Shimdog** ⬆️ 1

The correct statement is A. Table access rules are evaluated from the general to the specific.

In ServiceNow, the evaluation of Access Control rules follows a general-to-specific order. This means that the more general rules are evaluated first, followed by more specific rules. The system checks the access control rules for a table, starting with the global (most general) rules, and then proceeds to evaluate table-specific and field-specific rules.

---

**👤 Max191** ⬆️ 1

**Selected Answer:** D
Answer is C
page 371 of the book ServiceNow Administration Fundamentals - Tokyo

---

**👤 SunnyNik** ⬆️ 1

Answer is A

In ServiceNow, if both a row-level rule and a field-level rule exist, both rules do not need to be true for an operation to be allowed. The rules are evaluated independently, and if either the row-level rule or the field-level rule allows the operation, it will be permitted.

*...and 8 more comments*


</details>

---

## Question 357

### ServiceNow contains a resource which provides the following:
✑ A standard and shared set of service related definitions across ServiceNow products and platform that will enable and support true service level reporting.
✑ A CMDB framework across our products and platform that will enable and support multiple configuration strategies.
What resource do these statements describe?

### Answer Choices:
**A.** Common Services Data Model (CSDM)

**B.** Information Technology Service Management (ITSM)

**C.** Configuration Management Database (CMDB)

**D.** Information Technology Infrastructure Library (ITIL)



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 GilSAtx** ⬆️ 2

**Selected Answer:** A
Csdm is correct

---

**👤 Milenablvr** ⬆️ 2

A, the correct answer is CSDM.



</details>

---

## Question 358

### An IT manager is responsible for the Network and Hardware assignment groups, each group contains 5 team members. These team members are working on many tasks, but the manager cannot see any tasks on the Service Desk > My Groups Work list. What could explain this?
//IMG//

### Answer Choices:
**A.** The Service Desk > My Groups Work list shows active work tasks that are not yet assigned.

**B.** The manager does not have the itil role.

**C.** The manager is not a member of the Service Desk group.

**D.** The manager is not a member of the Network and Hardware groups.

**E.** The Assignment Group manager field is empty.



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (51%), D (46%), 3%

</details>

<details>
<summary><strong>💬 View Discussion (42 comments)</strong></summary>

**👤 MattMeyer2022** ⬆️ 20

B is wrong. The manager clearly has ITIL role if he can see "Service Desk > My Groups Work list."
The correct answer is D> Once he is a member of the groups, he can see the work assigned to the group that is unassigned

---

**👤 JDHarp2112** ⬆️ 1

**Selected Answer:** A
My Groups Work when selected in PDI is showing a filter where:
* matches the assignment group
* active = true
* assigned to is empty
* state != pending

Since the question mentions that the team is working on many items, this implies those items are assigned. I think the answer is A since the assigned to for these items is not empty

---

**👤 roopsrich** ⬆️ 1

**Selected Answer:** A
I have checked it in PDA correct anser is A

---

**👤 KV_CTS** ⬆️ 1

**Selected Answer:** D
The "My Groups Work" list in ServiceNow only displays tasks that:
1. Belong to the user's assignment groups (Network and Hardware in this case).
2. Are not yet assigned to a specific user (i.e., Assigned to field is empty).
If tasks are already assigned to specific individuals within the groups, the manager will not see them in My Groups Work.

---

**👤 Nexfo** ⬆️ 2

**Selected Answer:** A
Looking at the conditions correct answer cannot be \My Groups Work:

Assignment group = groups you're a member of
Active = true
Assigned to = EMPTY
State is not Pending

The breadcrums show that these groups were added to the filter, therefore he MUST be member of these groups.

I select A

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
I choose A. 

This is the correct statement, if there are no unassigned tasks, you cannot see anything via My Groups Work. 

B - false – the manager has ITIL role if he can run the above filter from the view and he can access the SD module from the Navigator

C – false – The Service Desk group vs the Service Desk module are 2 different things and the SD group is not mentioned in the question

D – false - If he has the ITIL role, he doesn’t have to be a member of the groups, assuming there are no rules in place to restrict the view only to those members. There is no mention of such a rule so we cannot assume it exists. 

E – potentially true – this is a very particular case, he is not a member of those groups, he is not a manager of those groups so there is nothing there. But I think the most correct and complete answer is A. 

Very confusing phrasing for this question, all in all.

---

**👤 Kflo** ⬆️ 1

**Selected Answer:** A
A - I think the hint is that members are working on many tasks. Therefore, most likely no more unassigned tasks.

My Groups Work:
Assignment group = groups you're a member of
Active = true
Assigned to = EMPTY
State is not Pending

---

**👤 SnowNick** ⬆️ 1

**Selected Answer:** A
It's A. My groups work only shows unassigned tickets Out of the box. 
If the groups are shown in the list the manager is clearly that the manager is member of the groups.

---

**👤 aper** ⬆️ 1

**Selected Answer:** D
Answer is D. 
In my opinion, ff the question indicated that the manager is aware of unassigned tasks but is unable to view them, then option A would be the correct choice.

---

**👤 96d187b** ⬆️ 1

I would say A — the reason he's not seeing the tasks that they're working on is because the My Groups Work only shows tasks that are unassigned. If the team members are actively working on the tasks, then the manager won't see anything in this section. I'm assuming the first sentence makes it clear he belongs to the appropriate groups, but it's definitely a tricky one where multiple responses could work.

*...and 32 more comments*


</details>

---

## Question 359

### What do you need to do before you can use an Application-based trigger in your flow?

### Answer Choices:
**A.** Activate application trigger spoke

**B.** Activate trigger security rules

**C.** Activate application spoke, and plug-ins as needed

**D.** Assign Application trigger role [sn_app_trigger_write] to SME

**E.** Activate application plugins only



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (10 comments)</strong></summary>

**👤 Erp88** ⬆️ 11

**Selected Answer:** C
Extract from Admin course in Service Now: 
Application-based triggers are added when the associated application spoke* is activated. For example, these types of triggers allow you trigger a flow when an item is requested from the Service Catalog. In some instances, a plug-in might need to be activated as well (e.g.
Flow Designer Support for Service Catalog).
*A spoke contains Flow Designer triggers and actions dedicated to a particular application. For example, the ITSM Spoke contains actions for managing Task records such as the Create Task action.
Spokes are activated when their parent application is activated.

---

**👤 Markster** ⬆️ 6

**Selected Answer:** C
https://developer.servicenow.com/dev.do#!/learn/courses/quebec/app_store_learnv2_flowdesigner_quebec_flow_designer/app_store_learnv2_flowdesigner_quebec_using_flow_designer/app_store_learnv2_flowdesigner_quebec_triggering_flows

---

**👤 Cplatt** ⬆️ 3

The answer is C. From the Fundamentals course below...
"Application-based triggers are added when the associated application spoke is activated. in some instance, a plug-in might need to be activated as well (All>System Definition> Plugins).

---

**👤 ba14741** ⬆️ 3

**Selected Answer:** C
Correct answer should be C

---

**👤 Adamkrt** ⬆️ 1

**Selected Answer:** C
C by documentation to Vancouver

---

**👤 doode** ⬆️ 1

yep it's C

---

**👤 Ghouse1** ⬆️ 2

C is the answer.

---

**👤 ALDADI** ⬆️ 3

**Selected Answer:** C
Correct answer

---

**👤 fauxmaux** ⬆️ 5

in Flow Designer OOB we have ready to use:
Application Triggers:
SLA Task
Inbound Email
Service Catalog
No need to set anything.
But I don't understand the answer. This is not a good question.

---

**👤 Miiila** ⬆️ 5

Is there a source for this? Never heard of anything like that



</details>

---

## Question 360

### The ServiceNow platform includes which types of interfaces? (Choose three.)

### Answer Choices:
**A.** Now Mobile Apps

**B.** Agent Control Center

**C.** Back Office Dashboard

**D.** Service Portals

**E.** Now Platformֲ® User Interfaces

**F.** Field Service Taskboard



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ADE
**Community voting results:** ADE (100%)

</details>

<details>
<summary><strong>💬 View Discussion (18 comments)</strong></summary>

**👤 Jo98778** ⬆️ 36

**Selected Answer:** ADE
A. https://docs.servicenow.com/bundle/rome-mobile/page/administer/tablet-mobile-ui/concept/mobile-config-navigation.html

D and E. https://www.servicenow.com/products/service-portal.html


It is almost like ServiceNow is deliberately flooding these websites with wrong correct-answers so people fail the test.

---

**👤 TimmyLam** ⬆️ 1

**Selected Answer:** ADE
It's always been ADE

---

**👤 saurav_21** ⬆️ 2

**Selected Answer:** ADE
why are they providing wrong answers ??
just to confuse us. :-\

---

**👤 Mariina_003_B** ⬆️ 4

**Selected Answer:** ADE
Explanation:

Now Mobile Apps (A): ServiceNow provides mobile applications that allow users to access the platform from mobile devices, providing a mobile-friendly interface.

Service Portals (D): Service Portals are user interfaces that allow users to access specific services and information. They can be customized to meet specific business needs and provide a user-friendly experience.

Now Platform® User Interfaces (E): This refers to the various user interfaces available on the Now Platform, including forms, lists, and other interfaces that users interact with while working with ServiceNow.

Options B, C, and F are not typically referred to as types of interfaces on the ServiceNow platform.

---

**👤 Mariina_003_B** ⬆️ 1

**Selected Answer:** ADE
A. https://docs.servicenow.com/bundle/rome-mobile/page/administer/tablet-mobile-ui/concept/mobile-config-navigation.html

D and E. https://www.servicenow.com/products/service-portal.html

---

**👤 babafakruddin** ⬆️ 1

**Selected Answer:** ADE
ADE are correct

---

**👤 saiakhilpalle** ⬆️ 1

ADE are correct

---

**👤 Ranaway** ⬆️ 1

it must be ADE

---

**👤 ainkechik** ⬆️ 1

**Selected Answer:** ADE
ADE is the correct answer

---

**👤 Chrispin_Vino** ⬆️ 1

**Selected Answer:** ADE
ADE is correct

*...and 8 more comments*


</details>

---

## Question 361

### What is generated from the Service Catalog once a user places an order for an item or service?

### Answer Choices:
**A.** A change request

**B.** An Order Guide

**C.** A request

**D.** An SLA



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (11 comments)</strong></summary>

**👤 AMITKSI** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 Mahindra_Reddie** ⬆️ 1

**Selected Answer:** C
Is correct answer

---

**👤 techead** ⬆️ 1

C is the correct answer.
Request is generated from Service Catalog once a user places an order for an item or service? The order could be placed in Production Environment or Service Portal (to produce a record).

---

**👤 Augusto0007** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 mrnow** ⬆️ 1

**Selected Answer:** C
C is correct

---

**👤 saiakhilpalle** ⬆️ 1

C is correct

---

**👤 KIKI_AI** ⬆️ 2

**Selected Answer:** C
C is correct

---

**👤 Bonche** ⬆️ 1

C is the correct answer

---

**👤 Nicko1990** ⬆️ 1

C - A request

---

**👤 S_040** ⬆️ 1

Correct

*...and 1 more comments*


</details>

---

## Question 362

### Which of the following are not included in an Update Set, by default? (Choose four.)

### Answer Choices:
**A.** Homepages

**B.** Data

**C.** Published Workflows

**D.** Business Rules

**E.** Schedules

**F.** Database changes

**G.** Related Lists

**H.** Report Definitions



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** ABEI
**Community voting results:** ABEI (56%), BEFI (44%)

</details>

<details>
<summary><strong>💬 View Discussion (27 comments)</strong></summary>

**👤 ANCANC** ⬆️ 19

5 options are correct Homepages, Data, Schedules, Database changes and Scheduled jobs.

---

**👤 Erp88** ⬆️ 10

**Selected Answer:** BEFI
What is NOT captured in an update set:
-Data
-New Data
-Records
-Modified Data
-Records
-Tasks
-Modified Cls
-New Users and Groups
-Schedules
-Scheduled Jobs
-Dashboards*

---

**👤 Caro111222** ⬆️ 2

**Selected Answer:** ABEI
xanadu version

---

**👤 adyrip** ⬆️ 1

**Selected Answer:** ABEI
According to Washington CSA manual, not captured:
New Data Records, Modified data records, Tasks, modified CIs, new Users and groups, schedules, scheduled jobs, Dashboards

---

**👤 Frost312321** ⬆️ 2

**Selected Answer:** ABEI
F. Database changes is not a correct answer because tables and views are included in an update set

---

**👤 rrshah83** ⬆️ 1

**Selected Answer:** ABEI
tried it hands-on

---

**👤 JR83** ⬆️ 3

ABEI It seems A vs. F is the only contention here.
Homepages and content pages are not added to Update Sets by default. You must manually add pages to the current Update Set by unloading them. https://www.servicenow.com/community/itsm-forum/adding-homepage-customizations-to-update-set/m-p/812803

---

**👤 gprieto** ⬆️ 1

The correct is ABEI

---

**👤 96d187b** ⬆️ 2

A: Homepages are the same as Portal Pages or Dashboards, which are not included by default. They can be, but they require the Unload Dashboard function. 
Data: not captured 
Schedules: not captured 
Schedule Jobs: not captured 

Refer to the visual in Workbook section 7.2: What is Captured in a Data Set? Notice that in the X column, Dashboards appears with an asterisk* — I think they are using Homepage to represent Dashboard in this question.
B: Data is not captured like an Incident record
E: Schedules
I: Scheduled Jobs

F is incorrect: Database updates like a new table or a new field in a table would be included

---

**👤 ae6ab69** ⬆️ 5

**Selected Answer:** ABEI
A: Homepages ~ Portal Pages ~ Dashboards. Dashboards need to be added via Unlaod Dashboard function.
B: Data is not captured like an Incident record
E: Schedules 
I: Scheduled Jobs

F is incorrect: Database updates like a new table or a new field in a table would be included

*...and 17 more comments*


</details>

---

## Question 363

### You are showing your customer a new form that you have created for their new application. They would like to add a field to the form. Where could you do that?
(Choose two.)

### Answer Choices:
**A.** Select Fields and Columns module

**B.** Right click on form header, select Configure > Form Layout

**C.** Click on context menu, select Configure > Form Designer

**D.** Select Field Class Manager module



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BC
**Community voting results:** BC (82%), Other

</details>

<details>
<summary><strong>💬 View Discussion (23 comments)</strong></summary>

**👤 40a783b** ⬆️ 1

1. A is not correct as the module name should be Tables & Columns
2. Form Designer is not an option on the context menu (The option is Form Design not Designer)
The only correct answer is B

---

**👤 wer3ball** ⬆️ 3

B & C.

Go to section "Adding New Fields" in the ebook and it gives both options.

---

**👤 Adamkrt** ⬆️ 1

**Selected Answer:** B
only B is correct. Right click on form header, select Configure > Form Layout

---

**👤 PeterK0** ⬆️ 2

Every BC person, ask yourself, is it form designer or form design ? (:

---

**👤 Mariina_003_B** ⬆️ 2

**Selected Answer:** AB
The correct options are:

A. Select Fields and Columns module
B. Right click on form header, select Configure > Form Layout

Explanation:

Select Fields and Columns module (A): This module allows you to manage fields and columns on forms. You can add, modify, or remove fields from a table using this module.

Right click on form header, select Configure > Form Layout (B): This option provides access to the form layout configuration, where you can customize the arrangement and properties of form fields.

---

**👤 FATMASAAL2** ⬆️ 2

**Selected Answer:** BC
The answer should be B and C

---

**👤 netbuzz123** ⬆️ 1

Answer should be B&C

---

**👤 BenBose** ⬆️ 1

**Selected Answer:** B
The only correct answer is B. The reason C is incorrect is because a) you cannot click context menu (A context menu appears when you right click) and b) Form Designer is not an option on the configure sub-menu (The option is Form Design)

---

**👤 LovMi21** ⬆️ 1

**Selected Answer:** BC
Tested it! It's BC

---

**👤 ramana4r** ⬆️ 4

we don't have a module called "Fields & Columns". In ServiceNow, we have a module called "Tables & Columns". So option A is not correct. Correct options are B and C.

*...and 13 more comments*


</details>

---

## Question 364

### Which ServiceNow resource is a framework that ensures the data your ServiceNow application requires maps correctly to the appropriate CMDB tables?

### Answer Choices:
**A.** Common Service Data Model (CSDM)

**B.** Service Mapping Utility (SMU)

**C.** Service Schema Map (SSM)

**D.** CMDB Class Manager (CMDBCM)

**E.** CI Class Manager (CICM)



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 adyrip** ⬆️ 1

**Selected Answer:** A
Text book definition of the CSDM framework from the CSA Book.

---

**👤 4learning2u** ⬆️ 1

**Selected Answer:** A
A is the correct answer. Check the "Benefits of using the CSDM" section from the below link: 
https://docs.servicenow.com/bundle/tokyo-servicenow-platform/page/product/csdm-implementation/concept/csdm-basics.html

---

**👤 Nasman096** ⬆️ 2

**Selected Answer:** A
CSDM framework map to the physical model objects (tables and CI classes) in the CMDB.



</details>

---

## Question 365

### What do you activate when you want to add applications or functionality within your development instance?

### Answer Choices:
**A.** App Package

**B.** Updated Pack

**C.** Patch

**D.** Plugin

**E.** App Updated Set



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 GilSAtx** ⬆️ 1

**Selected Answer:** D
D Plugin

---

**👤 saiakhilpalle** ⬆️ 3

D is correct



</details>

---

## Question 366

### What field contains a record's 32-character, unique identifier?

### Answer Choices:
**A.** sn_rec_id

**B.** rec_id

**C.** u_id

**D.** sys_id

**E.** sn_gu_id

**F.** sn_sys_id

**G.** id



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** D
**Community voting results:** D (100%)

</details>

<details>
<summary><strong>💬 View Discussion (2 comments)</strong></summary>

**👤 skibumvermont** ⬆️ 1

**Selected Answer:** D
A unique 32-character GUID (Globally Unique ID), called a Sys ID (sys_id) identifies each record in an instance.

https://www.servicenow.com/docs/csh?topicname=c_UniqueRecordIdentifier.html&version=latest

---

**👤 sephereth** ⬆️ 4

**Selected Answer:** D
correct



</details>

---

## Question 367

### Your company is giving all first line workers a special T-shirt as a recognition for their hard work. Management team wants a way for employees to order the T- shirt, with the ability to specify the preferred size and color. How would you ensure that only first line workers (non-managers) can submit the order?

### Answer Choices:
**A.** Create Record Producer and use the Available For list to specify First Line [sn_first_line] role

**B.** Create Catalog Item and use the Not Available list to specify the Manager Group

**C.** Create Catalog Item and use the Available For list to specify ITIL [itil] role

**D.** Create Order Guide and use the User Criteria list to specify First Line [sn_first_line] role



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (79%), 14%, 7%

</details>

<details>
<summary><strong>💬 View Discussion (8 comments)</strong></summary>

**👤 Sam_Sonic** ⬆️ 6

**Selected Answer:** B
Not Order guide, as it is used to order mutiple items.
Not record producer, as it used for incidents and cases in most cases.
We only left with catalogue items. which is B abd C. and C does not exclude managers. so answer is B.

---

**👤 Joe15** ⬆️ 1

**Selected Answer:** B
B is the only one correct, by excluding the others.

---

**👤 servicepow** ⬆️ 1

**Selected Answer:** A
Available for First Line [sn_first_line] role.

---

**👤 gumarrita** ⬆️ 1

sn_first_line role does not exist. Itil role can include managers. So I chose option B.

---

**👤 Sam_Sonic** ⬆️ 1

Not as Order guide is used to order mutiple items. 
Not record producer, as it used for incidents and cases in most cases. 
We only left with catalogue items. which is B abd C. and C does not exclude managers. so answer is B.

---

**👤 Void8427** ⬆️ 2

**Selected Answer:** D
This isn't a record producer issue this is an order they want to allow. I believe it is D. You create an order guide that only those in that criteria can buy.

---

**👤 Thattechgirl** ⬆️ 1

To ensure that only first line workers (non-managers) can submit the order for the T-shirt, you can create a Record Producer and use the "Available For" list to specify the First Line [sn_first_line] role.
Option A is the correct answer

---

**👤 Nasman096** ⬆️ 4

**Selected Answer:** B
Procedure

Navigate to All > Service Catalog > Maintain Items or Service Catalog > Maintain Categories.
Open the relevant catalog item or category.
Configure the form to add the appropriate Available or Not available lists.



</details>

---

## Question 368

### What is used frequently to move customizations from one instance to another?

### Answer Choices:
**A.** Update Sets

**B.** Code Sets

**C.** Update Packs

**D.** Configuration Logs

**E.** Remote Sets

**F.** Local Sets

**G.** Code Packs



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** A
**Community voting results:** A (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 Schoolboz** ⬆️ 1

**Selected Answer:** A
A is the right answer

---

**👤 GilSAtx** ⬆️ 2

**Selected Answer:** A
A Update sets

---

**👤 NokoNice** ⬆️ 1

A is the correct ans



</details>

---

## Question 369

### What icon do you use to change the label on a Favorite?

### Answer Choices:
**A.** Clock

**B.** Hamburger

**C.** Pencil

**D.** Three dots

**E.** Triangle.

**F.** Star



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** C
**Community voting results:** C (100%)

</details>

<details>
<summary><strong>💬 View Discussion (4 comments)</strong></summary>

**👤 Foodiepanda** ⬆️ 2

**Selected Answer:** C
It says which Icon for EDIT .... bit confusing one '

---

**👤 PeterK0** ⬆️ 1

**Selected Answer:** C
Accurate correct answer is a pencil within square with round corners

---

**👤 anku15** ⬆️ 3

This is correct. The answer would be C only, Pencil icon, because the Question says, what icon is used for editing the label.

---

**👤 anku15** ⬆️ 1

It should be a star icon for favorites.



</details>

---

## Question 370

### What needs to be specified, when creating a Business Rule? (Choose four.)

### Answer Choices:
**A.** UI action

**B.** Table

**C.** Fields to update

**D.** Who can run

**E.** Script to run

**F.** Application scope

**G.** Update set

**H.** Timing



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** BEHI
**Community voting results:** BEHI (53%), BEFI (22%), BCDH (22%), 2%

</details>

<details>
<summary><strong>💬 View Discussion (35 comments)</strong></summary>

**👤 trye_indicators** ⬆️ 14

**Selected Answer:** BCDH
A. UI action - not applicable
B. Table - mandatory
C. Fields to update - important settings under ACTION tab. This is the purpose we creating business rule 
D. Who can run - optional - WHEN-TO-RUN tab -> role conditions 
E. Script to run - optional - advanced scenarios only - script box
F. Application scope - auto fill
G. Update set - not applicable
H. Timing - WHEN-TO-RUN tab
I. Condition to evaluate - optional - advanced scenarios only - script box

A, G - not applicable
B, C, D, H - required
E, I - advanced scenarios 
F - auto fill

except A, G, all is valid
B,C,D,H is generally required

---

**👤 Johnlego** ⬆️ 10

Answer is "BEHI" according to the updated San Diego ServiceNow Learning platform

---

**👤 adyrip** ⬆️ 2

**Selected Answer:** BEHI
According to the CSA Book: Every business includes what table to run against and timing (before or after insert), what conditions to evaluate and what script to run based on the evalation.

---

**👤 Kflo** ⬆️ 1

**Selected Answer:** BEHI
Every Business Rule includes what table to run against and timing (before or after insert and more), what actions to evaluate, and what script to run based on the evaluation.

---

**👤 Frost312321** ⬆️ 1

**Selected Answer:** BEHI
From ebook - chapter 7.1

Every Business Rule includes:
what table to run against > B. Table
and timing (before or affer insert and more) > H. Timing
what conditions to evaluate > I. Condition to evalue
and what script to run based on the evaluation > E. Script to run

---

**👤 aper** ⬆️ 1

**Selected Answer:** BEHI
The answer is BEHI

---

**👤 96d187b** ⬆️ 3

Straight from the SNAF Course / Book: 

Every business rule includes: 
1) what table to run against = B TABLE
2) timing (before or after insert) = H TIMING
3) what conditions to evaluate = I CONDITION TO EVALUATE
4) what script to run based on the evaluation = E SCRIPT TO RUN

Answer is: BEHI

---

**👤 PassPls** ⬆️ 1

**Selected Answer:** BEFI
H. Timing - I don't thik this mean When to run

---

**👤 nlaksdmrqe** ⬆️ 1

**Selected Answer:** BEFI
I went to dev instance, picked "UI Action", "New record". I needed to update table, script to run, application scope, condition to evaluate

---

**👤 40a783b** ⬆️ 3

**Selected Answer:** BEFI
eBook chapter 7.1 - script tipes: business rules

quote: 
Every Business Rule includes what table to run against and timing (before or affer insert and more), what conditions to evaluate, and what script to run based on the evaluation.

*...and 25 more comments*


</details>

---

## Question 371

### What feature can track the amount of time that a task has been open, to ensure that tasks are completed within an allotted time?

### Answer Choices:
**A.** Task Escalation Clock

**B.** Service Level Agreements

**C.** Inactivity Monitor

**D.** Response Time Clock

**E.** Business Time Remaining



<details>
<summary><strong>🔍 View Correct Answer</strong></summary>

**Correct Answer:** B
**Community voting results:** B (100%)

</details>

<details>
<summary><strong>💬 View Discussion (3 comments)</strong></summary>

**👤 tim4896** ⬆️ 1

**Selected Answer:** B
B is correct

---

**👤 Joe15** ⬆️ 1

**Selected Answer:** B
Only SLAs can track metrics

---

**👤 Budzitsu** ⬆️ 1

**Selected Answer:** B
It's B: https://docs.servicenow.com/ja-JP/bundle/vancouver-application-development/page/administer/task-table/reference/r_ToolsForDrivingTasks.html :
Service levels
Service level agreements can track the amount of time that a task has been open, to ensure that tasks are completed within an allotted time.



</details>

---

